# ACT-515 R1 · 2x-const · Full Receipt

```
RECEIPT — ACT-515 R1 · variant=2x-const
SELECT now();  →  2026-07-26T03:08:06.006Z
── CAVEATS ────────────────────────────────────────────────────────
...
── INVOCATION + COUNTS ────────────────────────────────────────────
  variant=2x-const  leverage=2x  mode=const
  ...
  admits: TOTAL=4,902  LONG=4,693  SHORT=209
  cap-bind telemetry:
    refusals — allocation_cap=643  position_already_open=2,934
               daily_budget=14,648  short_daily_budget=0
...
── THE VERDICT ROW (config-matrix.md columns) ─────────────────────
  ...
  ending_equity            = $171,111.44
  total_return_pct         = 71.11%
  cumulative_carry_usd     = $50.31
  worst_calendar_year      = 2024
  worst_calendar_year_ret  = 3.59%
  max_drawdown_pct         = 20.00%
  dd_peak_date             = 2024-11-11
  dd_trough_date           = 2025-04-08
  dd_recovery_date         = 2025-10-06
  dd_duration_sessions     = 100
  dd_recovery_sessions     = 124

── TERMINAL IDENTITY (study-mode envelope) ────────────────────────
  starting_equity_cents      = 10,000,000
  Σ realized_cents           = 7,116,195   (LONG 8,942,333 / SHORT -1,826,138)
  − cumulative_carry_cents   = 5,031
  + terminal_unrealized_c    = 0   (open lots at last session = 0)
  = predicted_end_cents      = 17,111,164
  actual  end_cents          = 17,111,144
  Δ = -20c over 4,902 lots — envelope=4,902c → WITHIN study-mode rounding envelope
...
── EQUITY-SHAPE NOTES ─────────────────────────────────────────────
  ...
  sessions_with_carry      = 12 / 1,011
  peak_cash_debit_usd      = $34,900.88
