# ACT-515 R1 · 1x-const · Full Receipt

```
RECEIPT — ACT-515 R1 · variant=1x-const
SELECT now();  →  2026-07-26T03:08:05.910Z
── CAVEATS ────────────────────────────────────────────────────────
...
── INVOCATION + COUNTS ────────────────────────────────────────────
  variant=1x-const  leverage=1x  mode=const
  ...
  cap-bind telemetry:
    refusals — allocation_cap=643  position_already_open=2,934
               daily_budget=14,632  short_daily_budget=0
...
── THE VERDICT ROW (config-matrix.md columns) ─────────────────────
  ...
  ending_equity            = $135,137.67
  total_return_pct         = 35.14%
  cumulative_carry_usd     = $0.00
  worst_calendar_year      = 2024
  worst_calendar_year_ret  = 2.07%
...
── TERMINAL IDENTITY (study-mode envelope) ────────────────────────
  ...
  Σ realized_cents           = 3,513,809   (LONG 4,417,299 / SHORT -903,490)
  − cumulative_carry_cents   = 0
  + terminal_unrealized_c    = 0   (open lots at last session = 0)
  = predicted_end_cents      = 13,513,809
  actual  end_cents          = 13,513,767
  Δ = -42c over 4,902 lots — envelope=4,902c → WITHIN study-mode rounding envelope
...
── EQUITY-SHAPE NOTES ─────────────────────────────────────────────
  ...
  sessions_with_carry      = 0 / 1,011
  peak_cash_debit_usd      = $0.00
