/**
 * longshort-reversal-compute-manual — operator-triggered manual reversal
 * compute. FP-040 / Signal #7. Sibling of the cron handler
 * `longshort-reversal-compute`.
 *
 * Mirrors `longshort-momentum-compute-manual/index.ts`:
 *   - Operator JWT (`authenticateRequest`) + `longshort.manage` permission.
 *   - `POST` with `{ "as_of": "YYYY-MM-DD" }` body; reuses the shared
 *     `parseAsOfDate` helper.
 *   - Dual audit envelope: `longshort.reversal.compute.manual_triggered`
...
Deno.serve(createHandler(async (req: Request) => {
  ...
  await writeStrategyAuditEvent({
    ...
    action: 'longshort.reversal.compute.manual_triggered',
  ...
  // KEEP IN SYNC with longshort-reversal-compute/index.ts cron handler.
  ...
  try {
    ...
    if (persist_error) {
      await writeStrategyAuditEvent({
        strategyKey: 'longshort',
        action: 'longshort.reversal.compute.manual_failed',
        actorId: authCtx.user.id,
        correlationId,
        ipAddress: authCtx.ipAddress ?? undefined,
        userAgent: authCtx.userAgent ?? undefined,
    ...
    await writeStrategyAuditEvent({
      ...
      action:
        result.outcome === 'completed'
          ? 'longshort.reversal.compute.manual_completed'
          : 'longshort.reversal.compute.manual_failed',
      actorId: authCtx.user.id,
      correlationId,
      ipAddress: authCtx.ipAddress ?? undefined,
      userAgent: authCtx.userAgent ?? undefined,
    ...
    return apiError(500, 'manual_reversal_compute_failed', { correlationId });
  }
}));