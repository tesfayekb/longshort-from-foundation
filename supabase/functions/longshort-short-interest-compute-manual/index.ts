/**
 * longshort-short-interest-compute-manual — operator-triggered manual
 * short-interest compute. FP-041 / Signal #5. Sibling of the cron handler
 * `longshort-short-interest-compute`.
 *
 * Mirrors `longshort-reversal-compute-manual/index.ts`:
 ...
 *   - NEW fetcher: `PolygonShortInterestFetcher` (entitlement-aware).
 *
...
Deno.serve(createHandler(async (req: Request) => {
  ...
  await writeStrategyAuditEvent({
    ...
    action: 'longshort.short_interest.compute.manual_triggered',
  ...
  // KEEP IN SYNC with longshort-short-interest-compute/index.ts cron handler.
  ...
  try {
    ...
    if (persist_error) {
      await writeStrategyAuditEvent({
        strategyKey: 'longshort',
        action: 'longshort.short_interest.compute.manual_failed',
        actorId: authCtx.user.id,
        correlationId,
        ipAddress: authCtx.ipAddress ?? undefined,
        userAgent: authCtx.userAgent ?? undefined,
    ...
    await writeStrategyAuditEvent({
      ...
      action:
        result.outcome === 'completed'
          ? 'longshort.short_interest.compute.manual_completed'
          : 'longshort.short_interest.compute.manual_failed',
      actorId: authCtx.user.id,
      correlationId,
      ipAddress: authCtx.ipAddress ?? undefined,
      userAgent: authCtx.userAgent ?? undefined,
    ...
    return apiError(500, 'manual_short_interest_compute_failed', { correlationId });
  }
}));