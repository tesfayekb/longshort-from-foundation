...
Deno.test('(2) happy path — ASC-sorted reports (Polygon DESC reversed)', async () => {
...
Deno.test('(5) HTTP 401 throws OvershootFetchError with ticker context', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
  ...
  assertEquals((err as OvershootFetchError).operation, SHORT_INTEREST_OPERATION_ID);
  assertEquals((err as OvershootFetchError).ticker, 'AAPL');
  assertStringIncludes((err as Error).message, 'AAPL');
});

Deno.test('(5b) HTTP 500 throws OvershootFetchError (5xx-not-swallowed)', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({}, false, 500, 'Internal Server Error'),
  );
  const err = await assertRejects(
    () => fetcher.fetchShortInterest('AAPL', AS_OF),
    OvershootFetchError,
...
Deno.test('(6) rows without short_interest are dropped (anti-phantom — no fabricated zero)', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
        { settlement_date: '2026-05-31', short_interest: 80_000_000 },
        // No short_interest → dropped, NOT defaulted to 0.
...
Deno.test('(6a) negative short_interest is dropped', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
        { settlement_date: '2026-05-31', short_interest: 80_000_000 },
...
Deno.test('(6c) malformed row (missing settlement_date) → surfaced by drop, not silent 0-fill', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
        { short_interest: 80_000_000 }, // missing settlement_date → dropped
        { settlement_date: '2026-05-15', short_interest: 90_000_000 },
      ],
    }),
  );
  const out = await fetcher.fetchShortInterest('AAPL', AS_OF);
  if (out.kind !== 'reports') throw new Error('unreachable');
  assertEquals(out.reports.length, 1);
  assertEquals(out.reports[0].report_date, '2026-05-15');
});
...
Deno.test('(9-DTC) carries explicit days_to_cover when Polygon returns it', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
...
Deno.test('(10-DTC) derives days_to_cover from short_interest / avg_daily_volume when missing', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
...
Deno.test('(11-DTC) absent/non-finite DTC inputs yield null (typed-absence, NOT fabricated zero)', async () => {
  const fetcher = new PolygonShortInterestFetcher('test-key', async () =>
    jsonResp({
      results: [
        { settlement_date: '2026-05-31', short_interest: 80_000_000 },
