...
Deno.test('(5) HTTP 401 throws OvershootFetchError with ticker context (5xx-not-swallowed sibling coverage)', async () => {
  const fetcher = new PolygonSharesOutstandingFetcher('test-key', async () =>
  ...
  assertEquals((err as OvershootFetchError).operation, SHARES_OUTSTANDING_OPERATION_ID);
...
Deno.test('(6) missing share_class_shares_outstanding → data_unavailable (anti-phantom — no fabricated denominator)', async () => {
  const fetcher = new PolygonSharesOutstandingFetcher('test-key', async () =>
...
Deno.test('(7) zero shares → data_unavailable (divide-by-zero trap intercepted at fetcher)', async () => {
  const fetcher = new PolygonSharesOutstandingFetcher('test-key', async () =>
...
Deno.test('(9) NaN / Infinity shares → data_unavailable', async () => {
  const fetcher1 = new PolygonSharesOutstandingFetcher('test-key', async () =>
    jsonResp({ results: { share_class_shares_outstanding: Number.NaN } }),
  );
...
Deno.test('(10) malformed-row (results absent entirely) → data_unavailable', async () => {
  const fetcher = new PolygonSharesOutstandingFetcher('test-key', async () =>
    jsonResp({}),
  );
  const out = await fetcher.fetchShares('AAPL');
...
Deno.test('(11) URL carries apiKey + ticker', async () => {
  let capturedUrl = '';
