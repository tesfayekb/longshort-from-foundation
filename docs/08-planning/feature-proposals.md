...
**Cross-references**: Phase 1 closure addendum at `docs/08-planning/phase-closures/plan-trading-001-longshort-003-closure.md` § "Honest Re-closure Addendum (2026-05-30)" eligibility-caveat subsection (the contract Commit 2 enforces); INC-24/25/26/27/28/29 in `docs/06-tracking/incidental-findings.md`; DW-079/080/081 in `docs/08-planning/deferred-work-register.md`; MIG-055 in `docs/07-reference/database-migration-ledger.md`; FP-008.3 entry above (side-awareness contract — predecessor pass that surfaced the same enforcement-gap mechanism class four times before FP-008.4 codified the gate-strengthening discipline).
