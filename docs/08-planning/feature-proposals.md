...
| **Reference Impact** | • `permission-index.md` — no new permissions (FP-008 does NOT introduce `longshort.execute`; remains deferred to Phase 5+). • `route-index.md` — no new routes (universe component is signal-stack infrastructure; not user-facing at this phase). • `event-index.md` — register universe-related audit events: `longshort.universe.refresh.completed`, `longshort.universe.cross_check.divergence`, `longshort.universe.constituent.added`, `longshort.universe.constituent.removed`, `longshort.universe.hard_exclusion.added`, `longshort.universe.hard_exclusion.removed` (events authored during sub-step 8.7 verify_universe_membership real implementation + sub-step 8.9 health monitoring). • `function-index.md` — register `verify_universe_membership` real implementation (was stub at FP-006 Gate 6.3); register `universeIngestionFetcher`, `hardExclusionRefresher`, `quarterlyAtomicRefreshJob`, `ingestionTimeCrossCheck` (authored during execution sub-steps). • `config-index.md` — register universe-component configuration row (e.g., refresh cadence overrides, cross-check tolerance bands per DEC-034 clause (6) asymmetric-tolerance discipline). • `env-var-index.md` — register `POLYGON_REFERENCE_DATA_API_KEY`, `EARNINGS_CALENDAR_API_KEY`, `SECONDARY_CONSTITUENT_SOURCE_API_KEY` (or equivalent per sub-step 8.1 selection). • `database-migration-ledger.md` — register **MIG-048** (NEW table `universe_membership`) + **MIG-049** (NEW table `hard_exclusions`) + further MIGs as needed during sub-step 8.6 (numbering follows MIG-047 from FP-006 closure). • `artifact-index.md` — register the new module docs (`docs/04-modules/longshort/universe/universe.md` at sub-step 8.10; ART-NNN per artifact-index convention). • `dependency-map.md` — add `longshort/universe` row under the strategy-module tier. |
...
| **Decision IDs (forward-reference)** | DEC-038 (Phase 1 universe-component invariants; to be authored at sub-step 8.0a). DEC-038.1 (Phase 1 universe-component architecture; to be authored at sub-step 8.0a if needed). DEC-032 clause (2)+(3) (FP-006 reservation honored — Phase 0B residual + entire Phase 0B closed; FP-008 opens Phase 1 as the natural successor). DEC-031 (strategy-module pattern; FP-008 inherits verbatim). |
