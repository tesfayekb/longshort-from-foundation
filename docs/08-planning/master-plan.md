...
**Sub-step inventory (4 — 3.0 only authored at this commit):**
  ...
  - [ ] 3.2 — R4 market-index/SPY regime fetcher + jsonb feature columns (DW-101; FP-052.2 entry authored at ACT-291 implementing ratified DEC-066; first consumer = lambdarank feature vector at 3.3)
    - [x] 3.2-a — FP-052.2 entry + DEC-066 §(h) `signal_observations` constraint-admissibility resolution (live read-only probe: ticker NOT NULL → sentinel `__MARKET__` chosen; signal_id free-text admits `market_*` with no widening; value/is_present + carried_forward CHECKs satisfied by always-present fail-loud regime features; NO migration needed, Gate-3.2 "without migration" holds) — ACT-291.
    - [x] 3.2-b — SPY regime fetcher edge function + cron `longshort.spy_regime.compute` (seeded DISARMED) + `signal_registry` rows for `market_24m_cumulative_return` + `market_realized_vol_6m` + `REGIME_PRICE_HISTORY_LOOKBACK_DAYS = 730` constant; writes to `signal_observations` under sentinel ticker `__MARKET__`; typed-fail-loud per DEC-066 §(e); NO `FEATURE_ORDER` change. LANDED at ACT-292 (2026-06-23, MIG-117): pure `compute-regime.ts` (`computeRegime24mReturn` + `computeRegimeVol6m`; `REGIME_24M_MIN_BARS=504` per DEC-066 §(f) Feature 1 + `REGIME_VOL_6M_MIN_BARS=126` per Feature 2; `sqrt(252)` annotated convention-not-grounding); `regime-orchestrator.ts` (single-ticker SPY pipeline, DISTINCT typed-fail-loud reasons `regime_data_missing_current_bar` vs `regime_data_insufficient_history` vs `regime_fetch_error` vs `regime_persistence_error` — never collapsed); cron handler `longshort-spy-regime-compute/index.ts` (mirrors carry-compute skeleton; cron-auth → as_of → 3-event audit envelope → orchestrator → cron_last_fire); manual sibling `longshort-spy-regime-compute-manual/index.ts` (operator JWT + `longshort.manage`); MIG-117 (signal_registry rows + DISARMED `job_registry` row `0 19 * * 1-5` weekdays — collision-free, pre-positioned upstream of assemble 23:35 + rank 23:50). Live-verified post-MIG: `read_query` confirms 2 signal_registry rows (status=planned, criticality=NULL, display_order 100/101, job_registry_id='longshort.spy_regime.compute') + job_registry row enabled=false. Tests: compute-regime 19/19, orchestrator 9/9, handler source-sentinel 13/13 — all green. Gate-6 wall-clock self-scan CLEAN. `deno.lock` v3 preserved (Catalog #58). NO assembler / `lgbm-inference.ts` / `feature_contract.py` / `FEATURE_ORDER` / `EXPECTED_FEATURE_KEY_COUNT` / `feature_order_hash` touch — `feature_order_hash` does NOT flip in 3.2-b (3.2-d work). NO DDL on `signal_observations` — Gate-3.2 "without migration" holds. Rows are INERT (no reader) until 3.2-c lands the assembler regime-broadcaster.
    - [x] 3.2-c — Assembler regime-broadcaster wiring + assembler-side fail-loud propagation. LANDED at ACT-293 (2026-06-23, no migration). `feature-assembler-orchestrator.ts` reads regime rows from `signal_observations` as a SEPARATE projection (`.select('signal_id, value, is_present').eq('operator_id', …).eq('as_of_date', as_of).in('signal_id', [MARKET_24M_CUMULATIVE_RETURN_SIGNAL_ID, MARKET_REALIZED_VOL_6M_SIGNAL_ID])` — NO `universe_membership` join: the `__MARKET__` sentinel is intentionally outside the per-name universe). `feature-assembler.ts` adds `RegimeFeatures` typed param + `buildFeaturesJsonb` appends the 2 regime keys as BARE NUMERICS (NOT `__value`/`__is_present` — market-level category per §6.5.1.1) IDENTICALLY to every per-name `features` jsonb row at the as_of (broadcast). Per-name signal keys UNCHANGED (broadcast is purely additive); regime keys appended LAST so deterministic key order is preserved. Assembler-side fail-loud (DEC-066 §(e) family): if regime read returns `< 2` rows OR any row has `is_present=false` / `value=null`, orchestrator returns `{outcome:'failed', failure_reason:'regime_data_unavailable_at_assemble', persisted_count:0, included_count:0}` — ZERO feature-vector rows written, NO book that day (a regime-less or null-filled vector would silently poison training). Invariant check widened to `EXPECTED_FEATURE_KEY_COUNT + REGIME_FEATURE_COUNT (16+2=18)` — `EXPECTED_FEATURE_KEY_COUNT` itself NOT touched (3.2-d folds the +2 into the catalog constant and flips the hash). Tests: 27 feature-assembler + 13 orchestrator + 3 carry-flag-isolation = 43/43 green; new orch tests cover (a) separate-projection / no-join evidence, (b) IDENTICAL broadcast across all per-name rows, (c) regime ABSENT → fail-loud + zero upserts, (d) partial 1-of-2 → fail-loud, (e) `is_present=false` → fail-loud. NO touch to `lgbm-inference.ts` / `signal-catalog.ts EXPECTED_FEATURE_KEY_COUNT` / `FEATURE_ORDER` / `feature_contract.py` — `feature_order_hash` does NOT flip in 3.2-c (3.2-d work). NO migration (no DDL). Gate-6 wall-clock self-scan CLEAN. `deno.lock` v3 preserved (Catalog #58).
    - [x] 3.2-d — Paired TS+Python `FEATURE_ORDER` 16→18 (`lgbm-inference.ts` + `training/combiner/feature_contract.py` SAME PR per DEC-064 Clause 4 / DEC-066 §(d)) + `EXPECTED_FEATURE_KEY_COUNT 16→18` in `signal-catalog.ts` (+ new `REGIME_FEATURE_COUNT=2` catalog constant; assembler invariant simplified to single `EXPECTED_FEATURE_KEY_COUNT`); new `[16]=market_24m_cumulative_return`, `[17]=market_realized_vol_6m` (BARE NUMERICS, NOT `__value`/`__is_present` pairs — market-level category per DEC-066 §(c)); `feature_order_hash` flipped from `1054bbc1d7352855801dcc0d9ddb7f276310dcebd4cd1a4f6774240f0317e20b` to `d4aac3e3e58740543de51764c05b8688595eb025ec41bd55677c9c27f24ce348` (TS featureOrderHash() and Python feature_order_hash() byte-identical; `scripts/assert_feature_order_parity.py` green — 18 keys + hash parity). ADDITIVE-NOT-BREAK at change time per DEC-066 §(f) — zero artifacts exist (trainer skip-gate per ACT-288 has never let one promote); POST-PROMOTION LOCK takes effect once the first artifact promotes. The 16-key prefix `[0..15]` is byte-unchanged; only `[16]/[17]` appended. LANDED at ACT-295 (2026-06-23, no migration): test_feature_contract.py hardcoded hash updated + new `test_market_regime_keys_appended_last_as_bare_numerics` + `test_missing_market_regime_raises`; lgbm-inference_test.ts (lgbm-1) extended to 18-key sequence + new (lgbm-1b) hash lock; ranker-orchestrator_test.ts fixtures `max_feature_idx` 15→17 + `fullIncludedRow` adds market keys. Tests: 171/171 combiner + regime Deno green; 8/8 Python feature_contract green; parity assert green. NO migration, NO assembler logic change (3.2-c already broadcasts the keys), NO trainer fit, NO cron arming.
    - [x] 3.2-e — Trainer re-validation against the new 18-key hash (CONTRACT-SHAPE check only — no fit). LANDED at ACT-297 (2026-06-23, no migration): (i) `scripts/assert_feature_order_parity.py` green — `OK: FEATURE_ORDER parity (18 keys) + feature_order_hash=d4aac3e3e58740543de51764c05b8688595eb025ec41bd55677c9c27f24ce348` (TS `featureOrderHash()` extracted from `lgbm-inference.ts` == Python `feature_order_hash()` over `feature_contract.FEATURE_ORDER`); (ii) 8/8 `tests/test_feature_contract.py` + 5/5 `tests/test_trainer_skip.py` green — locks the 18-key sequence + the hash + the typed skip-gate shape (zero-rows, rows-below-floor, seed-dates-below-floor, both-cleared-does-not-skip, typed-stdout); (iii) trainer SHAPE coherence verified — `trainer.py:270` builds the feature matrix via `np.array([features_to_ordered_row(r.features) for r in ordered])`, which iterates `feature_contract.FEATURE_ORDER` (all 18 keys) by construction, and `trainer.py:328` stamps `meta['feature_order_hash'] = feature_order_hash()` (the live 18-key value, refused at inference load by DEC-064 Clause 4 if any mismatch); (iv) trainer's live dry-run skip-gate exercise deferred to the GHA workflow's next fire (the runtime path requires `lightgbm` + live Supabase per `training/combiner/README.md` §"What is NOT in this directory"; in-sandbox test_trainer_skip.py is the runnable substitute and is green). Stale-shape sweep: `training/combiner/trainer.py:10` docstring updated `16-feature → 18-feature` with DEC-066 §(c) citation + new hash; `training/combiner/README.md` lines 12, 25 updated `16-key → 18-key` + new hash. NO trainer fit, NO model artifact, NO `combiner_model_registry` insert, NO migration, NO cron arming.
    - [x] Gate-3.2 — CLOSED 2026-06-23 (ACT-297). Closure attestation: (1) 3.2-a/b/c/d/e all LANDED green (ACT-291/292/293/295/297); the dual-toolchain CI reds en route resolved (functions-lock #58 first firing, lint-directive at ACT-294, root-lock #58 second firing at ACT-296). (2) FEATURE_ORDER parity verified at 18 keys + `feature_order_hash=d4aac3e3e58740543de51764c05b8688595eb025ec41bd55677c9c27f24ce348`, TS==Python (parity assert + Python tests + TS hash-lock test `lgbm-1b` all green). (3) "WITHOUT MIGRATION" invariant preserved across the entire 3.2 ladder: the only migration in the 3.2 window was MIG-117 (3.2-b: `signal_registry` rows + DISARMED `job_registry` seed for `longshort.spy_regime.compute`) — NOT a feature-carrier DDL change. ZERO `ALTER TABLE`/`CREATE TABLE` on `signal_observations` or `combiner_feature_vectors` anywhere in 3.2-a..3.2-e (confirmed via migration ledger grep). Phase-3.2 / DW-101 regime features COMPLETE — the trainer's first real candidate (when T+10 labels accrue) will carry regime features from day one, meta-stamped with `d4aac3e3…`; no forced retrain (DEC-066 §(f) ADDITIVE-NOT-BREAK at change time + POST-PROMOTION LOCK from the first promoted artifact). PLAN-TRADING-001-LONGSHORT-007 as a whole stays OPEN — 3.3 (LambdaRank training + atomic promotion) remains in progress; only the 3.2 sub-gate closes here.
  - [ ] 3.3 — LambdaRank training + atomic promotion (FP-052.3; flips BOTH exit-gate queries to non-zero — first row with `status='active'` in `combiner_model_registry` + first ranking row with non-fallback `ranker_source` — giving a clean before/after diff against the 3.0 attestation surface)
    - [x] 3.3a — Promotion-gate foundation: MIG-115 (`combiner_forward_returns.horizon_td` widened to admit T+10 per §6.1/§6.2 LOCK; `promote_combiner_model(p_model_id uuid)` + `rollback_combiner_model(p_side text)` `SECURITY DEFINER` `service_role`-only RPCs honoring `uq_combiner_model_registry_active_per_side` via retire-first ordering) + 3.M forward-returns fetcher T+10 pair-edit (`HORIZONS_TD` + `MATURATION_FLOOR_CAL_DAYS`; `emptyByHorizon` derived from `HORIZONS_TD`; T+1/5/20 logic byte-unchanged) + pure criteria evaluator `promotion-criteria-evaluator.ts` (C1/C2/C9/C10/C12 with NAMED CONSTANT CANDIDATE thresholds; SHAP/C6 carved out per DEC-063; `TRAINING_HORIZON_TD=10`) + Deno tests + DEC-063 (SHAP/C6 temporal relocation + atomic RPC contract). LANDED at ACT-283 (2026-06-23, MIG-115). No model exists; evaluator returns `{not_computable, not_yet}` against current substrate — that is the CORRECT v1 state.
     ...
     - [~] 3.3b-ii-B — PARTIAL (trainer files LANDED at ACT-288; GHA workflow + bucket/secret provisioning PENDING operator package): committed `training/combiner/` (the repo's FIRST Python dir per DEC-064) containing `feature_contract.py` (byte-identical Python mirror of TS `FEATURE_ORDER`; `feature_order_hash()` = `1054bbc1d735...20b`, matches TS `featureOrderHash()` byte-for-byte — DEC-064 Clause 4 contract honored at the source-of-truth level) + `trainer.py` (LightGBM-lambdarank per §6.1/§6.2 LOCK, T+10 per §6.2, winsorize 1st/99th per day per §6.2, exp-time-weight half-life 1.5y per §6.3, NDCG@25 per §6.1, TWO models per §6.1; ASSERTS `horizon_td=10` widen live before training; SKIP-GRACEFUL when T+10 labels below `LABEL_MIN_ROWS_PER_SIDE * LABEL_MIN_SEED_DATES` floor — no degenerate model on thin data; writes model.txt + meta.json to Storage; INSERTs `combiner_model_registry` with `status='candidate'` ONLY — promotion is the 3.3a RPC's job, criteria-gated) + `requirements.in` + `requirements.txt` (hash-pinned per DEC-064 Clause 2; operator regenerates with `pip-compile --generate-hashes` to expand transitives) + `tests/test_feature_contract.py` (6 tests; locks hash to constant) + `tests/test_trainer_skip.py` (5 tests; skip-gate floors) + `scripts/assert_feature_order_parity.py` (cross-language CI assert; TS↔Python FEATURE_ORDER + hash parity; DEC-064 Clause 4 enforcement). 11/11 Python tests green + parity assert green in Lovable sandbox. NO `.github/workflows/*.yml` (operator package), NO bucket provisioning (operator Dashboard per DEC-065 Clause 4), NO `SUPABASE_SERVICE_ROLE_KEY` GHA secret (operator), NO TS / `supabase/functions/` edit (deno.lock untouched at v3), NO migration. PENDING-operator: GHA weekly Sunday workflow per §6.3; bucket + secret provisioning; missingness-profile capture + masking-stress harness (C3 KILL / C4 / C5); promotion-RPC caller edge function; missingness-profile stability evaluator (C11); full Optuna search (§6.4 quarterly); data-fetch + fit path runtime exercise (trainer skeleton present but data-fetch RPC + chunked pagination land alongside the GHA workflow). Candidates cannot promote until 3.3a evaluator returns `all_pass=true` AND DEC-063 Clause-3 re-gate clears post-DW-136. **UPDATE 2026-06-26 (ACT-335) — OPERATOR PACKAGE COMPLETE; PHASE-3.3 NOW PURELY SUBSTRATE-GATED.** Live-state confirmation: (a) GHA workflow `.github/workflows/combiner-train.yml` LANDED — schedule `0 6 * * 0` (Sundays 06:00 UTC per §6.3) + `workflow_dispatch` for manual fire; entrypoint `python trainer.py` under `training/combiner/`; pre-step `assert_feature_order_parity.py` enforces DEC-064 Clause 4 cross-language hash parity; fast-fail guards on both required secrets. (b) Supabase Storage bucket `combiner-models` PROVISIONED (created 2026-06-23; private; MIG-116 RLS policies + retention purge now active rather than inert). (c) GitHub Actions secrets `SUPABASE_PROJECT_REF` + `SUPABASE_SERVICE_ROLE_KEY` (plus `SUPABASE_URL` + `SUPABASE_ACCESS_TOKEN`) PRESENT — operator-verified in GitHub Settings → Secrets → Actions ~2 weeks ago. **Remaining gate (substrate maturation, benign):** trainer skips until T+10 labels cross `LABEL_MIN_ROWS_PER_SIDE=500 × LABEL_MIN_SEED_DATES=20`. Current substrate (2026-06-26): T+1 = 2,560 rows (seeds 06-16…06-24); T+5 = 1,000 rows (seeds 06-16…06-19); **T+10 = 0 rows**. **STATE: MATURATION-PENDING (NOT a fetch-gap)** — `HORIZONS_TD = [1, 5, 10, 20]` in `forward-return-constants.ts` (10 IS in the attempt set); `MATURATION_FLOOR_CAL_DAYS[10] = 10`; earliest seed 2026-06-16 is exactly 10 calendar days old today and the bar-array authoritative check (the load-bearing maturation gate per the constants-file docstring) requires 10 *trading* days forward (06-16 + 10 RTH ≈ 2026-06-30) — so the 06-16 seed will start producing T+10 rows ~2026-06-30 and accrue daily; first weekly cron (Sunday 2026-06-28 06:00 UTC) still skips; the FP-052.3 floor (500 rows × 20 seed-dates per side) clears after ≥20 seed-dates × ~20 names/side accrue past T+10 maturation, ETA late-July 2026 assuming the daily seeding cadence holds. NO seed-date older than the floor produced T+1/T+5 without T+10 (the fetch-gap signature is ABSENT).
