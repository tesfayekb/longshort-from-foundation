...
**Sub-step inventory (4 — 3.0 only authored at this commit):**
  ...
  - [ ] 3.2 — R4 market-index/SPY regime fetcher + jsonb feature columns (DW-101; FP-052.2 entry authored at ACT-291 implementing ratified DEC-066; first consumer = lambdarank feature vector at 3.3)
    ...
    - [ ] 3.2-d — Paired TS+Python `FEATURE_ORDER` 16→18 (`lgbm-inference.ts` + `training/combiner/feature_contract.py` SAME PR per DEC-066 §(d)) + `EXPECTED_FEATURE_KEY_COUNT 16→18` in `signal-catalog.ts`; new `[16]=market_24m_cumulative_return`, `[17]=market_realized_vol_6m`; `feature_order_hash` flips from `1054bbc1…20b` to a new value (ADDITIVE-NOT-BREAK at change time per DEC-066 §(f) — zero artifacts exist; POST-PROMOTION LOCK takes effect once first artifact promotes); `assert_feature_order_parity.py` re-run.
