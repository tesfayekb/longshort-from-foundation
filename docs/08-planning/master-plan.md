...
**Sub-step inventory (4 — 3.0 only authored at this commit):**
  ...
  - [ ] 3.2 — R4 market-index/SPY regime fetcher + jsonb feature columns (DW-101; FP-052.2 entry authored at ACT-291 implementing ratified DEC-066; first consumer = lambdarank feature vector at 3.3)
    ...
    - [ ] 3.2-c — Assembler regime-broadcaster wiring: read regime rows from `signal_observations` by `signal_id LIKE 'market\_%'` (separate projection, no universe intersection) and broadcast values into every per-name `combiner_feature_vectors.features` jsonb row at that `as_of_date`; typed-fail-loud propagates to assemble outcome; NO `FEATURE_ORDER` change.
