...
**Sub-step inventory (4 — 3.0 only authored at this commit):**
  ...
  - [ ] 3.2 — R4 market-index/SPY regime fetcher + jsonb feature columns (DW-101; FP-052.2 entry authored at ACT-291 implementing ratified DEC-066; first consumer = lambdarank feature vector at 3.3)
    ...
    - [ ] 3.2-b — SPY regime fetcher edge function + cron `longshort.spy_regime.compute` (seeded DISARMED) + `signal_registry` rows for `market_24m_cumulative_return` + `market_realized_vol_6m` + `REGIME_PRICE_HISTORY_LOOKBACK_DAYS = 730` constant; writes to `signal_observations` under sentinel ticker `__MARKET__`; typed-fail-loud per DEC-066 §(e); NO `FEATURE_ORDER` change.
