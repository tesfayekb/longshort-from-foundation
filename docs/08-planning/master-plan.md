...
**Sub-step inventory (4 — 3.0 only authored at this commit):**
  ...
  - [ ] 3.2 — R4 market-index/SPY regime fetcher + jsonb feature columns (DW-101; FP-052.2 entry authored at ACT-291 implementing ratified DEC-066; first consumer = lambdarank feature vector at 3.3)
    ...
    - [ ] 3.2-e — Trainer re-validation against the new hash (contract-shape check only — parity assert + trainer dry-run skip-gate; no fit because T+10 labels haven't accrued per ACT-288).
    - [ ] Gate-3.2 — 3.2-d + 3.2-e land green; FEATURE_ORDER parity verified; "without migration" invariant preserved (no DDL change on `signal_observations` or `combiner_feature_vectors`).
