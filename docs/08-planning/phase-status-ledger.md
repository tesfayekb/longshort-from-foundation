...
| **6 — Full Integration / Cadence Rebuild** | **PARTIAL — FP-062 ladder shipped (ACT-381..ACT-400); discrete-phase closure pending [6I.7 (DW-156) + `verify_corporate_action_clean` orchestration wire + DW-058-B2 disposition + DW-145/146/147 calibration-DEC ratifications + end-to-end live-fills validation]** (status flipped at ACT-411 — Rule 8 additive; the prior "IN-PROGRESS by aggregation — UNVERIFIED" framing is now stale because FP-062 IS the discrete §10-Phase-6 FP and its sub-step ladder is substantially built; live-fills validation is the discrete-closure gate — NOT CLOSED) | FP-057 RESOLVED 2026-06-29 (ACT-349; all 6 sub-steps landed); FP-062 chartered ACT-368, ladder built ACT-381..ACT-400 (CHARTER → built per its Status roll-up at ACT-411) — **FP-062 IS the discrete §10-Phase-6 integration FP**; **Drift D4 RESOLVED** at ACT-411 (the prior "no discrete Phase-6 FP exists" claim is corrected — FP-062 closes that gap). The rest is the build-to-paper aggregate window. | FP-057 (cadence rebuild RESOLVED); **FP-062 (Phase-6 integration — PARTIALLY-BUILT per its Status roll-up at ACT-411; 9 sub-steps closed, 5 closure gaps open)** | DW-058 (incl. B2 halt feed) — 6I.2a BUILT at ACT-386 (BP + Position mocks retired); B2 PROBED-DEFERRED at ACT-400 (see escalation cell); DW-060 (periodic-sweep scheduler arming) — **BUILT + CLOSED at ACT-393** (MIG-145 re-enable + 2-tick AC verified healthy); DW-105 (§1.4 state-machine) — **BUILT at ACT-398 / MIG-147**; DW-144 (kill-switch surface) — **BUILT at ACT-394 / MIG-146**; DW-151 (`pdt_block` PAUSE) — **BUILT at ACT-396** (routes through DW-144 via `account-pause-escalator.ts`); DW-152 (persistent-BP detection) — **BUILT at ACT-397** (in `tick-scheduler.ts`); DW-156 (entry-freshness / signal-trajectory gate) — **CONDITIONAL-OPEN** (6I.7 confirm-or-adopt has no closing ACT; remains a Phase-6 closure gap); DW-145 / DW-146 / DW-147 (calibration DEC-only) — **PENDING** (in-code constants exist; DEC-authoring turn unauthored) | DW-058-B2 (halt feed — HARD for live-order code paths; PROBED-DEFERRED at ACT-400 — vendor-procurement-vs-typed-absence disposition PENDING); end-to-end live-fills validation (market-gated; ACT-403 fill-path + ACT-407 B1 substrate gate deployed but UN-EXERCISED — first `longshort.execution.filled` not observed — the discrete-closure GATE per FP-062 Status roll-up gap (e)); `verify_corporate_action_clean` orchestration call-site wire (FP-061-ADD-01 / gap (b)) |
