...
|---|---|---|
| **Phase-4 money-path FP** → **AUTHORED AS FP-061** (ACT-367, 2026-06-29); **4M.1 BUILT** (ACT-370, MIG-139); **AMENDED** (ACT-371, FP-061-ADD-01) — 4M.5 split into 4M.5a (per-exit realized-PnL + exit-columns + `verify_realized_pnl` seam) + 4M.5b (terminal year-end aggregation); dependency chain re-sequenced to 4M.1 → {4M.2, 4M.4, 4M.5a} parallel-safe → 4M.3 → 4M.5b (the original "4M.3 before 4M.5" inverted per §7.7 broker-confirmed-loss dependency); 4M.2 carries own `settled_at` migration; §7.8 retroactive ships with 4M.3. Column-on-lots binding for DW-160 unchanged; soft-Phase-6 dependency unchanged. Builds deferred to separate authorized execution turns per sub-step. | DW-157, DW-158, DW-159, DW-160, DW-161 — ALL ADOPTED as named sub-steps (DW-158→4M.1 [DONE ACT-370], DW-160→4M.1+4M.2 [`settled_at` in 4M.2 per ADD-01], DW-157→4M.3 [+§7.8 retroactive per ADD-01], DW-161→4M.4, DW-159→**4M.5a + 4M.5b** [split per ADD-01]). | Hard pre-live (5) |
