...
| **4 — Portfolio / Sizing / Wash-sale / Lot / PnL / Settlement / CA** | **PARTIAL — UNVERIFIED (no `phase-4-closure.md` exists)** | Sizing + book-layer landed under FP-055 (no standalone closure doc — see Drift **D7**); tax/lot/PnL/settlement/corp-actions infra **BUILT under FP-061 (CLOSED in-entry 2026-06-29 at ACT-380, MIG-139→144); broker-reconciliation discharge pending FP-062.** Row overall stays PARTIAL — FP-055 sizing strand closure remains via-reference per D7, and the money-path infra's pre-live (Phase-8) discharge tail (verifier broker-side flip mock→real) awaits FP-062. | FP-055 (sizing — closure recorded via reference, see D7); **FP-061 (CHARTERED ACT-367; CLOSED in-entry ACT-380 2026-06-29 — all 5 sub-steps built + applied + CI-green, MIG-139→144)** — Drift **D4 RESOLVED** (authored + built + closed). | DW-157 / DW-158 / DW-159 / DW-160 / DW-161 (each "PRE-LIVE BLOCKER"; provenance = the DW-143 umbrella split, see Drift **D6**) — **BUILT under FP-061 (ACT-380); broker-reconciliation discharge pending FP-062.** | DW-157 / DW-158 / DW-159 / DW-160 / DW-161 (5 hard pre-live blockers — substrate BUILT; pre-live discharge requires FP-062 broker fetchers) |
...
| **6 — Full Integration / Cadence Rebuild** | **CLOSED 2026-06-30 at ACT-429 (FP-062 CLOSED in-entry; §10.10 gate MET — `crosswind_spec_v09_part3b.md:146`).** *(Prior "PARTIAL — FP-062 ladder shipped (ACT-381..ACT-400); discrete-phase closure pending …" framing PRESERVED below per Constitution Rule 8 pointer pattern; the "discrete-phase closure pending" clause is SUPERSEDED-IN-PLACE by this closure.)* **PARTIAL — FP-062 ladder shipped (ACT-381..ACT-400); discrete-phase closure pending [6I.7 (DW-156) + `verify_corporate_action_clean` orchestration wire + DW-058-B2 disposition + DW-145/146/147 calibration-DEC ratifications + end-to-end live-fills validation]** (status flipped at ACT-411 — Rule 8 additive; the prior "IN-PROGRESS by aggregation — UNVERIFIED" framing is now stale because FP-062 IS the discrete §10-Phase-6 FP and its sub-step ladder is substantially built; live-fills validation is the discrete-closure gate — NOT CLOSED). **Closure 2026-06-30 at ACT-429:** all 5 closure gaps cleared — (a) DW-156 CONFIRM-DEFERRED at ACT-412; (b) `verify_corporate_action_clean` WIRED at ACT-413; (c) DW-058-B2 DISPOSED at DEC-068 clause (s) + FP-064 / ACT-416 (PAPER-ONLY-ACCEPTABLE; live-fire gated on FP-064); (d) DW-145/146/147 RATIFIED at DEC-076 / ACT-415; **(e) live-fills validation DISCHARGED 2026-06-30** via the manual chain (36 accepted Alpaca orders, correlation_id `e1b02a34-5d89-4277-a691-fa8c650cdbfa`, rebalance.completed 16:13:42Z; 27 `execution.filled` recon events, 20 lots with `source_order_id` populated, zero MIG-148 dedup violations, §11.0.11 firings root-caused, combiner_book 43/43 ratio 1.00 DW-201 favorable re-eval, ranking-snapshot sidecar 176 rows joinable to all 20 accrued symbols). FP-062 flipped PARTIALLY-BUILT → **CLOSED IN-ENTRY at ACT-429** per FP-061 / FP-054 / FP-037 non-phase-FP convention; **NO separate `phase-closure.md` authored, NO `PLAN-TRADING-001-LONGSHORT-NNN` ID invented**. Non-blocking follow-up: residual ~7 PENDING-NEXT-TICK lots + 23 pending_timeout orders reconstruct on subsequent execute-ticks (normal lag). | FP-057 RESOLVED 2026-06-29 (ACT-349; all 6 sub-steps landed); FP-062 chartered ACT-368, ladder built ACT-381..ACT-400 (CHARTER → built per its Status roll-up at ACT-411) — **FP-062 IS the discrete §10-Phase-6 integration FP**; **Drift D4 RESOLVED** at ACT-411 (the prior "no discrete Phase-6 FP exists" claim is corrected — FP-062 closes that gap); **FP-062 CLOSED in-entry at ACT-429 2026-06-30**. | FP-057 (cadence rebuild RESOLVED); **FP-062 (Phase-6 integration — CLOSED in-entry at ACT-429 2026-06-30; all 9 sub-steps + 5 closure gaps discharged)** | DW-058 (incl. B2 halt feed) — 6I.2a BUILT at ACT-386 (BP + Position mocks retired); B2 PROBED-DEFERRED at ACT-400 → DISPOSED at DEC-068 clause (s) + FP-064 / ACT-416 (paper-acceptable; FP-064 is the named pre-live blocker); DW-060 (periodic-sweep scheduler arming) — **BUILT + CLOSED at ACT-393** (MIG-145 re-enable + 2-tick AC verified healthy); DW-105 (§1.4 state-machine) — **BUILT at ACT-398 / MIG-147**; DW-144 (kill-switch surface) — **BUILT at ACT-394 / MIG-146**; DW-151 (`pdt_block` PAUSE) — **BUILT at ACT-396** (routes through DW-144 via `account-pause-escalator.ts`); DW-152 (persistent-BP detection) — **BUILT at ACT-397** (in `tick-scheduler.ts`); DW-156 — **CONFIRM-DEFERRED at ACT-412** (data-gated on Phase-3.3 first artifact + post-first-fills window per DW-156-ADD-01); DW-145 / DW-146 / DW-147 — **RATIFIED at DEC-076 / ACT-415** (v1 defaults homed; empirical recalibration pre-authorized per clause (d)). | FP-064 (Polygon Advanced WS halt-fetcher BUILD — named pre-live blocker per DEC-068 clause (s) GUARDRAIL 3; scheduled to START at gap-(e) discharge which is now satisfied). |
...
|---|---|---|
| **Phase-4 money-path FP** → **AUTHORED AS FP-061** (ACT-367, 2026-06-29); **4M.1 BUILT** (ACT-370, MIG-139); **AMENDED** (ACT-371, FP-061-ADD-01) — 4M.5 split into 4M.5a (per-exit realized-PnL + exit-columns + `verify_realized_pnl` seam) + 4M.5b (terminal year-end aggregation); dependency chain re-sequenced to 4M.1 → {4M.2, 4M.4, 4M.5a} parallel-safe → 4M.3 → 4M.5b (the original "4M.3 before 4M.5" inverted per §7.7 broker-confirmed-loss dependency); 4M.2 carries own `settled_at` migration; §7.8 retroactive ships with 4M.3. Column-on-lots binding for DW-160 unchanged; soft-Phase-6 dependency unchanged. Builds deferred to separate authorized execution turns per sub-step. **ALL 5 SUB-STEPS BUILT + CI-GREEN 2026-06-29** (MIG-139→144; ACT-370 / 372 / 374 / 376 / 377 / 378; CI fixes ACT-373 / 375 / 379). **FP-061 CLOSED in-entry at ACT-380** (per FP-054/FP-037 non-phase-FP convention — no `phase-04-closure.md`). Broker-reconciliation discharge tail → **FP-062** (successor DWs **DW-196 / DW-197 / DW-198 / DW-199**). | DW-157, DW-158, DW-159, DW-160, DW-161 — ALL ADOPTED as named sub-steps (DW-158→4M.1 [DONE ACT-370], DW-160→4M.1+4M.2 [`settled_at` in 4M.2 per ADD-01], DW-157→4M.3 [+§7.8 retroactive per ADD-01], DW-161→4M.4, DW-159→**4M.5a + 4M.5b** [split per ADD-01]). **ALL BUILT — pre-live discharge pending FP-062.** | Hard pre-live (5) |
...
| **Signal-substrate-hardening FP** → **AUTHORED AS FP-059 (ACT-369, 2026-06-29)** — unified-but-sub-clustered carrier (3 sub-clusters: A cross-signal / B insider / C catalyst). DW-114 is the Tier-A anchor (pre-live blocker per its `trigger_conditions(a)`); A.2 (DW-132) is one-orchestrator-per-PR (blast-radius); A.3 (DW-130) is **Defect-3-ONLY** (D1 CLOSED ACT-277, D2 CLOSED MIG-111/ACT-274 — DW-148-style residual adoption). Insider (B.1 DW-093 / B.2 DW-094) cross-references FP-042/DEC-044 for provenance; catalyst (C.1 DW-097 / C.2 DW-098) cross-references FP-049 — FP-042 and FP-049 are NOT reopened. DW-094 homing **DETERMINED post-paper §6.5 NON-CRITICAL imputation** (operator-flip-to-pre-live escape at activation time). Builds deferred to separate authorized execution turns per sub-step. **Adopted as named sub-steps (7):** DW-114, DW-132, DW-130-D3, DW-093, DW-094, DW-097, DW-098. **DROPPED from scope:** ~~DW-095~~ — **CLOSED by FP-045 Phase 4 (MIG-085, `signal_registry status='live'` for `options_flow_imbalance_5d`)** at 2026-06-10; not adopted (annotation-only, original mention preserved above per Constitution Rule 8 / DW-148 normalization pattern). | NTB substrate fidelity |
...
- **FP-059 "Signal Substrate Hardening"** — **AUTHORED 2026-06-29 at ACT-369** (pre-reserved by ACT-365; slotted into its held number, NOT next-free at authoring). Adopts **7 DWs** unified-but-sub-clustered: DW-093 / DW-094 / DW-097 / DW-098 / DW-114 / DW-130 (Defect-3-ONLY) / DW-132. ~~DW-095~~ **DROPPED — CLOSED by FP-045 Phase 4 (MIG-085, 2026-06-10);** original adoption-list mention preserved per Constitution Rule 8 / DW-148 normalization. See FP-059 charter for DW-094 homing determination + sub-cluster ladder + STOP conditions.
...
- **Signal-substrate hardening** under FP-059 (per STEP 0): DW-093 / DW-094 / DW-097 / DW-098 (~~DW-095 DROPPED — CLOSED by FP-045 Phase 4 / MIG-085 at 2026-06-10; ACT-369 annotation~~). Cross-signal substrate items (DW-114 Tier-A anchor, DW-132, DW-130-D3) sequence within FP-059's sub-cluster A — DW-114 is a Phase-6-or-earlier pre-live blocker, not STEP 3.
