...
| **Status** | open (Defect 1 + Defect 3); Defect 2 closed in MIG-111. |
...
| **Status** | Partially closed (primary defect closed by MIG-112 / ACT-275; forward regression guard `scripts/check-rbac-helper-grants.ts` queued here). |
...
| **Cross_ref** | DW-119 (the recreate whose missed grant re-issue is the regression source); MIG-109 (DW-119 closure migration that did NOT include the GRANT block — the gap); MIG-112 (this primary fix); ACT-275 (execution); `sql/02_rbac_security_helpers.sql` (canonical source with co-located grants now); `supabase/functions/_shared/authorization.ts` `checkPermissionOrThrow` (the consumer that mapped 42501 -> 403); `docs/ai-failure-modes.md` Catalog rule on SECURITY DEFINER recreate (codifies the binding rule forward). |
