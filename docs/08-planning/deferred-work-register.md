...
| D | ACT-572 (**new — 2026-07-24; charter**) | IBKR shadow lane — isolated mirror of Alpaca admits/exits via CP Web API; own tables (`ibkr_shadow_lots/orders/equity/reconciliation_events`), own kill-switch (`ibkr_shadow_enabled` default false), own crons (`overshoot_ibkr_shadow_mirror` + `_reconcile`); NEVER feeds money path; CI separation guard for `_shared/overshoot-shadow-ibkr/*` imports; nightly comparator → per-fill evidence rows (spread/slippage/borrow); ACT-565 baseline | **Weekend skeleton + operator-secrets gate** (§4 TO-DO: IBKR paper account + CP Gateway URL + 5 named secrets, then flip `ibkr_shadow_enabled=true`; dormant until then) | 2026-07-24 | after ACT-565 lands + operator §4 completes | `docs/06-tracking/ACT-572-charter-ibkr-shadow-lane.md`; ACT-565 (baseline evidence); Phase-L (decision consumer); Constitution Rule T5 |
