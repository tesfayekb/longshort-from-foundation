...
| **Status** | open (mechanism CONFIRMED; fix is a dedup/disambiguation rule for the no-exclusion variant — not yet designed). |
...
| **Cross_ref** | ACT-409 (REGISTRATION); FP-062 Tuesday-AM cycle watch (the surfacing); `shadow-ranker-orchestrator.ts:267-270` (the throwing assertion + the all-12-variants blast-radius site); `shadow-ranker.ts:14,113,~195,~207` (the no-exclusion symmetric composite + long/short rank assignment with mirrored ticker-ASC tiebreaker — the overlap source); `combiner_book_shadow` → `combiner_forward_returns` (the starved substrate); `LABEL_MIN_SEED_DATES`/`TRAINING_HORIZON_TD` (the Phase-3.3 gate the substrate feeds); ACT-288 (the trainer skip-gate). |
