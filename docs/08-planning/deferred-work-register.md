...
| **Status** | open — **resolution-path RATIFIED 2026-06-26 at ACT-338 via [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (b) (planner reads working orders + positions via the EXISTING `reconstructInFlight(ts)` path; honors E3 SURFACE-1 invariant; cancel-storm + pending-intent-ledger considered-and-rejected with rationale). Landing site: [FP-057](feature-proposals.md#fp-057) Sub-step 2 (working-order visibility + ranking-freshness gate land together).** Closes on FP-057 Sub-step 2 commit. **CADENCE-REBUILD PREREQUISITE** (STREAM-3) — tick-driven rebalance cannot ship until the planner can see what's already working at the broker, otherwise a 5-minute tick would re-place over already-open orders. NOT immediately blocking for the once-daily un-pause (the daily cadence resolves naturally between fires). |
...
| **Cross_ref** | The conformance audit (ACT-pending — accidental-discovery surfacing); STREAM-3 cadence-rebuild parent; ACT-336 (the un-pause investigation that surfaced the gap); **ACT-338 (resolution-path ratification — [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (b) + [FP-057](feature-proposals.md#fp-057) Sub-step 2)**; `supabase/functions/_shared/longshort-execution/rebalance-submit-orchestrator.ts:256-257` (the blind spot anchor); CROSSWIND §4.4 / §122 (tick-cadence intent). |
