...
| **Attestation** | Open. Closure SHA + live-DB read confirming `enabled=true` for all 5 ids + Gate-15 sentinel re-run showing migration-derived state now matches live state. Cross-references: INC-43 (the surfacing entry); MIG-058 (the §22.5.1 binding-evidence discipline pattern this migration applies); `supabase/migrations/20260525093303_*.sql` + `supabase/migrations/20260525103115_*.sql` (the seed migrations whose `enabled=false` was overridden out-of-band). |
