...
| **Cross_ref** | FP-055 (the FP that authored this DW); ACT-302 (the landing action); DEC-032 clause-4 (the `longshort.execute` permission key that is separately gated — Phase 8 also authors that); DW-046 (the paper-exec layer that is upstream of any leverage relaxation); CROSSWIND §1 L95 / L155 (the spec invariant being superseded); `supabase/functions/_shared/longshort-targets/target-position-builder.ts` `LEVERAGE_PAPER_LOCK` constant + `LeverageLockViolationError` (the code surface the DEC must coordinate with). |
