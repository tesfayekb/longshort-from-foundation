...
|---|---|
| **ID** | DW-199. |
| **Status** | **REGISTERED 2026-06-29 at ACT-378 — BROKER-GATED, mirrors DW-196.** 4M.4 ships the soft-dependent `createInternalCorporateActionStatusFetcher` (`internal-corporate-action-status-fetcher.ts`) that reports `broker_basis_adjusted = (corporate_actions.applied_at IS NOT NULL)` — a TAUTOLOGICAL internal correctness proof ("the applier ran"), NOT a real broker cross-check. The verifier `verify_corporate_action_clean.ts` consumes the internal substate via its 4-outcome time-windowed contract (24h/48h). Real broker cross-check waits on the live Alpaca client surface. |
