...
**Future split trigger (UPDATED to reflect 7-instance population):** Instance density now 4-of-7 platform/tool-behavior-shaped (instances #3 + #4 + #5 + #6) vs 2-of-7 schema/column/path-shaped (#1 + #2) + 1-of-7 supervisor-verification-tier-shaped (#7). Original split trigger said "if instance #4 also §22.8.5-platform-constraint-shaped". Now 4 of 7 instances match the platform/tool-behavior shape (3 of them: #3 §22.8.5(b); #4 + #5 + #6 Vitest/test-runner behavior — closer to §22.3 (l) scope coverage of test-runner output states than §22.8.5 verbatim). Tighter framing: **(a) §22.8.5-explicit-constraint-shape** = #3 only (1-of-7); **(b) test-runner/build-tool-behavior shape** = #4 + #5 + #6 (3-of-7); **(c) schema/column/path-reference shape** = #1 + #2 (2-of-7); **(d) supervisor-discipline-application shape** = #7 (1-of-7). Supervisor recommends preserving unified DW-077 discipline pending operator decision: the four shapes share the same root cause (artifact-authoring without grep-verification of relevant state) per §22.3 (l) intent. If operator prefers tightened taxonomy, splitting candidates are: DW-078 narrower-discipline for shape (b) test-runner/build-tool-behavior coverage; DW-079 narrower-discipline for shape (d) supervisor-discipline-application. Decision deferred to operator. Lineage preserved for future decision-making with full instance breakdown. |
