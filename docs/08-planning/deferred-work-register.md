...
>
> **Catalog #65 — NO ARTIFACT, NO ASSERTION (binding both agents, adopted 2026-07-23 alongside INC-131).** (a) Any claim of the form "X = Y" about a register row, ratified constant, schema shape, or file content MUST be accompanied in the same turn by the fresh `grep` / `SELECT` line that proves it — otherwise it is phrased "unverified; checking". (b) Any claim that something was "filed / landed / updated" MUST carry the commit sha; when the writing agent cannot mint a sha under sandbox git-write restrictions, that constraint must be stated explicitly and the file-edit diff surfaced instead. (c) The un-greppable default is STOP. This extends the existing Standing Format Rule (quantitative claims + `date -u` timestamp) to identity claims, closing the lane all five 2026-07-23 fabrications (INC-130 / INC-131) drove through.
...
| D | DW-225 | wall-clock scanner overshoot-scope extension | after step (a) annotations land | 2026-07-22 | 2026-07-29 | step (a) SCAN-ONLY report: `docs/06-tracking/artifacts/DW-225-wall-clock-overshoot-scan.md` (29 hits, 0 Tier-A defects). Step (b) `U`-class dispositions (reconcile.ts:65, OvershootOverview.tsx:101) filed in `docs/06-tracking/artifacts/receipts-2026-07-22-corrections.md` §(5); annotations + scanner arm (step c) queued behind receipts. UI-clock policy split as DW-225.a. |
| D | DW-229 (**new — 2026-07-23**) | Universe-chip cadence-aware unit tests (fresh ≤9d / stale >9d / alert >35d thresholds on `UniverseFreshnessChip`) — the concrete test-writing item that the 2026-07-23 UI truth pack introduced; DISTINCT from ACT-565 (which is the IBKR-vs-Alpaca decision-prep artifact and was mis-conflated with chip tests in the fabricated last-turn queue per INC-131) | small — docs/receipts window | 2026-07-23 | 2026-07-30 | `src/features/overshoot/components/OvershootUniverse.tsx` (chip component landed 2026-07-23); cadence source `DEC-504-4-AMENDMENT` (26d SI window) + universe-refresh cadence policy |
| D | DW-230 (**new — 2026-07-23; PROPOSAL — operator veto pending**) | Typed lifecycle/exclusion flag on `overshoot_lots` for the 4 stranded 2026-04 test-cohort rows (Rule-8-compliant, house-style typed-absence; **do NOT delete rows** per operator ruling on INC-131 turn). **CONDITIONAL on first pasting** the 4 lot_ids + entry context proving test-cohort origin AND the 6-surface `WHERE lifecycle_class IS DISTINCT FROM 'test_cohort_exempt'` audit (surfaces: `OvershootPnL`, `useOvershootRealizedToday`, `useOvershootPortfolioPositions`, `overshoot_dial_daily` view, portfolio reconciliation banner, `overshoot-equity-snapshot`). No row edits beyond the new column. | proposal — operator one-word veto | 2026-07-23 | 2026-07-25 | operator ruling text 2026-07-23 turn; superseded prior fabricated wording per INC-131 |
| D | FIX-2-NOTE-01 (**new — 2026-07-23; register anchor for FIX-2 motivating instance**) | VICR's 2026-07-23 senior-close realized **−14.67% / −1467 bps** materially includes ~3 sessions stranded past horizon by genuine-stale SI refusals (ordinals 8–10 refused as `si_above_squeeze_threshold` under pre-FIX-1 20d window, then repriced into weakness after the DEC-504-4-AMENDMENT 21→26d flip landed at ordinal 11). Cite this row verbatim in tomorrow's FIX-2 deploy note as the concrete motivating instance: ~3 sessions of forced hold on VICR under stale-refusal semantics ≈ −900 bps of the −1467 bps outcome. Distinct row from DW-227 (typed `snapshotAgeMs`) — this is the *motivating story*, DW-227 is the *typed rewrite*, FIX-2 (tomorrow) is the *behavioural fix*. | opportunistic — folds into FIX-2 deploy note | 2026-07-23 | 2026-07-24 | 07-23 senior-tail heartbeat receipts; `_shared/overshoot/si-freshness.ts` (DEC-504-4-AMENDMENT); DW-227 (typed rail) |
| A | DEC-084 (**new — 2026-07-24**) | overshoot short-daily-pacing gate (`overshoot_short_daily_budget` default=1; typed `short_daily_budget_reached`; TERMINAL in FIX-8 allow-list) | LIVE — `SOURCE_VERSION=fb5fdf13+fix2+fix8+sp1`, echo probe-verified pre-13:30Z 07-24; clone 9b033d5f | 2026-07-24 | — | `docs/decisions/DEC-084-overshoot-short-daily-pacing.md`; `_shared/overshoot-execution/daily-budget.ts`; `_shared/overshoot-execution/completion-pass-allow-list.ts`; `overshoot-entry-run/index.ts` |
| A | FIX-9 (**new — 2026-07-24**) | pass-scoped idempotency gate in `overshoot-entry-run` (marker query gains `AND COALESCE(metadata->>'pass','primary') = ${passLabel}`; DST dual-slot preserved; legacy-null-marker treated as primary) | LIVE — `SOURCE_VERSION=fb5fdf13+fix2+fix8+sp1+fix9`; maiden manual completion invoke returned clean `budget_exhausted_pre_loop` no-op (correlation `69bcbd09-…`); Monday 14:05Z scheduled fire = routine | 2026-07-24 | 2026-07-27 (scheduled-fire proof-point) | `docs/04-modules/overshoot/fix-9.md`; `overshoot-entry-run/index.ts` (~L612+, :863 marker); `index_test.ts` (7 new pins; 73/73 green); INC-136 charter-before-build |
| D | ACT-571 (**2026-07-25 — BRIDGED-OPTION-A / BUILD-COMPLETE**) | overshoot universe-refresh source re-point Polygon `I:RUT` → IVV+IJH composite landed (reuse `parseIsharesCsv` + fetch primitives from ACT-548); sanity `[850,950]` unchanged; provenance `source='ivv_ijh_composite'`; probe-only `I:RUT` retained; 7-test §3 set GREEN; deployed `0c5ad0d9`. Operator ruling **OPTION A-PLUS** (2026-07-25 05:17Z): FMP Ultimate ($139/mo, ETF Holdings) rejected on ROI (DEV-10 tier-correct); operator monthly ~5-min ritual (browser download IVV+IJH → ACT-548 `probe:'seed_apply'`) is the accepted primary. **INC-141-b FIXED this turn**: every default-path `!ok` early-return now writes typed `overshoot.universe.refresh.refused` audit row (helper `writeUniverseRefusalAudit`; grep-lock test `inc141b-refusal-audit_test.ts`; 11/11 GREEN; deployed). Monday 10:00Z expected outcome re-pinned: `{ok:false, status:'both_sources_failed'}` + 1 audit row + zero writes = GREEN. Roster stays 905 @ 07-21 until next seed (~2026-08-04). DW-237 filed (SSGA SPY+MDY XLSX probe returned real spreadsheets 54.4KB/47.3KB — durable free auto-lane candidate for future build slot) | HARD gate satisfied (loud-fail contract in place) | 2026-07-24 | 2026-07-25 | `docs/06-tracking/2026-07-25-act-571-build-and-cdn-block.md` (Operator Ruling section); `docs/06-tracking/ACT-571-charter-refresh-repoint.md`; INC-140 (open, downgraded); INC-141-b (fixed); DEV-9/DEV-10; DW-237; ACT-548 (seed path — primary) |
| D | DW-237 (**2026-07-25 — FILED**) | SSGA composite as durable free auto-lane candidate for `overshoot-universe-refresh`. Probe: SPY XLSX `www.ssga.com/us/en/intermediary/library-content/products/fund-data/etfs/us/holdings-daily-us-en-spy.xlsx` → 200/54,400B/`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`; MDY same path → 200/47,318B same type; `file` = `Microsoft OOXML`. 301→CDN redirect required (`www.ssga.com/library-content/...`). SPY∪MDY ≈ IVV∪IJH by identity (S&P 500 + MidCap 400). Not built this turn (operator ruling: manual ritual accepted; auto-lane deferred). Requires XLSX parser + sheet-shape reverse-engineering + charter amendment + tests | future weekend slot | 2026-07-25 | TBD | `docs/06-tracking/2026-07-25-act-571-build-and-cdn-block.md` (DW-237 section) |
| D | ACT-572 (**skeleton LANDED 2026-07-25**) | IBKR shadow lane skeleton delivered: four isolated tables (`ibkr_shadow_lots/orders/equity/reconciliation_events`) migrated with grants/RLS (service_role writes, `overshoot.view` reads); kill-switch `ibkr_shadow_enabled=false` seeded in `system_config`; edge fn skeletons `overshoot-ibkr-shadow-mirror` + `overshoot-ibkr-shadow-reconcile` deployed dormant (`SOURCE_VERSION=act572-skel-v1`, POST returns `{status:'shadow_disabled'}`, OPTIONS echoes `x-source-version` — probe verified 2026-07-25 17:21Z); shared client stub `_shared/overshoot-shadow-ibkr/cp-client.ts` with typed error surface + 5 named secret constants + IBKR CP endpoint locks; CI separation guard `scripts/check-ibkr-shadow-separation.ts` (5/5 tests GREEN, tree scan 0 violations); ACT-565 §5 comparator schema referenced verbatim | **DORMANT — awaiting operator §4 TO-DO**: (1) open IBKR paper account, (2) enable CP API, (3) enable equities market data, (4) provision CP Gateway URL, (5) set 5 secrets `IBKR_PAPER_GATEWAY_URL/ACCOUNT_ID/USERNAME/PASSWORD/SESSION_TOKEN`, (6) confirm in chat then flip `ibkr_shadow_enabled=true`. Phase-1 wiring (live CP fetch + nightly comparator) NOT included in skeleton | 2026-07-24 | operator-gated | `docs/06-tracking/ACT-572-charter-ibkr-shadow-lane.md`; `sql/43_act_572_ibkr_shadow_tables.sql`; `supabase/functions/overshoot-ibkr-shadow-{mirror,reconcile}/index.ts`; `supabase/functions/_shared/overshoot-shadow-ibkr/cp-client.ts`; `scripts/check-ibkr-shadow-separation{,_test}.ts`; ACT-565 §5 (comparator schema); Constitution Rule T5 |
| C | L-05 DoW-pacing (**STEP-0 CORPUS PRIOR TEST — FAIL 2026-07-25**) | quarantine gate per operator ruling: n≥10,000/DoW ✓, but Wed lift = **−0.36 bps** (required ≥ +25), non-monotone (Wed < Fri), analytic 95% CI [−2.05,+1.33] straddles zero, non-robust across 2022–2025 (Wed best in only 1/4 years), earnings-scrubbed residual = **−1.17 bps**. **NOT-A-SIGNAL** under Constitution Rule 8. **$25K/yr projected prize RETRACTED on the record.** ACT-576 §B menu top prize now L-01 ($11k) + L-02 ($6.6k). Data-snooping caveat governs any future DoW re-slice | CLOSED-REJECTED | 2026-07-25 | 2026-07-25 | `docs/06-tracking/L-05-step0-corpus-prior.md`; `docs/06-tracking/charters/L-05-dow-pacing-quarantined.md`; ACT-576 §B (menu source); ACT-570 Phase-1 Stage-2 §Q4 (data-snooping precedent) |
| D-PHASE-1-DELIVERED | ACT-573 (charter + Phase-1 landed 2026-07-25) | refused-winners forensics — SQL/study of REFUSED short-candidate pool (12 sessions); apples-to-apples per-tier entry/exit basis; status-quo-bias tie rule pre-committed | **Weekend Phase-1 SQL + corpus-graduation gate** (INSUFFICIENT-N tags on thin cells; verdict = adopt / hold / operator ruling) | 2026-07-24 | weekend | `docs/06-tracking/ACT-573-charter-refused-winners-forensics.md`; `docs/06-tracking/ACT-573-phase1-refused-winners-forensics.md` (Phase-1 delivery, 2026-07-25); ACT-569 (short-arm feed) |
| D-PHASE-1-DELIVERED | ACT-574 (charter + Phase-1 landed 2026-07-25) | entry-day-offset grid — Phase-1 verdict: current config (T1@T+2, T2@T+1) WINS/TIES its column, status-quo holds, $/yr at BLEND = $0; Phase-2 regime×year split + T1 bootstrap-CI queued (DEV-13/14) | **PHASE-1 CLOSED status-quo** — no R-1 amendment; Phase-2 non-blocking | 2026-07-24 | Phase-2 mid-week | `docs/06-tracking/ACT-574-charter-entry-day-offset-grid.md`; `docs/06-tracking/ACT-574-phase1-entry-day-offset-grid.md` (Phase-1 delivery, 2026-07-25) |
...
| C | ACT-515 (kernel bring-up) | **RE-SEQUENCED 2026-07-25 per operator ruling B3.** Engine kernel is multi-turn workstream: `types→clock→admit→size→mark→exit→equity/DD`, one module per turn with unit tests. Layer-1 gate = selection-parity replay GREEN + hand-truth fixture #1 (`fixtures/overshoot-backtest/2024-05-02-hand-truth.jsonl`, sha `d06bd24c...`) byte-exact GREEN + hand-truth fixture #2 (2023-Q2, built during `mark`/`equity` bring-up) byte-exact GREEN. Matrix computes AFTER kernel + BOTH fixtures green. Parallel to SQL-only weekend chains (ACT-573/574/570). | kernel-modules 5–10 turns + fixture-2 build + per-config chains ~8 turns | 2026-07-25 | **mid-week 2026-07-29..31** | `scripts/act-515/compute-plan.md §5 B3`; `scripts/act-515/hand-truth-fixture-verification.md`; INC-144 |
| C | ACT-515(e)-sector-ingest | **NEW 2026-07-25 per B2 amendment.** Sector-cap config (e) is chartered with pre-committed levels {uncapped, 30% notional/sector, 20% notional/sector} but BLOCKED on sector-metadata ingest — `overshoot_universe` has no sector column; longshort `universe_membership.gics_sector` is wrong universe (fabricated-classification risk). Ingest path: FMP `/stable/profile` primary + iShares CSV verification cross-check. Adds `sector_source`/`sector_asof` provenance. ~3 turns (MIG + ingest + backfill/verify). No fabricated GICS strings. | operator OK on ingest path + build slot | 2026-07-25 | operator-gated | `scripts/act-515/charter-amendment-e-sector-cap.md §3-§4` |
| C | ACT-509 Stage-2 (exit-minute leg) | **ELEVATED 2026-07-23; AMENDED same day** — operator variant correctly encoded: SAME-DAY-EARLIER exit on ordinal-10 at {09:31,09:35,09:45} (NO extra overnight); freed cash funds same morning's 09:35 admission wave. Three-term REVISED: `net(m) = redeploy_gain − morning_exec_cost(m) − final_day_forfeit`. **Pre-screen delivered TONIGHT** on daily bars (n=523,646): winsorized ALL mean = **+3.05 bps** (long +4.66 / short +1.46) — well under +20 bps failure threshold → **VARIANT ALIVE**; Stage-2 prices only execution term. Regime caveat: 2024 flat / 2023 hot. Live 25-lot cross-check thin (n=18, survivor-biased). Alpaca paper/margin same-day reentry non-blocking. Priced prize = ACT-558 v4 blend $2.98K/yr / floor $12.0K/yr. | sequenced-immediately-after-engine-verdict (Monday) | 2026-07-23 | days-after-engine | `docs/08-planning/artifacts/ACT-509-STAGE2-EXIT-MINUTE-LEG-CHARTER.md`; `docs/08-planning/artifacts/ACT-509-STAGE2-PRESCREEN-final-day-forfeit.md`; corpus 3.1M 1-min bars |
...
| A | INC-129 + DEC-504-4 AMENDMENT | (a) cron pickup lag (twice-monthly → daily Mon-Fri) + (b) 21d→26d cadence-aware staleness cap; ritual DEC-504-4 engage extinguished; motivating instance = 2026-07-22 sleeve-engage (would not have occurred under amended cap) | code+tests+docs landed 2026-07-23 pre-receipts; sql/30 authored (operator applies ~13:55Z 2026-07-24); one-shot invoke ~14:00Z 2026-07-24; UI cadence-aware chips fold next turn alongside receipts | 2026-07-23 | 2026-07-24 (DISENGAGE acceptance test — freshest=07-15 → age 9 → sleeve DISENGAGES) | `_shared/overshoot/si-freshness.ts` (constant 21→26 + amendment block); `si-freshness_test.ts` (age 24/26/27 + default=26); `sleeve-reallocation-writer_test.ts` (date re-anchor); `sql/30_overshoot_short_interest_cron_schedule.sql` (schedule + header amendment); `docs/08-planning/artifacts/DEC-504-4-AMENDMENT-cadence-aware-si-staleness.md`; `docs/06-tracking/incidental-findings.md` INC-129 (a)+(b) |
...
| C | DW-212-CONF (**legacy cross-check**) | 06-30 short-book `reversal_1w` sanctioned-null producer-vs-consumer timestamp reconciliation | opportunistic; unblocks DW-212 forensic closure | 2026-07-22 (surfaced 2026-07-02) | 2026-08-15 | `deferred-work-register.md:5014` (original defer note); manual-chain audit for 2026-06-30 |
| E | curl-tool-401 (**legacy cross-check**) | Lovable curl-tool always-injects-preview-JWT → 401 on cron slugs | **SUPERSEDED-BY** (a) `supabase--curl_edge_functions` caller-supplied header pass-through, (b) `_shared/cron-auth.ts` X-Cron-Secret branch (INC-97 F1) | 2026-07-22 | — | `supabase/functions/_shared/cron-auth.ts:18-36`; `incidental-findings.md:1542,1545` |
| D | DW-225 (**new — legacy cross-check**) | Wall-clock scanner overshoot-scope extension | pre-committed "DONE-with-receipt" **REJECTED**: scanner still longshort-only per `check-wall-clock.ts:43-48` + `banned-patterns.md:18`; ACT-554-a is analyst backfill, not a scanner extension; filed OPEN under INC-125 raw-evidence rule | 2026-07-22 | 2026-08-15 | `scripts/check-wall-clock.ts:43-48`; `docs/banned-patterns.md:18` |
| A | DW-226 (**new**) | INC-125.b decoupled book-level SI corpus-MAX read + two-constant design annotation | landed 2026-07-22 tonight; PR bundles (i) unwindowed `SELECT MAX(as_of_date) FROM overshoot_short_interest` in `overshoot-detection-run/index.ts` (replaces the per-row-map derivation that returned NULL whenever corpus-MAX exceeded the 20d envelope), (ii) TWO-CONSTANT DESIGN header blocks in `_shared/overshoot/si-freshness.ts` and at `overshoot-detection-run/index.ts:126` documenting `DETECTOR_SI_STALENESS_MAX_DAYS=20` (per-row usable-data envelope) vs `OVERSHOOT_SI_STALENESS_MAX_DAYS_DEFAULT=21` (book-level staleness flag, strict `>`), (iii) three CI ratchets in `index_test.ts` (INC-125.b a/b/c), (iv) `sleeves.prior` echo on transition edges, (v) Rule-8 correction-note stamp on audit row `68f54ed3` via data migration (immutable original reason left in place), (vi) SPY idempotent backfill tail in `overshoot-equity-snapshot/index.ts` + 07-22 one-shot (747.41). **Supervisor note (on the record):** the "~587 events found SI in per-ticker joins" inference in the root-cause demand was wrong — the windowed map was empty; the funnel masked it. The durable diagnostic is CORPUS-MAX-vs-FRESHEST divergence, annotated in both header blocks. | 2026-07-22 | — | `supabase/functions/_shared/overshoot/si-freshness.ts` (header); `supabase/functions/overshoot-detection-run/index.ts` (~L126 + L586); `supabase/functions/overshoot-detection-run/index_test.ts` (INC-125.b a/b/c); `supabase/functions/overshoot-equity-snapshot/index.ts` (SPY tail) |
| A | INC-125.b (**new — sub of INC-125**) | dial-class fabrication vector: corpus-vs-window-map coupling misfired `si_corpus_absent` | closed by DW-226 fix; standing rule for future readers → CORPUS-MAX-vs-FRESHEST divergence is the durable tell (never re-diagnose from scratch) | 2026-07-22 | — | `_shared/overshoot/si-freshness.ts` (header block); DW-226 row above |
| A | DW-225.b (**new — step (b) of DW-225**) | scanner overshoot-scope annotation companion to INC-125.b (fold pending) | small — folds onto next scanner-extension touch under DW-225 rollup; no code change tonight (DW-225 remains OPEN for step (b) + (c)) | 2026-07-22 | 2026-08-15 | DW-225 row above |
...
| state | count |
|---|---|
| A (DONE-with-receipt)   | 17 (+DW-226, +INC-125.b, +DW-225.b) |
...
| **Addendum — DW-058-B2 PROBE FINDING (2026-06-29, ACT-400; Rule-8 additive — original fields PRESERVED VERBATIM)** | FP-062 sub-step **6I.1b** executed read-only probe against the in-hand Polygon tier using a one-shot operator-supplied `POLYGON_API_KEY_PROD_PROBE`. Endpoints probed: `/v1/marketstatus/now` (exchange-level open/closed only — `nasdaq: open`, no ticker halt), `/v3/snapshot?ticker.any_of=AAPL` (per-ticker `market_status: "open"` coarse string; **no `halted` / `luld_indicator` / `trading_status` boolean**), `/v2/snapshot/locale/us/markets/stocks/tickers/AAPL` (no halt field), `/v3/reference/tickers/AAPL` (`active: true` — static metadata, not real-time), `/v2/last/trade/AAPL` + `/v3/trades/AAPL?limit=1` (trade `conditions: [14,41]` / `[37]` — halt/LULD/T1/T12 status only **inferable by decoding per-trade condition codes**, with halt-edge latency = trade-tape gap detection). **Verdict: PARTIAL — HALT-FIELD INFERRED-ONLY-VIA-CONDITION-CODES under the in-hand subscription.** No dedicated halt boolean exists at any probed REST endpoint. Clean per-second halt signal requires the SIP halts WebSocket feed (Stocks Advanced+ tier) — **Phase-7 / tier-upgrade-gated CONFIRMED.** **Disposition:** keep `verify_halt_status` tick-dispatch **unwired** (existing intentional gating preserved); `AlpacaHaltStatusFetcher` remains structurally phantom for live-order code paths per the original B2 wording ("phantom non-halt fetcher is structurally worse than no halt check"). **Sub-step 6I.1b CLOSED as PROBED-DEFERRED** — probe deliverable complete; substantive halt-fetcher wiring deferred to Phase-7 tier-upgrade FP (or, as documented fallback option, a condition-code degraded-mode fetcher chartered separately if operator elects pre-tier-upgrade partial coverage). Original Required-Tests-for-Closure item B2 wording UNCHANGED; this addendum records the probe finding that grounds the Phase-7 homing as evidence-backed rather than presumed. |
| **Addendum — DW-058-B2 PROCUREMENT-FRAMING CORRECTION (2026-06-30, ACT-416; Rule-8 additive — DW-058-B2-ADD-01 above and all original fields PRESERVED VERBATIM; this addendum is APPENDED, NOT a rewrite, per the clause-(p) preserved-record precedent)** | The DW-058-B2-ADD-01 (ACT-400 probe addendum, immediately above) sentence *"Clean per-second halt signal requires the SIP halts WebSocket feed (Stocks Advanced+ tier) — Phase-7 / tier-upgrade-gated CONFIRMED"* is **REST-only-probe-scoped** and is **FALSE as a procurement claim** under grep-verified facts at HEAD `0e9ac02a`: **(i)** Polygon ships **NO dedicated halt-boolean REST field on ANY tier** — the ADD-01 probe exhaustively enumerated every relevant REST surface (`/v1/marketstatus/now`, `/v3/snapshot`, `/v2/snapshot/.../tickers/...`, `/v3/reference/tickers/...`, `/v2/last/trade`, `/v3/trades`) and none expose a halt boolean; this is a vendor-architecture property, NOT a subscription gate, so "tier-upgrade-gated" is not a real procurement remedy. **(ii)** We **already own Polygon "Stocks Advanced"** — DEC-068 clause (r) line 635 ratifies the tier; `supabase/functions/_shared/longshort-execution/broker-bootstrap.ts:246` consumes it today via `PolygonQuoteFetcher` for the real-time SIP-consolidated NBBO quote path. The tier we hold includes the WS trade tape on which halt-class condition codes (T1 resumption / T12 paused / LULD / regulatory) flow in real-time. **(iii)** There is **no Polygon tier above Advanced that adds a halt boolean** to substitute for WS condition-code decoding; "Stocks Advanced+" as a procurement target does not exist as a halt-coverage solution. **(iv)** The canonical halt mechanism on every Polygon tier is **real-time per-trade condition-code decoding on the WS trade tape** — which is a BUILD on the tier we hold, NOT a BUY on a tier we don't. **DISPOSITION CORRECTION:** the pre-live discharge is a BUILD (FP-064, chartered concurrently at ACT-416 — Polygon Advanced WS condition-code halt fetcher with mandatory WS-probe-first Step A, scheduled to START at FP-062 gap-(e) discharge), **not a procurement**. The paper-window posture is **typed-absence-on-paper, NOT-LIVE-ACCEPTABLE**, ratified at **DEC-068 clause (s)** (POSTURE / RATIONALE / SCOPE-BOUNDARY / 3 GUARDRAILS; mirrors clause (n) / DW-154 SSR twin); clause (s) GUARDRAIL 3 names FP-064 closure as a hard prerequisite for live-fire ratification. **STATUS TRANSITION:** DW-058-B2 moves from `open / PROBED-DEFERRED / procurement-pending` → **`open-RATIFIED-V1-TYPED-ABSENCE-PAPER-NOW-PENDING-FP064-BUILD-PRE-LIVE`** (mirrors the `open-CONTAINED` shape used for DW-204; the deferred work persists until FP-064 closes for live, but the paper-window disposition is RATIFIED and gap (c) of the FP-062 closure roll-up is DISCHARGED). **THE ACT-400 ADDENDUM (DW-058-B2-ADD-01) IS NOT EDITED** — it is preserved verbatim as the audit record of the REST-only probe finding (clause-(p) preserved-record precedent); this addendum is the framing correction the audit record sits under, NOT a redaction. **Cross-references:** DEC-068 clause (s) (ACT-416 — paper-v1 halt typed-absence posture); DEC-068 clause (r) line 635 (Stocks Advanced tier confirmation); DEC-068 clause (n) / DW-154 (SSR twin precedent); FP-064 (ACT-416 — pre-live halt-coverage BUILD; WS-probe-first Step A; scheduled at FP-062 gap-(e) discharge); FP-062 gap (c) (DISCHARGED at ACT-416); ACT-400 (the REST-only probe whose framing this corrects); ACT-416 (this correction's authoring action); `broker-bootstrap.ts:246` (the in-code tier-ownership anchor); CROSSWIND §11.0.7 #6 (the `verify_halt_status` contract FP-064 Step B will wire against). |
...
| **Cross-reference addendum (2026-06-29, FP-062 session)** | The gap DW-086 addresses for the single `liveness_check.stop` metric is generalized across all armed longshort bypass-crons by **DW-200** (push-alert enrollment + `cron.job_run_details` dispatch-vs-outcome cross-check). DW-086 remains its own narrow entry (liveness-check STOP rung-(b)); it is NOT absorbed into DW-200. When DW-200's pipeline-enrollment lands for the reconciliation surface, DW-086's liveness-specific rung is satisfied by the general mechanism — at which point DW-086 may be marked resolved-via-DW-200. Original DW-086 scope preserved verbatim above. |
...
| **Scope extension (2026-06-29 — FP-062 6I.3c / ACT-393 closure; additive per Constitution Rule 8)** | DW-138 now ALSO covers the `longshort-reconciliation-tick` handler's internal-expected reconciliation inputs (`expected_bp` + `verify_position` `expected_qty` / `expected_cost_basis` / probe `symbol`) — currently the `BOOTSTRAP_PLACEHOLDER_*` named constants in `supabase/functions/longshort-reconciliation-tick/index.ts` (values unchanged: `100_000`, `'AAPL'`, `0`, `0`) that correctly escalate `failure_escalated` pre-first-trade against the live paper account (~$321K BP, no positions). These resolve to real internal equity/position state when DW-138's stub-capital → real-equity wiring lands and a `longshort_equity_snapshots` row exists post-first-rebalance. **Rejected (do NOT do)**: seeding the expected side from a broker snapshot — that makes expected==observed by construction (tautology) and defeats drift detection (§9 anti-phantom: visible sentinel → invisible tautology). The only legitimate internal-expected source is our own ledger. Surfaced at FP-062 6I.3c HEALTHY-CONFIRMED closure (ACT-393) when the reconciliation cron first fired against live paper state — the engine is firing + watching + escalating TRUE-positives; PASS-class outcomes deferred to natural resolution post-DW-138. Cross_ref adds: **FP-062 6I.3c / ACT-393** (the closure surfacing this scope); `supabase/functions/longshort-reconciliation-tick/index.ts` (the new SENTINEL surface — `BOOTSTRAP_PLACEHOLDER_EXPECTED_BP_USD` / `_PROBE_SYMBOL` / `_EXPECTED_QTY` / `_EXPECTED_COST_BASIS`); **ACT-389** (the cron-only handler-auth conversion that made the tick fire); **MIG-145 / ACT-387** (the arming that exposed this stub against live state). |
...
#### DW-148 CLOSURE (ACT-337, 2026-06-26) — feed-accuracy gate satisfied; CLOSED

| Field | Value |
|-------|-------|
| **Closing action** | ACT-337 — STEP B–D of the feed-fix charter: `PolygonQuoteFetcher` (edge + src parity) implementing `BrokerQuoteFetcher` over Polygon `/v2/last/nbbo/{ticker}` (consolidated SIP NBBO; nanos-`t` → epoch-ms tested explicitly against `verify_quote_freshness`); injected at `broker-bootstrap.ts:194` behind a `LONGSHORT_QUOTE_FEED=alpaca` emergency-revert env flag. Polygon "Stocks Advanced" tier verified real-time SIP-consolidated (STEP-A gating probe). |
| **STEP-C re-measurement (RTH 2026-06-26 16:12:36Z; corr `2e0413ee`)** | Read-only probe over the 23-name live broker book (Alpaca paper positions) cross-checking the new feed (`/v2/last/nbbo`) against an independent Polygon endpoint (`/v2/snapshot/locale/us/markets/stocks/tickers/{ticker}.lastQuote`). 23/23 valid pairs. **NBBO-vs-snapshot divergence**: median **0.0 bp**, p95 **13.95 bp**, max **20.9 bp** (F = Ford, a halted/thinly-quoted name whose NBBO endpoint returned a 45-min-old quote while the snapshot was fresher — symbol-specific liquidity artifact, NOT feed-fabrication). |
| **Divergence collapse vs IEX baseline** | median **58.63 bp → 0.0 bp** (Δ −58.6 bp); p95 **265.91 bp → 13.95 bp** (Δ −252 bp); max **689.03 bp (ENSG) → 20.9 bp** (Δ −668 bp). Both pre-committed gates that tripped on the IEX feed (median > 5 bp; p95 > 20 bp) now **PASS** on the median; p95 13.95 bp is well under the 20-bp ceiling. The IEX-single-venue artifact (e.g., ENSG 14%-wide intra-quote spread) is structurally absent on the consolidated feed. |
| **Stale-skip rate (corroborating health signal)** | 10/23 = **43.5%** of book symbols had `ts_age > 5s` at probe-call time. The IEX baseline did not measure stale-skip in Q4, so an exact ratio-comparison is unavailable; however the new feed's stale rows are **honestly stale** (most ~5.5–6.7 s, reflecting per-symbol quote arrival cadence on the consolidated feed), not feed-fabricated. The `verify_quote_freshness` gate (`max_age_s=5`) remains the binding gate and will correctly skip these symbols on the placement path. The 43.5% rate is a real-feed property to monitor, not a regression. |
| **Nanos→ms conversion correctness (unit-trap defence)** | Polygon `results.t` is nanoseconds; `polygonNanosToDate(t) = new Date(t / 1_000_000)` returns epoch-ms `Date`, matching the unit `verify_quote_freshness` consumes (`call_ts.getTime() - observed.ts.getTime()` over ms). Test suite `polygon-quote-fetcher_test.ts` cases 6–7 drive the freshness gate directly: a 2 s-old quote PASSES the 5 s gate; a 10 s-old quote FAILS — proving the unit chain Polygon→fetcher→verifier is correct end-to-end and the "every quote looks stale / every quote looks fresh" trap is closed. |
| **STOP conditions** | All cleared. Re-measurement does NOT still show divergence (it collapsed). Cadence rebuild remains separate. Cron 107/108 stay disarmed pending the cadence-rebuild charter; the once-daily strategy is now un-paused on correct prices when the operator re-arms. |
...
| **Status** | **Component 1 RESOLVED (ACT-344)** — the ≥15% short-stop P&L monitor (`short-stop-evaluator.ts`) is wired into `runTick` AFTER `reconstructInFlight` and BEFORE `advanceTick` + the rebalance-aggregate assertion; the state-machine defensive guard is REMOVED and `short_stop` is natively handled (single-step ladder, 20s fill-wait, §8.6.2:152 parallel-market-cover race at the evaluator without cancelling the limit); the §8.6.2 elevated-200bps cover mechanic is reused; aggregate-gate transient-vs-persistent annotation activates when a forced cover co-occurs with a band-violation (log not alert; persistence escalation deferred to a follow-up cross-tick observer DW). Open follow-ups: **DW-149-B persistence-aware aggregate gate** (cross-tick observer that escalates a SECOND-tick band-violation to operator alert); **DW-149-C short-stop-obligation persistence** (the elevated-200bps RESTART across ticks if the intra-tick race fails to fill, currently single-tick fire-and-let-broker-race); Components 2-3 (DW-165 days-to-cover entry screen + DW-162 borrow-rate spike monitor — the defense-in-depth avoidance + early-warning layers). |
| **Tier** | A — financial-critical (the risk-management exit surface; live-money territory once authorized). |
| **Title** | Short-stop execution branch — Component 1 (the active ≥15% circuit-breaker: P&L monitor + intent producer + parallel-cover race). Component 1 RESOLVED ACT-344; persistence + Components 2-3 carried forward. |
| **Scope** | **Component 1 (RESOLVED)** — the active circuit-breaker: (i) `short-stop-evaluator.ts` reads broker-truth `listOpenPositions(ts)`, computes `loss% = (current_price − avg_entry_price) / avg_entry_price` per short, fires a +200bps marketable-limit cover on `loss% ≥ SHORT_STOP_LOSS_THRESHOLD` (default 0.15, env-overridable via `LONGSHORT_SHORT_STOP_THRESHOLD` with strict parse + 0<x<1 range guard); (ii) `state-machine.ts` `short_stop` native handling (single-step `SHORT_STOP_LADDER` + 20s `STEP_FILL_WAIT_S_SHORT_STOP`; the defensive `isSupportedTradeType` rejection is REMOVED); (iii) §8.6.2:152 parallel-market-cover race at the evaluator (no cancel of the limit; broker races; loser cancelled by next tick reconstruct); (iv) idempotency via deterministic minute-bucket COIDs (`lsep-shortstop-{symbol}-{minute}` + `lsep-shortstopmkt-{symbol}-{minute}`); (v) tick-scheduler seam placement AFTER reconstructInFlight + BEFORE advanceTick + BEFORE rebalanceAggregateAssertion, with in-flight RE-RECONSTRUCT after a fire so the cover enters the SAME tick's advance-path (no 15-min lag); (vi) aggregate-gate transient-vs-persistent annotation `short_stop_adjusted_aggregate` set on the result with `console.warn` log (not operator-alert) when a forced cover co-occurs with a band-violation. **Component 1 carryover DWs** — DW-149-B: cross-tick persistence observer for the aggregate gate (escalate to operator alert when band-violation persists into the NEXT tick after a short-stop fire; today's annotation is log-only). DW-149-C: the §8.6.1 L113 elevated-200bps RESTART persistence — if the intra-tick race fails to fill, today the next tick treats the symbol as a fresh breach (re-fires both legs under fresh minute-bucket COIDs); a future short-stop-obligation persistence (sibling `longshort_short_stop_obligations` table, NOT a position-state column) would let the elevated cover persist a ladder across ticks per CROSSWIND §8.6.1 L113. |
| **Reason for Resolution (Component 1)** | Operator directive elevated DW-149 to PRE-LIVE BLOCKER (Tier A) ahead of intraday-cadence ramp because a short without squeeze protection makes short-side P&L uninterpretable (can't distinguish strategy-failure from getting-squeezed); the seams existed (`BrokerPositionFetcher.listOpenPositions` returns `avg_entry_price + current_price`; advance-tick already reads positions; `short_stop` `TradeType` already in the union; the 200bps elevated cover mechanic already in the rank-exit ladder), so Component 1 is bounded and buildable. |
| **Components 2-3 (open, Tier A)** | **DW-165 days-to-cover entry screen** (avoidance layer — refuse short entries where short-interest-to-volume ratio exceeds a threshold; squeezes don't happen in random names). **DW-162 borrow-rate spike monitor** (early-warning layer — flag a name whose borrow rate has spiked, indicating institutional crowding ahead of a potential squeeze). Both Tier A; both pre-live blockers per the squeeze-protection charter. |
| **Cross_ref** | DEC-068 Addendum (ACT-311; the original E3-v1 scope cut now superseded for Component 1); CROSSWIND §6.3 (the ≥15% stop-loss spec); §8.6.2 (the elevated-200bps mechanic + the §8.6.2:152 parallel-cover race anchor); §8.6.1.1 (parallel-order anchor — the safety posture is "race, don't cancel"); ADR-002 (parallel-order block carryover — the evaluator implements the race AT THE APPLICATION layer rather than via Alpaca's `replaces` so the ADR-002 dispositive 40310000 finding does not apply); `supabase/functions/_shared/longshort-execution/short-stop-evaluator.ts` (Component 1 source); `supabase/functions/_shared/longshort-execution/state-machine.ts` (native `short_stop` ladder + fill-wait helpers); `supabase/functions/_shared/longshort-execution/tick-scheduler.ts` (the SEAM + the transient-vs-persistent aggregate annotation); ACT-344 (Component 1 RESOLVED action). |
...
#### DW-156-ADD-01 (Rule 8 additive, appended 2026-06-30 at ACT-412 — FP-062 sub-step 6I.7 confirm-or-adopt disposition)

**6I.7 verdict: CONFIRM-DEFERRED** (mirrors the 6I.1b PROBED-DEFERRED shape). The FP-062 charter scheduled 6I.7 as a conditional sub-step: "adopt only if not auto-resolved by cadence + Phase-3.3 closure at activation time; otherwise mark RESOLVED." The confirm-or-adopt evaluation was performed at ACT-412 with the following grounded outcome — DW-156 is **neither auto-resolved nor adoptable-now**, and re-homes to a post-data window. **Original DW-156 fields above are PRESERVED VERBATIM (Constitution Rule 8); this addendum is appended-only.**

**Why NOT auto-resolved.** The cadence-rework half of DW-156's trigger (Future Owner Phase, register line 13: "Post-cadence-rework AND/OR post-Phase-3.3 trained-combiner first-artifact") DID land — FP-057 RESOLVED 2026-06-29 at ACT-349, all 6 sub-steps shipped. The trained-combiner half did NOT — Phase 3.3 first artifact has not landed (substrate accruing; T+10-label-gated; ETA late-July 2026 per ACT-335; further contained at ACT-410 for the DW-204 shadow-rank ShadowBookOverlapError starvation). Even when the model lands, register line 11 is explicit that the model "MAY partially auto-resolve... [w]hether it actually does so is empirical and unknowable until the first non-skip training run produces a model artifact" — i.e., the auto-resolution branch is NOT determinable in advance; it must be measured against the first artifact, and DW-156 "remains a distinct refinement even if the trained combiner learns part of it" (register line 11 closing clause). Therefore: do NOT mark RESOLVED.

**Why NOT adoptable now.** DW-156's Resolution shape (register line 14) requires a two-gate backtest (age-bucket exclusion + trajectory-tilt re-rank) against a min-incremental-Sharpe threshold, evaluated on **(i) accrued `combiner_rankings` history** AND **(ii) filled-position P&L**. Input (i) is in-hand (daily-retained since 2026-06-16+ per register line 12), but input (ii) is **absent — zero `longshort.execution.filled` events observed end-to-end** (this is itself FP-062 Phase-6 closure gap (e) per the ACT-411 Status roll-up; the ACT-403 fill-accounting path + the ACT-407 / DEC-070 clause (i) B1 substrate gate are deployed but un-exercised). Additionally, even if filled P&L existed, the trained-combiner artifact (register line 13's "Future Owner Phase" trigger half) is needed to **size the incremental gain over the model** (register line 11 — without it, any gate ratified now would over- or under-state its prize because the model might subsume part of the trajectory dimension). Both inputs are data-gated; the evaluation cannot honestly precede them.

**Disambiguation — DW-156 is NOT the §11.0.7 ranking-freshness staleness gate.** DW-156 is a **ranking-INPUT enrichment** (planner reads two new derived features per candidate: top-K-age bucket + 5-day score-slope sign — then either excludes age-stale or re-ranks by trajectory tilt). The §11.0.7 ranking-freshness gate (ratified under DEC-070 clause (c), wired in FP-057) is a **STALENESS refusal** (the planner refuses to act on `combiner_rankings` rows older than N ticks). They share the word "freshness" only; they consume different signals (DW-156: per-name score history within an in-date ranking row; §11.0.7: row-level `as_of` timestamp) and produce different actions (DW-156: re-rank/exclude candidates; §11.0.7: refuse the whole rebalance). The §11.0.7 gate landing does NOT discharge DW-156. (This was the FP-062 6I.7 confirm-or-adopt's load-bearing conflation check.)

**Re-homing.** DW-156 remains **open / CONDITIONAL-DEFERRED**, re-homed to the **post-Phase-3.3-first-artifact AND post-first-fills window** (whichever lands SECOND becomes the activation trigger — both inputs must be present for the Resolution-shape backtest). The Future Owner Phase row (register line 13) stays valid as written ("post-cadence-rework AND/OR post-Phase-3.3 trained-combiner first-artifact (whichever comes first). Evaluation precedes any implementation.") with this addendum tightening "first-artifact" to also require filled-position P&L for the backtest input. No FP carrier is allocated at this turn (the future FP will be authored at the activation turn against whichever of the two data-gates clears second).

**Net effect on FP-062 Phase-6 closure.** FP-062 sub-step 6I.7 is **CLOSED-AS-CONFIRM-DEFERRED at ACT-412** (the disposition is itself the deliverable; no planner code lands). The FP-062 Status roll-up gap (a) flips from "UNADDRESSED" to "CONFIRM-DEFERRED at ACT-412 — evaluation data-gated on trained-combiner first artifact AND filled-position P&L; DW-156 re-homed post-Phase-3.3 / post-first-fills." Phase-6 discrete-closure remains gated on gaps (b)–(e) (verify_CA orchestration wire / DW-058-B2 disposition / DW-145-147 DEC ratifications / live-fills validation); gap (a) is now discharged-with-rationale.

**Cross_ref (addendum-specific):** ACT-412 (this disposition); ACT-411 (the FP-062 Status roll-up that named 6I.7 as open gap (a) — being updated in the same turn); ACT-368 (FP-062 charter — the conditional-sub-step framing 6I.7 inherits); ACT-349 / FP-057 (the cadence-rework half of DW-156's trigger — LANDED, the auto-resolution-by-cadence claim is examined and rejected because the second trigger half remains absent and register line 11 makes the partial auto-resolution empirically-unknowable); ACT-335 (Phase-3.3 substrate / late-July ETA — the trained-combiner data gate); ACT-410 (DW-204 containment — Phase-3.3 substrate-accrual recovery underpinning the late-July ETA); ACT-403 / ACT-407 / DEC-070 clause (i) at ACT-408 (the deployed-but-un-exercised live-fills path — the filled-position P&L data gate); DEC-070 clause (c) (§11.0.7 ranking-freshness staleness gate — explicitly distinguished, NOT a discharge for DW-156); FP-062 sub-step 6I.7 (the conditional charter slot this addendum discharges); Constitution Rule 8 (original DW-156 fields preserved verbatim above; this addendum is appended-only).
...
| **Status** | **PARTIALLY DISCHARGED** — substrate BUILT under FP-061 4M.4 (ACT-378: MIG-144 `corporate_actions` + Polygon CA adapter [splits + dividends] + ex-date ratio applier + composer `unapplied_corporate_action` BLOCK + soft-dependent `verify_corporate_action_clean`). **Basis-cross-check branch DISCHARGED 2026-06-29 at ACT-382** (FP-062 6I.2b — `AlpacaCorporateActionFetcher` real `/v2/positions.avg_entry_price` cross-check; DW-199 also DISCHARGED). **STILL OPEN**: (i) DW-197 (merger / spinoff CA source — Polygon adapter currently covers splits + dividends only); (ii) DW-198 (cash-dividend cash-ledger accrual). Verifier orchestration call-site wire is pending (ACT-382 surfaced as a gap — `verifyCorporateActionClean` has no live caller yet; bootstrap exposure + interface slot are in place for the eventual wire). PRE-LIVE BLOCKER posture retained until DW-197 + DW-198 close AND the verifier orchestration wire lands. ORIGINAL STATUS (preserved per Rule 8): open. **PRE-LIVE BLOCKER** — basis adjustments across splits/divs/mergers (silent-corruption risk if a held name splits between fill and close); NOT a paper-v1 blocker (paper-broker auto-adjusts; no internal ledger to reconcile). |
...
### DW-163 (+ DW-149-B): rolling-window aggregate gate — transient-vs-persistent reframe — **RESOLVED (ACT-348)**

| Field | Value |
|---|---|
| **ID** | DW-163 (unified with DW-149-B at ACT-348 — they are the SAME mechanism: the cross-tick escalation of the aggregate gate after exempt/short-stop ticks). |
| **Status** | **RESOLVED 2026-06-29 at ACT-348 — FP-057 Sub-step 5 landed.** Per-tick assertion (ACT-pending → now RESOLVED) extended with `exempt_cause: 'short_stop'|'partial_fill'|'working_order'|null` threaded from the tick-scheduler in-process signals (no new fetchers; precedence `short_stop > partial_fill > working_order`); written to `reconciliation_events.divergence` (canonical sink — NO new table). New `rebalance-aggregate-persistence.ts` closure runs AFTER the per-tick assertion: reads the last `LONGSHORT_REBALANCE_AGGREGATE_PERSIST_N` (default 3, env strict-parse ≥1, NEVER silently 0) `reconciliation_events` rows for `call_name='verify_rebalance_aggregate'` ordered DESC, counts CONSECUTIVE `failure_escalated` rows with `exempt_cause IS NULL`. **NON-COMPOUNDING**: an exemption SKIPS (neither resets nor advances) — a stale prior-tick `short_stop` does NOT silence a current unexplained tick (a single stop is NOT a permanent silencer). **RESET-ON-IN-BAND** (not M-of-N): the counter resets to 0 on any `false_positive_within_tolerance` row — matches the transient model (transient resolves → in-band → reset). **LATCH + COOLDOWN** (§11.0.9 surface): on ≥N unexplained → emit `call_name='verify_rebalance_aggregate_persistence'`, `outcome='failure_escalated'`, `action_taken='persistent_band_violation_operator_alert_N_consecutive_ticks'`, `tier='strong'` (the PAGER event); LATCH suppresses re-pages while within cooldown; RE-ESCALATES after cooldown if still stuck (no silent drop). Per-tick `failure_escalated` continues to fire (audit trail per fire); the `short_stop_adjusted_aggregate` annotation generalizes to `aggregate_band_break_exempted` with the cause (log-only). Wired into `longshort-execute/index.ts` + `longshort-execute-cron/index.ts`; threaded `ts` is the SOLE clock source (DEC-034 cl.4; no `new Date()`). Tests (12 in `rebalance-aggregate-persistence_test.ts` + assertion exempt-cause threading + tick-scheduler precedence + persistence-invocation + persistence-throw-survives): env strict-parse (≥1, malformed throws build-time); counter — reset on in-band; non-compounding (short_stop@T then unexplained@T+1 → unexplained counts); exempt-only → 0; `system_bug` skip; closure below-threshold no-write; at-threshold + no-prior → ESCALATES; at-threshold + within-cooldown → LATCH no-write; past-cooldown → RE-ESCALATES; ts-threaded (no clock leak); reader-throw propagates. Full execution suite: **248/248 green**. Locks v3. |
...
| **Why transient-vs-persistent (NOT noise-smoothing)** | A short-stop cover, a partial fill, and a working order all INTENTIONALLY break neutrality for one tick — paging on a single fire would page on every defensive cover. A PERSISTENT break (≥3 consecutive unexplained) is the supervised escalation: by tick-3 the cause is no longer "we just covered" or "fill is en route" — it is a real placement defect. M-of-N (e.g. 3-of-5) was REJECTED: that smooths noise, the reframe is transient-vs-persistent — reset on in-band, count consecutive unexplained. |
| **Cross_ref** | DEC-070 clause (g) (the rolling-window mandate); DW-149 (Component 1 — the short-stop action layer whose annotation this generalizes); CROSSWIND §11.0.7 #17, §11.0.9 (rolling-window/cooldown surface), §22.5.1; `supabase/functions/_shared/longshort-verifiers/verify_rebalance_aggregate.ts` (the spec — `exempt_cause` extension); `supabase/functions/_shared/longshort-execution/rebalance-aggregate-assertion.ts` (per-tick assertion — `exempt_cause` threading); `supabase/functions/_shared/longshort-execution/rebalance-aggregate-persistence.ts` (NEW — the rolling-window closure); `supabase/functions/_shared/longshort-execution/tick-scheduler.ts` (`deriveExemptCause` + persistence injection seam); `supabase/functions/longshort-execute/index.ts` + `supabase/functions/longshort-execute-cron/index.ts` (the dispatch sites); `supabase/functions/_shared/longshort-execution/rebalance-aggregate-persistence_test.ts` + `rebalance-aggregate-assertion_test.ts` + `tick-scheduler_test.ts` (the proof-tests); **ACT-348 (RESOLUTION)**. |
...
| **Status** | **RESOLVED 2026-06-27 at ACT-340 — FP-057 Sub-step 2 landed.** Orchestrator (`rebalance-submit-orchestrator.ts:runRebalanceSubmit`) now calls `broker.reconstructInFlight(ts)` (the EXACT same path the advance-tick has always used at `tick-scheduler.ts:86` — reuse, no new fetcher) and forwards the resulting in-flight set to `planRebalance({ workingOrders })`. The planner subtracts signed working notional (`workingOrderSignedNotional`) using REMAINING qty (`shares − filled_qty`) and the working order's `current_limit_price` as the notional basis. Partial-fill accounting verified non-double-counting (the filled segment lives only in `position_market_value`). E3 SURFACE-1 invariant preserved (broker authoritative; no projection table introduced). Tests: `rebalance-planner-working-orders_test.ts` (8 cases — sign convention, partial-fill, noop-at-target, orphan, back-compat) + `freshness-gate_test.ts` last case asserts `reconstructInFlight` is invoked. Daily path unbroken (fixtures without working orders preserve prior behavior; the orchestrator-test suite — 11 tests — green unchanged). |
...
| **Blocks** | STREAM-3 cadence-rebuild ship; any tick-driven rebalance trigger. (Resolution-path now ratified at [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (b); landing at [FP-057](feature-proposals.md#fp-057) Sub-step 2.) |
...
### DW-168: Intraday-group lambdarank sanity at first-candidate train (the promotion-trust precondition named by DEC-070 clause (h).4)

| Field | Value |
|---|---|
| **ID** | DW-168 (next-free after DW-167). |
| **Status** | **REGISTERED 2026-06-29 at ACT-349 — DATA-GATED (Phase-7-adjacent).** Answerable only once slot>0 forward-return labels clear the skip-gate floor (`LABEL_MIN_ROWS_PER_SIDE=500 × LABEL_MIN_SEED_DATES=20 × TRAINING_HORIZON_TD=10`) and the first intraday-inclusive candidate is produced by the weekly-Sunday GHA cron. No code lands at this DW now (a `_test.py` scaffold or observability emitter pre-built before the candidate exists is speculative shelf-ware). |
| **Tier** | A — financial-critical promotion-trust gate on the first intraday-trained combiner candidate. NAMED in [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (h).4 as the substrate-ready → candidate-trains → DW-168 sanity-gate → promotion chain. |
| **Title** | At the first intraday-trained candidate, sanity-check (i) lambdarank group-size cardinality under intraday-slot row inflation and (ii) NDCG@25 stability vs the slot-0-only baseline — to determine whether the candidate is TRUSTED for promotion through the existing slot-agnostic `promote_combiner_model` path. |
| **Why it's not a code gap today** | The trainer (`training/combiner/trainer.py`) groups on `seed_as_of_date` (DATE); under the additive `intraday_slot` rotation (MIG-122…MIG-126) each daily group now spans `rows-per-day × #slots` with redundant T+H labels (slot>0 FR rows share the daily realised return by construction per clause (a)/(e)). The lambdarank pairwise loss may degrade if the inflated-group redundancy biases pair sampling; this is an OBSERVATIONAL question answerable only against the first real candidate. Pre-building a synthetic sanity harness now would assert against fabricated structure, not the real loss surface. |
| **Scope (when activated)** | (a) Group-cardinality histogram at first-candidate train (per-`seed_as_of_date` group sizes pre- vs post-intraday-inclusion); (b) NDCG@25 absolute + per-fold values vs the slot-0-only baseline computed against the same held-out fold; (c) SHAP feature-importance delta vs the slot-0-only baseline (named-feature drift detector); (d) PASS criterion: NDCG@25 ≥ baseline − ε (ε ratified at activation time, NOT now — pre-ratifying ε against zero data is speculative); (e) FAIL handling: candidate is NOT promoted; FP-057-adjacent investigation FP authored to determine whether the failure is (i) substrate-bug (redundancy mis-weighted by lambdarank — fix is a per-group weighting change in the trainer), (ii) signal-cadence-bug (a lifted signal is contaminating the slot>0 features), or (iii) genuine no-lift (intraday slots do not add information at the current data volume — re-evaluate at the next monthly accrual). |
| **Depends-on** | FP-057 RESOLVED (substrate plumbed — yes, ACT-349); slot>0 FR labels accrued past the skip-gate floor (DATA-GATED); the first intraday-trained candidate produced by the existing weekly-Sunday GHA cron. |
| **Blocks** | The first intraday-trained candidate's PROMOTION (the DEC-070 clause (h).4 sanity-gate). Does NOT block ongoing slot=0 daily training + promotion (the daily path runs unaffected on the same registry per the clause (h).1 slot-agnostic invariant). |
| **Future Owner Phase** | Phase-7-adjacent (fires as soon as the first intraday candidate exists, which is data-driven not phase-driven — likely Phase-6/7 transition window). |
| **Resolution shape** | (1) The first-candidate train run emits the (a)/(b)/(c) observability artefacts; (2) operator review against the PASS criterion (d); (3) PASS → candidate promotes through the existing path + §10.7 Phase-3 exit gates (NDCG@25 floor, walk-forward Sharpe, SHAP) apply on top; FAIL → the (e) investigation FP. |
| **Cross_ref** | [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (h).4 (the naming authority); DEC-048 (the Phase-7 measurement provision DW-168 is adjacent to); DEC-066 §c (the 18-feature representation lock — DW-168 sanity-checks the representation under intraday-inclusive training); FP-057 RESOLVED at ACT-349 (the substrate); FP-058 (the Phase-7 measure-and-lock — DW-168 PASS is upstream of FP-058's "intraday-trained candidate promoted" precondition); `training/combiner/trainer.py` (the train surface); `combiner_model_registry` + `promote_combiner_model` (MIG-115 — the slot-agnostic promotion path); MIG-122…MIG-126 (the substrate rotations); **ACT-349 (REGISTRATION)**. |

#### DW-168-ADD-01 (Rule 8 additive, appended 2026-06-30 at ACT-422 — reconciled supervisor↔Lovable dual-investigation disposition-reopen + two new sanity dimensions + adjacent contingent sibling registration)

**Status transition:** `DATA-GATED` → **`PARTIALLY DATA-GATED`** (mirrors the `open-CONTAINED` addendum shape used by DW-145-ADD-01 / DW-058-B2-ADD-01 / DW-204). **Original DW-168 fields above are PRESERVED VERBATIM (Constitution Rule 8); this addendum is appended-only.** **This disposition-reopen does NOT authorize the probe / harness build** — the pre-buildable shelf (P1–P4 below) is queued as a separate later FP; this turn only reopens the disposition and durably records the two new sanity dimensions + the adjacent contingent sibling.

**Reopen rationale (the reconciled dual-investigation finding).** The original DW-168 status row collapses three distinct work-classes into one verdict ("speculative shelf-ware"). Disaggregated against `training/combiner/trainer.py` (group-by `seed_as_of_date`, `_labels_to_relevance` at trainer.py:278+) and `promotion-criteria-evaluator.ts:146-160` (the existing `ndcgAtK`): (1) **substrate-observational probes** = pre-buildable now (read-only against today's accruing substrate, zero candidate dependency); (2) **pure harness code** (NDCG-delta extension of the existing `ndcgAtK`, group-cardinality histogram emitter) = pre-buildable now, testable on synthetic; (3) **candidate-conditional measurements** (SHAP-delta numerical eval, slot-0-only baseline retrain, PASS/FAIL declaration) = genuinely data-gated. The "speculative shelf-ware" framing is correct ONLY for class (3). Both independent investigations converged on this disaggregation.

**PRE-BUILDABLE-NOW set (P1–P4; read-only, zero candidate dependency — NOT authorized this turn, queued as a separate later FP):**
- **P1 — Substrate-observational probes.** (i) Group-cardinality histogram emitter (per-`seed_as_of_date` group sizes WITH vs WITHOUT `intraday_slot` grouping) — directly measures the row-inflation factor the original scope (a) defers to first-candidate time, against today's accruing substrate; plus (ii) the **two NEW sanity dimensions** below (NEW-1 within-group label-tie ratio; NEW-2 feature-variation across slot rotation).
- **P2 — NDCG-delta harness.** Extension of the existing `ndcgAtK` at `promotion-criteria-evaluator.ts:146-160` to emit slot-stratified ΔNDCG@25; the harness itself is pure code, testable on synthetic, runnable against the first candidate when it lands. Discharges the scope (b) tooling pre-requisite ahead of the candidate.
- **P3 — ε-tolerance DEC.** Mirrors the DEC-076 clause (a) v1-now / supersede-on-first-DW-168-run pattern: ratify a v1 ε (literature anchor) NOW with an explicit empirical-recalibration supersession trigger fired by the first real DW-168 run. Discharges the scope (d) "ε ratified at activation time, NOT now" deferral by replacing "decide later against zero data" with "ratify v1 now, supersede on first evidence" — same DEC-076 shape that closed DW-145/146/147.
- **P4 — FAIL-investigation FP skeleton.** CHARTERED-CONTINGENT (mirrors the FP-064 pattern at ACT-416 — chartered now, START gated on a named trigger). Skeleton enumerates the original scope (e) (i)/(ii)/(iii) branches as the three pre-named investigation arms; activated only on a FAIL declaration from the first DW-168 run.

**DEFERRED-UNTIL-CANDIDATE set (genuinely data-gated, unchanged from original scope):** the SHAP-delta numerical evaluation, the slot-0-only baseline retrain on the same held-out fold, and the PASS/FAIL declaration itself. These require the first intraday-inclusive candidate to exist and are correctly held until the late-July 2026 substrate-clear window (per ACT-335 / ACT-410 substrate-accrual trajectory).

**TWO NEW SANITY DIMENSIONS (the (a)–(e) scope misses these — both forced by trainer evidence, both measurable TODAY against accruing substrate, both higher-value than the existing checks because they give early warning BEFORE the candidate exists):**
- **NEW-1 — Within-group label-tie ratio.** Per DEC-070 clause (h).2, slot>0 forward-return rows share the daily T+H label by construction (intraday rotation is additive; the realised T+H return is a daily quantity replicated across slots). Lambdarank pairs on tied labels are uninformative — Δrelevance = 0 after `_labels_to_relevance` at `trainer.py:278+`, so the pairwise loss accumulates zero gradient on those pairs. **Check (pre-buildable now):** per-`seed_as_of_date` ratio of (tied-pair count / total pairs), measurable from `combiner_forward_returns` alone, no model required. **Interpretation:** HIGH ratio ⇒ the candidate's lambdarank loss is largely uninformative regardless of NDCG — the headline metric can look fine while the optimization surface is degenerate. This dimension is the diagnostic for the failure-mode the original scope (e)(i) names but cannot detect until a candidate exists; the probe detects it NOW.
- **NEW-2 — Feature-variation across slot rotation.** If features at `(date, ticker, slot=0)` ≈ `(date, ticker, slot=N)` for the same `(date, ticker)`, the intraday rows are pure label duplication (worst case for lambdarank — same X, same y, replicated). **Check (pre-buildable now):** per-`(date, ticker)` cosine / L2 distance distribution of `combiner_feature_vectors.features` across slots; aggregate to a distribution moment per `seed_as_of_date`. **Interpretation:** this is THE dimension that decides whether intraday rotation is **information-additive** (distances bounded away from zero — slot-N carries new feature state) or **noise** (distances ≈ 0 — slot rotation is label-duplication only). Answerable now, against accruing substrate, no model required.

**KEY FINDING (the §14 / time-axis payoff of the reopen).** The two new probes (NEW-1 + NEW-2) can determine NOW, against today's accruing substrate, whether the intraday rotation is information-additive or pure label-duplication — weeks before the late-July 2026 first-candidate train window. If the answer is "label-duplication" that's learned BEFORE a wasted training cycle and BEFORE the DW-205 trainer-decision (below) is forced blind; if "information-additive" the candidate proceeds with the diagnostic baseline already on file. Either way the reopen converts a calendar-week of opaque waiting into measured early warning — a real ROI / time-axis finding under §14.

**Explicit no-build-authorized note.** This addendum is **disposition-reopen + documentation only**. The P1–P4 pre-buildable shelf is queued as a separate later FP; no probe code, no harness code, no DEC, no FP charter lands this turn. The adjacent contingent sibling (DW-205 below) is registered as decision-pending-evidence, NOT a pre-committed trainer fix.

**Cross_ref (addendum-specific):** ACT-422 (this disposition-reopen + new dimensions + DW-205 registration); reconciled supervisor↔Lovable dual-investigation (the convergent finding source); `training/combiner/trainer.py` group-by `seed_as_of_date` + `_labels_to_relevance` at trainer.py:278+ (the trainer evidence forcing NEW-1); DEC-070 clause (h).2 (the by-design daily-T+H-label-shared-across-slots invariant NEW-1 measures the consequence of); `combiner_forward_returns` (the NEW-1 substrate, no-model probe); `combiner_feature_vectors.features` (the NEW-2 substrate, no-model probe); `promotion-criteria-evaluator.ts:146-160` `ndcgAtK` (the existing harness P2 extends); DEC-076 clause (a) (the v1-now / supersede-on-first-run pattern P3 mirrors); FP-064 charter at ACT-416 (the CHARTERED-CONTINGENT pattern P4 mirrors); ACT-335 / ACT-410 (the late-July first-candidate substrate-clear ETA the deferred set is gated on); DW-205 (the adjacent contingent sibling registered this turn); Constitution Rule 8 (original DW-168 fields above PRESERVED VERBATIM; this addendum is appended-only).

### DW-169: Momentum risk-layer portfolio-vol-target overlay (Signal-#6 audit finding — risk-layer, NOT signal-change)

| Field | Value |
|---|---|
| **ID** | DW-169 (next-free after DW-168). |
| **Status** | **REGISTERED 2026-06-30 at ACT-350 — PHASE-7-GATED (homed under FP-058).** No code lands now. The overlay is a portfolio-vol-target scalar applied to the gross long-short book — a RISK-LAYER overlay, NOT a change to `compute-momentum.ts` (the spec-correct §4.4.1 raw 12-1 signal stays as the baseline). |
| **Tier** | A — strongest-conviction momentum ROI lever in the academic record (Barroso/Santa-Clara 2015; Daniel/Moskowitz 2016 — ~2× Sharpe + momentum-crash elimination on US equity momentum, STRONG consensus). |
| **Title** | Scale gross long-short book by `target_vol / realized_vol(portfolio, trailing 6m)` so the portfolio targets a constant realized volatility — the published mechanism that drives the strong-consensus Sharpe lift and the 1932/2009 momentum-crash elimination. |
| **Why it's not a code gap today** | The combiner+regime feature `market_realized_vol_6m` (DEC-066 §c representation) can in principle learn the time-series volatility conditioning ONCE a model trains; double-counting between a signal-level σ-divide AND a portfolio-vol-target AND the regime feature is the failure mode. The right venue is Phase-7 measure-and-lock against accrued PnL — pre-building the overlay before measurement risks overlapping with the regime feature's learned conditioning. |
| **Scope (at activation)** | (a) Shadow-overlay implementation: book-level realized-vol estimator (trailing 6m of strategy daily returns), target-vol parameter (literature anchor 12% annualized, NOT a now-decision), scalar leverage in `[L_min, L_max]` bounded by the existing sizing surface (DEC-067); (b) Phase-7 shadow ablation: with-overlay vs without-overlay vs regime-feature-only — three-way to detect double-counting against the regime feature; (c) crash-window stress: 2026-onward analogues to the published 1932/2009 momentum crashes; (d) promotion DEC if PASS (a sizing-surface change, not a signal change — under DEC-067 not DEC-066). |
| **Depends-on** | FP-058 activated (Phase-7 measure-and-lock); accrued live strategy PnL (≥ the FP-058-ratified minimum); DEC-067 sizing-surface seam clarified for a scalar leverage parameter; DW-168 PASS (so the combiner candidate the overlay layers on top of is trusted). |
| **Blocks** | Nothing today (the strategy runs without it). Blocks the strongest-conviction momentum ROI lever from being captured if left un-chartered. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP authored under FP-058 measuring (a)/(b)/(c); on PASS, a sizing-surface DEC extending DEC-067 with the vol-target parameter; on FAIL or double-counting-with-regime-feature, the overlay is DECLINED with a recorded finding. |
| **Cross_ref** | FP-058 (the Phase-7 home); DEC-067 (sizing surface — the natural seam for a scalar leverage); DEC-066 §c (the regime feature `market_realized_vol_6m` — the double-counting axis); §4.4.1 (the signal stays raw 12-1 — this is NOT a signal change); the Signal ROI audit findings ledger at `docs/06-tracking/signal-roi-audit-findings.md`; literature: Barroso & Santa-Clara 2015 *JFE*; Daniel & Moskowitz 2016 *JFE*; **ACT-350 (REGISTRATION)**. |

### DW-170: Momentum signal-level construction ablation — Phase-7 shadow variants (raw 12-1 baseline vs σ-divide vs residual)

| Field | Value |
|---|---|
| **ID** | DW-170 (next-free after DW-169). |
| **Status** | **REGISTERED 2026-06-30 at ACT-350 — PHASE-7-GATED.** No `compute-momentum.ts` change today. Promoting either variant would MUTATE the CRITICAL §4.4.1 signal and require a DEC. The Signal-#6 audit (HEAD `1ce89c85`) found raw 12-1 is the spec-correct baseline; σ-divide has WEAK literature support at the signal level once the portfolio-vol overlay is available (DW-169 absorbs most of the benefit); residual momentum vs market-beta (Blitz/Huij/Martens) has MODERATE support as the stronger signal-level candidate. |
| **Tier** | A — touches a CRITICAL signal (§4.3.5); any promotion is a DEC-grade change. |
| **Title** | Phase-7 shadow-variant ablation via §6.5 `combiner_shadow_variant_config`: Variant 0 = raw 12-1 (current baseline, kept); Variant 1 = per-name σ-divide `(P[T-21]/P[T-252]−1) / σ_252d`; Variant 2 = residual momentum (regress 12-1 on trailing-12m market beta, take the residual). |
| **Why it's not a code gap today** | The shadow-variant harness `combiner_shadow_variant_config` is designed for exactly this; firing a variant pre-data is fabricated-candidate (§9 SENTINEL) and pre-decision the operator can't compare against live PnL. The Signal-#6 audit explicitly recommended NO now-change to the CRITICAL signal. |
| **Scope (at activation)** | (a) Implement Variants 1 + 2 behind shadow-variant flags (NO change to Variant 0 / the live ranker path); (b) accrue shadow rankings + shadow PnL alongside the live strategy; (c) Phase-7 ablation against the FP-058 measurement methodology; (d) promotion DEC if Variant 2 (or, less likely, Variant 1) PASSES — promotion would supersede §4.4.1's verbatim raw 12-1 formula and is a DEC-grade signal mutation; (e) crash-window stress (Variant 2's residualization weakens during regime breaks — measure explicitly). |
| **Depends-on** | FP-058 activated; the `combiner_shadow_variant_config` substrate; trailing-12m market-beta feature available (already present in DEC-066 §c representation per `market_realized_vol_6m` axis — confirm at activation); DW-169 outcome (the risk-layer overlay is the higher-conviction win — sequence DW-169 ablation BEFORE DW-170 to size the residual benefit). |
| **Blocks** | Nothing today. |
| **Future Owner Phase** | Phase 7 (sequenced after DW-169). |
...
| Field | Value |
|---|---|
| **ID** | DW-171 (next-free after DW-170). |
| **Status** | **REGISTERED 2026-06-30 at ACT-350 — PHASE-7-GATED, DATA-ACCRUING.** No `compute-pead.ts` change today. The Signal-#2 audit (HEAD `1ce89c85`) identified the σ_proxy standardization shape as the HIGHEST-LEVERAGE correctable PEAD finding. Current denominator is `(epsHigh − epsLow) / (2 × 1.349)` — the analyst-dispersion (range-based) family, the LESS-favored standardization (Cheong-Thomas 2011: poor cross-firm-size scaling). The classic Bernard-Thomas PEAD uses the firm's OWN past-surprise time-series std (8-quarter own-firm SUE) — the canonical and stronger denominator. |
| **Tier** | A — promoting the classic variant would MUTATE §4.4.6 (a CRITICAL-class signal under §4.3.5) and requires a DEC. |
| **Title** | Phase-7 two-way ablation: σ_proxy_RANGE (current — analyst-dispersion range from Finnhub epsHigh/epsLow) vs σ_proxy_OWN-FIRM-SUE (classic — std of the firm's trailing-8-quarter own-firm SUE series, built on accruing `signal_observations` per-name actual/consensus pairs). |
| **IMPORTANT CORRECTION (anti-phantom)** | A cross-sectional-σ "cheap fix" computed from individual analyst estimates is NOT available. Finnhub exposes ONLY `epsHigh / epsLow / epsAvg / numberAnalysts` — NOT the panel of individual estimates. A true cross-sectional std of the analyst panel cannot be computed from a panel SUMMARY. The range IS the best-available dispersion proxy from the data we have. Anyone re-investigating this DW MUST confirm the Finnhub endpoint contract before chartering a third "cross-sectional std" variant — it would be a phantom. The ablation is STRICTLY two-way (range vs own-firm-time-series). |
| **Why it's not a code gap today** | The own-firm-time-series-SUE denominator requires ≥ 8 quarters of per-name actual/consensus pairs in `signal_observations`. The PEAD orchestrator persists these going forward; the series accrues organically. Implementing pre-accrual would assert against a denominator with too few points to be std-stable (own-firm std on 1–3 points is noise). |
| **Scope (at activation)** | (a) Add a `σ_proxy_own_firm` computation path inside `compute-pead.ts` reading the trailing-8q SUE series from `signal_observations`; (b) shadow-variant under §6.5 harness — both variants compute and persist; live ranker stays on the range variant; (c) Phase-7 ablation against FP-058 methodology; (d) promotion DEC if PASS — DEC mutating §4.4.6 (signal-change, CRITICAL-class); (e) hybrid-fallback policy at promotion: if a name has < 8 quarters, fall back to range (typed-absence rule, NOT fabricated zero — §9 SENTINEL). |
| **Depends-on** | FP-058 activated; ≥ 8 quarters of per-name actual/consensus pairs accrued in `signal_observations` (data-gated, ~2 years from PEAD signal arming); the §6.5 shadow-variant harness; DW-172 (the consensus-snapshot capture is independent — does NOT block this DW, but the two combine for the strongest PEAD construction). |
| **Blocks** | Nothing today (PEAD's standalone edge in a liquid universe is near-dead per Chordia et al. 2009; this DW is about the combiner-feature shape, not unlocking a standalone strategy). |
| **Future Owner Phase** | Phase 7 (after the 8-quarter own-firm SUE series accrues). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058; on PASS, a §4.4.6-mutating DEC promoting the classic variant with the hybrid-fallback rule; on FAIL/marginal, the range variant is kept with a recorded finding (we did the work and the answer is: at our universe, the range proxy is acceptable). |
| **Cross_ref** | FP-058; DEC-066 (signal-representation lock); §4.4.6 (the current spec-literal σ_proxy); §4.3.5 (CRITICAL classification); §6.5 (shadow-variant harness); §9 (SENTINEL — fallback discipline); DW-172 (the consensus-snapshot capture companion); the Signal ROI audit findings ledger; literature: Bernard & Thomas 1989/1990 (classic SUE); Cheong & Thomas 2011 (analyst-dispersion poor cross-firm-size scaling); Chordia/Goyal/Sadka/Sadka/Shivakumar 2009 *FAJ* (PEAD ~dead in liquid stocks); **ACT-350 (REGISTRATION)**. |

### DW-172: PEAD T-0 consensus snapshot capture — zero-cost forward action against walk-down dampening

| Field | Value |
|---|---|
| **ID** | DW-172 (next-free after DW-171). |
| **Status** | **REGISTERED 2026-06-30 at ACT-350 — LOW priority; BUILD-NOW-vs-DEFER is the operator's call.** ZERO-COST forward action (no I/B/E/S procurement, no backfill). Cheap plumbing: start STORING our own T-0 Finnhub consensus snapshots NOW so a true T-5 consensus series exists ~3 months forward. |
| **Tier** | B — second-order to DW-171 (σ_proxy fix) and to PEAD's small liquid-universe edge. Reassess priority ONLY if Phase-7 (FP-058) finds PEAD is materially edge-positive as a combiner feature. |
| **Title** | Snapshot Finnhub T-0 consensus (epsAvg, epsHigh, epsLow, numberAnalysts, consensus_timestamp) per ticker on a pre-RTH daily cadence into a new `pead_consensus_snapshots` table, indexed by (ticker, snapshot_date), so a Phase-7 ablation can compute a true T-5-vs-T-0 walk-down series rather than the current T-0-frozen consensus (which is already walked down). |
| **Why it's a Phase-7-blocking forward action despite being LOW priority** | The walk-down (consensus drifts toward the true number as report-date approaches; using T-0 consensus dampens our SUE because the dampening has already happened) is a known PEAD construction leak. The mitigation is a T-5 (or earlier) consensus baseline — which DOES NOT EXIST in our data today. A snapshot capture started now creates the series going forward at zero data cost; backfill is impossible (vendor does not expose historical consensus snapshots). If we don't start now, the Phase-7 walk-down ablation will be unable to run on our own data at FP-058 activation. |
| **Why this MIGHT be a small now-action** | Storing snapshots is cheap plumbing (one Finnhub call/day per ticker via the same `FinnhubEstimateFetcher` the PEAD orchestrator already uses; one new table; one writer; no kernel/ranker/combiner change). Operator decides build-now vs defer based on Phase-7 priority — the audit's recommendation is to charter and let the operator decide; the cost of NOT starting is a 3-month delay on the walk-down ablation's data series. |
| **Scope (at activation OR at build-now decision)** | (a) NEW table `public.pead_consensus_snapshots` (RLS-first per D1; idempotent UPSERT per D3; GRANTs per the public-schema-grants rule); (b) pre-RTH daily snapshot job consuming the existing `FinnhubEstimateFetcher`; (c) writer wired into the existing PEAD orchestrator path (extends, NOT replaces — current PEAD compute path untouched); (d) Phase-7 ablation under FP-058: T-0 SUE (current) vs T-5 SUE (using snapshot series) — measures the walk-down dampening magnitude on OUR universe; (e) promotion DEC if PASS (would mutate §4.4.6's `epsAvg` source from "fetched at report" to "snapshot at T-5"). |
| **Depends-on** | Nothing technical (the fetcher and orchestrator exist). PRIORITIZATION depends on FP-058 finding PEAD is combiner-meaningful. |
| **Blocks** | The Phase-7 walk-down ablation (which requires ~3 months of pre-RTH snapshots to have a usable T-5 series). |
| **Future Owner Phase** | Phase 7 ablation (FP-058) for the analysis; build-now or Phase-7-build is the operator's call. |
| **Resolution shape** | EITHER (i) operator approves a small now-build FP (table + writer + cron — one focused PR), OR (ii) DW is parked until FP-058 activation and the walk-down ablation runs against whatever series has accrued by then. Note in the resolution: PEAD's STANDALONE edge in a liquid universe is, on the public record, close to dead — PEAD earns its seat as a COMBINER FEATURE / short-side risk-gate, not a standalone strategy; its combiner weight is a Phase-7 measurement question (FP-058). Do NOT cut PEAD pre-measurement; do NOT expect standalone weight. |
| **Cross_ref** | FP-058 (the ablation home); §4.4.6 (the current PEAD formula — promotion would mutate it); §4.3.5 (CRITICAL classification); DW-171 (the σ_proxy companion — together they form the strongest PEAD construction); `FinnhubEstimateFetcher` (the existing fetcher path); `pead-orchestrator.ts` (the existing orchestrator path); the Signal ROI audit findings ledger; literature: Chordia/Goyal/Sadka/Sadka/Shivakumar 2009 *FAJ* (PEAD ~dead in liquid stocks — the priority context); the walk-down dampening literature (Easterwood & Nutt 1999; Richardson/Teoh/Wysocki 2004); **ACT-350 (REGISTRATION)**. |

### DW-173: SI level / days-to-cover (DTC) as-alpha shadow variant — Signal #5 audit's highest-conviction cheap finding

| Field | Value |
|---|---|
| **ID** | DW-173 (next-free after DW-172). |
| **Status** | **REGISTERED 2026-06-30 at ACT-351.** SHADOW STAND-UP is **WEEKEND-BUILDABLE** (stands up now via the §6.5 `combiner_shadow_variant_config` harness so it starts ACCRUING free comparison data immediately); the PROMOTION decision stays Phase-7-gated (no trained model / accrued labels exist to validate against today — promoting pre-data is the fabricate-a-candidate anti-pattern FP-057 Sub-step 6 closed against). |
| **Tier** | A — promoting would MUTATE Signal #5 (CRITICAL §4.3.5) and require a DEC. |
| **Title** | Stand up a SHADOW signal computing the SI LEVEL (`short_interest_pct_float`) and DAYS-TO-COVER (`short_interest_days_to_cover` — already populated by the DW-165 squeeze-screen fetcher path) as ALPHA variants alongside the live ΔSI signal (#5), so a Phase-7 ablation can decide promote / stack / keep with measured data. |
| **The audit finding driving this DW** | Signal #5 is ΔSI (`−(SI[T] − SI[T−2])`, falling SI → bullish). The SI literature locates the predictive edge primarily in the LEVEL (Asquith/Pathak/Ritter 2005 *JFE* top-decile-shorted-low-IO names ≈ −2.15%/mo abnormal; Boehmer/Jones/Zhang 2008 *JF* shorting-flow level ≈ 1.16%/mo) and especially DAYS-TO-COVER (Hong/Li/Ni/Scheinkman/Yan 2016 *JFE* canonical DTC > SI% result, ≈ 1.2%/mo). The CHANGE dimension is weaker, smaller-magnitude, concentrated in illiquid names, and the most damaged by FINRA's ~8-biz-day publication lag + twice-monthly cadence (our ΔSI is structurally ~2-3 weeks stale; LEVEL/DTC survive the lag because the papers are point-in-time-lagged constructions). We ALREADY compute DTC for the DW-165 squeeze screen — the stronger alpha is in the system, spent ONLY on risk. |
| **WEEKEND-BUILDABLE PIECE (mark explicitly)** | The shadow stand-up: register a `si_level` (and optionally `si_dtc`) variant in `combiner_shadow_variant_config` reading the existing fetcher outputs the squeeze screen already pulls (`short_interest_pct_float` from the FINRA/Polygon path; `short_interest_days_to_cover` from the MIG-131 table). The harness writes shadow rankings alongside live; ZERO change to `compute-short-interest-change.ts`; ZERO change to the live ranker path; ZERO change to the squeeze screen. Cheap because the data and the harness already exist. The point is to START THE MEASUREMENT CLOCK NOW on free data so Phase-7 ablation has months of accrued comparison instead of a cold start. |
| **What is NOT weekend-buildable (Phase-7-gated)** | The PROMOTION decision (replace ΔSI with level/DTC / STACK both / KEEP ΔSI). Promotion requires a trained model + accrued labels + the FP-058 measurement methodology — none of which exist today. The ex-ante promotion rule pinned now (to avoid p-hacking at Phase-7): if level/DTC beats ΔSI beyond the NDCG@25 noise band → replace; if both carry independent (low-correlation) edge → stack with regularization tuned per DW-175; else keep ΔSI. |
| **Scope (weekend stand-up)** | (a) `combiner_shadow_variant_config` row(s) registering `si_level` (and `si_dtc`) variants under §6.5; (b) shadow producer reading the existing fetcher outputs (NO new vendor surface, NO new fetcher); (c) shadow write into the shadow tables already used by DW-170's variant pattern; (d) typed-absence on missing SI/DTC (§9 SENTINEL — never fabricate 0); (e) NO touch to live ΔSI compute path, NO touch to the live ranker, NO touch to the squeeze screen, NO DEC, NO migration if the existing shadow infra suffices. |
| **Scope (Phase-7 promotion, deferred)** | Phase-7 ablation under FP-058 using whatever shadow series has accrued; promotion DEC if PASS (would mutate Signal #5's compute path); hybrid/stack rule per the ex-ante criterion above. |
| **Squeeze-composition note** | The composition between Signal #5 alpha and the DW-165 DTC ≥ 7 squeeze screen is CORRECT: alpha proposes (ranks), risk disposes (PreflightComposer hard planner-reject AFTER the rank); no code path lets alpha override the screen; a GME-class name (rising SI + high DTC) is ranked "short" by #5 but hard-excluded by the screen. Promoting level/DTC-as-alpha does NOT change this composition — the screen still wins on its axis. The binary-vs-continuous seam at DTC = 7 is a separate Phase-7 sensitivity probe, registered as DW-175. |
| **Depends-on** | §6.5 `combiner_shadow_variant_config` harness (already MIG-100); existing SI fetcher path (DW-165 / MIG-131); FP-058 activation for the PROMOTION decision only. The SHADOW STAND-UP has no Phase-7 dependency. |
| **Blocks** | Nothing today. Blocks the Phase-7 #5 measurement on its own data if NOT stood up this weekend (cold start at FP-058 activation, ~months of delay). |
| **Future Owner Phase** | NOW (shadow stand-up) + Phase 7 (promotion decision under FP-058). |
| **Resolution shape** | Weekend stand-up PR (separate, scope-disciplined: shadow variant + config + tests + docs only); Phase-7 ablation FP under FP-058; on PASS, a §4.4-mutating DEC promoting the level/DTC variant (or stacking it); on FAIL/marginal, ΔSI is kept with a recorded finding. |
| **Cross_ref** | FP-058 (Phase-7 ablation home); §4.4 (Signal #5 spec — promotion would mutate); §4.3.5 (CRITICAL classification); §6.5 (shadow-variant harness); §9 (SENTINEL — typed-absence on missing SI/DTC); DEC-066 §c (representation-lock — promotion would touch); MIG-100 (shadow-variant config table); MIG-131 (DTC table populated by DW-165 fetcher path); DW-165 (the squeeze screen consuming the same DTC column for risk, NOT alpha — the duplication this DW resolves); DW-174 (the daily-SI vendor charter — sequenced AFTER this DW); DW-175 (the orthogonality + DTC=7 sensitivity probe — Phase-7 companion); the Signal ROI audit findings ledger; literature: Asquith/Pathak/Ritter 2005 *JFE*; Boehmer/Jones/Zhang 2008 *JF*; Hong/Li/Ni/Scheinkman/Yan 2016 *JFE*; **ACT-351 (REGISTRATION)**. |

### DW-174: Daily-SI vendor procurement charter — sister to DW-166 (borrow-rate vendor)

| Field | Value |
|---|---|
| **ID** | DW-174 (next-free after DW-173). |
| **Status** | **REGISTERED 2026-06-30 at ACT-351 — VENDOR-GATED, sequenced AFTER DW-173.** Pre-live nice-to-have, NOT a pre-live blocker. |
| **Tier** | B — does not block live readiness; sized by whether ΔSI (or the level/DTC variant) survives DW-173's ablation at all. |
| **Title** | Evaluate daily-estimated short-interest vendors (S3 Partners, Ortex, S&P Global / IHS Markit Securities Finance (FIS Astec)) for procurement, to collapse the FINRA ~8-biz-day publication lag (twice-monthly cadence → our ΔSI structurally ~2-3 weeks stale). |
| **Why VENDOR-GATED** | FINRA publishes SI on a semi-monthly settlement-date basis with multi-business-day lag. Daily-estimated SI requires a paid vendor with point-in-time-clean history (no look-ahead backfile rewriting), licensed redistribution rights, and our universe coverage. The CHANGE dimension is the one most damaged by the lag; LEVEL/DTC survive the lag as point-in-time-lagged constructions. |
| **Why SEQUENCED AFTER DW-173** | If the free DW-173 ablation finds the LEVEL/DTC variant dominates ΔSI on our universe (the literature's prediction), the lag-damaged CHANGE dimension is the wrong thing to spend vendor budget on. Run the free ablation first; the procurement decision should be informed by whether ΔSI carries independent edge worth de-lagging. |
| **Scope (at activation)** | (a) Vendor short-list + commercial RFP (S3, Ortex, S&P/Astec); (b) point-in-time-integrity audit (no look-ahead backfile); (c) universe coverage check vs our liquid universe; (d) license / redistribution review; (e) total-cost-of-ownership vs Phase-7-measured ΔSI edge size; (f) trial integration via the existing SI fetcher interface (`ShortInterestFetcher` in `longshort-hard-exclusion-interfaces.ts`) if procurement approved. |
| **Depends-on** | DW-173 outcome (sequenced AFTER); operator procurement approval. |
| **Blocks** | Nothing today. |
| **Future Owner Phase** | Phase 7 / procurement (post-DW-173 ablation). |
| **Resolution shape** | EITHER (i) DW-173 finds level/DTC dominates → DW-174 declined (the de-lagged CHANGE is not worth procurement); OR (ii) DW-173 finds ΔSI carries independent edge worth de-lagging → operator-approved vendor procurement FP. |
| **Cross_ref** | DW-166 (sister borrow-rate vendor charter — same procurement pattern); DW-173 (the gating free ablation); DW-167 (sister options-flow vendor — Polygon Options Advanced); `ShortInterestFetcher` interface in `supabase/functions/_shared/longshort-hard-exclusion-interfaces.ts` (the integration seam); the Signal ROI audit findings ledger; **ACT-351 (REGISTRATION)**. |

### DW-175: ΔSI / level orthogonality + DTC = 7 squeeze-gate sensitivity probe (Phase-7)

| Field | Value |
|---|---|
| **ID** | DW-175 (next-free after DW-174). |
| **Status** | **REGISTERED 2026-06-30 at ACT-351 — PHASE-7-GATED, MEASUREMENT-ONLY (no compute change implied today).** Companion to DW-173. |
| **Tier** | B — measurement / regularization-tuning, not a primary signal mutation. |
| **Title** | Two coupled Phase-7 measurements once labels accrue: (i) RANK CORRELATION between ΔSI (Signal #5 live) and the level/DTC shadow variants (they share the SI-level driver → likely correlated combiner features → combiner regularization matters; informs whether to stack vs replace at DW-173 promotion); (ii) MARGINAL-NAME behavior around the binary DW-165 DTC = 7 squeeze gate (alpha is continuous, the gate is binary — a name at DTC = 6.9 is shorted full-conviction, DTC = 7.1 is hard-excluded; the discontinuity is real and should be measured before deciding soft-penalty vs hard-exclude). |
| **Why Phase-7 only** | Both measurements are observational on accrued shadow + live data; neither is buildable pre-accrual. Synthetic measurement now would assert against fabricated structure. |
| **Scope (at activation)** | (a) Spearman / Kendall rank correlation between ΔSI and `si_level` / `si_dtc` shadow rankings over the accrued window; (b) feature-importance / SHAP-delta in the combiner if both are stacked; (c) marginal-name P&L around DTC ∈ [6, 8] (live ΔSI excluded names within the band vs included names just below); (d) sensitivity to the DTC threshold (rerun excluding 5 / 7 / 9) WITHOUT changing the production gate; (e) finding-only output: PASS = hard-exclude at 7 is correct; SOFT-PENALTY = recommendation FP authored mutating DW-165 from hard planner-reject to a magnitude-graded penalty; NO touch to the squeeze screen this turn or at probe time. |
| **Squeeze-screen LOOSENING is OUT OF SCOPE** | The composition between Signal #5 (proposes) and the DW-165 squeeze screen (disposes) is correct. DW-175 measures only. Any change to the DTC = 7 gate (soft-vs-hard, threshold value) is a separate post-probe FP gated on operator approval, NOT implied by registering this DW. |
| **Depends-on** | DW-173 shadow stand-up accruing data; FP-058 activation; trained combiner using both ΔSI and level/DTC as features (Phase-7 outcome). |
| **Blocks** | Nothing today. Informs DW-173's promotion shape (replace vs stack) and the DW-165 gate-design Phase-7 review. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 measurement FP under FP-058 producing the orthogonality table + the marginal-name-behavior chart; outcome is a recorded finding + (only on SOFT-PENALTY outcome) a downstream gate-design FP for DW-165. |
...
| Field | Value |
|---|---|
| **ID** | DW-176 (next-free after DW-175). |
| **Status** | **LIVE 2026-06-28 at ACT-358 (HEAD `a81c1b3d`) — SHADOW ROWS ACCRUING.** The `reversal_ungated_observations` shadow table (MIG-135) shipped with sub-step 3b; verified accrual at 3b closure = 652 none + 96 catalyst + 90 news shadow rows on day 2026-06-26. Phase-7 (FP-058) consumes the accrued comparator. (Original registration ACT-352 preserved for audit trail.) |
| **Tier** | B — measurement / retrospective over-gating check; not a primary signal mutation; rides on existing §6.5 harness. |
| **Title** | Stand up `reversal_ungated` as the pre-DEC-071 contract (raw §4.4.2 with NO news/catalyst gate and NO magnitude cap) alongside the live gated reversal so Phase-7 can measure RETROSPECTIVELY what DEC-071 gave up: (i) variant-comparison NDCG@25 on the accrued window; (ii) hit-rate of the live-gated path on the failure-mode cases the gate exists for (falling-knife / short-into-strength names where #8/#9 fired or `|R_5d| > 3σ`); (iii) the OVER-GATING rate (names the gate suppressed that subsequently mean-reverted as the ungated path would have predicted). The shadow variant is the ONLY retrospective check we have on over-gating (there is no retrospective check on under-gating — an unbounded short loss is permanent; the asymmetry is the operator-decided basis for shipping the cap pre-measurement). |
| **Why Phase-7 for the measurement** | The comparison is observational on accrued shadow + live data; meaningful only after enough labels accrue for NDCG and hit-rate stability. The STAND-UP is now (rides with the DEC-071 build); the MEASUREMENT is Phase-7. |
| **Scope (at activation — Phase-7 read)** | (a) NDCG@25 / Spearman comparison of the gated vs ungated reversal rankings across the accrued window; (b) failure-mode hit-rate on the cases the gate fires (news-day moves, catalyst-day moves, `|R_5d| > 3σ` moves) — confirms the gate actually filters the worst-realized-PnL cases; (c) over-gating rate — names suppressed where the ungated path would have correctly faded; (d) recommendation output: PASS = keep the gate as-shipped; THRESHOLD-TUNE = recommendation FP under FP-058 to revisit the 3σ default (input to DW-177); NEEDS-NARROWER-GATE = recommendation FP to tighten clause (a) presence semantics (e.g., catalyst-tier severity threshold instead of any-presence); NO touch to DEC-071 this DW. |
| **Build-NOW piece (the ride-along)** | Add `reversal_ungated` row to `combiner_shadow_variant_config` (MIG-100); orchestrator emits two rows per `(operator_id, ticker, as_of_date)` — one to live `signal_observations` (gated) and one to the §6.5 shadow channel (ungated). ZERO change to live ranker / live execution / live PnL. The shadow channel is consumed only by Phase-7 measurement scripts. |
| **Depends-on** | DEC-071 ratified (the gated path being measured against); §6.5 shadow harness (MIG-100, in-system). |
| **Blocks** | Nothing today. Informs whether DEC-071's gate shape (the news/catalyst presence semantics + the 3σ default) needs Phase-7 revision; feeds DW-177. |
| **Future Owner Phase** | Phase 7 (FP-058) for the measurement read. The stand-up is the weekend-build now. |
| **Resolution shape** | A Phase-7 measurement FP under FP-058 producing the gated-vs-ungated comparison table + the over-gating-rate + the recommendation; outcome is a recorded finding + (only on THRESHOLD-TUNE or NEEDS-NARROWER-GATE) a DEC-071 amendment FP. |
| **Cross_ref** | DEC-071 (the now-fix this shadow measures against); FP-058 (Phase-7 home); DW-177 (Phase-7 threshold/lookback/weaker-fixes ablation — sister); §4.4.2 (spec-literal compute — UNCHANGED in shadow path too; the variant differs only in the gate, not the compute); §6.5 (shadow-variant harness); §9 (typed-absence preserved in shadow path: ungated still emits raw §4.4.2; suppression rules are the differential, NOT a fabricated zero); MIG-100 (shadow-variant config table); literature: Tetlock 2010, Savor 2012, Da-Liu-Schaumburg 2014, Nagel 2012; **ACT-352 (REGISTRATION)**. |

### DW-177: Reversal Phase-7 gate-parameter ablation — 3σ threshold + news/catalyst lookback window + the two weaker fixes deferred from DEC-071 clause (g)

| Field | Value |
|---|---|
| **ID** | DW-177 (next-free after DW-176). |
| **Status** | **REGISTERED 2026-06-30 at ACT-352, EXPANDED 2026-06-28 at ACT-359 — PHASE-7-GATED, MEASUREMENT-ONLY** (no compute change implied today; pre-data tuning of a CRITICAL signal is p-hacking, which DEC-071 clause b explicitly forbids). **EXPANSION (ACT-359):** DW-177 now ALSO carries the **magnitude-cap go/no-go (DEFAULT-NO)** as the FIRST measurement, before the threshold ablation — per the DEC-071 Clause (b) DEFERRED addendum (the probe estimated <1pp incremental gating beyond news∪catalyst at ~5× Polygon payload cost). The `PRICE_HISTORY_LOOKBACK_DAYS` 20→~95 lookback bump is deferred TOGETHER with the cap (the lookback expansion is not needed without the σ denominator). Sequencing: (1) magnitude-cap go/no-go on the DW-176 ungated shadow (default-NO unless the shadow shows the violent-no-news / sub-3σ-trend cohort pulls weight news∪catalyst misses); (2) ONLY IF (1) is GO, the threshold-sensitivity sweep; (3) the lookback-window sweep on clause (a); (4) the two weaker-fix shadow variants. |
| **Tier** | B — measurement / parameter-ablation; not a primary signal mutation. |
| **Title** | Three coupled Phase-7 ablations against the DEC-071 gated reversal once data accrues: (i) the magnitude-cap threshold (clause b currently pins 3σ trailing-60d as the literature-anchored default — Nagel 2012 / Da-Liu-Schaumburg 2014; ablate 2σ / 2.5σ / 3σ / 3.5σ / 4σ to confirm the default is approximately optimal vs the realized failure-mode distribution); (ii) the news/catalyst lookback window (clause a currently uses same-`as_of` semantics; ablate whether a 1-2 day trailing window improves the gate hit-rate without inflating over-gating); (iii) the two weaker fixes deferred from DEC-071 clause (g) — `wait_for_turn` (fade only after the first counter-trend bar) and `volume_gate` (suppress reversal when 5-day volume is in the top decile of trailing volume) — Phase-7 shadow variants under §6.5 to measure marginal contribution beyond the DEC-071 gate. |
| **Why Phase-7 only** | All three measurements need accrued labels; running them pre-data would be the fabricate-a-candidate anti-pattern FP-057 Sub-step 6 closed against. The 3σ default is LITERATURE-ANCHORED; ablating it pre-data without measurement evidence is p-hacking. DEC-071 clause (b) pins the default explicitly so the build PR cannot drift the threshold under the guise of "tuning." |
| **Scope (at activation)** | (a) Threshold sensitivity sweep (2σ / 2.5σ / 3σ / 3.5σ / 4σ) on the trailing-60d σ denominator — output: hit-rate vs over-gating curve; recommendation = keep 3σ if within noise band, else propose DEC-071 amendment FP; (b) lookback-window sweep on clause (a) presence semantics (T-0 only vs T-0..T-1 vs T-0..T-2) — output: hit-rate vs over-gating curve; (c) the two weaker-fix shadow variants `reversal_wait_for_turn` and `reversal_volume_gate` against the DEC-071 baseline gated path — output: marginal NDCG@25 / failure-mode hit-rate; recommendation = keep / stack / discard. NO production change implied; all three are measurement-only. |
| **Depends-on** | DEC-071 ratified + built (the gated baseline being ablated against); DW-176 shadow stand-up (the comparator infrastructure); FP-058 activation; accrued shadow + live data over a window covering at least one vol-spike regime to test Nagel 2012's regime-dependency claim. |
| **Blocks** | Nothing today. Informs DEC-071 parameter stability; feeds any downstream DEC-071 amendment FP. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058 producing the three sensitivity tables + recommendation per axis; outcome is a recorded finding + (only on threshold-tune / lookback-tune / weaker-fix promotion) downstream DEC-071 amendment FPs. |
| **Cross_ref** | DEC-071 clauses (b) and (g) (the parameters being ablated); FP-058 (Phase-7 home); DW-176 (sister — provides the gated-vs-ungated comparator the threshold ablations layer on); §4.4.2 + §4.3.5 (spec / CRITICAL classification — unchanged regardless of ablation outcomes); §6.5 (shadow-variant harness); §9 (typed-absence preserved across all variants); literature: Nagel 2012 (2-3σ framing source), Da-Liu-Schaumburg 2014 (non-monotonicity tail), Tetlock 2010 / Savor 2012 (news lookback window basis); **ACT-352 (REGISTRATION)**. |

### DW-178: Analyst per-revision-outcome CAPTURE step — **TIME-SENSITIVE**, the only near-term build candidate in the DEC-072 cluster

| Field | Value |
|---|---|
| **ID** | DW-178 (next-free after DW-177; grep-verified at HEAD `3c259f07`). |
| **Status** | **REGISTERED 2026-06-30 at ACT-353 — TIME-SENSITIVE NEAR-TERM BUILD CANDIDATE** (NOT Phase-7). The CAPTURE is time-sensitive-now; the WEIGHTING that uses it (DW-179) is Phase-7. Every day without capture = a day of analyst-track-record accrual PERMANENTLY lost (FMP /price-target-news is a forward stream; vendor backfill of historical analyst-attributed revisions is not reliably available at the per-analyst granularity needed for accuracy weights). |
| **Tier** | A — time-sensitive forward-data accrual; gates DW-179 (the LARGE skill lever) and any future Bayes-shape combination with DEC-072 tier-weight. |
| **Title** | Persist EACH analyst price-target revision row (analystName, analystCompany, ticker, prior_target, new_target, direction, magnitude, published_at, as_of_date) into a dedicated `analyst_revision_events` table at the moment `compute-analyst-revision.ts` ingests it, BEFORE the per-name z-scored aggregation that lands in `signal_observations`. Today the orchestrator persists only the z-scored aggregate (Stage 4 within-sector GICS z-score) — the per-revision rows with analyst identity + forward-outcome-joinable timestamps are NOT preserved. Build a per-revision audit row alongside the existing persist. |
| **Why TIME-SENSITIVE (not Phase-7)** | DW-179 measured accuracy-weights need ≥ 6–12 months of (analyst → revision → forward-N-day-return outcome) triples to stabilize. The forward return is computed in Phase-7 by joining captured revisions to the `dim_prices` bar table — but only IF the (analyst, revision, timestamp) rows exist. They do not exist today. Starting capture NOW is the only way to have weights ready when Phase-7 activates; deferring capture to Phase-7 delays accuracy-weights by the full 6–12 month accrual window AFTER FP-058 starts. The capture itself is cheap (one INSERT per revision; volume ≈ 200–500 revisions/day × ~25 fields ≈ negligible); the weighting that USES the capture is Phase-7. Distinct from DW-172 (PEAD T-0 consensus capture, also TIME-SENSITIVE for the same reason) — same anti-pattern, same fix. |
| **Scope (at build, if authorized in the post-audit weekend slate)** | (a) New table `analyst_revision_events` (MIG-NNN at build) with columns covering the per-revision tuple above + a stable revision-id (analyst_normalized + ticker + published_at_microseconds composite); (b) INSERT alongside existing persist in `analyst-revision-orchestrator.ts` (no change to z-scored aggregate, no change to `signal_observations` shape, no change to ranker, no change to PnL); (c) idempotent UPSERT on the composite key to absorb FMP duplicate emissions; (d) typed-absence preserved (no fabricated rows when `findSameAnalystPrior` returns null); (e) RLS + grants per D1; (f) confirm at build whether an existing per-revision audit row already exists upstream (if so, this DW resolves as no-op + clarifying note). |
| **Depends-on** | None for the capture itself. Blocks DW-179 (accuracy-weight requires this capture having accrued). |
| **Blocks** | DW-179. Indirectly improves the Bayes-shape combinability of DEC-072 tier-weight × DW-179 accuracy-posterior. |
| **Future Owner Phase** | Near-term (weekend-buildable; ranked in the post-audit weekend slate); the consumer (DW-179) is Phase 7 (FP-058). |
| **Resolution shape** | A small build PR landing the table + MIG-NNN ledger row + the alongside-persist hook + idempotency test + RLS evidence. ROI-flagged per the project guardrails as a forward-data accrual that does NOT change live prediction/sizing/execution (orthogonal — the live z-scored signal is unchanged). |
| **Cross_ref** | DEC-072 clause (e) (do-not-conflate — this DW is the upstream of DW-179, not of the tier-weight); DEC-055 §a (the v1 placeholder DW-179 ultimately replaces with a measured weight); DW-179 (the consumer); DW-172 (sister TIME-SENSITIVE capture for PEAD T-0 consensus — same anti-pattern); `compute-analyst-revision.ts` (the orchestrator the alongside-persist hooks into); `analyst-identity.ts:38-39` (the surfacing this capture preserves); ROI guardrails (forward-data accrual is ROI-flagged even though prediction is untouched); **ACT-353 (REGISTRATION)**. |

### DW-179: Analyst per-analyst rolling accuracy-weight — Phase-7, the LARGE skill lever (DEPENDS-ON DW-178 capture)

| Field | Value |
|---|---|
| **ID** | DW-179 (next-free after DW-178). |
| **Status** | **REGISTERED 2026-06-30 at ACT-353 — PHASE-7-GATED + ACCRUAL-GATED**. Depends on DW-178 capture having accrued ≥ 6–12 months of revision-outcome triples. Cannot run pre-data; pre-data multipliers would be the fabricate-a-candidate anti-pattern FP-057 Sub-step 6 closed against. |
| **Tier** | A — the LARGE documented skill lever for Signal #1; supersedes the DEC-072 tier-weight in the limit (firm-tier is a proxy for individual analyst skill; per-analyst accuracy is the direct measurement). |
| **Title** | Replace (or stack on top of) the DEC-072 tier-weight with a per-analyst rolling accuracy weight `accuracyWeight(analystName, ticker_universe_slice, as_of)` computed from accrued DW-178 capture: for each `(analyst, revision)`, join forward N-day return; compute analyst-level hit-rate / IR / decile rank over a trailing window (1y / 2y / since-inception); map to a credibility multiplier. Literature: Loh-Stulz "influential" subset (~12% of changes carry the edge); Clement (1999) top-quintile-accuracy; Mikhail-Walther-Willis (1997, 2007); Stickel (1992). |
| **Why Phase-7 only** | Multiplier mapping is a measured ablation against accrued labels; running it pre-data would be p-hacking on a CRITICAL signal. The Bayes-shape combination with DEC-072 tier-weight is itself a Phase-7 design (firm-tier prior × per-analyst accuracy posterior — both literature-supported; the right combination is measured, not guessed). |
| **Scope (at activation)** | (a) Outcome-join job over `analyst_revision_events` × `dim_prices` to compute per-(analyst, revision) forward-N-day returns at N ∈ {5, 21, 63}; (b) per-analyst accuracy aggregates over trailing windows; (c) multiplier mapping (decile / continuous IR / Bayesian shrinkage to prior); (d) shadow variant under §6.5 vs (i) current DEC-055 §a uniform = 1.0, and (ii) DEC-072 tier-weight if shipped; (e) recommendation per FP-058: replace tier-weight / stack with tier-weight (Bayes shape) / keep tier-weight standalone (if measured accuracy adds nothing beyond tier). |
| **Depends-on** | DW-178 capture accrued; FP-058 activation; combiner model present (or §6.5 harness sufficient for shadow comparison). |
| **Blocks** | Nothing today. Informs DEC-072 promotion / amendment / supersession. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058 producing the per-analyst accuracy distribution + multiplier-mapping ablation + recommendation; outcome is a recorded finding + (only on promotion) a DEC-072 amendment or successor DEC. |
| **Cross_ref** | DEC-055 §a (uniform placeholder); DEC-072 (the near-term proxy this measures against); DW-178 (the capture this depends on); DW-180 (parallel — EPS revision type; orthogonal Phase-7 ablation); FP-058 (Phase-7 home); literature: Fang-Yasuda 2009, Loh-Stulz 2011, Clement 1999, Stickel 1992, Mikhail-Walther-Willis 1997/2007, Hong-Kubik 2003; **ACT-353 (REGISTRATION)**. |

### DW-180: Analyst EPS-estimate revision parallel stream + rating-change third stream — Phase-7 revision-type ablation

| Field | Value |
|---|---|
| **ID** | DW-180 (next-free after DW-179). |
| **Status** | **REGISTERED 2026-06-30 at ACT-353 — PHASE-7-GATED**. The current Signal #1 is built on the WEAKER of three available revision types per FMP (price-target); the literature-strongest (EPS-estimate revisions) and the sparse-but-real (rating changes) are AVAILABLE-BUT-UNUSED. |
| **Tier** | A — revision-type is the SECOND-LARGEST documented lever for Signal #1 (after per-analyst accuracy). |
| **Title** | Stand up TWO parallel signal streams alongside (NOT replacing) the live price-target Signal #1: (i) `analyst_eps_revision` consumed from FMP `/analyst-estimates` consensus deltas (the IBES "Earnings Revisions" factor analog — Gleason-Lee 2003, Chan-Jegadeesh-Lakonishok 1996); (ii) `analyst_rating_change` consumed from FMP `/grades` (Womack 1996, Barber-Lehavy-McNichols-Trueman 2001). Both Phase-7-gated for promotion; both stood up as separate signal IDs so the combiner can learn revision-type weights. |
| **Why Phase-7 only** | Promotion (replace #1 with EPS; stack all three; weight by combiner) requires a trained combiner; pre-data revision-type choice would be fabricate-a-candidate. The PARALLEL stand-up itself is buildable now once capture infrastructure is in place, but is part of the consolidated post-audit weekend slate (ranked against DW-178 etc.). Note: FMP /analyst-estimates is consensus-snapshot, not per-analyst-delta — building per-revision rows requires diffing consecutive snapshots (which has its own staleness/coverage caveats vs the gold-standard IBES feed; DW-181 II vendor charter covers the gold-standard path). |
| **Scope (at activation)** | (a) Build FMP /analyst-estimates fetcher + consecutive-snapshot differ to produce per-period EPS-estimate revision events; (b) build FMP /grades fetcher for rating-change events; (c) compute analogous Signal #1 contributions (direction × magnitude × decay × credibility-or-1.0); (d) emit as `analyst_eps_revision` and `analyst_rating_change` signal IDs (parallel, not replacement); (e) Phase-7 ablation under FP-058 measuring marginal NDCG@25 / IC vs the live price-target Signal #1; (f) recommendation = replace / stack / drop per revision-type. |
| **Depends-on** | FP-058 activation for promotion decision; FMP /analyst-estimates + /grades endpoints (both confirmed available on existing FMP plan, no procurement needed); the consolidated weekend-slate ranking for build-slot allocation. |
| **Blocks** | Nothing today. Informs combiner revision-type weighting. |
| **Future Owner Phase** | Build = post-audit weekend slate (ranked); promotion = Phase 7 (FP-058). |
| **Resolution shape** | A build FP for the two parallel fetchers + signal emitters (when ranked in the weekend slate) + a Phase-7 ablation FP under FP-058 with the three-stream comparison and recommendation. |
| **Cross_ref** | DEC-055 (the §a uniform-credibility placeholder applies symmetrically to all three streams pre-DW-179); DEC-072 (tier-weight; if built, applies symmetrically to all three streams); literature: Gleason-Lee 2003 *AR*, Chan-Jegadeesh-Lakonishok 1996 *JF*, Brav-Lehavy 2003 *JF* (price-target accuracy ~38%), Bradshaw-Brown-Huang 2013, Womack 1996, Barber-Lehavy-McNichols-Trueman 2001 *JF*; FP-058 (Phase-7 home); **ACT-353 (REGISTRATION)**. |

### DW-181: Analyst II All-America ranking weight — vendor procurement charter (LARGEST skill lever, vendor-gated)

| Field | Value |
|---|---|
| **ID** | DW-181 (next-free after DW-180). |
| **Status** | **REGISTERED 2026-06-30 at ACT-353 — VENDOR-GATED, PRE-LIVE NICE-TO-HAVE, NOT A BLOCKER**. Sister to DW-166 (borrow-rate vendor) and DW-174 (daily-SI vendor). |
| **Tier** | B — vendor procurement; the LARGEST literature-documented skill lever (II All-America rankings are the gold-standard analyst-skill measurement per Fang-Yasuda 2009 and the broader rankings literature) but DW-178 + DW-179 give us a near-gold-standard internal proxy that does not require the vendor spend. |
| **Title** | Charter procurement of Institutional Investor All-America Research Team rankings (paid product; not in FMP/Finnhub free or standard tiers) as a direct ranked-vs-not skill weight `iiWeight(analystName, year)` that can layer on top of DEC-072 tier-weight and DW-179 accuracy-weight (or supersede them at the high end if measured II coverage dominates). |
| **Why deferred** | Internal proxies (DEC-072 tier + DW-179 accuracy) are sufficient for FP-058 baseline; II procurement is a budget call best made once we know whether the internal proxies are leaving meaningful edge on the table. Sequenced AFTER DW-179 evaluation so the marginal value of II is measurable, not assumed. |
| **Scope (at activation)** | (a) Vendor evaluation (II direct vs Refinitiv/IBES bundle that includes II coverage); (b) coverage check (what % of our FMP-surfaced analystName entries have II coverage in any given year); (c) join-key strategy (II uses analyst names; FMP also uses names; entity-resolution caveats); (d) DEC charter for the weight shape if procured. |
| **Depends-on** | DW-179 outcome (informs the marginal-value case); operator budget approval. |
| **Blocks** | Nothing today; informs the long-run analyst-skill weighting shape. |
| **Future Owner Phase** | Pre-live nice-to-have; not phase-gated. |
| **Resolution shape** | A procurement charter / vendor-evaluation FP; promotion to live weighting requires a DEC amendment to DEC-072 or a successor DEC. |
| **Cross_ref** | DEC-072 (the near-term internal proxy); DW-179 (the internal-measurement alternative); DW-166 (borrow-rate vendor charter — sister); DW-174 (daily-SI vendor charter — sister); literature: Fang-Yasuda 2009 (II rankings effectiveness); **ACT-353 (REGISTRATION)**. |

### DW-182: Analyst decay-horizon ablation — Phase-7 (5d / 10d / 20d half-life shadow variants)

| Field | Value |
|---|---|
| **ID** | DW-182 (next-free after DW-181). |
| **Status** | **REGISTERED 2026-06-30 at ACT-353 — PHASE-7-GATED, MEASUREMENT-ONLY**. The current `exp(−age/5)` decays revisions to ~nothing within ~14 days, but documented analyst-revision drift persists WEEKS-TO-MONTHS (Gleason-Lee 2003 ≥ 3 months; Womack 1996 ~ 1–6 months; Brav-Lehavy 2003 ~ 1 month). Also inconsistent with PEAD's 20-day half-life for a similar underreaction phenomenon. Pre-data tuning (5d → 20d on intuition) is the silent ROI change explicitly forbidden by the ROI guardrails. |
| **Tier** | B — calibration / parameter ablation; not a primary signal mutation. |
| **Title** | Phase-7 ablation of the decay-horizon parameter in `compute-analyst-revision.ts` `exp(−age/decay_days)`: shadow variants at decay_days ∈ {5, 10, 15, 20, 30} measured against the live decay_days = 5; expected outcome (per literature) is that a longer half-life captures more accrued drift, but the ablation MEASURES, does not assume. Output: hit-rate / IC vs decay curve; recommendation = keep 5d if within noise band, else propose a DEC amendment FP. |
| **Why Phase-7 only** | Decay-horizon is a literature-anchored CALIBRATION parameter — properly resolved by measurement, not guess. Guess-tuning 5d → 20d would itself be the "silent ROI change" the guardrails forbid; the right resolution is the shadow-variant comparison. |
| **Scope (at activation)** | (a) Stand up 4 shadow variants alongside live `analyst_revision`; (b) measure IC / NDCG@25 / decay-curve fit vs accrued labels; (c) regime-conditional check (does the optimal decay differ in high-vol vs low-vol regimes?); (d) recommendation per FP-058. |
| **Depends-on** | FP-058 activation; accrued labels. Independent of DW-178/179/180/181 (decay is orthogonal to credibility and revision-type). |
| **Blocks** | Nothing today; informs decay-parameter tune at amendment time. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058 producing the decay-horizon sensitivity table + recommendation; outcome is a recorded finding + (only on tune) downstream DEC-055 amendment FP. |
| **Cross_ref** | DEC-055 §b (the decay specification this measures); PEAD's ~20-day half-life as the comparable underreaction-drift calibration in our own signal stack; literature: Gleason-Lee 2003 *AR*, Womack 1996 *JF*, Brav-Lehavy 2003 *JF*, Stickel 1995; FP-058 (Phase-7 home); ROI guardrails (decay-tune is ROI-changing → MEASURED not GUESSED); **ACT-353 (REGISTRATION)**. |

### DW-183: Insider buy/sell-asymmetry Phase-7 ablation — measures whether the dropped sell-side carries recoverable edge

| Field | Value |
|---|---|
| **ID** | DW-183 (next-free after DW-182). |
| **Status** | **REGISTERED 2026-06-30 at ACT-354 — PHASE-7-GATED, MEASUREMENT-ONLY**. Stands up the recovery comparator for DEC-073 (insider buys-only): if the dropped sell-side turns out to carry edge in OUR universe, this DW provides the measured β that a DEC-073 amendment FP would adopt. |
| **Tier** | A — Phase-7 ablation of a signal-construction now-fix; the comparator that makes the DEC-073 decision recoverable. |
| **Title** | 3-arm Phase-7 ablation of Signal #4 sign-handling: (i) **symmetric** (the §6.5 shadow variant stood up by DEC-073 clause g — today's `+1/−1`), (ii) **sell-down-weight** (β grid e.g. `{0.2, 0.3, 0.4}`, literature-anchored range from Jeng-Metrick-Zeckhauser asymmetry magnitude), (iii) **buys-only** (the live signal after DEC-073). Output: IC / NDCG@25 / hit-rate / decile-spread curves for each arm vs accrued forward returns; recommendation = keep buys-only / amend DEC-073 with a measured β / re-introduce symmetric (if equal). |
| **Why Phase-7 only** | The buys-only NOW-FIX (DEC-073) is literature-direct and parameter-free; the down-weight β is the parameter-introducing alternative explicitly REJECTED as the now-fix per DEC-073 clause (b) because shipping a guessed β would be the analyst-tier-weight anti-pattern. Phase-7 MEASURES β rather than guessing it. |
| **Scope (at activation)** | (a) Confirm symmetric shadow variant landed by DEC-073 build (clause g); (b) stand up the 3-arm sell-down-weight β grid as additional §6.5 shadow variants alongside the live buys-only and the symmetric shadow; (c) measure IC / NDCG@25 / decile-spread vs accrued labels under FP-058; (d) recommendation per FP-058. |
| **Depends-on** | DEC-073 build (clauses a + g — buys-only live + symmetric shadow); FP-058 activation; accrued labels. Independent of DW-184 (decay) and DW-185 (CMP classifier) — they compose. |
| **Blocks** | Nothing today; informs any DEC-073 amendment FP. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058 producing the buy/sell-asymmetry sensitivity table + recommendation; outcome is a recorded finding + (only on a non-buys-only winner) a DEC-073 amendment FP with measured β. |
| **Cross_ref** | DEC-073 clauses (b) + (g) (this measures what they pinned); DEC-058 (insider role-weights + dedup); §4.4.4 (the spec); literature: Jeng-Metrick-Zeckhauser 2003 *RES*, Lakonishok-Lee 2001 *RFS*, Seyhun 1998; FP-058 (Phase-7 home); ROI guardrails (β is ROI-changing → MEASURED not GUESSED); **ACT-354 (REGISTRATION)**. |

### DW-184: Insider decay-horizon Phase-7 ablation — `exp(−age/14)` is likely too fast vs months-long insider drift

| Field | Value |
|---|---|
| **ID** | DW-184. |
| **Status** | **REGISTERED 2026-06-30 at ACT-354 — PHASE-7-GATED, MEASUREMENT-ONLY**. The current `exp(−age/14)` decays insider-trade contribution to ~37% by day 14, ~14% by day 28, ~5% by day 42, ~1% by day 64 — near-total truncation by ~6 weeks. Documented insider-trade drift persists 6–12 MONTHS (Lakonishok-Lee 2001 *RFS*: abnormal-return drift on insider-buy portfolios out to 6–12 months; Cohen-Malloy-Pomorski 2012 *JF*: opportunistic-buy ~82 bp/month sustained over the post-trade year; Ali-Hirshleifer 2017 replication: drift horizon ~1 year for skilled-insider trades). Same finding-shape as DW-182 (analyst 5d decay), MORE SEVERE because insider drift is longer than analyst-revision drift. |
| **Tier** | B — calibration / parameter ablation; not a primary signal mutation. |
| **Title** | Phase-7 ablation of the decay-horizon parameter in `compute-insider.ts` `exp(−age/decay_days)`: shadow variants at decay_days ∈ {14 (live), 30, 60, 90, 180} measured against accrued forward returns; expected outcome (per literature) is that a longer half-life captures more of the months-long drift, but the ablation MEASURES, does not assume. Output: IC / hit-rate / decile-spread vs decay curve; recommendation = keep 14d if within noise band, else propose a DEC amendment FP. |
| **Why Phase-7 only** | Decay-horizon is a literature-anchored CALIBRATION parameter — properly resolved by measurement, not guess. Guess-tuning 14d → 60d or 90d on intuition would itself be the silent ROI change the guardrails forbid; the right resolution is the shadow-variant comparison. Same disposition as DW-182 (analyst decay). |
| **Scope (at activation)** | (a) Stand up 4 shadow variants alongside the live `insider` signal at decay_days ∈ {30, 60, 90, 180}; (b) measure IC / NDCG@25 / decay-curve fit vs accrued labels; (c) regime-conditional check (does the optimal decay differ in high-vol vs low-vol regimes?); (d) recommendation per FP-058. |
| **Depends-on** | FP-058 activation; accrued labels (months-long, by construction of the question). Independent of DW-183 (sign-handling) and DW-185 (CMP classifier). |
| **Blocks** | Nothing today; informs decay-parameter tune at amendment time. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | A Phase-7 ablation FP under FP-058 producing the decay-horizon sensitivity table + recommendation; outcome is a recorded finding + (only on tune) downstream DEC amendment FP. |
| **Cross_ref** | DEC-058 (the live decay spec this measures); DW-182 (analyst decay — sister ablation, same shape); literature: Lakonishok-Lee 2001 *RFS*, Cohen-Malloy-Pomorski 2012 *JF*, Ali-Hirshleifer 2017; FP-058 (Phase-7 home); ROI guardrails (decay-tune is ROI-changing → MEASURED not GUESSED); **ACT-354 (REGISTRATION)**. |

### DW-185: Insider opportunistic-vs-routine CMP classifier — Phase-7 + free-EDGAR backfill ETL (NOT time-sensitive)

| Field | Value |
|---|---|
| **ID** | DW-185. |
| **Status** | **REGISTERED 2026-06-30 at ACT-354 — PHASE-7-GATED**. **EXPLICITLY NOT TIME-SENSITIVE** (distinct from analyst DW-178): MIG-095 (ACT-191) already persists `owner_cik` on every parsed Form 4 row, and the DEC-058 §h dedup key `(issuer_cik, owner_cik, transaction_date, transaction_seq)` already keys by per-insider identity. Forward per-insider history accrues passively — no urgent capture step required. The time-sensitive list stays at TWO (DW-172 PEAD T-0 capture, DW-178 analyst per-revision capture); insider does NOT join. |
| **Tier** | A — the FRONTIER lever per Cohen-Malloy-Pomorski 2012 (~95% of the insider-trade edge concentrates in the opportunistic subset; routine insiders ~0 alpha). Buildable on FREE EDGAR data, NOT vendor-gated. |
| **Title** | Phase-7 classification of insiders into **routine** (traded in the same calendar month for 3+ consecutive prior years — mechanical / non-informational) vs **opportunistic** (everyone else — irregular, on information); restrict the live signal contribution (or stand up as a shadow variant) to the opportunistic subset. Per CMP 2012, the opportunistic subset earns ~82 bp/month long-short alpha; routine subset ~0; ~95% of the signal's predictive power sits with opportunistic. Opportunistic insiders also lead earnings surprises and analyst-forecast revisions — genuine information advantage. |
| **Why Phase-7 only (NOT a now-fix)** | The classification requires ≥ 3 years of per-insider Form 4 history. Forward accrual already happens (MIG-095). One-shot historical backfill of 3–5 years of EDGAR Form 4 filings for universe insiders is required to classify regularity at promotion time. The backfill is FEASIBLE (EDGAR is free; our pipeline parses Form 4 cleanly) but non-trivial throughput/storage work — not a weekend-bundle item, and gains compound with DEC-073 (buys-only) and DW-184 (decay) so sequencing makes sense. |
| **Scope (at activation)** | (a) **Backfill ETL spec**: pull 3–5 years of historical Form 4s for all universe-resident `owner_cik`s from EDGAR (free, public, paced per the §SEC-EDGAR-pipeline hardening this session); store in a new `insider_history` table keyed by `(owner_cik, transaction_year_month)`. (b) **Classifier**: for each `(owner_cik, current_calendar_month)`, label `routine` iff the insider transacted in the same calendar month in 3+ consecutive prior years; else `opportunistic`. (c) **Shadow variants**: opportunistic-only insider signal alongside the live (all-insider) signal on §6.5 harness. (d) **Compose with DEC-073**: opportunistic-buys-only is the natural composition; measure both subsets. (e) Recommendation per FP-058. |
| **Depends-on** | MIG-095 forward `owner_cik` persistence (LANDED at ACT-191 — confirmed). FP-058 activation. Independent of DW-183 (sign-handling) and DW-184 (decay) — they compose. |
| **Blocks** | Nothing today; informs the largest documented insider-signal lever. |
| **Future Owner Phase** | Phase 7 (FP-058) — backfill ETL is a Phase-7 build prerequisite, not a weekend-slate item. |
| **Resolution shape** | A Phase-7 FP under FP-058 implementing the backfill ETL + classifier + shadow variants + measurement; outcome is a recorded finding + (on a clean opportunistic-subset winner) a DEC-073 amendment FP or new DEC restricting Signal #4 contribution to the opportunistic subset. |
| **Cross_ref** | MIG-095 / ACT-191 (the owner_cik persistence that makes this NOT-time-sensitive — the load-bearing distinction from analyst DW-178); DEC-058 §h (dedup key); DEC-073 (buys-only composes with opportunistic-only); §4.4.4 (the spec); literature: Cohen-Malloy-Pomorski 2012 *JF* "Decoding Inside Information" (the canonical result); Ali-Hirshleifer 2017 (replication, drift-horizon extension to ~1 year); FP-058 (Phase-7 home); **ACT-354 (REGISTRATION)**. |

### DW-186: News articleCount attention-feature — TIME-SENSITIVE capture (3rd time-sensitive item; meta discarded at persist; feed_items TTL 24h)

| Field | Value |
|---|---|
| **ID** | DW-186. |
| **Status** | **REGISTERED 2026-06-30 at ACT-355 — TIME-SENSITIVE CAPTURE**. STEP-A verification (HEAD `cd3abfd8`): the news kernel `computeNewsSentiment` (compute-news-sentiment.ts:218) returns `meta.articleCount` + `meta.prExcludedCount` + `meta.unmappedPublisherCount`, BUT the orchestrator path drops meta — `captureSignalObservations` (shared/missingness-capture.ts:44-59) persists only `{value, is_present, gics_sector, computed_at, carried_forward}` to `signal_observations`; no meta column exists. The intermediate `signal_queue_feed_items` table retains per-article rows (recovery surface), but its `stagingTtlSec = 86400` (news-sentiment-queue-registration.ts:92) means the sweeper prunes it 24h after run terminal-state → **historical articleCount is NOT recoverable beyond 24h**. Joins the time-sensitive list at THREE (DW-172 PEAD-T0, DW-178 analyst per-revision, DW-186 news articleCount). |
| **Tier** | A — the ONE modest non-zero ROI lever surfaced by the #8 audit; the discrete `{−1, 0, +1}` mapping (DEC-056 §a) deliberately discards intensity for noise-defense (Loughran-McDonald 2011: sign > magnitude for financial text), and `articleCount` recovers the discarded magnitude WITHOUT trusting Benzinga's calibration. Da-Engelberg-Gao 2011 ("In Search of Attention", *J. Finance*): news-attention has DOCUMENTED INDEPENDENT predictability distinct from sentiment-direction. |
| **Title** | Persist `meta.articleCount` (and `prExcludedCount`, `unmappedPublisherCount`) alongside the z-scored news sentiment value at capture time, then surface `articleCount` as a SEPARATE combiner feature (news-attention/intensity), distinct from the existing sentiment-direction value. |
| **Why time-sensitive (not cheap-surface-it)** | Two alternative recovery surfaces verified absent: (1) `signal_observations` schema has no meta JSONB column → meta is structurally dropped at persist; (2) `signal_queue_feed_items` retains raw rows but is sweeper-pruned at TTL 24h → historical counts unrecoverable. Every day without capture = a day of permanent loss of the attention series for Phase-7 measurement. Sister to DW-172 (PEAD T-0) and DW-178 (analyst per-revision) in the time-sensitive-capture anti-pattern. |
| **Scope (at activation — weekend-bundle candidate)** | (a) Schema-additive: either add an `article_count INTEGER NULL` column to `signal_observations` (gated to news rows; NULL elsewhere) OR stand up a sidecar `news_attention_observations(operator_id, signal_id, as_of_date, ticker PK; article_count, pr_excluded_count, unmapped_publisher_count)` — pick the lighter governance path at build. (b) Threading: extend `SignalRow` (or a news-specific extension) to carry the meta counts; wire the news queue finalizer to populate from `computeNewsSentiment` result's `meta`. (c) Combiner surfacing: register `news_attention_7d` as a separate combiner feature alongside `news_sentiment_7d` — DOES NOT modify the existing sentiment value or its z-score. (d) Anti-completion-theater: no change to the sentiment-direction kernel; no change to the discrete `{−1, 0, +1}` mapping (which remains CORRECT not over-conservative per Loughran-McDonald). |
| **Depends-on** | DEC-056 (news construction, UNCHANGED); the news-sentiment queue path (FP-048); FP-058 for the eventual combiner weighting decision. |
| **Blocks** | Phase-7 Da-Engelberg-Gao-style attention-as-independent-feature measurement; cannot retrospectively analyze attention dynamics without persisted history. |
| **Future Owner Phase** | Weekend-bundle candidate (cheap forward-only build; sister to DW-172 / DW-178). Phase-7 (FP-058) decides whether `news_attention` earns combiner weight once data accrues. |
| **Resolution shape** | (a) one-shot schema migration + capture wiring as one PR (no live ranker / live PnL change — additive only; combiner does not exist yet so no model retrain triggered); (b) Phase-7 measurement of standalone attention IC + incremental combiner contribution under FP-058. |
| **Cross_ref** | DEC-056 §a (discrete sentiment map — UNCHANGED) + §e (PR-wire exclusion — UNCHANGED); compute-news-sentiment.ts:218 (the meta producer); shared/missingness-capture.ts:44 (the persistence surface that DROPS meta — the verified gap); news-sentiment-queue-registration.ts:92 (stagingTtlSec=86400 — the 24h prune that makes feed_items non-recoverable); literature: Da-Engelberg-Gao 2011 *J. Finance* "In Search of Attention" (the independent-attention-predictability anchor); Loughran-McDonald 2011 *J. Finance* "When Is a Liability Not a Liability?" (sign > magnitude for financial text — the discrete-mapping defense); DW-172 / DW-178 (sister time-sensitive captures); DW-187 (the standalone-vs-combiner Phase-7 decomposition this attention feature feeds); FP-058 (Phase-7 home); **ACT-355 (REGISTRATION)**. |

**Split note (2026-06-28, DW-186a build):** Operator chose Option C — narrow DW-186a to the one observationally-real field (`article_count`) and charter the two structural-0 fields as DW-186b. RESOLVED-PARTIAL: DW-186a landed (generic `captureMeta` seam at queue-finalizer + typed `news_attention_observations(article_count NOT NULL)` store; capture-only; zero PnL/ranker effect; §22.5.1 live-DB verified). The two remaining fields (`pr_excluded_count`, `unmapped_publisher_count`) move to DW-186b below — they are structural-0 today (PR-excluded dropped pre-stage at news-sentiment-queue-registration.ts:225; `tierMapped` hardcoded `true` at :254 because `FeedItemRecord` carries no `tier_mapped` channel) and capturing them as 0/NULL would be a phantom in the operator's "every value real" sense.

### DW-186b: News attention-capture upstream channels — PR-excluded + tier-mapped survival

| Field | Value |
|---|---|
| **ID** | DW-186b. |
| **Status** | **CHARTERED 2026-06-28 at DW-186a build — DEFERRED, NOT TIME-SENSITIVE**. Split from DW-186 per operator Option C: DW-186a captured `article_count` (observationally real); DW-186b captures the two structural-0 fields once their upstream channels exist. NOT time-sensitive — unlike `articleCount`, these fields would be structural-0 today regardless of whether we wrote a row, so deferral costs nothing recoverable. Recovery becomes possible only after the upstream `fetchPage` / `signal_queue_feed_items` carry the channels. |
| **Tier** | A — completes the news-attention triple from DEC-056 + Da-Engelberg-Gao 2011; the two fields add (i) PR-noise observability (how much of the in-window flow was wire-spam) and (ii) tier-mapping coverage (how many articles fell back to DEFAULT_TIER_WEIGHT — the calibration-trust signal). |
| **Title** | Carry PR-excluded entries through staging with an `excluded` flag; extend `FeedItemRecord` + `signal_queue_feed_items` with `tier_mapped`; teach news reconstruction to read the real `tierMapped`; add `pr_excluded_count` and `unmapped_publisher_count` (NOT NULL) to `news_attention_observations`. |
| **Why structural-0 today (the verified phantom risk)** | (a) `prExcludedCount`: PR-excluded entries are dropped at `news-sentiment-queue-registration.ts:225` (`if (cls.excluded) continue` — DEC-056 §(e)) BEFORE the staging push, so they never reach `signal_queue_feed_items`; the finalizer's feed-mode reconstruction never sees them; `prExcludedCount` is 0 for every row, every run, by architecture. (b) `unmappedPublisherCount`: `FeedItemRecord` (queue-config.ts:66-77) has fields `articleId, ticker, sentimentNum, tierWeight, publishedUtc` — no `tier_mapped` channel. The news reconstruction at `news-sentiment-queue-registration.ts:254` hardcodes `tierMapped: true`; `unmappedPublisherCount` is therefore 0 for every row, every run, by architecture. Writing either as 0 (or as NULL with NOT-NULL schema) would persist a structurally-fabricated value — exactly the operator's "every value real, no phantom" rule forbids. |
| **Scope (at activation)** | (a) extend `FeedItemRecord` with `excluded: boolean` and `tier_mapped: boolean`; (b) migration: add `excluded boolean NOT NULL DEFAULT false` and `tier_mapped boolean NOT NULL DEFAULT true` columns to `signal_queue_feed_items`; (c) news `fetchPage` stops dropping PR-excluded rows at line 225 — instead pushes them with `excluded: true, sentimentNum: 0, tierWeight: 0`; (d) news reconstruction at line 245-260 reads the real `excluded` and `tier_mapped` fields; (e) migration: add `pr_excluded_count INTEGER NOT NULL` and `unmapped_publisher_count INTEGER NOT NULL` to `news_attention_observations`; (f) `captureNewsAttention` extended to write both fields exactly as `computeNewsSentiment` produces them in `meta`. Each step ADDITIVE, capture-only — no change to news signal value / z-score / ranker / PnL. |
| **Depends-on** | DW-186a landed (the typed store + generic seam — DONE); DEC-056 §(e) PR-exclusion semantics UNCHANGED (rows are marked excluded, not re-included in the score). |
| **Blocks** | Phase-7 Da-Engelberg-Gao-style PR-noise and tier-coverage measurement; the two fields are the news-attention quality lens (`articleCount` is the volume lens — landed by DW-186a). |
| **Future Owner Phase** | Sequenced AFTER DW-178 (analyst per-revision) and DW-172 (PEAD T-0 consensus) land — the three captures share the DW-186a generic seam and should not race the upstream feed_items migration. |
| **Resolution shape** | (a) one-shot upstream migration (FeedItemRecord + signal_queue_feed_items columns) + news fetchPage/reconstruction update + news_attention_observations columns + capture extension, one PR; (b) §22.5.1 live-DB evidence; (c) backfill is NOT possible (historical data already dropped pre-stage / collapsed via hardcoded `tierMapped: true`) — series starts at the build day, same forward-only posture as DW-186a. |
| **Cross_ref** | DW-186a (the seam + article_count store this completes); DEC-056 §(e); `news-sentiment-queue-registration.ts:225` (pre-stage drop point that loses prExcludedCount); `news-sentiment-queue-registration.ts:254` (hardcoded `tierMapped: true` that loses unmappedPublisherCount); `queue-config.ts:66-77` (FeedItemRecord — channel-gap surface); Da-Engelberg-Gao 2011 *J. Finance*; Loughran-McDonald 2011 *J. Finance*. |

### DW-187: News standalone-vs-combiner-feature decomposition — Phase-7 (PEAD-analogue; honest down-weight-or-drop)

| Field | Value |
|---|---|
| **ID** | DW-187. |
| **Status** | **REGISTERED 2026-06-30 at ACT-355 — PHASE-7-GATED**. Direct analogue of the PEAD treatment: measure standalone IC + incremental combiner contribution under FP-058; honest down-weight-or-drop decision if standalone IC ≈ 0 AND incremental combiner contribution is dominated by other features. |
| **Tier** | A — the EXISTENTIAL question for Signal #8 per the #8 audit: literature consensus is that news-sentiment is the WEAKEST and MOST-CONTESTED of the nine signals — small pre-cost effect (Tetlock 2007/2008: ~bp/day), TWO-PHASE drift-then-reversion (NOT clean drift), heavily commoditized post-2010 (Heston-Sinha 2017; Ke-Kelly-Xiu 2019: daily-horizon predictability decayed materially), likely a combiner-feature only, NOT standalone alpha. Same class as PEAD. |
| **Title** | Phase-7 decomposition of Signal #8 into (a) standalone IC in the long-short universe net of execution costs, (b) incremental combiner contribution above the other eight signals + the regime feature, (c) interaction with the DW-186 `news_attention` feature once accrued. Recorded outcome: keep / down-weight at combiner / drop. |
| **Why Phase-7 only** | Cannot measure standalone or incremental IC without (i) a trained combiner (does not yet exist at HEAD `cd3abfd8`) and (ii) accrued forward returns over a meaningful horizon. Pre-data tuning of the combiner weight is silent ROI change and is forbidden under the ROI guardrails. |
| **Scope (at activation)** | (a) compute per-as-of IC of the live `news_sentiment_7d` signal against forward returns at multiple horizons (1d / 5d / 20d) to test the Tetlock drift-then-reversion shape on OUR universe; (b) compute SHAP / leave-one-out combiner contribution vs the other eight; (c) once DW-186 capture has accrued ≥ 6 months, compose `news_attention` orthogonally and re-measure; (d) honest recommendation: keep current weight / down-weight at combiner / drop signal. |
| **Depends-on** | A trained combiner (Phase-7 work); accrued forward returns; DW-186 capture (for the attention-feature arm). |
| **Blocks** | Nothing today; the live signal continues to run at its current (uncalibrated) combiner weight until FP-058 makes a measured decision. |
| **Future Owner Phase** | Phase 7 (FP-058) — measure-and-lock home. |
| **Resolution shape** | A recorded Phase-7 finding plus (on a down-weight or drop verdict) a DEC pinning the combiner-weight or signal-removal change. |
| **Cross_ref** | DEC-056 (Signal #8 construction); §4.4.8 (the spec); literature: Tetlock 2007 *J. Finance* "Giving Content to Investor Sentiment" (drift-then-reversion); Tetlock-Saar-Tsechansky-Macskassy 2008 *J. Finance* "More Than Words" (firm-level effect); Heston-Sinha 2017 (post-2010 daily-horizon decay); Ke-Kelly-Xiu 2019 NBER WP 26186 (declining horizon-sensitive returns); Da-Engelberg-Gao 2011 *J. Finance* (attention as independent channel — composes via DW-186); DW-186 (attention-feature capture); DW-188 (biphasic-kernel ablation — composes); DW-189 (vendor-eval — gated by this decomposition's standalone result); FP-058 (Phase-7 home); **ACT-355 (REGISTRATION)**. |

### DW-188: News biphasic-kernel ablation — Phase-7 (drift-then-reversion vs `exp(−age/24)`)

| Field | Value |
|---|---|
| **ID** | DW-188. |
| **Status** | **REGISTERED 2026-06-30 at ACT-355 — PHASE-7-GATED**. The current `age_weight = exp(−age_hours / 24)` is "defensible by accident" per the #8 audit: a 24h half-life makes the reversion-phase weight tiny (~6% at day-4), so the current kernel DODGES rather than EXPLOITS Tetlock's documented reversion. A biphasic kernel would let the signal exploit the reversion leg (short past-week's most-positive / long past-week's most-negative on the reversion phase). |
| **Tier** | A — modest non-zero lever conditional on DW-187's standalone result being non-trivial; if Signal #8 is a combiner-feature-only with near-zero standalone IC, this ablation's ROI ceiling is correspondingly low. Sequence after DW-187. |
| **Title** | Phase-7 shadow variants of the news age-weight: (a) live `exp(−age/24)` (baseline); (b) scaled-difference-of-exponentials biphasic (positive lobe day-0→day-T_flip; negative lobe day-T_flip→day-7); (c) sign-flip-at-horizon-T parametric family. Fit T_flip from accrued data; pre-data guess-tuning of T_flip forbidden as silent ROI change. |
| **Why Phase-7 only** | Fitting the sign-flip horizon requires accrued per-name news + forward returns; pre-data tuning is silent ROI change. Also: the reversion phase MAY HAVE COLLAPSED post-2010 commoditization (Heston-Sinha; Ke-Kelly-Xiu) — verifying the reversion still exists in liquid post-2020 names is a Phase-7 prerequisite, not an assumption. |
| **Scope (at activation)** | (a) build the shadow variants on the §6.5 harness; (b) measure standalone IC of each variant at the daily / weekly horizon; (c) measure combiner-contribution delta vs DW-187 baseline; (d) compose with DW-186 attention feature; (e) recorded outcome + (on a clear winner) a DEC-056 amendment FP. |
| **Depends-on** | DW-187 (standalone-vs-combiner result conditions the ROI ceiling); accrued forward returns; FP-058 activation. Independent of DW-186 in build, composes in measurement. |
| **Blocks** | Nothing today; live `exp(−age/24)` continues. |
| **Future Owner Phase** | Phase 7 (FP-058). |
| **Resolution shape** | Recorded finding + (on a clear winner) DEC-056 amendment FP. |
| **Cross_ref** | DEC-056 (Signal #8 construction); compute-news-sentiment.ts (the kernel); §4.4.8 (the spec — `exp(−age_hours / 24)`); literature: Tetlock 2007 (drift-then-reversion at the ~5-trading-day horizon — the biphasic shape this ablation tests); Tetlock-Saar-Tsechansky-Macskassy 2008 (firm-level confirmation); Heston-Sinha 2017 (post-2010 commoditization — the reversion-may-have-collapsed caveat); Ke-Kelly-Xiu 2019 (declining horizon-sensitivity); DW-187 (sequencing dependency); DW-186 (composes in measurement); FP-058 (Phase-7 home); **ACT-355 (REGISTRATION)**. |

### DW-189: News sentiment-vendor evaluation — charter, EXPLICITLY DEFERRED pending DW-187

| Field | Value |
|---|---|
| **ID** | DW-189. |
| **Status** | **REGISTERED 2026-06-30 at ACT-355 — CHARTER, EXPLICITLY DEFERRED**. Sister to DW-166 (rate-vendor charter), DW-174 (daily-SI vendor charter), DW-181 (II All-America ranking vendor charter) — same anti-pattern shape (don't procure behind an unmeasured signal). |
| **Tier** | B — explicitly second-order per the #8 audit: provider-sentiment quality (current source: Benzinga-via-Polygon, opaque rules+ML hybrid not academically benchmarked) IS a real bottleneck, but the EDGE itself (DW-187) is the binding constraint. A better classifier × a small/contested edge is still a small/contested edge. |
| **Title** | Evaluate RavenPack ESS / MarketPsych / Loughran-McDonald in-house classification against the incumbent Benzinga-via-Polygon `insights[].sentiment` field. Charter only; build EXPLICITLY DEFERRED pending the DW-187 standalone-vs-combiner result. |
| **Why deferred (not now-procure)** | RavenPack / MarketPsych pricing reported in the mid-five to low-six figures USD/year — NOT justified by a combiner-feature-grade signal. If DW-187 returns standalone IC ≈ 0 and incremental combiner contribution dominated by other features, the right response is down-weight at combiner, NOT vendor upgrade. Loughran-McDonald 2011 is FREE (academic dictionary + replication code) but requires in-house pipeline + maintenance — same don't-build-behind-unmeasured-signal logic applies. The Benzinga classifier's documented failure mode (false-positive negatives on neutral-but-pessimistic-toned earnings prose) is partly mitigated by the discrete `{−1, 0, +1}` collapse (DEC-056 §a) which sticks to the sign-only component Loughran-McDonald argue is the reliable part. |
| **Scope (at activation, post-DW-187)** | (a) RavenPack ESS trial evaluation (sign-agreement and IC-delta vs Benzinga on a shadow window); (b) MarketPsych trial evaluation (same shape); (c) Loughran-McDonald in-house pilot (build cost + maintenance estimate vs measured IC-delta); (d) procurement recommendation conditional on DW-187 returning non-trivial standalone or incremental contribution. |
| **Depends-on** | DW-187 (the gating standalone-vs-combiner decomposition); pricing reconfirmation at activation; legal/data-licensing review for vendor trials. |
| **Blocks** | Nothing today; Benzinga-via-Polygon continues as incumbent. |
| **Future Owner Phase** | Phase 7 (FP-058) — conditional activation only on DW-187 returning non-trivial result. |
| **Resolution shape** | (a) On DW-187 = drop/down-weight: VENDOR CHARTER CLOSES, no procurement; (b) on DW-187 = non-trivial contribution: vendor trial → measured IC-delta → procurement DEC or in-house build FP. |
| **Cross_ref** | DEC-056 (Signal #8 construction); compute-news-sentiment.ts + news-filters.ts (the Benzinga-via-Polygon consumer surface); literature: Loughran-McDonald 2011 *J. Finance* "When Is a Liability Not a Liability?" (free academic baseline + sign-over-magnitude argument); RavenPack ESS white-papers; MarketPsych vendor docs; Hutto-Gilbert 2014 (sentiment-classifier benchmarking methodology); sister vendor charters: DW-166 (borrow-rate vendor) / DW-174 (daily-SI vendor) / DW-181 (II All-America vendor); DW-187 (the gating measurement); FP-058 (Phase-7 home); **ACT-355 (REGISTRATION)**. |

### DW-190: Catalyst-as-interaction Phase-7 ablation (additive vs interaction vs excluded vs conditioning-gate) + per-event-type half-life grid calibration

| Field | Value |
|---|---|
| **ID** | DW-190. |
| **Status** | **REGISTERED 2026-06-30 at ACT-356 — PHASE-7-GATED**. Composes with DEC-074 (the now-fix that excludes catalyst from the §6.4 fallback summation set) and DEC-075 clause (d) (the Phase-7 acceptance criterion — trained combiner must outperform the exclusion-disciplined fallback on interaction-valued signals). NOT time-sensitive: per-event records (`active_catalyst_events`) persist at Stage 7 → catalyst events are recoverable for retrospective Phase-7 measurement. Time-sensitive-capture list stays at THREE (DW-172, DW-178, DW-186). |
| **Tier** | A — the EXISTENTIAL Phase-7 question for Signal #9: does catalyst earn its seat as an INTERACTION feature in a trained combiner at all? If the trained combiner does not measurably use catalyst as a salience gate beyond what direct signals already capture, the catalyst signal's value is unsubstantiated and a DEC dropping the signal becomes available on evidence. |
| **Title** | Phase-7 ablation of Signal #9 active-catalyst across four deployment shapes — catalyst-as-INTERACTION (trained combiner uses tree splits like `catalyst_z > τ AND pead_z > 0`); catalyst-as-ADDITIVE (the pre-DEC-074 baseline — pure additive direction-blind contribution); catalyst-EXCLUDED (the DEC-074 fallback regime — control); catalyst-as-CONDITIONING-GATE (horizon-selector that increases PEAD/news/momentum coefficients when a fresh event is present without contributing additively). Fold in the per-event-type half-life grid (DEC-057 §a is defensible PRIORS not measured): Earnings 48h / M&A 96h / FDA 72h / Regulatory 96h / Guidance 48h / Exec 72h / Analyst 24h / Partnership 36h / Dividend 36h / Splits 24h — calibrate exact hours against post-event return autocorrelation per event type; the relative ordering (M&A > Earnings > Analyst) is literature-defensible-now and is treated as a prior. |
| **Why Phase-7 only** | Measuring catalyst-as-interaction lift requires a trained combiner (does not exist at HEAD `cb17a66d` — `model_active_artifact_loader_not_wired_pending_3_3b_ii`). Pre-data tuning of the half-life table is silent ROI change and is forbidden under the ROI guardrails. The relative half-life ordering is literature-defensible enough to defer exact-hour calibration to measurement. |
| **Scope (at activation)** | (a) build the four deployment shadow variants on the §6.5 harness; (b) measure standalone IC + incremental combiner contribution of each across the universe and across regimes (vol-spike vs calm); (c) per-event-type half-life grid search against post-event return autocorrelation (24h / 36h / 48h / 72h / 96h / 168h candidates per event type, anchored to the literature ordering); (d) regime-conditional measurement (does interaction-use of catalyst help most in high-event-density windows?); (e) recorded outcome + (on a clear winner) a DEC-074 amendment FP or, on a measured null, a DEC dropping the signal. |
| **Depends-on** | A trained combiner (FP-058 work); accrued forward returns over a meaningful post-event horizon; DEC-074 (the fallback-regime baseline this ablation compares against); DEC-075 clause (d) (the acceptance criterion). Independent of DW-186 (news attention) in build; may compose at measurement if news_attention also reaches Phase-7 weight. |
| **Blocks** | Nothing today; live fallback continues at the post-DEC-074 exclusion-disciplined posture. |
| **Future Owner Phase** | Phase 7 (FP-058) — measure-and-lock home. |
...
### DW-197: Merger / spinoff corporate-action source (Polygon `/v3/reference/` has no merger or spinoff feed)

| Field | Value |
|---|---|
| **ID** | DW-197. |
| **Status** | **REGISTERED 2026-06-29 at ACT-378 — VENDOR-GATED, money-path correctness gap.** v1 4M.4 ships `polygon-corporate-actions-fetcher.ts` against `/v3/reference/splits` + `/v3/reference/dividends` only. Polygon does not expose a merger or spinoff feed on `/v3/reference/`. The applier dispatch table fully implements both action types (merger-cash via 4M.5a `closeLots`; merger-stock via ratio mutate + `successor_symbol` rename; spinoff via parent-trim + child-open via 4M.1 `writeOpenLot`), so the moment a source is wired the rows flow through unmodified. Until then: **degrades safely** — no rows in the table → no §7 BLOCK fires, no lot mutation, no silent corruption. |
| **Tier** | A — money-path basis correctness (an unprocessed merger or spinoff leaves the lot ledger reconciling against a delisted or wrong-symbol parent for as long as the position is open). |
| **Title** | Source merger and spinoff corporate-action rows from a vendor that exposes them (candidates: Tradier `/markets/etb/...` — investigate scope; SEC EDGAR Form 8937 — manual or scraped; commercial CA vendor — Wall Street Horizon / FactSet / EventVestor). |
| **Scope** | (a) Investigate vendor coverage (Polygon does NOT cover; Tradier may have a partial via the catalyst endpoints but uses a DIFFERENT normalized type — `CatalystFetchResult` — and lives in the signal-#9 active-catalyst domain, not the execution domain). (b) Build a new edge-resident execution-domain CA fetcher (sibling of `polygon-corporate-actions-fetcher.ts`) that upserts into `public.corporate_actions` with `action_type IN ('merger','spinoff')`. (c) Validate the typed-throw discipline (missing `successor_symbol` / `basis_allocation_pct` → THROW; missing `cash_per_share` on merger-cash → THROW). (d) Confirm the existing applier dispatch table consumes the new rows unchanged (the dispatch is action-type-discriminated; new sources flow through `applyOne`). (e) Document the operator backstop: if no vendor source is feasible, an operator manual-entry flow for merger/spinoff rows. |
| **Depends-on** | None (vendor-investigation). |
| **Blocks** | Phase-8 live-money authorization on any portfolio that may hold merger or spinoff candidates (Russell 3000 universe → non-trivial frequency). |
| **Future Owner Phase** | FP-061 closure pass or FP-062 broker MOVE cluster (whichever lands the next CA-vendor surface). |
| **Resolution shape** | One commit adding the new CA fetcher + adapter test + an end-to-end synthetic merger/spinoff applier run; OR a documented operator manual-entry SOP + admin UI. |
| **Cross_ref** | ACT-378 (REGISTRATION); MIG-144 (the `corporate_actions` table the new fetcher writes to); `corporate-action-applier.ts` (the dispatch table that consumes new rows); `polygon-corporate-actions-fetcher.ts` (the splits+dividends sibling); CROSSWIND §7 composer BLOCK; §7.6 / §7.8. |

### DW-198: Cash-dividend cash-ledger accrual (applier no-op stamps `applied_at`; cash-side not booked)

| Field | Value |
|---|---|
| **ID** | DW-198. |
| **Status** | **REGISTERED 2026-06-29 at ACT-378 — NOT a basis-correctness gap, a cash-balance accuracy gap.** Cash dividends do NOT mutate `cost_basis` (IRS treatment: dividends are income, not basis adjustments), so the 4M.4 applier is correctly a no-op on lots for `cash_dividend` rows. It DOES stamp `applied_at` to clear the §7 composer gate. The MISSING piece: the cash credit (`qty × cash_per_share`) is not booked into any internal cash-ledger row. Today the broker's `buying_power` reflects the credit (Alpaca records the dividend), so the §7 BP-read is correct at the broker layer; the GAP is the internal mirror. |
| **Tier** | B — accounting / reporting parity (does NOT cause a divergent trade; DOES cause an internal vs broker cash-balance reconciliation gap that surfaces at FP-062 broker-cash reconciliation and at year-end tax reporting for the dividend income portion). |
| **Title** | Book cash-dividend accruals into an internal cash-ledger table (or column on a future cash-balance table) so `(internal_cash + sum(qty × entry_price on open lots))` reconciles against broker `cash`. |
| **Scope** | (a) Decide cash-ledger shape (sibling table `longshort_cash_events` vs column on a future cash-balance row). (b) Extend the 4M.4 applier `cash_dividend` branch from no-op-on-lots to no-op-on-lots + write-cash-credit. (c) Wire the cash-credit into the year-end aggregator dividend-income reporting (currently empty — dividends are NOT capital gains, separate 1099-DIV not 1099-B). (d) Decide handling for short-side cash dividends (the SHORT pays the dividend to the lender — `qty × cash_per_share` DEBIT not credit; opposite sign). |
| **Depends-on** | A cash-ledger table OR a settled-cash row design (today there is none; deferred until the FP-062 broker-cash reconciliation pass forces the shape). |
| **Blocks** | FP-062 broker-cash reconciliation (the internal vs broker cash diff will surface uncredited dividends as drift); year-end 1099-DIV reporting parity. |
| **Future Owner Phase** | FP-062 broker MOVE cluster (cash-balance reconciliation is a natural home) or a dedicated cash-ledger FP. |
| **Resolution shape** | One commit adding the cash-ledger write at the `cash_dividend` branch + the short-side debit handling + a reconciliation test. |
| **Cross_ref** | ACT-378 (REGISTRATION); MIG-144 (the `corporate_actions` table that feeds the cash credit); `corporate-action-applier.ts:applyOne` `cash_dividend` branch (the call site); FP-062 / DW-058; CROSSWIND §7.6. |

### DW-199: Real broker corporate-action basis cross-check (Alpaca `/v2/positions.avg_entry_price` vs internal `cost_basis`)

| Field | Value |
|---|---|
| **ID** | DW-199. |
| **Status** | **DISCHARGED 2026-06-29 at ACT-382 (FP-062 sub-step 6I.2b).** `AlpacaCorporateActionFetcher` (`supabase/functions/_shared/longshort-broker/alpaca-corporate-action-fetcher.ts`) implements the real cross-check: composes `createInternalCorporateActionStatusFetcher` for recent-action provenance, overlays `GET /v2/positions/{symbol}.avg_entry_price` vs internal qty-weighted `SUM(cost_basis*qty)/SUM(qty)` over open lots, 1¢ tolerance, typed-throw `BrokerPositionMissingForCAReconciliation` on 404 / empty-lots (NO sentinel). Wired into `BrokerInterfaces.corporateActionFetcher` at `broker-bootstrap.ts`. The internal stand-in is RETAINED per §2 axiom 2 (cross-check posture) — it provides the creds-free fallback at the eventual verifier orchestration call site. ORIGINAL REGISTRATION (2026-06-29 at ACT-378 — preserved per Rule 8): 4M.4 shipped the soft-dependent `createInternalCorporateActionStatusFetcher` that reported `broker_basis_adjusted = (corporate_actions.applied_at IS NOT NULL)` — a TAUTOLOGICAL internal correctness proof, NOT a real broker cross-check; real broker cross-check waited on the live Alpaca client surface. |
| **Tier** | A — money-path basis correctness (a divergence between internal post-CA basis and broker `avg_entry_price` is a silent corruption that surfaces only at exit). |
| **Title** | Implement an `AlpacaCorporateActionBasisFetcher` that reads `/v2/positions/{symbol}.avg_entry_price` (Alpaca's post-CA-adjusted avg entry) and compares against internal `longshort_lots.cost_basis` for the symbol, returning `broker_basis_adjusted = (|broker_avg − internal_avg| ≤ tolerance)` instead of the tautological internal-applied_at proxy. |
| **Scope** | (a) Implement `AlpacaCorporateActionBasisFetcher` against the live Alpaca client surface. (b) Aggregate internal `cost_basis` across all open lots for the symbol (qty-weighted average) for comparison against Alpaca's single `avg_entry_price`. (c) Tolerance discipline (Strong+ class — 0.01 USD or 0.01% — TBD per §11.0.9). (d) Flip the verifier wire from `createInternalCorporateActionStatusFetcher` to the new fetcher; keep the internal stand-in as fallback for diagnostic. (e) Operator alert pipeline on `divergence_escalated` (broker-internal basis mismatch = potential 1099-B defect at year-end). |
| **Depends-on** | FP-062 / DW-058 (live Alpaca client surface); FP-061 4M.4 (this turn — the internal applier + soft-dependent shell). |
| **Blocks** | Phase-8 live-money authorization on any portfolio that holds positions through a corporate action (silent basis corruption is a Strong+ tier failure per §11.0.7 / §11.0.9). |
| **Future Owner Phase** | FP-062 broker MOVE cluster (the natural home — same Alpaca-live client surface as DW-196). |
| **Resolution shape** | One commit adding `AlpacaCorporateActionBasisFetcher` + the wire-flip at the `verify_corporate_action_clean` call site + a synthetic post-split end-to-end test. |
| **Cross_ref** | ACT-378 (REGISTRATION); DW-196 (broker-1099-B fetcher activation — the sibling broker-flip pattern this DW mirrors); MIG-144 (the `corporate_actions` store that feeds the internal stand-in); `internal-corporate-action-status-fetcher.ts` (the soft-dependent shell this DW replaces); `verify_corporate_action_clean.ts` (the 4-outcome verifier the new fetcher consumes); FP-057 `verify_rebalance_aggregate` (the broker-flip precedent); FP-062 / DW-058; CROSSWIND §7.8 / §11.0.7 verifier inventory / §11.0.9 zero-tolerance class. |

### DW-200: Bypass-cron monitoring gap — push-alert enrollment + dispatch-vs-outcome cross-check for armed longshort crons

| Field | Value |
|---|---|
| **ID** | DW-200 (next-free after DW-199; grep-verified at HEAD `bb16037d`). |
| **date_deferred** | 2026-06-29 (FP-062 session; surfaced by the shadow-rank accrual probe + operator-raised visibility concern). |
| **source** | Operator-raised during FP-062 6I.x work, grounded by the shadow-rank deploy-state investigation. `longshort.combiner_shadow_rank.compute` failed at chunk-0 (ON CONFLICT) on its 2026-06-26 fire and went undetected ~4 weekdays; discovered only by a manual accrual probe, not by any alert. |
| **title** | Enroll the armed longshort bypass-crons in the `job-alert-evaluation` push-alert pipeline AND add a `cron.job_run_details` (dispatch prime) vs `cron_last_fire` (self-report derivative) cross-check, so a pre-writer crash is detected without waiting for the staleness window to elapse. |
| **problem (two complementary halves)** | **(a) No push-alert (escalation gap):** the bypass-crons write `cron_last_fire` for the pull-based AdminJobsPage staleness pill, but are NOT enrolled in the push-based `job-alert-evaluation` pipeline (`alert_configs` threshold over `system_metrics`). A silent failure shows as a stale pill on `/admin/jobs` only if someone looks; nothing escalates. **(b) Self-report dependency (detection gap):** `cron_last_fire.completed_at` is written by the function's OWN writer. A crash BEFORE the writer's outcome-record line (the shadow-rank 06-26 failure mode — aborted at the `combiner_book_shadow` upsert) writes NO `cron_last_fire` row; the staleness model stays frozen at the last success and only flags stale once `nextExpectedFire + slack` elapses. §2 axiom 2: the derivative (self-report) cannot be the only watch on the prime (was the work done). |
| **why both halves (not just push-alert)** | Push-alert enrollment alone would NOT have caught the 06-26 incident: the crash was pre-writer, so there was no `system_metrics` value to threshold against. The `cron.job_run_details` cross-check is the only signal that catches dispatch-without-outcome (pg_cron logs the `net.http_post` dispatch regardless of function-body success) without waiting for the staleness window. The two mechanisms close complementary halves of the same gap. |
| **scope (enumerated — falsifiable, NOT "all crons")** | Named-now armed bypass-crons (real `job_registry` ids, grep-confirmed): `longshort.combiner_shadow_rank.compute`, `longshort.combiner_forward_returns.compute`, `longshort.reconciliation_periodic_sweep` (+ the `longshort-reconciliation-liveness-check` handler), `longshort.combiner_assemble.compute`, `longshort.combiner_rank.compute`, `longshort.spy_regime.compute`, the combiner train + promotion crons, and the 6I.6a/6I.6b auto-pause escalators. Future crons are added by addendum, not absorbed silently. |
| **scope_sketch** | (a) For each enumerated cron: emit a heartbeat/outcome `system_metrics` row (`metric_key` per-cron) + a matching `alert_configs` row (threshold + comparison + cooldown matching the cron cadence), so `job-alert-evaluation` writes `alert_history` on breach. Mirror the DW-086 rung-(b) shape. (b) A dispatch-vs-outcome reconciler: read `cron.job_run_details` (dispatch prime) for each enumerated jobid; for any dispatch with no matching `cron_last_fire` outcome row within N seconds, emit a `system_metrics` pre-writer-crash signal → `alert_configs` → `alert_history`. (c) AdminJobsPage: surface the cross-check result (dispatch-without-outcome) as a distinct badge state from "stale" (a crashed cron is a different condition from an overdue one). (d) Tests: simulate a pre-writer crash (dispatch row, no outcome row) → cross-check fires; clean fire → no alert. |
| **blocking_dependencies** | The `job-alert-evaluation` pipeline must be verified operational (same precondition DW-086 carries). If not yet operational, half (a) is blocked on that; half (b) (the `cron.job_run_details` cross-check) can proceed independently (it reads pg_cron's own log, not `system_metrics`). |
| **cross-references** | **DW-086** (Liveness-check STOP ladder rung (b), register line 1932) addresses this gap for ONE metric (`liveness_check.stop`). DW-200 is the GENERALIZED class across all enumerated bypass-crons. DW-200 does NOT subsume or rewrite DW-086 — DW-086 remains its own narrow, individually-trackable entry; once DW-200's pipeline-enrollment lands for the reconciliation surface, DW-086's liveness-specific rung is satisfied by the general mechanism (note added to DW-086). |
| **status** | logged (alert-pipeline-operational-gated for half (a); half (b) buildable independently). |
| **estimated_complexity** | M-L (per-cron metric emission + alert_configs seeds + the dispatch-vs-outcome reconciler + AdminJobsPage badge state + tests). Candidate for its own FP given the cross-cutting surface. |
| **related_decisions** | §2 axiom 2 (external/prime vs internal/derivative); DW-086 (the one-metric precedent); the shadow-rank 06-26 incident (motivating evidence). |
| **Cross_ref** | ACT-399 (REGISTRATION); DW-086 (the one-metric precedent, cross-referenced not folded); shadow-rank 06-26 incident; `job-alert-evaluation` / `system_metrics` / `alert_configs` / `alert_history` (the pipeline); `cron.job_run_details` (the dispatch prime); `cron_last_fire` (the self-report derivative); AdminJobsPage / `cron-staleness.ts` / `useCronLastFire.ts` (the existing pull-based surface). |

### DW-201: Structural dollar-neutrality short-skew — book runs short-heavy outside the §1.6 90-110% band

| Field | Value |
|---|---|
| **ID** | DW-201 (next-free after DW-200; grep-verified at HEAD `a61f29a0`). |
| **date_deferred** | 2026-06-29 (FP-062 first-armed-RTH-session health sweep; surfaced by the verify_rebalance_aggregate CLASS-B disambiguation). |
| **Tier** | A — STRUCTURAL correctness (§14 priority 1), NOT merely ROI (priority 3). Dollar-neutrality is a by-construction invariant of the strategy (§1, §1.6); a sustained breach is a structural finding. |
| **Status** | open. |
| **source** | FP-062 first-RTH health sweep. verify_rebalance_aggregate (reconciliation verifier #17, zero_tolerance class) escalated on every fire this session. Disambiguation (this turn) confirmed CLASS-B (real divergence), NOT CLASS-A (empty-book / bootstrap-stub). |
| **title** | The live paper book runs structurally short-heavy: short-side gross dollar exposure sustained ~13-24% above long-side, outside the §1.6 dollar-neutrality band (ratio short/long must be 0.90-1.10; observed 1.13-1.24). Sustained across consecutive ticks with exempt_cause=null (NOT explained by a transient force-cover), and predates the current session (06-26 history shows the same skew). |
| **evidence (verbatim from the disambiguation read)** | reconciliation_events verify_rebalance_aggregate failure_escalated rows: 2026-06-29 14:30 long=$28,049.09 short=$31,814.66 ratio=1.1342 (band 0.90-1.10, exempt=null); 14:15 long=$27,852.03 short=$31,583.18 ratio=1.1340 (exempt=null); 14:00 long=$28,064.76 short=$34,776.58 ratio=1.2392 (exempt=short_stop — a force-cover legitimately broke neutrality for that one tick); 06-26 19:45 ratio=1.2388; 06-26 19:30 ratio=1.2395. The 14:15/14:30 null-exempt rows show the book did NOT mean-revert after the 14:00 force-cover — short stayed ~$3.7k above long. |
| **NOT a verifier defect** | verify_rebalance_aggregate is firing CORRECTLY. Band math verified (ratio short/long, 0.90-1.10 per §1.6; empty-book guard handles long==0 && short==0 → ratio=1.0 → within band; no divide-by-zero). The verifier is doing its job — detecting a real breach. The finding is the SKEW, not the verifier. |
| **candidate root causes (hypotheses — not yet investigated)** | (a) ranker producing more short conviction than long this session (signal-side asymmetry); (b) planner sizing asymmetry (rebalance-planner.ts allocating short gross > long gross); (c) long-side partial-fills / rejects not re-attempted while short side filled cleanly (execution asymmetry); (d) post-force-cover re-shorting (the 14:00 short_stop reduced short, but the planner subsequently re-established short exposure above long). The 06-26 history rules OUT a one-session blip — the skew is persistent/structural. |
| **scope_sketch** | (a) Investigate which layer produces the asymmetry: ranker output (long vs short conviction counts + score distribution), planner sizing (target long gross vs short gross per rebalance), execution fills (long-side fill rate vs short-side fill rate, partial/reject asymmetry). Read-only DB + code investigation FIRST (dual-independent). (b) Determine whether it's signal-driven (the strategy genuinely finds more short conviction — may be acceptable within a wider band, or a §1.6 band-tightening question), planner-driven (a sizing bug), or execution-driven (fill asymmetry leaving the book lopsided). (c) Fix at the identified layer; re-verify the book returns within the §1.6 band. |
| **blocking_dependencies** | None for the investigation (read-only, data on disk). A fix may depend on which layer is implicated. |
| **relationship to the escalation-path question** | SEPARATE but adjacent: whether the verify_rebalance_aggregate_persistence escalator FIRED on the null-exempt sequence (or is a 2nd DW-200-class detection-without-escalation instance) is tracked under the (a) escalator-fired check this turn. DW-201 is the SKEW root-cause; the escalation-path gap (if confirmed) is DW-200-class and cross-references DW-200. |
| **paper-vs-live note** | On a paper account this is not capital-at-risk, but the MECHANISM producing the skew would carry into live — so it is a now-finding (structural), not a defer-to-live one. A book that cannot hold dollar-neutrality is not Phase-8-eligible (market-neutrality is by-construction per §1). |
| **related_decisions** | §1 / §1.6 (dollar-neutrality by-construction, 90-110% band); §14 priority stack (structural correctness > ROI); DW-163 (the rebalance-aggregate rolling-window reframe — the verifier that caught this); verify_rebalance_aggregate (#17). |
| **Cross_ref** | ACT-pending (REGISTRATION); FP-062 first-RTH health sweep; the verify_rebalance_aggregate CLASS-B disambiguation (this turn); `rebalance-planner.ts` (candidate root-cause anchor — planner sizing); `verify_rebalance_aggregate.ts` (the verifier that caught it); DW-200 (the adjacent escalation-path gap if the persistence escalator is detection-only); §1.6 (the band). |

### DW-202: Persistence-escalator pre-WRITE silent failure — rebalance-aggregate pager throws on `action_taken` column-mismatch, swallowed at the runTick seam

| Field | Value |
|---|---|
| **ID** | DW-202 (next-free after DW-201; grep-verified at HEAD `9492eb51`). |
| **date_deferred** | 2026-06-29 (FP-062 first-RTH health sweep follow-up; surfaced by the (a) escalator-fired check on the DW-201 short-skew). |
| **Tier** | A — the operator-pager for verify_rebalance_aggregate (#17, §11.0.9 ZERO-tolerance, dollar-neutrality structural invariant) has NEVER fired since being wired, because its write throws and the seam swallows the throw. A silent-failed pager on a zero-tolerance structural verifier. |
| **Status** | open (root cause CONFIRMED; fix is small + falsifiable). |
| **source** | FP-062 health-sweep (a) escalator-fired check. The verify_rebalance_aggregate_persistence escalator reached its N=3 threshold (14:15 today: 14:45-null=1, 14:30-null=2, 14:15-null=3; short_stop SKIPS non-compounding per the counter rules, does NOT reset — grep-confirmed `countConsecutiveUnexplained`) AND on 06-26 (~7 consecutive null rows) — yet ZERO `verify_rebalance_aggregate_persistence` rows have ever been written. |
| **ROOT CAUSE (confirmed from edge log, verbatim)** | `longshort-execute-cron` log: `longshort_rebalance_aggregate_persistence.failed — aggregate_persistence_write_failed: Could not find the 'action_taken' column of 'reconciliation_events' in the schema cache`. The persistence writer's insert `row` (rebalance-aggregate-persistence.ts, the `.from('reconciliation_events').insert(row).select('event_id')` block) sets a TOP-LEVEL key `action_taken` — which is NOT a column on reconciliation_events. The real column is `failure_action text` (grep-confirmed: reconciliation_events columns = event_id, operator_id, ts, call_name, symbol, divergence jsonb, outcome, failure_action, phase_0b_run_id — `action_taken` count=0, `failure_action` count=1). NOTE: `action_taken` IS a legitimate NESTED key inside the verifier's own `failure_action` value (verify_rebalance_aggregate.ts:122-124 — `failure_action: async (ctx) => {... action_taken: '...'}`); the bug is the persistence writer put it at the TOP LEVEL of the insert where a real column is required. |
| **THE SWALLOW (why it was silent)** | tick-scheduler.ts:312-320: `try { rebalance_aggregate_persistence = await p.rebalanceAggregatePersistenceCheck(p.ts); } catch (e) { console.error('longshort_rebalance_aggregate_persistence.failed', ...); rebalance_aggregate_persistence = null; }`. The write-throw is caught, logged to console.error (which nobody reads), and the result set to null — the tick continues normally. No reconciliation_events row, no surfaced error, no operator signal. THIRD silent-failure class. |
| **distinct from DW-200** | DW-200 = "detects, writes the detection row, but no PUSH-alert (not enrolled in job-alert-evaluation)." DW-202 = "the detection WRITE itself throws (column-mismatch) and the seam swallows it — the row is never even written." Pre-WRITE, not pre-alert. The two compound: even after DW-202's write is fixed, the written row still needs DW-200's push-alert enrollment to actually page. |
| **fix scope (small + falsifiable)** | (a) **Writer fix**: in rebalance-aggregate-persistence.ts, correct the insert `row`'s top-level key from `action_taken` to `failure_action` (the real text column); the value (the BAND_VIOLATION_ACTION const `persistent_band_violation_operator_alert_N_consecutive_ticks`) goes into `failure_action`. Align the full insert payload to the real reconciliation_events column set. NO migration (align to existing schema, do NOT add an action_taken column). (b) **Structural guard** (the lesson): add a writer-level test asserting the insert column-set is a subset of reconciliation_events' real columns — would have caught this at author time (same class as the lockfile-drift structural-guard pattern; structural, not per-commit vigilance). (c) **Seam-swallow decision**: the runTick seam swallowing a persistence-check throw to console.error+null on a ZERO-tolerance verifier is itself a finding — at minimum emit a system_metrics heartbeat-failure metric so a swallow is observable (ties into DW-200's pipeline once landed). Whether to make the seam fail-louder for zero-tolerance checks is a design call to resolve at fix time. |
| **blocking_dependencies** | None — root cause confirmed, fix is a code change (no migration, no external data). |
| **relationship to DW-201** | DW-201 = the SKEW (book runs short-heavy). DW-202 = the PAGER for that skew has never fired (write-throw swallowed). DW-201 is the disease; DW-202 is the broken alarm. Both surfaced same-turn from the same health sweep. Independent fixes. |
| **related_decisions** | §11.0.9 (zero-tolerance class — the pager that never fired); §2 axiom 2 (derivative vs prime — a console.error is not a monitored signal); DW-200 (the adjacent push-alert gap); DW-163 (the persistence escalator's origin); Catalog #50 / the column-name-from-mental-model discipline (the class of bug — writer authored against a reconstructed column name). |
| **Cross_ref** | ACT-pending (REGISTRATION); FP-062 health-sweep (a) check; the verbatim edge log; `rebalance-aggregate-persistence.ts` (the writer with the bug + the structural-guard-test target); `tick-scheduler.ts:312` (the swallow seam); reconciliation_events schema (`failure_action` the real column); verify_rebalance_aggregate.ts:122-124 (the canonical nested-action_taken pattern the writer should have mirrored); DW-200 (push-alert enrollment — the compounding gap); DW-201 (the skew this pager should have caught). |
### DW-203: Intraday combiner-tick claims as_of_date=current_date before daily signal producers fire → 100% vector exclusion → empty book → zero orders

| Field | Value |
|---|---|
| **ID** | DW-203 (next-free after DW-202; grep-verified at HEAD `075db926`). |
| **date_deferred** | 2026-06-29 (FP-062 first-RTH arc; surfaced by the zero-orders → no_included_vectors → signal-producer investigation chain). |
| **Tier** | A — STRUCTURAL: this starves the entire trade pipeline (no book → no orders → no fills). Blocks live paper trading. FP-057-introduced. |
| **Status** | open (root cause CONFIRMED; fix direction corrected from the initial investigation — see below). B1 fix RATIFIED via DEC-070 clause (i) at ACT-408; B2 deferred as refinement FP. |
| **source** | FP-062 first-RTH zero-orders investigation. Chain: rebalance places 0 orders ← stale/empty combiner_book ← combiner_rank.compute fails `no_included_vectors` ← 100% of combiner_feature_vectors carry `excluded_reason='missing_critical_signal_6'` ← the morning combiner-tick wrote vectors for `as_of_date=current_date` before that day's daily signal #6 producer fired. |
| **producers are HEALTHY (NOT a stall)** | All 6 daily-cadence signal producers (momentum #6, reversal #7, pead #2, short_interest #5, insider, options_flow) are scheduled, enabled, active, and fired successfully EOD M-F (last fire Fri 06-26 20:00-23:00 UTC; weekend correctly skipped; next fire tonight). signal_observations stable ~834-837 rows/day. The "stall since 06-26" was the weekend gap + Monday's not-yet-fired EOD slot. (Two prior diagnoses in-session were superseded: the "unripe date" call was actually correct; the "crons disarmed by FP-057" hypothesis was wrong — cron-state read confirmed healthy.) |
| **ROOT CAUSE (confirmed)** | The FP-057 intraday combiner-tick (`longshort-combiner-tick`, the tick-poll-dirty-bit recompute trigger) resolves `floor_as_of_date` to `current_date` and composes combiner_feature_vectors for today BEFORE today's daily-cadence signal producers have fired (they run EOD 20:00-23:00 UTC; the tick ran 03:42 UTC). Daily signal #6 for today does not exist until tonight → the assembler's strict signal load (`.eq('as_of_date', floor_as_of_date)`, feature-assembler-orchestrator.ts:162) finds no signal #6 → `missing_critical_signal_6` on 100% of 839 vectors → ranker reads zero included → `no_included_vectors`. It also wrote vectors for as_of_date=06-27 (Saturday, no producer fire at all) — same outcome. |
| **THE FIX DIRECTION (CORRECTED — important)** | The initial investigation proposed switching the signal load to T-1-tolerant `as_of_date <= floor ORDER BY DESC LIMIT 1`. **That is the WRONG fix.** The strict `.eq` is INTENTIONAL: feature-assembler-orchestrator.ts:16-18 documents it preserves **T8 replay-determinism** ("replaying a historical as_of with a `<= as_of` filter [would break determinism]"). Switching to `<=` would regress the T8 replay invariant. The CORRECT fix is in the **`floor_as_of_date` resolution** (feature-assembler-orchestrator.ts:134) and/or the **intraday combiner-tick's as_of_date selection**: the tick must NOT advance the floor to `current_date` until that day's daily-cadence signals exist. The morning/intraday tick should build for the last as_of_date that HAS complete daily signals (gating on daily-producer completion), rolling forward to current_date only after the EOD daily window completes. This preserves the strict `.eq` (replay-determinism intact) AND fixes the starvation. |
| **the design judgment (operator fork)** | Two fix shapes within the corrected direction, with different ROI/freshness semantics — operator decision: **(B1) floor-gates-to-last-complete-day:** intraday tick builds for last_business_day (complete daily signals) until tonight's daily window completes, then rolls to current_date. Clean same-day vintage; morning intraday book lags daily signals by one day (no morning intraday freshness on the daily base). **(B2) intraday-slot-carries-prior-daily:** the intraday tick at slot>0 composes today's intraday signals (analyst/catalyst/news/regime, which DO fire intraday) layered on the most-recent COMPLETE daily snapshot, explicitly recording the daily-signal vintage — but achieved via the floor/slot mechanism, NOT via a `<=` filter that breaks T8. This is closer to the FP-057 intraday intent (fresh intraday on slower daily base) but needs careful design to preserve replay-determinism. §14: B2 aligns with the intraday-cadence ROI intent; B1 is simpler/safer but clamps the morning intraday benefit. Resolve on which freshness semantics the strategy should have. |
| **DEC-070 gap (not a spec violation)** | DEC-070 (the cadence rebuild) ratified the intraday_slot superset-PK rotation + substrate dual-capture but did NOT specify how daily-cadence signals are handled when the intraday tick fires before the daily producers (grep-confirmed: no T-1/floor/vintage clause). This is a genuine FP-057 IMPLEMENTATION GAP — the intraday tick was enabled (claims current_date) without the companion floor-gating for not-yet-fired daily signals. May warrant a DEC-070 addendum ratifying the chosen fix shape. |
| **fix layer** | Consumer-side: the intraday combiner-tick's as_of_date selection + the feature-assembler-orchestrator floor_as_of_date resolution. NOT producer-side (producers healthy). NOT operator cron SQL. NOT the strict `.eq` (T8-protected). A code change in `longshort-combiner-tick` and/or `feature-assembler-orchestrator.ts:134`. |
| **blocking_dependencies** | None for the fix (code change). The operator fork (B1 vs B2 freshness semantics) gates which shape. |
| **why this blocks live paper trading** | Until fixed, every morning/intraday tick writes a 100%-excluded vector set for current_date, the rebalance reads it, places zero orders, no fills. The ACT-403 fill-fix is correct but starved. Signal #6 firing tonight (20:00 UTC) will populate today's daily signals, so the canonical 23:35 combiner (cron 102) will build a non-excluded book tonight — but the MORNING tick will re-break it tomorrow without this fix. (Tonight's canonical book may unblock a single rebalance, but the morning re-break recurs daily until fixed.) |
| **related_decisions** | DEC-070 (the cadence rebuild — the gap); FP-057 (the intraday tick that introduced it); T8 replay-determinism (the invariant the strict `.eq` protects — the fix MUST preserve it); §14 (the freshness-vs-ROI fork). |
| **Cross_ref** | ACT-pending (REGISTRATION); the FP-062 zero-orders investigation chain (this turn); `feature-assembler-orchestrator.ts:16-18` (T8 strict-.eq rationale) + `:134` (floor_as_of_date resolution — the fix surface) + `:162` (the strict load); `longshort-combiner-tick/index.ts` (the intraday tick claiming current_date); DEC-070 (the cadence rebuild missing this clause); the 6 healthy daily producers (NOT the bug). |

#### DW-203-ADD-01 (Rule 8 additive, appended 2026-06-30 at ACT-425) — B1 gate firing-as-designed confirmation + DW-203 B2 (morning-vintage refinement) re-evaluation trigger

**Status correction (supersedes the original `Status: open` cell by pointer; original PRESERVED VERBATIM above per Constitution Rule 8):** DW-203 **B1 fix is CLOSED-AND-CORRECT** (Gate-3 in `longshort-combiner-tick`, built at ACT-407, ratified via DEC-070 clause (i) at ACT-408). The 2026-06-30 gap-(e) fills-watch at HEAD `76cb2f65` (recorded in `feature-proposals.md` FP-062-ADD-04 / ACT-425) confirmed the gate **FIRES CORRECTLY**: the 10:30 ET / 14:30 UTC morning rebalance saw 12× `longshort.combiner.tick.skipped` with reason `critical_signals_absent` (zero `combiner.tick.completed`) because today's EOD daily-cadence critical signals (cross_sectional_momentum_12_1 #6, short_term_reversal_1w #7) do not exist at 14:30 UTC — they are produced EOD ~20:00–23:00 UTC. The morning rebalance therefore submitted 0 orders (`metadata.submission_counts` all-zero) and produced 0 lots, 0 ranking-snapshot rows, 0 fills. **This is the DESIGNED consequence of the DW-203 B1 fix preventing the empty-book composition pathology — NOT a residual DW-203 defect.** The next session reviewing the watch must NOT read this as "DW-203 still blocking"; B1 is closed-and-correct, the gate is firing as DEC-070 clause (i) specifies.

**DW-203 B2 (morning-vintage refinement) — DEFERRED-PENDING-FILL-EVIDENCE (NOT deferred indefinitely; explicit re-arm trigger below).** Per DEC-070 clause (i).3, the deferred B2 refinement (letting the morning intraday tick compose against the prior day's COMPLETE daily signals — T-1 base — instead of waiting for today's EOD, achieved via the floor/slot mechanism without violating the T8 replay-determinism strict-`.eq` invariant) is gated on **measured bp cost** from post-trade analysis. The 2026-06-30 watch demonstrated the morning-vintage MECHANISM (every morning rebalance under the current design will skip-to-empty until tonight's EOD signals exist; first fills come from the EOD → next-day cadence) but NOT the COST (how much ROI a T-1 base would have captured versus waiting for the EOD → next-day cycle). Cost measurement is structurally unobtainable until Path 1 (the EOD → next-day cadence) produces the first fills + the post-attribution substrate (the ACT-419 ranking-snapshot sidecar + FP-065 full §1.1 entry-attribution lineage) catches up enough to measure morning-vintage opportunity cost. **Surfacing DW-203 B2 (morning-vintage refinement) now on mechanism-only evidence would invert its own justification gate** — building ahead of the evidence the deferral was explicitly written to require.

**EXPLICIT RE-EVALUATION TRIGGER (so the deferral does not ossify into "never"):** DW-203 B2 (morning-vintage refinement) gets CHARTERED — and only gets chartered — when post-fill attribution analysis (enabled by Path 1's first fills + the ranking-snapshot / FP-065 entry-attribution substrate) shows the morning-vintage gap costs ≥ a **material bp threshold**. The threshold itself is set at first-attribution time (it cannot be set without any fill data — pre-setting a number would be cargo-culting). At the trigger point, DW-203 B2 (morning-vintage refinement) is chartered WITH the measured cost number in its justification. Until then: status = **DEFERRED-PENDING-FILL-EVIDENCE**, re-evaluated at the first post-attribution analysis turn.

**Falsification path (recorded so the deferral does not ossify in EITHER direction):** if the NEXT post-EOD rebalance cycle ALSO produces zero fills, the finding sharpens — it is NOT a cadence-vintage gap but a deeper book-construction / submission defect, and DW-203 B2 (morning-vintage refinement) is STILL not the answer. That path triggers a fresh diagnosis turn (re-watch + root-cause), NOT a B2 charter. The trigger is asymmetric on purpose: B2 charters only on POSITIVE evidence of morning-vintage cost from REAL fills, never on absence-of-fills.

**Critical naming guard (do NOT collapse in any future grep or charter):** always write "**DW-203 B2 (morning-vintage refinement)**" — never bare "B2". There are TWO unrelated "B2"s in the register: (1) **DW-203 B2 = morning-vintage refinement** (THIS note's subject; intraday tick composing on prior-day complete daily signals); (2) **DW-058 B2 = halt-feed integration** (Polygon Advanced WS condition-code halt fetcher; UNRELATED; already disposed via FP-064 + DEC-068 clause (s) per ACT-416). Bare "B2" in any future prompt is ambiguous and a STOP-condition for the responding session.

**Cross-references:** ACT-425 (this Rule-8 note + verdict-correction); ACT-407 (B1 fix BUILT — Gate-3 in `longshort-combiner-tick`); ACT-408 (B1 ratification via DEC-070 clause (i)); DEC-070 clause (i).3 (the verbatim B2 deferral text — gated on "post-trade analysis shows measurable basis-point cost from morning-window vintage gap"); FP-062-ADD-04 (the companion gap-(e) PENDING-NEXT-CADENCE verdict in `feature-proposals.md`); ACT-419 (ranking-snapshot sidecar — substrate for future morning-vintage cost measurement); FP-065 (post-window full §1.1 entry-attribution — supplies the substrate the re-arm trigger requires); FP-064 + DEC-068 clause (s) + ACT-416 (the UNRELATED DW-058 B2 halt-feed disposition — naming-collision guard); ACT-403 (fill-accounting path — DEPLOYED, un-exercised end-to-end, awaiting next post-EOD cycle); Constitution Rule 8 (this block is additive; originals preserved verbatim).

#### DW-203-ADD-02 (Rule 8 additive, appended 2026-06-30 at ACT-426) — freshness-gate mechanism that concretely blocks DW-203 B2 (morning-vintage refinement)

**What happened (2026-06-30 manual-chain run, operator-invoked with signed-in JWT):** the manual combiner chain against `as_of=2026-06-29` SUCCEEDED through assemble (839 universe, 108 included, 725 `below_coverage_threshold` + 5+1 `missing_critical_signal` — all normal sparsity-class exclusions) AND rank (`book_size_long=21`, `book_size_short=20`, `ranker_source=count_normalized_fallback` — the §6.4 documented degraded path, expected pre-trained-combiner). A real, populated 21-long / 20-short book was written to `combiner_rankings` for `as_of_date=2026-06-29`. But `longshort-rebalance-submit` REFUSED with `reason=rankings_stale`: `latest_computed_at=2026-06-29T00:00:00Z`, `age_s=141990` (~39h), `tolerance_s=600`. Zero orders placed.

**THE CONCRETE MECHANISM (this is the sharpening — DW-203 B2 (morning-vintage refinement) was abstract; now it has a named blocker):** the rebalance freshness gate (`supabase/functions/_shared/longshort-execution/rebalance-submit-orchestrator.ts:215-237`; `RANKING_FRESHNESS_TOLERANCE_S=600` from `rebalance-planner.ts:123`; §11.0.7 #1) compares **wall-clock-now** against `combiner_rankings.computed_at`. But `computed_at` is **determinism-stamped from `as_of`** (`supabase/functions/_shared/longshort-combiner/ranker-orchestrator.ts:21-22` + `:161` — verbatim: "computed_at = as_of.toISOString(), explicitly overrides the schema DEFAULT now() per DEC-034 (4)"), a T8 replay-determinism invariant: two replays of the same `as_of` must produce byte-identical rows, which is impossible if `computed_at` reads wall-clock — so the override is correct, NOT a bug. **Consequence:** ANY rebalance acting on a prior-`as_of` book sees `computed_at = that prior date's midnight` → `age_s` always ≫ 600 → always refuses `rankings_stale`. DW-203 B2 (morning-vintage refinement) — letting the morning intraday tick compose against the prior day's COMPLETE daily signals (T-1 base) without violating the strict-`.eq` T8 invariant — is therefore **concretely blocked at the rebalance freshness gate**, not just at the upstream signal-presence gate: even if a T-1-vintage book were composed, the 600s intraday-tick tolerance would refuse it at submit time because a T-1-vintage book is by-construction stale against that tolerance.

**Why the gate is correct, NOT a defect:** the 600s tolerance is calibrated for the intraday-tick regime per ACT-340: "production daily pattern — `computed_at` ~10:30Z, fire seconds later → age ≈ 0-5s, comfortably under 600s — gate does not bite." The normal automated fill path is the intraday tick building `current_date` rankings stamped fresh, with the rebalance firing seconds-to-minutes later → passes. The freshness-gate-vs-prior-`as_of` collision is **specific to manually replaying a stale-dated book (the off-cadence exception), NOT a blocker on the automated current_date → fresh-rankings → rebalance path.** Tonight's EOD producers → fresh `current_date` book → next post-EOD rebalance is unaffected. The gate is doing exactly the job §11.0.7 #1 specifies: refusing to trade on rankings older than the intraday-cadence freshness window.

**Scoping note for whoever picks up DW-203 B2 (morning-vintage refinement):** the fix shape is **undetermined** and requires production evidence; pre-building any candidate now would be exactly the premature refinement DW-203-ADD-01's deferral-pending-fill-evidence was written to prevent. Candidate shapes recorded so they are not forgotten when the re-arm trigger fires:
(i) a distinct EOD-rebalance freshness tolerance separate from the 600s intraday-tick tolerance (read via a SECOND env var so the intraday-tick path's 600s remains untouched);
(ii) market-session-aware staleness (the rebalance reads `as_of_date` + session boundaries — e.g., "prior-RTH-session EOD vintage" — rather than a flat seconds tolerance);
(iii) the EOD-rebalance path reading a different vintage column entirely (e.g., a separate `submitted_at_floor` derived from the rebalance-tick's wall-clock, decoupled from the deterministic `computed_at`).
Determining which shape is correct requires (a) how often the EOD-rebalance path actually fires against a prior-`as_of` book in production, (b) the right tolerance value for a daily-cadence regime, (c) the real freshness regime for a daily-cadence strategy as measured against post-fill attribution. None of (a)–(c) is observable until Path 1's first fills + the FP-065 entry-attribution substrate accrue — the same evidence gate DW-203-ADD-01 already records.

**DO-NOT-DO (recorded so a future session does not reach for the blunt lever):** do **NOT** set `LONGSHORT_RANKING_FRESHNESS_TOLERANCE_S` to force a stale-book rebalance through. The override env var (`rebalance-submit-orchestrator.ts:222-229`) reads on the **same code path the live intraday-tick rebalance uses** — widening it to admit a 39h-old EOD-vintage book silently loosens the gate protecting every automated same-day rebalance that the intraday tick produces. The cost of one cadence cycle of waiting (tonight's EOD producers → fresh `current_date` book → next post-EOD rebalance → first fills) is strictly cheaper than silently widening a live money-path safety gate that is otherwise firing correctly. Any future "just bump the env var to get fills today" instinct must STOP here: the env var is global to the gate, not scoped to the manual EOD-replay path.

**Status (UNCHANGED by ADD-02):** DW-203 B1 remains **CLOSED-AND-CORRECT** (per ADD-01); DW-203 B2 (morning-vintage refinement) remains **DEFERRED-PENDING-FILL-EVIDENCE** (per ADD-01) with the SAME explicit re-arm trigger (≥ material bp from post-fill attribution, threshold set at first-attribution time). ADD-02 adds the concrete-mechanism scoping ONLY; it is NOT a status change, NOT a charter, NOT a re-arm. The freshness gate is NOT touched. No env var is set. No code is changed.

**Critical naming guard (carried forward from ADD-01):** always write "**DW-203 B2 (morning-vintage refinement)**" — never bare "B2" — to avoid the unrelated **DW-058 B2 (halt-feed integration)** disposed via FP-064 + DEC-068 clause (s) per ACT-416.

**Cross-references:** ACT-426 (this Rule-8 note); DW-203-ADD-01 (the deferral + re-arm trigger this sharpens); ACT-340 (the freshness-gate authoring + "production daily pattern" calibration rationale); DEC-034 clause (4) (the T8 replay-determinism injected-clock contract that mandates `computed_at = as_of.toISOString()`); DEC-070 clause (i).3 (the verbatim B2 deferral text — measured-bp gate); `supabase/functions/_shared/longshort-execution/rebalance-submit-orchestrator.ts:215-237` (the gate read site); `supabase/functions/_shared/longshort-execution/rebalance-planner.ts:123` (the 600s default); `supabase/functions/_shared/longshort-combiner/ranker-orchestrator.ts:21-22` + `:161` (the deterministic stamp site); FP-062-ADD-04 (the gap-(e) PENDING-NEXT-CADENCE companion verdict); FP-065 (post-window §1.1 entry-attribution substrate — gates the re-arm trigger); FP-064 + DEC-068 clause (s) + ACT-416 (the UNRELATED DW-058 B2 halt-feed disposition — naming-collision guard); Constitution Rule 8 (this block is additive; ADD-01 + originals preserved verbatim above).

#### DW-203-ADD-03 (Rule 8 additive, appended 2026-06-30 at ACT-427) — correction to ADD-02's stamp-generalization: `computed_at` is path-polymorphic, NOT uniformly determinism-stamped; the freshness gate is NOT a latent always-refuse defect

**What ADD-02 got wrong (preserved above verbatim per Rule 8; corrected — NOT redacted — here per the clause-(p) preserved-record precedent):** ADD-02 stated `computed_at` is "determinism-stamped from `as_of`" such that "ANY rebalance acting on a prior-`as_of` book" is blocked at the 600s freshness gate. The dual-investigation (live-DB-grounded, HEAD `d19bbfe4`) **REFUTES** the generalization. The ranker (`supabase/functions/_shared/longshort-combiner/ranker-orchestrator.ts:162`, `computed_at: as_of.toISOString()`) is **PATH-POLYMORPHIC** — it stamps whatever `Date` the caller passes:

- **CRON rank** (`supabase/functions/longshort-combiner-rank/index.ts:119`) passes `productionClock.getWallClockTs()` → `computed_at` = **wall-clock-at-fire** → FRESH → passes the gate.
- **INTRADAY TICK** (`supabase/functions/longshort-combiner-tick/index.ts:180`) passes `productionClock.getWallClockTs()` → **wall-clock-fresh** → passes the gate when not skipped upstream by the DW-203 B1 Gate-3 critical-signals check.
- **MANUAL rank** (`supabase/functions/longshort-combiner-rank-manual/index.ts:55`) passes `parseAsOfDate('YYYY-MM-DD')` → **UTC-midnight of the supplied date** → STALE for any prior day → refused by the 600s gate.

**Live DB evidence (`combiner_rankings.computed_at`, this turn):** 2026-06-25 / 06-24 / 06-22 cron rows stamped `~23:50:01Z` (wall-clock-at-fire); 2026-06-29 / 06-23 manual rows stamped `00:00:00Z` (as_of-midnight). Two regimes, same column. ADD-02's "ANY rebalance acting on a prior-`as_of` book" claim is true only for the manual-handler regime.

**The corrected truth:** the freshness gate is **NOT a latent always-refuse defect** and is **NOT broken**. It works correctly for the automated path (cron + intraday tick stamp wall-clock-fresh and pass the gate). The defect is **NARROW** and lives in the **manual handlers only**: they pass parsed-midnight as `as_of`, so a manual rebalance against a prior-day book is always stale. The gate, the ranker, and the automated cron/tick path are all correct.

**Impact on DW-203 B2 (morning-vintage refinement):** ADD-02's "three candidate fix shapes in the gate" framing is **SUPERSEDED for the manual-path issue** — the manual-path bug is fixed in the **manual handler** (see the manual-handler wall-clock fix tracked at ACT-428), NOT in the gate. **DW-203 B2 (morning-vintage refinement) remains a SEPARATE, still-valid deferred question** about whether the intraday MORNING tick should compose against prior-day-COMPLETE daily signals (T-1 base) — but that is about the upstream tick's `as_of` SELECTION, NOT the freshness gate, and NOT the manual-handler midnight stamp. Status **UNCHANGED**: DW-203 B2 (morning-vintage refinement) remains **DEFERRED-PENDING-FILL-EVIDENCE** with the SAME re-arm trigger (≥ material bp from post-fill attribution, threshold set at first-attribution time) from ADD-01.

**The do-not-set-`LONGSHORT_RANKING_FRESHNESS_TOLERANCE_S` guard from ADD-02 STANDS** — it is still correct, for the same reason (widening the env var would silently loosen the live intraday-tick gate that protects every automated same-day rebalance). Never blunt-override the gate.

**Critical naming guard (carried forward from ADD-01 + ADD-02):** always write "**DW-203 B2 (morning-vintage refinement)**" — never bare "B2" — to avoid the unrelated **DW-058 B2 (halt-feed integration)** disposed via FP-064 + DEC-068 clause (s) per ACT-416.

**Cross-references:** ACT-427 (this Rule-8 correction note); DW-203-ADD-02 (the framing this corrects — PRESERVED VERBATIM above); DW-203-ADD-01 (the original deferral + re-arm trigger, unchanged); `supabase/functions/_shared/longshort-combiner/ranker-orchestrator.ts:162` (the path-polymorphic stamp site); `supabase/functions/longshort-combiner-rank/index.ts:119` (cron — wall-clock-fresh); `supabase/functions/longshort-combiner-tick/index.ts:180` (intraday tick — wall-clock-fresh); `supabase/functions/longshort-combiner-rank-manual/index.ts:55` (manual — midnight, the narrow defect); DEC-034 clause (4) (the T8 replay-determinism contract — still correct, simply not what the cron/tick paths invoke); DEC-070 clause (i).3 (the DW-203 B2 measured-bp deferral text); ACT-428 (the narrow manual-handler wall-clock fix tracked separately — NOT a gate change); Constitution Rule 8 (additive correction-under-pointer; ADD-02 + ADD-01 + originals preserved verbatim above).

### DW-204: Shadow-rank ShadowBookOverlapError starves the Phase-3.3 training-label substrate — no-exclusion symmetric composite places a ticker on both sides → pre-persistence assertion throws → zero write → forward-return labels never accrue

| Field | Value |
|---|---|
| **ID** | DW-204 (next-free after DW-203; grep-verified at HEAD `82dd20b4`). |
| **date_deferred** | 2026-06-30 (FP-062 Tuesday-AM cycle watch surfaced the daily failure during the live-fills readiness sweep). |
| **Tier** | A — STRUCTURAL data-collection: the shadow-rank substrate (`combiner_book_shadow` → `combiner_forward_returns`) is the T+10-label feeder for the Phase-3.3 combiner-model promotion gate. While it fails daily, the training labels do NOT accrue — the system is NOT "collecting all data" for the downstream model track, even when the live trade path works. |
| **Status** | open-CONTAINED (ACT-410, 2026-06-30) — fix (b) per-variant skip-on-overlap SHIPPED in `shadow-ranker-orchestrator.ts` (blast radius reduced from 12/12-variants-lost to 1/12-per-overlap; substrate accrues from sibling variants as `completed_with_skipped_variants`). Structural tiebreaker fix (b)'s sibling option (a) — asymmetric short-side tiebreaker on BOTH live + shadow — re-homed to **FP-063** (Tier-A money-path; build deferred pending ≥1 week of `variants_skipped_overlap` audit-metadata frequency evidence). DW-204 stays OPEN until FP-063 lands the structural root-cause fix; the daily-throw symptom is contained, not the root cause. |
| **source** | FP-062 Tuesday-AM cycle watch (2026-06-30): `longshort.combiner_shadow_rank.compute` failed 2026-06-29 23:30 UTC with `ShadowBookOverlapError` (tickers HIMS, QCOM, SMCI, SNDK observed on both long and short sides of the seeded shadow book; universe=839, vectors_assembled=839 — grep-verified in `longshort_audit_logs` this turn). Distinct from the earlier shadow-rank ON CONFLICT stall (a write-conflict, last seen 2026-06-26) — this is a both-sides-overlap assertion throw. |
| **MECHANISM (confirmed)** | The shadow ranker uses a **criticals-symmetric, no-exclusion §6.4 composite** (`shadow-ranker.ts:14` "every signal contributes iff present, never throws"; `:113` "Criticals-symmetric §6.4 composite"). Unlike the LIVE ranker — which excludes names failing the critical-coverage gate — the shadow variant ranks the FULL universe with no exclusion. The long sort key is `(adjusted DESC, ticker ASC)` and the short sort key is `(adjusted ASC, ticker ASC)` (`shadow-ranker.ts` longOrder/shortOrder, ~lines 195/207). When a tied block of identical `adjusted` values straddles the seed-cut on either side, the SAME alphabetical tiebreaker on BOTH sides surfaces the SAME tickers in the top-`BOOK_SEED_SIZE=20` of both — `seedShadowBook` places them on both sides. The pre-persistence overlap assertion (`shadow-ranker-orchestrator.ts:267-270`) detects this and throws `ShadowBookOverlapError` BEFORE the `pendingBook` chunked UPSERT — so the ENTIRE shadow fire fails with NO write (no partial book, all 12 variants lost). |
| **IMPACT (the data-collection gap)** | The shadow-rank orchestrator's chunked UPSERT into `combiner_book_shadow` never runs on an overlap-throwing fire → no shadow book rows → `combiner_forward_returns` (the T+10-label accrual, keyed off matured shadow seeds) has no new seeds to mature → the Phase-3.3 promotion-gate substrate (`LABEL_MIN_SEED_DATES=20` × `TRAINING_HORIZON_TD=10`, per FP-052.3 / ACT-288) does NOT accrue. The combiner model stays un-promotable not just because it's un-trained, but because the label substrate it would train on is starved. This is the Phase-3.3-track equivalent of the DW-203 live-track starvation. |
| **why it overlaps when the LIVE ranker does not** | The live ranker excludes critical-coverage failures (so a name absent on a critical signal is dropped, never ranked on either side). The shadow ranker's no-exclusion composite deliberately ranks everything (the whole point of the shadow — to measure what exclusion gives up). A name in a tied `adjusted` block can legitimately rank in the top-N from BOTH ends when the symmetric alphabetical tiebreaker breaks the tie identically on both sides. The overlap is a real artifact of the no-exclusion variant's sort symmetry, not a bug in the live path. |
| **fix direction (not yet designed — the disambiguation rule)** | A dedup/disambiguation rule for the no-exclusion shadow seed. Options under consideration: (a) **asymmetric tiebreaker** — flip the short-side tiebreaker to `ticker DESC` so a tied block cannot surface the same alphabetical heads on both sides (smallest blast, deterministic, no exclusion); (b) **per-variant partial-write** — narrow the catch to per-variant so a single overlap fails only that variant, not all 12 (narrows blast radius from 12/12-lost to 1/12-lost); (c) **per-ticker side-resolution** — assign the overlapping ticker to ONE side (e.g., stronger |adjusted|, or by net composite sign). Design must: (1) preserve the shadow's measurement intent (it measures no-exclusion ranking — the dedup must not silently re-introduce exclusion); (2) keep the resolution deterministic (replay-safe); (3) not contaminate the live ranker (shadow-only change). Recommended shape (subject to reconciliation): (a)+(b) together — fixes the structural cause AND narrows blast radius. |
| **blocking_dependencies** | None for the fix (shadow-only code change). Does NOT block the live trade path (shadow-rank is the measurement substrate, not the live fills path). Blocks the Phase-3.3 training-label accrual clock. |
| **ROI / §14** | The label-accrual clock is calendar-bound (T+10 maturation × 20 seed-dates ≈ weeks of accrual). Every day the shadow fire throws is a day of training-label accrual PERMANENTLY lost (forward stream, the shadow book for a past as_of cannot be reconstructed with the live signal vintage). Silent ROI leak on the model-promotion timeline — does not affect live PnL today, but un-stalls the clock toward the trained combiner that §6 expects to outperform the fallback ranker. |
| **related** | The earlier shadow-rank ON CONFLICT stall (distinct — a write-conflict, this is a both-sides-overlap throw); DW-203 (the live-track starvation sibling — same "starved substrate" shape, different pipeline); FP-052.3 / 3.3b (the trainer that skip-gates until T+10 labels accrue, ACT-288); the §6.5 shadow-variant harness (the measurement framework this feeds); DEC-059 (the shadow measurement window). |
| **Cross_ref** | ACT-409 (REGISTRATION + reconciled dual-investigation); ACT-410 (containment ship: per-variant skip + `variants_skipped_overlap` evidence stream + FP-063 registration); FP-063 (the deferred structural tiebreaker fix gated on the evidence window); FP-062 Tuesday-AM cycle watch (the surfacing); `shadow-ranker-orchestrator.ts` Step 5 (the per-variant try/catch now contains the throw — blast radius narrowed); `shadow-ranker.ts:14,113,188-189,197-198` (the no-exclusion symmetric composite + the long/short symmetric `ticker ASC` tiebreaker — root cause, deferred to FP-063); `ranker.ts:190-205` (the live ranker with the IDENTICAL symmetric tiebreaker — the reason the fix is not shadow-only); DEC-059 (the gated_k0 ≡ live_gated byte-identity invariant the FP-063 fix must preserve); `combiner_book_shadow` → `combiner_forward_returns` (the starved substrate — now accrues from non-overlapping siblings); `LABEL_MIN_SEED_DATES`/`TRAINING_HORIZON_TD` (the Phase-3.3 gate the substrate feeds); ACT-288 (the trainer skip-gate). |

### DW-205: Trainer intraday-slot-awareness decision (DW-168-adjacent; contingent on probe findings — decision-pending-evidence, NOT a pre-committed trainer fix)

| Field | Value |
|---|---|
| **ID** | DW-205 (next-free after DW-204; registered 2026-06-30 at ACT-422). |
| **Status** | **REGISTERED-CONTINGENT** — gated on the DW-168 probe findings (especially the two new sanity dimensions DW-168-ADD-01 NEW-1 within-group label-tie ratio + NEW-2 feature-variation across slot rotation). No code lands now; no trainer change is pre-committed. The question this DW frames is **decision-pending-evidence**, not a fix-to-apply. |
| **Tier** | A — money-path trainer surface (`training/combiner/trainer.py`); the trainer is the upstream of `promote_combiner_model` and therefore of every live ranking that consumes the promoted combiner. |
| **Title** | Whether the trainer SHOULD become `intraday_slot`-aware (per-group weighting to correct for the N_slots² pair-count inflation, OR slot-stratified grouping) — the open question DW-168's probes INFORM. |
| **The by-design framing (NOT a latent defect).** | `training/combiner/trainer.py` groups strictly by `seed_as_of_date` with NO `intraday_slot` awareness. **This is the DOCUMENTED INTENDED behavior** per [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (h).2 — *"no trainer code change required for substrate ingestion"* — the slot-rotation is additive at the substrate layer; the trainer ingests the rotated rows under the existing daily-group identity by ratified design. The slot-merge is **by-design**, NOT a bug. DW-205 exists because by-design does not foreclose a future optimization decision, not because the current behavior is wrong. |
| **The open question (decision-pending-evidence)** | Given DW-168-ADD-01 NEW-1/NEW-2 results, decide between three branches: **(α)** keep the current daily group-by unchanged (right answer if NEW-2 shows features ARE varied across slots AND NEW-1 tie ratio is low — slot rotation is information-additive and the existing trainer captures it correctly); **(β)** add per-group weighting to correct for the `N_slots²` pair-count inflation (right answer if NEW-2 shows variation but NEW-1 tie ratio is elevated — the loss surface needs reweighting to avoid the inflated tied-pair count biasing pairwise gradient); **(γ)** slot-stratified grouping — group by `(seed_as_of_date, intraday_slot)` rather than `seed_as_of_date` alone (right answer if NEW-2 shows features ARE varied AND the rotation behaves more like independent within-day folds than additive replication). The branch is selected by evidence, not by anticipation. |
| **Why no fix lands now** | Pre-committing any of (α)/(β)/(γ) before NEW-1/NEW-2 land would: (i) violate the DEC-070 clause (h).2 by-design baseline without evidence; (ii) risk a fix-for-symptom that the probes would have shown unnecessary or wrong-branched; (iii) double the FP-064-style "build against unmeasured assumption" failure mode this register exists to prevent. The slot-merge being by-design (not a defect) is the load-bearing reason this DW is decision-pending-evidence, not a fix queued behind a calendar. |
| **Scope (when activated — only after DW-168 probe findings land)** | (a) Consume the NEW-1 (within-group label-tie ratio) + NEW-2 (feature-variation across slot rotation) distributions emitted by the DW-168 probes (P1); (b) select branch (α)/(β)/(γ) against the evidence-decision table above; (c) if (β) or (γ) is selected, author a DEC-070 amendment clause ratifying the trainer surface change (the (h).2 by-design baseline supersession requires an explicit clause, mirroring the DEC-070 clause-amendment pattern); (d) implement the selected branch under a named FP with the §10.7 Phase-3 exit gates (NDCG@25 floor, walk-forward Sharpe, SHAP) applied on top; (e) if (α) is selected, record the decision and close DW-205 RESOLVED-DECLINED with the evidence cited. |
| **Depends-on** | DW-168-ADD-01 P1 probes landed (the NEW-1 + NEW-2 measurements); first intraday-trained candidate emitted (to validate the branch decision against an actual loss surface, not just substrate moments). |
| **Blocks** | Nothing today (the trainer runs unchanged under the by-design baseline). Blocks an *optional* trainer optimization until the probe evidence lands. |
| **Future Owner Phase** | Phase-7-adjacent (downstream of DW-168 probe findings; not phase-driven but evidence-driven). |
| **Resolution shape** | (1) DW-168 P1 probes land and emit NEW-1 + NEW-2 distributions; (2) branch (α)/(β)/(γ) selected against the evidence-decision table; (3) (α) → DW-205 RESOLVED-DECLINED with cited evidence; (β)/(γ) → DEC-070 amendment clause + named FP under the §10.7 exit gates → DW-205 RESOLVED-BUILT on FP closure. |
| **Cross_ref** | DW-168-ADD-01 (the probe-findings dependency — NEW-1 + NEW-2 are the named inputs); [DEC-070](../decisions/DEC-070-longshort-cadence-rebuild.md) clause (h).2 (the by-design baseline this DW does NOT pre-empt — slot-merge is documented intended behavior, not a defect); `training/combiner/trainer.py` group-by `seed_as_of_date` logic (the trainer surface; the strict daily group-by anchor); MIG-122…MIG-126 (the substrate rotations the rows flow through unchanged); FP-058 (the Phase-7 measure-and-lock — this DW's branch (β)/(γ) FP would attach under FP-058's umbrella if landed); ACT-422 (this REGISTRATION); Constitution Rule 8 (this DW is registered fresh, not an addendum). |

### DW-206: Shadow-rank cron fires pre-EOD → structural starvation of the gated arm (baseline-integrity defect for the DW-109 07-22 read-out)

| Field | Value |
|---|---|
| **ID** | DW-206 (next-free after DW-205; registered 2026-06-30 at ACT-430). |
| **Status** | **OPEN** — Fix A pending operator cron re-schedule (Dashboard, §22.5.3 sql/19 template — NOT applied this turn); Fix B pending small build (port DW-203 Gate-3 critical-present guard to the shadow path); Fix C pending post-A+B (one-shot gated-arm backfill for 06-30 + any residual missed days). Fix decision is **A+B durable pair + C to repair**; A is the minimum-now move. No code, no cron change lands this turn — record only. |
| **Tier** | A — baseline-integrity defect for the ROI-critical DW-109 read-out; without repair, the 2026-07-22 h=10 re-read inherits an asymmetrically thinner gated baseline than the relaxed arms and cannot defensibly answer the gate-cost question DW-109 exists to answer. |
| **Defect** | The shadow-rank cron `longshort-combiner-shadow-rank` (`cron.job` jobid 97) fires at **23:30 UTC** — **BEFORE** the live assemble (23:35 UTC, jobid 102) and live rank (23:50 UTC, jobid 103). The gated arm at `supabase/functions/_shared/longshort-combiner/shadow-ranker.ts:12` (`if (!(c6 && c7)) return false`) requires both critical signals (#6 momentum, #7 reversal). When today's critical signals haven't landed by 23:30 UTC, the gated arm sees zero includable rows → writes zero rows. Non-gated arms (`no_gate` requires `presentCount >= 1`; `criticals_required` requires either critical) still write. The shadow path LACKS the DW-203 Gate-3 critical-present guard — verified: main tick imports `SIGNAL_IDS_CRITICAL` and guards at `supabase/functions/longshort-combiner-tick/index.ts:72/90/114`, but the shadow path has no such guard. DW-203 remediation was tick-only. |
| **Evidence** | 2026-06-30: non-gated shadow arms wrote 40 rows each into `combiner_book_shadow`; all four `gated_kN` variants wrote **zero rows**. This is a fresh recurrence, not a historical-only artifact — the timing race manifests on any weekday the EOD critical producers slip past 23:30 UTC. |
| **Impact on DW-109** | The gated baseline accumulates asymmetric gap-days vs the relaxed arms on every weekday EOD producers slip past 23:30 UTC. Projected to 2026-07-22, this bakes a systematically thinner gated baseline into the h=10 re-read — the exact bias that made the 06-30 read-out untrustworthy, worsened by 3 additional weeks of asymmetric accrual. Left unfixed, the 07-22 anchor cannot answer the gate-cost question defensibly. |
| **Fix decision (operator-ratified 2026-06-30, ACT-430; implementations are separate)** | **Fix A (operator, NOW):** reschedule `longshort-combiner-shadow-rank` post-EOD — target `10 0 * * 2-6` (00:10 UTC next day, after the 23:50 live rank) or ≥23:55 UTC. Operator-applied via Supabase Dashboard per §22.5.3 (sql/19 template). Prevents the race. **Fix B (small build, before 07-22):** port the DW-203 Gate-3 critical-present guard to the shadow path — typed-skip when `SIGNAL_IDS_CRITICAL` absent so a starved fire does not write a false zero-includable seed. Defense-in-depth for EOD-slip days that survive Fix A. **Fix C (one-shot, after A+B):** manual gated-arm backfill for 2026-06-30 (+ any residual missed days) via authenticated shadow-rank invocation once #6/#7 known-present — repairs the existing hole so the 07-22 sample is complete. Not sufficient alone (no recurrence prevention). |
| **Why A+B+C, not A alone** | Fix A prevents the race under the normal EOD cadence but does NOT cover the EOD-slip case where producers land after 00:10 UTC too. Fix B is the defense-in-depth guard that makes the shadow path robust to any timing skew (mirroring the tick-only DW-203 remediation). Fix C is the one-time repair; without it the 07-22 sample still carries the 06-30 hole. |
| **Depends-on** | Fix A: operator Dashboard cron re-schedule per §22.5.3. Fix B: `SIGNAL_IDS_CRITICAL` typed import into the shadow path (already exported by the main tick per `supabase/functions/longshort-combiner-tick/index.ts:72`). Fix C: A+B landed + a known-present critical-signal window for the backfill invocation. |
| **Blocks** | The **DW-109 07-22 re-read** (h=10 anchor per DW-109-ADD-01) — cannot be executed on a defensible baseline until at least Fix A lands and the gated arm resumes writing. |
| **Out-of-scope (separate failure mode)** | The 2026-06-26 and 2026-06-29 whole-harness gaps (BOTH gated AND non-gated arms wrote zero) are a DIFFERENT failure — most likely EOD-producer non-fire, not the 23:30 timing race. Explicitly out of DW-206 scope; register separately if a recurrence pattern emerges. |
| **Future Owner Phase** | Phase-6/Phase-7 (baseline-continuity substrate — DW-109 is Phase-7-adjacent). |
| **Resolution shape** | (1) Fix A applied (operator Dashboard cron re-schedule); (2) Fix B built + deployed (shadow-path critical-present guard); (3) Fix C executed (manual gated-arm backfill for 2026-06-30 + any residual missed days); (4) `combiner_book_shadow` audit shows gated_kN rows present for every trading day 2026-06-30 → 2026-07-22 → DW-206 RESOLVED-BUILT; (5) DW-109 07-22 re-read unblocked and executed. |
| **Cross_ref** | DW-109-ADD-01 (the read-out this defect protects, and the 07-22 anchor gated on Fix A); DW-203 (the tick-only critical-present guard Fix B ports to the shadow path); DW-204 (shadow harness); ACT-407 (DW-203 tick-side gate); `supabase/functions/longshort-combiner-tick/index.ts:72/90/114` (main-tick guard sites); `supabase/functions/_shared/longshort-combiner/shadow-ranker.ts:12` (gated-arm structural dependency); `cron.job` jobid 97 (shadow-rank cron currently 23:30 UTC); jobid 102 / 103 (live assemble 23:35 / live rank 23:50 UTC — the producers Fix A must fire after); §22.5.3 sql/19 (the operator cron-apply template Fix A uses); ACT-430 (this REGISTRATION); Constitution Rule 8 (this DW is registered fresh, not an addendum). |

#### DW-206-ADD-01 (2026-07-01, ACT-434): Fix B LANDED

| Field | Value |
|---|---|
| **Status transition** | OPEN → **PARTIALLY RESOLVED-BUILT** (Fix B done; Fix A operator-cron still pending; Fix C historical backfill still pending). |
| **What landed** | Fix B — shared critical-present guard extracted to `supabase/functions/_shared/longshort-combiner/critical-signals-present.ts` (single source of truth); tick's inlined copy DELETED and re-imports from shared (behavior byte-identical, Gate-3 semantics preserved); `shadow-ranker-orchestrator.ts` Step 0 calls the shared helper BEFORE any variant / universe / signal read; critical-absent → typed `{outcome:'skipped', reason:'critical_signals_absent'}` with ZERO `combiner_book_shadow` writes (whole-run skip honoring full-book-or-no-book at the run level). Both shadow entrypoints (cron + manual) classify `skipped` as SUCCESS (no false alerts on natural absence). |
| **Verification** | Unit-locked: `critical-signals-present_test.ts` (csp-1..5 including T8 strict-`.eq('as_of_date')` and one-missing→false); `shadow-ranker-orchestrator_test.ts` sorch-7 (whole-run skip: zero upserts + zero universe reads + zero variant reads on critical-absent) and sorch-8 (happy path still writes — no false positive from Step 0). Tick regression preserved (t13/t14 pass against imported helper). |
| **Residual work** | Fix A: operator Dashboard cron re-schedule for `longshort-combiner-shadow-rank` (jobid 97 → `10 0 * * 2-6` or ≥23:55 UTC) — still required for cadence hygiene even though Fix B removes the false-seed risk. Fix C: one-shot manual gated-arm backfill for 2026-06-30 (+ any residual missed days) once #6/#7 known-present for those dates. |
| **DW-109 07-22 re-read gating (updated)** | Fix B now protects the baseline from FUTURE critical-absent poisoning (the 06-30 defect pattern cannot recur — the gated arm now typed-skips instead of writing zero-includable seeds). Fix A remains recommended for cron-cadence hygiene; Fix C remains required to repair the pre-existing 06-30 hole in the historical sample. |
| **Cross_ref** | ACT-434 (this build); DW-203 (the guard's origin, now shared); DW-109 (the read-out protected); Constitution Rule 8 (additive addendum; original DW-206 body preserved verbatim above). |

#### DW-206-ADD-02 (2026-07-01, ACT-435): Fix B COMPLETION — single-definition invariant enforced in the tick

| Field | Value |
|---|---|
| **Status transition** | PARTIALLY RESOLVED-BUILT → **PARTIALLY RESOLVED-BUILT** (unchanged at the DW level; internal completion of the Fix B cleanup — Fix A cron re-schedule and Fix C historical backfill still pending). |
| **Trigger** | ACT-434 landed Fix B with a half-done tick extraction: `longshort-combiner-tick/index.ts:73` imported the shared helper aliased as `sharedCriticalSignalsPresentForDate` (dead alias — never referenced), while `longshort-combiner-tick/index.ts:99-105` still defined a LOCAL `async function criticalSignalsPresentForDate` wrapper that the call site (line 240) invoked. Two definitions of the same guard was the exact tick↔shadow drift failure mode DW-206 Fix B exists to prevent. |
| **What landed (ACT-435)** | Tick cleanup — DELETED the local `criticalSignalsPresentForDate` wrapper; dropped the `as sharedCriticalSignalsPresentForDate` alias on the import (now imports the shared helper under its plain name); rewrote the call site to invoke `criticalSignalsPresentForDate(supabaseAdmin, DEFAULT_OPERATOR_ID, as_of_date)` directly (the shared 3-arg signature — the caller adapted, not the helper). Test-sentinel `(t12)` anchor switched from `indexOf` to `lastIndexOf` so it robustly points at the CALL site regardless of layout (semantics preserved: Gate-3 must run before the dirty-bit poll). |
| **Single-definition invariant (grep-verified)** | `grep -rn 'function criticalSignalsPresentForDate' supabase/functions/` = **1 hit** — `supabase/functions/_shared/longshort-combiner/critical-signals-present.ts:25`. Zero definitions in the tick; zero dead imports. |
| **Behavior-identical** | Shared helper runs the EXACT same query (`.select('signal_id').eq('operator_id',…).eq('as_of_date',…).in('signal_id', SIGNAL_IDS_CRITICAL)`, strict `.eq` on `as_of_date` for T8 replay-determinism) the pre-ACT-434 inline probe ran. Gate-3 skip-vs-proceed decision is byte-identical. Runtime call-ordering in the handler is unchanged. |
| **Test evidence** | `deno test` for tick + shared helper + shadow orchestrator = **27 passed | 0 failed** (t1-t14 tick, csp-1..5 helper, sorch-1..8 orchestrator). |
| **Residual work (unchanged from ADD-01)** | Fix A (operator Dashboard cron re-schedule for jobid 97 → post-EOD) still pending. Fix C (one-shot manual gated-arm backfill for 2026-06-30 + any residual missed days) still pending. |
| **Cross_ref** | ACT-435 (this completion); ACT-434 (Fix B initial landing); DW-206-ADD-01 (Fix B ADD entry); `supabase/functions/_shared/longshort-combiner/critical-signals-present.ts:25` (the single remaining definition); Constitution Rule 8 (additive cleanup — shared helper untouched, tick sheds its duplicate). |

#### DW-206-ADD-03 (2026-07-01, ACT-436): Fix C OUTCOME — CLOSED-AS-UNREPAIRABLE (DW-204 thin-day overlap-skip, NOT starvation) + 07-22 read-out caveat

| Field | Value |
|---|---|
| **Status transition** | PARTIALLY RESOLVED-BUILT → **PARTIALLY RESOLVED-BUILT** (unchanged at the DW level; Fix A cron re-schedule still pending operator Dashboard action. Fix B LANDED at ADD-01/ADD-02. **Fix C CLOSED-AS-UNREPAIRABLE** — the 06-30 gated hole is a structural DW-204 overlap-skip, not a starvation gap; re-running produces the same overlap by design). |
| **Trigger** | The 06-30 gated backfill was run (`longshort-combiner-shadow-rank-manual`, `as_of_date=2026-06-30`, correlation `8cd1e352`) with the Fix B guard LIVE. **Result:** `outcome=completed_with_skipped_variants`. Guard PASSED (criticals #6/#7 present at manual-fire time — Fix B works exactly as designed). 8 non-gated variants (`no_gate_kN`, `criticals_required_kN`) wrote 20L/20S each. All 4 `gated_kN` variants SKIPPED with `variants_skipped_overlap` naming QCOM + SMCI (each landed on BOTH long AND short of the gated book → `ShadowBookOverlapError` → the seeder correctly refused to write an invalid book per `shadow-ranker.ts:seedShadowBook` pre-persistence assertion). |
| **The corrected 2-cause diagnosis** | The 06-30 gated gap has TWO causes, and Fix A/B address only ONE: **(1) critical starvation** (cron fired 23:30 UTC before EOD critical producers) — FIXED by Fix A (cron reschedule, ACT-430) + Fix B (shared guard, ACT-434/435). The Fix C backfill confirmed the guard passed on criticals-present, so (1) is genuinely closed. **(2) DW-204 thin-coverage overlap-skip** — on 06-30 the gated-includable set is structurally too thin to seed two disjoint sides of `BOOK_SEED_SIZE`; QCOM + SMCI necessarily land on both long and short; the seeder throws `ShadowBookOverlapError` per its documented contract ("full-book-or-no-book — writing an overlapping book would be worse"). This is CONTAINED-BY-DESIGN DW-204 behavior, NOT a defect, and NOT repairable by re-run (the gated book is structurally too thin on that date; the input rankings do not change on re-invocation). |
| **Fix C disposition** | **CLOSED-AS-UNREPAIRABLE.** The 06-30 gated seed-day is genuinely ABSENT and structurally UNREPAIRABLE — the hole is a DW-204 thin-day overlap-skip, not a fixable starvation gap. Re-invocation produces the identical overlap. The Fix C backfill attempt correctly diagnosed the hole rather than repairing it — that is the intended outcome given the corrected 2-cause diagnosis. No further backfill invocation is warranted for 06-30. |
| **07-22 read-out caveat (the honest bias note for DW-109)** | The gated arm structurally under-writes on any thin-coverage day where the gated-includable set < ~2×`BOOK_SEED_SIZE`, skipping via DW-204 overlap while the relaxed arms (`no_gate`, `criticals_required`) still write. So the 07-22 h=10 gated baseline is a **NON-RANDOM SUBSET** of days (systematically thicker-coverage days only). The gate-ablation comparison remains valid on days where ALL arms wrote (paired-on-common-days), but the 07-22 read-out MUST: (a) **pair only on common-support days** — days where gated AND all relaxed arms both wrote; (b) **report the count of gated-skipped-overlap days separately** — they are themselves informative (they indicate the gate is SO restrictive it cannot form a disjoint book, which is a real finding about the gate's structural cost, not a missing data point to impute); (c) **NOT treat gated-absent days as missing-at-random** — the absence mechanism is coverage-density-correlated. |
| **DW-204 status (unchanged, restated for cross-ref)** | CONTAINED-BY-DESIGN. The overlap-skip is the seeder's correct behavior (writing an overlapping book would be a worse failure mode than skipping the variant). No new defect to fix; no build queued. |
| **Residual work (revised)** | Fix A (operator Dashboard cron re-schedule for jobid 97 → post-EOD) still pending. Fix B LANDED (ADD-01/ADD-02). **Fix C CLOSED-AS-UNREPAIRABLE** (this entry). |
| **Cross_ref** | ACT-436 (this record); ACT-434/435 (Fix B); ACT-430 (Fix A charter); DW-204 (the CONTAINED-BY-DESIGN overlap-skip mechanism, ACT-410); DW-109 (the 07-22 gate-ablation read-out this caveats); `supabase/functions/_shared/longshort-combiner/shadow-ranker.ts:seedShadowBook` (the pre-persistence overlap assertion the gated variants correctly tripped); Constitution Rule 8 (additive addendum; DW-206 body + ADD-01 + ADD-02 preserved verbatim above). |

### DW-207: Rebalance current-book is lot-derived, not broker-derived (§2 inversion — money-path defect)

| Field | Value |
|---|---|
| **ID** | DW-207 (next-free after DW-206; registered 2026-07-01 at ACT-440). |
| **Status** | **OPEN** — diagnosis complete + fix direction known; Tier-A money-path build DEFERRED (separate, no time pressure). Immediate 06-30 orphan cleanup is a SEPARATE operator-gated Alpaca liquidation (Path A), not this DW. |
| **Tier** | A — money-path defect on `supabase/functions/_shared/longshort-execution/rebalance-planner.ts` (the close-intent input). §2-inverting (broker is prime; internal is derivative). |
| **Title** | Rebalance planner sources `currentPositions` from the internal lot ledger (`longshort_lots WHERE status='open'`) instead of the broker (`/v2/positions`). Any broker position outside the lot ledger is structurally invisible to the close path → accumulates as a permanent broker-orphan. |
| **How surfaced** | FP-068 W1 reconciliation banner rendered **40 broker / 28 internal, 12 broker-orphans** on 2026-07-01: BX, CF, CHTR, CMC, COST, FDX, FIX, GS, MU, NXT, PFGC, SNDK. Operator hypothesis was "stale prior-day book before intraday-signal rework." Read-only DB investigation (this turn) confirmed the mechanism as **pre-lot-ledger legacy**, not stale-close-orders. |
| **Diagnosis (evidence)** | (1) `SELECT * FROM longshort_lots WHERE symbol IN (12 orphans)` → **ZERO rows** (neither `open` nor `closed`); (2) all 28 currently-open lots have `entry_ts` in a narrow 2026-06-30 window — 20 lots at `16:15:03.465+00`, 8 lots at `16:30:02.360+00` — the FP-061 lot ledger **COLD-STARTED capturing fills on 2026-06-30 ~16:15 UTC**; (3) `SELECT * FROM longshort_lots WHERE status='closed'` → **empty** (no closed lot has ever been written). ⇒ The 12 orphans were opened at the broker BEFORE the lot ledger went live and are structurally invisible to it. Classified as **pre-lot-ledger legacy** (investigation hypothesis (c) confirmed); NOT (a) stale-close-not-executed and NOT (b) internally-closed-broker-didn't-execute. |
| **The defect (mechanism)** | `rebalance-planner.ts:39` (approx.) computes the close intent correctly — "current position NOT in selected set → intent='close', target_notional=0" — but `currentPositions` is sourced from the internal open-lot set, NOT from the broker `/v2/positions` snapshot. A broker position that never entered the lot ledger (cold-start gap, missed fill capture, corp-action re-issue, out-of-band manual position) is NEVER seen as `current` by the planner → NEVER emits a `close` intent → accumulates as a permanent orphan. This **inverts §2** (external snapshots are prime; internal tables are derivative). |
| **verify_position asymmetry (evidence + note)** | `verify_position.ts:27` (approx.) escalates the **internal-has / broker-lacks** direction (internal expects a position the broker doesn't hold). The orphan case is the **REVERSE** (broker-has / internal-lacks). Live evidence from `reconciliation_events` last 14d: `verify_position` = **539 `failure_escalated`** (latest 2026-07-01 07:05). The reconciler IS running and IS escalating, but (a) no cleanup path acts on the escalation, and (b) even if it did, the planner's close path cannot see the orphans to close them — so DW-207's fix (broker-sourced current) is what lets the close path act on the broker-has/internal-lacks direction. |
| **fix direction (not built this turn)** | `rebalance-planner`'s `currentPositions` input becomes **broker-sourced** (the fetcher's `listOpenPositions(ts)` result via `AlpacaPositionFetcher` — already E1-consumed elsewhere) OR the **union of broker-derived + lot-derived** (belt-and-braces: broker-primes the current-book, lot-ledger contributes attribution/cost-basis). Under either shape, any broker position outside the target set emits a `close` intent on the next rebalance and self-heals — closing the recurrence for any future lot-ledger gap (missed fill capture, corp-action re-issue, out-of-band manual position). Money-path change (Tier A) — build + verify carefully, no time pressure. |
| **cleanup options for the 12 existing orphans (operator-gated — SEPARATE from DW-207)** | **A (recommended for immediate cleanup)** — operator-triggered Alpaca liquidation of the 12 orphans (they are pre-rework legacy, not part of the clean book's P&L). Realizes any PnL on those 12 into the paper account outside the lot ledger. **B** — synthetic backfill of `open` lot rows from broker `avg_entry_price` × `qty`, then let rebalance close them next fire (fabricates internal history — wash-sale/FIFO integrity questionable; not recommended). **C** — leave the orphans; filter the banner to exclude the 12 known-legacy symbols (ignores §2 — not recommended). **D (this DW)** — the durable structural fix. Operator direction (this turn): **A now** (immediate cleanup, operator-executed outside this turn) **+ D as durable follow-up** (this DW). |
| **blocking_dependencies** | None for the DW-207 fix itself (self-contained planner input change + tests). The immediate 06-30 cleanup (Path A) is independent and operator-gated. |
| **ROI / §14** | Direct: closes a §2-inverting money-path gap that has already permitted 12 permanent broker orphans consuming ~30% of the position slot count (12/40). Silent buying-power drag + MV drift for every orphan until manually flattened; permanent unless the structural fix lands. Recurrence risk: **certain** on any future lot-ledger cold-start, missed fill capture, corp-action re-issue, or out-of-band manual position — DW-207's fix converts each such event from "permanent orphan" to "auto-closed on next rebalance." |
| **Future Owner Phase** | Phase-6-adjacent (money-path substrate hardening; siblings DW-203 / DW-204 / DW-206 already live in the same neighborhood). Named FP TBD when scheduled. |
| **Resolution shape** | (1) Named FP charters the planner-input change (broker-sourced `currentPositions` OR broker+lot union); (2) tests cover the orphan-close direction (broker-has / internal-lacks → close intent emitted); (3) verify_position reconciler check either narrows (once the close path handles this direction structurally) or is left as a belt-and-braces guard; (4) FP-068 reconciliation banner used as post-build validation — banner returns to `matched ✓` after next rebalance under a book with a synthetic orphan; (5) DW-207 → RESOLVED-BUILT on FP closure. |
| **Cross_ref** | ACT-440 (this REGISTRATION); ACT-437/438/439 (FP-068 W1/W2 — the reconciliation banner that surfaced the 40/28 discrepancy); FP-068 (the display surface that made the drift visible); §2 (broker-prime axiom this restores); `supabase/functions/_shared/longshort-execution/rebalance-planner.ts:39` (the correct close-intent logic that's currently lot-fed); `supabase/functions/_shared/longshort-execution/verify_position.ts:27` (the reconciler and the direction asymmetry — internal-has/broker-lacks only); FP-061 (the lot ledger whose 2026-06-30 ~16:15 UTC cold-start created the 12 orphans); `longshort_lots` (the derivative currently mis-driving the close path); `AlpacaPositionFetcher.listOpenPositions` (the broker-sourced input the fix should consume); DW-203 / DW-204 / DW-206 (money-path substrate siblings). |

#### DW-207-ADD-01 (Rule 8, ACT-445) — premise REFUTED, real cause re-attributed

**Status change:** DOWNGRADED from "money-path defect requiring a fix" to **NO-FIX-NEEDED / RESOLVED-ON-INSPECTION**. Original entry preserved verbatim above per Rule-8 additive discipline (clause-(p) precedent — wrong diagnosis is preserved-record, not rewritten).

**What the diagnosis got wrong:** DW-207 registered "rebalance planner sources `currentPositions` from the internal lot ledger, not the broker." This is **factually wrong** for the production path. Grep evidence at HEAD `d6acb39c`:
- `supabase/functions/_shared/longshort-execution/rebalance-submit-orchestrator.ts:383` — `const positions = await listOpenPositions.call(positionFetcher, ts)` — the BULK BROKER fetch (Alpaca `GET /v2/positions`).
- `:384` — `const currentPositions = positions.map(brokerPositionToCurrent)` — maps **BrokerPosition → CurrentPosition**.
- `:274` — `brokerPositionToCurrent(p: BrokerPosition)` — the mapper takes a `BrokerPosition`, not a lot.

So `currentPositions` **IS** broker-sourced on the production `full_rebalance` path. The §2 inversion the original entry describes does not exist.

**No lot-match filter drops orphans:** grep confirms no CID / `lse-` / managed-CID filter excludes orphans from the close-candidate set. `brokerPositionToCurrent` only throws on positions missing `market_value` / `current_price` (a data-completeness guard, not a lot-match filter). When planning runs, the 12 orphans **are** close candidates.

**The real cause the orphans persist:** the orchestrator returns `refused: rankings_stale` at `rebalance-submit-orchestrator.ts:355–375` **before** reaching the position fetch (`:383`) and planning. On every stale-rankings rebalance, planning never happens → no close intents emitted for any position → orphans persist. This is the **freshness** issue (manual-handler Fix A / the pending automated-path test), NOT a close-path or lot-derived defect. Separately, the 12 orphans predate the clean book, so they need **one clean rebalance to run** (broker-sourced current includes them → they are not in the new target book → close intents emit) to self-heal — which hasn't happened yet because the first fills only landed 2026-06-30 and subsequent rebalances were refusing on staleness.

**Implication for the 12 orphans:** they should **self-close on the next clean automated rebalance** (broker-sourced current sees them, they're not in the target book, close intents emit). Testable on tonight's automated cycle. The operator's planned manual liquidation at market open may be **unnecessary** if the automated rebalance runs clean first — check the FP-068 W5 Closed-today tab after the next clean rebalance before manually liquidating. A self-close would show there as **matched** (not unmatched), since it's a broker-sourced close of a real broker position.

**What does NOT need to be built:** no planner-input change, no broker+lot union, no orphan-close-direction tests (the existing planner logic already emits close intents for broker-current-not-in-target). The `verify_position` reconciler asymmetry noted in the original entry stays as a separate belt-and-braces observation — not a DW-207 obligation.

**Cross-ref:** `rebalance-submit-orchestrator.ts:383–384` (broker-sourced current — refutes premise); `:355–375` (`rankings_stale` refusal — the real gate); freshness Fix A (manual handler, DW-203 line); pending automated-fill test (tonight); FP-068 W5 Closed-today tab (self-close observation surface); ACT-445 (this correction).

### DW-208: Silent zero-submission rebalance (phantom-success signature; cause under-investigation)

| Field | Value |
| --- | --- |
| **ID** | DW-208 (next-free after DW-207; registered 2026-07-01 at ACT-446). |
| **Status** | OPEN — SYMPTOM-CONFIRMED / CAUSE-UNDER-INVESTIGATION (Claude ↔ Lovable mechanism divergence, unreconciled). |
| **Tier** | A (governance) — §9 phantom-success banned pattern: a rebalance that emits zero intents and reports `completed` without an explicit refusal LOOKS healthy while doing nothing. If the daily cron silently no-ops every run, the system APPEARS to trade autonomously while actually doing nothing — undetectable without inspecting intent counts. |
| **Confirmed symptom** | The 2026-07-01 14:30:02 UTC automated cron rebalance (correlation `d8c048e4…`, cron `job_id=110` `longshort.rebalance.daily`, trigger `cron`, mode `full_rebalance`, `long_only_mode=true`, `ssr_unavailable=true`) completed with **all submission counts = 0** (`accepted=0`, `rejected=0`, `noop_skipped=0`, `zero_share_skipped=0`, `quote_stale_skipped=0`, `insufficient_bp_skipped=0`). **No `rankings_stale` action was logged.** No error was surfaced. `longshort.rebalance.completed` fired normally. The planner emitted no opens, no closes, no noops — and the run reported success. |
| **Cause — UNDER INVESTIGATION (NOT settled)** | This is a **Claude ↔ Lovable divergence** that must be reconciled BEFORE any fix. **Lovable's verification proposal**: the planner ran against stale `longshort_target_positions` (newest `as_of=2026-06-23`) while `combiner_rankings` is current (`as_of=2026-06-30`). **Supervisor's independent grep (HEAD `44cc3b33`)**: `rebalance-submit-orchestrator.ts:196–214` reads the target set from `combiner_rankings`, **NOT** `longshort_target_positions`; the `rankings_stale` guard at `:8–40` would not fire because rankings are fresh (06-30). The supervisor could NOT locate (a) any writer for `longshort_target_positions`, or (b) any planner read of it. **The stale-target mechanism is therefore NOT code-confirmed.** |
| **Candidate mechanisms (disambiguate, do NOT assert)** | (1) stale `longshort_target_positions` IS read on a path the supervisor didn't grep; (2) rankings are fresh but the selected-set is empty for some reason (all names filtered out → zero intents *legitimately*); (3) a silent short-circuit / early-return swallows an empty-plan case as `completed`; (4) targets ARE current and the current book already matches target (nothing to do — a **LEGITIMATE zero-submission**, not a defect). The investigation must distinguish a DEFECT (silent swallow) from a LEGITIMATE no-op (book already matches target). |
| **Why it matters regardless of cause** | Even if this specific run was a legitimate no-op, the **SILENCE** is the defect. A zero-intent completion should be **EXPLICITLY classified** — `legitimate_no_op` vs `stale_input_refusal` vs `empty_selection` vs `book_matches_target` — not reported as an undifferentiated `completed`. §9 requires phantom-success to be impossible-by-construction: the terminal action MUST carry the classification. |
| **Related — possibly-separate, TBD (do NOT bundle)** | The concurrent AAPL `verify_position` divergence (225 `failure_escalated` events; broker `qty=-8` vs `expected qty=0`; AAPL). **Lovable's proposal**: `verify_position` reads its expected-source from the same stale `longshort_target_positions`, making it the same root as DW-208. **Supervisor**: could NOT confirm `verify_position`'s expected-source by grep. **Disposition**: flagged as POSSIBLY-RELATED, cause-unconfirmed; the investigation must determine whether planner and verifier share a root or are distinct. **Do NOT assume one root.** |
| **Orphan impact (cross-DW-207)** | The 12 (now 9 — operator manually closed 3) legacy orphans did NOT self-close today: the rebalance emitted no close intents, so the close-emit branch was never reached. **DW-207's self-close prediction is UNTESTABLE from a zero-intent rebalance** — it never got to the branch it predicted. The orphans persist by **inaction (this defect)**, not by a close-path defect. DW-207 remains RESOLVED-ON-INSPECTION for the close-path premise; the self-close observation is deferred until DW-208 is resolved and a rebalance actually emits plans. |
| **Next step (investigation, not fix)** | Dual-investigation to (a) confirm the actual target source the planner reads (grep the full submit path, not just the orchestrator surface); (b) determine why zero intents (stale-target / empty-selection / already-matched / silent-swallow) by instrumenting or replaying the 14:30 correlation; (c) determine `verify_position`'s expected-source (the AAPL divergence); (d) decide whether planner + verifier share a root. **NO code, NO fix, until the mechanism is code-confirmed and the Claude ↔ Lovable divergence is reconciled.** |
| **blocking_dependencies** | The Claude ↔ Lovable mechanism-divergence reconciliation MUST close before a fix charter opens. |
| **ROI / §14** | Direct: if the silent no-op is a defect (mechanisms 1 or 3), the daily cron has been shipping zero P&L while reporting healthy — an unbounded silent-drag on the money path. If it is a legitimate no-op (mechanisms 2 or 4), the ROI cost is telemetric only (operator cannot distinguish healthy-no-op from broken-no-op without SQL) — still §9-non-compliant. Either way, the fix (explicit classification on `rebalance.completed`) is Tier-A because it converts a phantom-success surface into an auditable one. |
| **Future Owner Phase** | Phase-6 money-path substrate (siblings DW-203 / DW-204 / DW-206 / DW-207). Named FP TBD once the mechanism is code-confirmed. |
| **Resolution shape** | (1) Investigation reconciles the Claude ↔ Lovable divergence and code-confirms the mechanism; (2) named FP either fixes the silent-swallow OR (if the run was legitimate) adds explicit `no_op_reason` classification to `longshort.rebalance.completed`; (3) tests cover every zero-intent branch — each emits a distinct classification; (4) the AAPL `verify_position` question is resolved as same-root or distinct-DW; (5) DW-208 → RESOLVED-BUILT on FP closure. |
| **Cross_ref** | ACT-446 (this REGISTRATION); §9 (phantom-success banned pattern this violates); DW-207 (the orphan self-close DW-208 blocks — see Orphan impact row); DW-203 (freshness / manual-handler Fix A — related but distinct source); `rebalance-submit-orchestrator.ts:196–214` (the `combiner_rankings` target read the supervisor grep-confirmed — refutes Lovable's stale-target mechanism on this path); `rebalance-submit-orchestrator.ts:8–40` (the `rankings_stale` guard that did NOT fire because rankings are fresh); the 14:30 cron (`cron.job_id=110`, correlation `d8c048e4…`); `verify_position` (the AAPL divergence, expected-source unconfirmed — POSSIBLY-RELATED, TBD); `longshort_target_positions` (Lovable's proposed source, writer + planner-read path unlocated by supervisor grep); `longshort.rebalance.completed` audit action (the phantom-success surface). |

#### DW-208-ADD-01 (Rule 8, ACT-447) — root cause CONFIRMED (two coupled defects); stale-target theory REFUTED

**Status change:** SYMPTOM CONFIRMED / CAUSE UNDER-INVESTIGATION → **ROOT CAUSE CONFIRMED (two coupled defects) / FIX DECISION PENDING** (Fix 1 requires architectural decision; Fix 2 buildable). Original entry preserved verbatim above per Rule-8 additive discipline.

**Stale-target theory REFUTED (both prior variants).** `longshort_target_positions` is read **0 times** in the planner/orchestrator (supervisor grep, HEAD `4512b564`). The planner reads targets from `combiner_rankings` (orchestrator `:196–214`). The zero-intent outcome on the 07-01 14:30 UTC fire was **NOT** caused by a stale target book on either the planner path or the verifier path (verifier's expected-source is unrelated — see AAPL/BP note below).

**Confirmed root cause — TWO COUPLED DEFECTS (all 4 claims grep-verified):**

**(1) CADENCE DEFECT — the substantive one.** The ranker cron writes `combiner_rankings` at `50 23 * * 1-5` (23:50 UTC) — verified in migration `20260621095443` line 85. The rebalance cron fires at `30 14 * * 1-5` (14:30 UTC) — verified in migration `20260625173743` line 11. The rebalance therefore consumes a ranking written **~14.7 hours earlier**, which is orders of magnitude above the DEC-070 clause (c) `RANKING_FRESHNESS_TOLERANCE_S = 600s` tolerance. The freshness gate (`rebalance-submit-orchestrator.ts:348–381`) refuses EVERY automated fire as designed. **The automated rebalance cannot pass the freshness gate under the current schedule** — every automated fire since fresh rankings landed 2026-06-30 23:50 UTC has refused with `refused: rankings_stale`. Live evidence: `max(combiner_rankings.computed_at) = 2026-06-30 23:50:01.521Z`; 14:30:02 UTC fire age = ~52,800s vs 600s tolerance.

**(2) AUDIT-SURFACE DEFECT — the §9 observability one.** The cron handler `supabase/functions/longshort-rebalance-submit-cron/index.ts:173–186` writes `longshort.rebalance.completed` with metadata `{submission_counts, ssr_unavailable, long_only_mode, shorts_skipped_locate_unavailable, htb_marks_persisted}` and **omits `result.refusal` entirely** (verified: **0** references to `refusal` in the audit metadata block). A refused-stale rebalance and a healthy zero-intent no-op produce **identical audit events**. The refusal is emitted only to `console.warn('longshort_rebalance.refused.rankings_stale', …)` at orchestrator `:355` — invisible to any operator reading audit logs. Same defect on the manual path: `longshort-rebalance-submit/index.ts:143`. This is a §9 phantom-success SURFACE violation (correct decision reported as if it were a healthy success).

**`long_only_mode: true` / `ssr_unavailable: true` on 07-01 were FALSE TELEMETRY.** On the refusal path, `preflight_summary` is `undefined` (early-return at orchestrator `:379` before preflight runs). `buildResponse` at `:671–681` then defaults `ssr_unavailable`, `locate_unavailable`, `shortability_unavailable` to `?? true`, which makes `long_only_mode = locate_unavailable && shortability_unavailable = true` as a default artifact. **Preflight never ran.** This is NOT evidence of a genuine long-only mode and NOT evidence of an SSR outage on 07-01. The separate architectural question "should long-only mode close un-maintainable shorts" is **N/A for this run** (preflight never ran) — deferred as an open architectural question, not a DW-208 defect.

**AAPL `verify_position` (225 escalations) + `verify_buying_power` (225 escalations) are DW-138, NOT DW-208.** `verify_position` reads a **hardcoded bootstrap placeholder** at `supabase/functions/longshort-reconciliation-tick/index.ts:144–146` — `BOOTSTRAP_PLACEHOLDER_PROBE_SYMBOL = 'AAPL'`, `BOOTSTRAP_PLACEHOLDER_EXPECTED_QTY = 0`, `BOOTSTRAP_PLACEHOLDER_EXPECTED_COST_BASIS = 0`. Broker holds AAPL short −8 (opened 06-30) → placeholder expects 0 → escalation. `verify_buying_power` same class: hardcoded `EXPECTED_BP_USD = 100_000` (reconciliation-tick `:143`). Both resolve when **DW-138** wires the real expected-source (target-position table for `verify_position`, live account snapshot for `verify_buying_power`). **Cross-ref DW-138. NOT a DW-208 defect. NOT a stale-target defect.**

**Orphan self-close (DW-207) — untestable from this run, gated on Fix 1.** The rebalance returned via early-return at orchestrator `:379` BEFORE `listOpenPositions` at `:383`, so no current-book fetch and no close intents were possible. DW-207-ADD-01's self-close prediction is **uncontradicted but untested** — it requires a rebalance that passes the freshness gate (i.e. requires Fix 1). The 9 remaining orphans (operator manually closed 3) persist by **freshness-refusal inaction**, not by any close-path defect.

**NOT a phantom-success SWALLOW; SAFE failure class.** The freshness gate is working **correctly** — refusing to trade on genuinely-stale 14h+ rankings per DEC-070 clause (c) as designed. The two defects are (1) the schedule guarantees refusal every automated fire, and (2) the refusal is reported invisibly at the audit surface. Nothing traded incorrectly — the system correctly declined to act on stale data. **Classification: SAFE (correct-decline + bad-cadence + bad-observability), NOT unsafe (trading on bad data).**

**Fix decision — two coupled fixes, built separately after operator authorization:**

- **FIX 1 (CADENCE — enabling; requires architectural decision).** Align the ranker and rebalance so the rebalance consumes a fresh ranking. Options (operator/architectural decision — **NOT pre-decided here**):
  - (a) move the rebalance to fire shortly after the 23:50 UTC ranker — but that is after-hours, orders wouldn't fill;
  - (b) add an intraday ranker fire in the rebalance's pre-window (e.g. ~14:00 UTC before the 14:30 rebalance) so a same-morning fresh ranking exists;
  - (c) revisit DEC-070 clause (c) design intent — the code comment at orchestrator `:340–347` says "the daily ranking computed at ~10:30 is acted on within the same fire," implying the DESIGN expected a ~10:30 ET (14:30 UTC) ranker, but the ranker actually runs 23:50 UTC the prior day. **Design intent vs actual schedule are mismatched — reconcile which is correct.**
  - Substantive fix; needs architectural decision on intended cadence before build.
- **FIX 2 (AUDIT SURFACE — §9 observability; buildable).** The rebalance audit event (cron + manual paths) MUST carry `result.refusal` and classify the zero-intent outcome as one of `{refused-stale, legitimate-no-op, empty-selection, all-matched}`. A refusal must never be indistinguishable from a healthy no-op. Makes §9 phantom-success impossible by construction. Self-contained, independent of Fix 1, buildable on operator authorization.

**Fix 1 blocks:** DW-207 self-close observation (untestable until an automated rebalance passes the freshness gate); the 9 remaining orphans' automated self-heal; every automated fire producing intents.

**Fix 2 blocks:** nothing structurally, but every future zero-intent audit event remains ambiguous until landed.

**Disposition:** DW-208 → **ROOT CAUSE CONFIRMED, FIX DECISION PENDING.** Fix 1 (cadence) is the enabling fix and needs an architectural cadence decision. Fix 2 (audit surface) is the §9 observability fix, buildable on authorization. AAPL/BP escalations → **DW-138** (not DW-208, not stale-target). Orphan self-close → **gated on Fix 1**.
...
| CLOSED-REJECTED | ACT-548 cell-add candidate (**closed 2026-07-25 19:49Z**) | short-side S_08_10 / S_10_INF × drawdown-bucket-4 × momentum-q-5 corpus verdict: **FAIL** vs 42.42 bps/slot-day floor. Only 1/10 target-cell windows clears at n≥100 (S_10_INF w=2, 56.96 bps); S_08_10 fails at every sufficient window and **sign-inverts at w=4 (n=259)**. Monotone-stability across dd±1 FAIL (dd=5 uniformly INSUFFICIENT-N in both bands). Regime-conditional UNEVALUABLE — pooled n never reaches 1000. **Rule-8 CLOSED-REJECTED**, no DEC drafted, no admission-grid change. DEC-084 short_daily_budget=1 unaffected. | closed | 2026-07-25 | 2026-07-25 | `docs/06-tracking/ACT-548-drawdown-b4-trial.md` (verdict); `docs/06-tracking/ACT-548-cell-add-candidate-drawdown-b4-shorts.md` (charter); ACT-573 §3 feeder; substrate `overshoot_study_cell_results` run `1888e113…c121` |
| D-PHASE-0-DELIVERED | ACT-570 (Phase-0 landed 2026-07-25) | naked runner-predicate (N=4 consec up ∧ cum≥15% ∧ no-earnings-±5d) — 1,693 events / 354 tickers / 2022-2026; fwd-5d +15 bps (rev 50.1%), fwd-10d +128 bps (rev 46.7%); **HYPOTHESIS-NOT-CONFIRMED at pooled corpus**; 2022 bear regime reverts, 2023-2026 continues. Phase-1 = FINRA Reg SHO SVR ingest (chartered §7) gated on DEV-15 endpoint probe | Phase-1 next turn after DEV-15 probe | 2026-07-25 | next turn | `docs/06-tracking/ACT-570-phase0-runner-predicate.md` |
| D-HALTED-AWAITING-RULING | DEV-15 (**probed 2026-07-25 06:09Z**) | FINRA `api.finra.org/regShoDaily` probe: OPEN/no-key, POST-JSON, ~27.5k rows/session, 5000 max/page, T-1 recency ✓, **BUT history depth = ~1 year rolling (2025-08-01 → T-1), not 17y as charter assumed**. Pre-committed n≥200/quintile UNREACHABLE on Phase-0 corpus (~127/quintile in coverage; 2022 bear regime entirely out). Options A/B/D presented; supervisor rec = **Option D** (api + CDN-archive hybrid) | operator ruling before MIG | 2026-07-25 | this turn | `docs/06-tracking/2026-07-25-dev-15-finra-endpoint-probe.md` |
| D | GUARD-CANDIDATE-D50 (**new — 2026-07-25 from ACT-570 Phase-0 §3**) | future short-side signals MUST EXCLUDE cum-4d ≥ 50% runners (D_50+ bucket): n=22, fwd-5d **+1,381 bps** violent continuation, 36% reversal — the untouchable class. Pre-committed exclusion; slot into any short predicate that graduates through corpus grammar | pre-adoption guard, no build until a short overlay is proposed | 2026-07-25 | pre-any-short-adoption | `docs/06-tracking/ACT-570-phase0-runner-predicate.md §3` |
| D | ACT-576 (charter filed 2026-07-25) | execution-drag decomposition — FILLED vs SELECTED gap (~152 bps, n=50) split into ENTRY_SLIP + CLUSTER_MIX + EXIT_SLIP + RESIDUAL with bootstrap CI; controllable share routes to ACT-506 bleed-box | slots immediately after ACT-570 Phase-1 | 2026-07-25 | post-ACT-570-P1 | `docs/06-tracking/ACT-576-charter-execution-drag-decomposition.md`; ACT-573 §3 feeder; ACT-506 downstream |
| CLOSED | FIX-7 capacity-arithmetic memo (delivered 2026-07-25) | corrects the 07-23 "shortfall table" framing that mis-read `capacity_remaining` (per-run K headroom) as book-level headroom; publishes honest book-full window **Fri 2026-07-31 (cap 36 @ K=5) → Tue 2026-08-04 (cap 40 @ observed 4.0/day)** from Fri 07-24 close of 12 open lots; no code change (framing memo only); shortfall language retired | n/a — memo | 2026-07-25 | delivered | `docs/04-modules/overshoot/fix-7.md` |
| GOVERNANCE-NOTE | L-05 quarantine-gate grammar → HOUSE STANDARD (2026-07-25) | per operator ruling post L-05 Step-0 FAIL, the L-05 quarantine grammar (n≥10,000/bucket historical prior test with lift + monotone + regime-robust + confound-residual sub-gates) is now MANDATORY for **all post-hoc composition/DoW/calendar patterns** before they may graduate to A/B; retracted-prize on FAIL is on the record (Rule 8 NOT-A-SIGNAL class) | permanent standing rule | 2026-07-25 | in force | `docs/06-tracking/L-05-step0-corpus-prior.md`; L-05 charter |
| CLOSED-CANCELLED | POLYGON-SUBSCRIPTION-CLOSURE (**2026-07-25** — operator ruling) | Polygon **Options Developer** + Polygon **Indices Advanced** subscriptions formally cancelled by operator 2026-07-25. Canonical provider inventory now published at `docs/04-modules/overshoot/overshoot.md` §Data-Providers; ACTIVE providers = Polygon Stocks Advanced + FMP Premium ONLY. Grep-sweep completed (`Options Developer\|Indices Advanced` across `docs/`) — each hit stamped `[CANCELLED-2026-07-25]`: `incidental-findings.md` INC-71 header; `approved-decisions.md` DEC-045 preface; `options-flow.md` L37; `_pattern-vendor-fetcher-filter-honesty.md` L89+L152; `2026-07-25-act-571-build-and-cdn-block.md` L167-168; `action-tracker.md` ACT-157 L4671; `feature-proposals.md` FP-043 L795. Topic permanently retired — any revival requires an explicit superseding DEC | permanent | 2026-07-25 | closed | `docs/04-modules/overshoot/overshoot.md` §Data-Providers (canonical); INC-71; DEC-045; ACT-571 |
| D-SCHEDULED | ACT-515(e) sector-ingest — **Turn 1 of 3 COMPLETE 2026-07-25** | Turn 1 landed: `sql/44_overshoot_universe_sector.sql` applied (adds `gics_sector`, `sector_source`, `sector_asof` to `overshoot_universe` with provenance CHECK constraint + partial index; NO data written — typed-absence honored). Turn 2 planned = FMP `/stable/profile` fetcher (`supabase/functions/overshoot-sector-ingest/`) + `verifyFieldsPresent` self-check + `supabase/functions/_shared/overshoot/fmp-sector-fetcher.ts`. Turn 3 = backfill 905 tickers + iShares CSV cross-check + coverage table + live-book concentration citation. Unlocks ACT-515(e) sector-cap engine column. Chartered `scripts/act-515/charter-amendment-e-sector-cap.md` §4 (FMP primary recommended path). | Turn 2 = 2026-07-26; Turn 3 = 2026-07-26/27 | 2026-07-25 | 2026-07-27 | `sql/44_overshoot_universe_sector.sql`; `scripts/act-515/charter-amendment-e-sector-cap.md`; MIG ledger entry pending |
| D-SCHEDULED | L-02 entry-minute corpus study — **DEADLINE 2026-07-26 / 2026-07-27** (calendar-lock restated) | Per operator's implementation-calendar lock (2026-07-25) and **L-02 charter amendment 2026-07-25**: substrate owned (MIG-167 `overshoot_minute_bars`, ≥95% coverage from 2026-07-01). Execute R-007 entry-side cost curve at 09:35..09:45 ET, k=1000 bootstrap CI, monotonicity envelope. Gates as chartered (`docs/06-tracking/charters/L-02-entry-minute-timing.md` §2). Deliverable = `docs/06-tracking/L-02-cost-curve.md`. Adoption ⟺ ALL 6 gates green (Δ ≥ 5 bps monotone). **Launch-independence:** paper lane runs continuously past ACT-577 mid-Aug live date; this lever adopts post-launch on its own evidence. | 2026-07-26/27 | 2026-07-25 | 2026-07-27 | `docs/06-tracking/charters/L-02-entry-minute-timing.md` (amended 2026-07-25); MIG-167 substrate |
| ARMED | L-01 limit-ladder A/B — **ARM LANDED Sat 2026-07-25 19:57Z; A/B LIVE Mon 2026-07-27 13:35Z** | Arm-B tighter-limit code path DEPLOYED tonight (one day early vs Sunday plan). `SOURCE_VERSION='fb5fdf13+fix2+fix8+sp1+fix9+l01a'` §22.5.1-verified via probe. Pure module `_shared/overshoot-execution/limit-arm.ts` (Arm A=50 bps ratified cap; Arm B=40 bps, −10 bps closer to quote-mid; deterministic FNV-1a over `(runId,ticker)` — replay-safe, session-matched pairing per charter §3). `overshoot-entry-run` calls `pickLimitArm` before `constructEntryLimitPrice`; arm+slippage stamped on submission-audit metadata AND on `overshoot_lots.metadata` jsonb (`{"limit_arm","limit_slippage_bps"}`). Migration `ALTER TABLE overshoot_lots ADD COLUMN metadata jsonb NOT NULL DEFAULT '{}'::jsonb` applied. 11/11 new tests green (6 module + 5 source-sentinel); 265 prior tests undisturbed. Randomization inactive until Monday 13:35Z primary-pass (Saturday/Sunday = no scheduled admits). Window is **rolling-until-gates-clear** — 90/120-day caps STRUCK; first-verdict look at n ≥ 100/arm (~4 weeks) is informational only. Gates: G-1 fill_rate ≥ 88% & |Δ| ≤ 4pts, G-2 entry_slip ≤ +20 bps & <ctrl, G-3 paired-t p<0.10, G-4 n≥200/arm, G-5 dollar-weighted primary, G-6 fwd-5d ≥ ctrl−30 bps. Weekly receipt to `docs/06-tracking/L-01-weekly-receipts.md`. **Launch-independence:** paper lane runs continuously past ACT-577 mid-Aug live date; this lever adopts post-launch on its own evidence. | armed 2026-07-25 19:57Z; live Mon 07-27 13:35Z; **rolling — no calendar deadline** | 2026-07-25 | rolling | `docs/06-tracking/charters/L-01-limit-ladder-tightening.md` (amended 2026-07-25); `supabase/functions/_shared/overshoot-execution/limit-arm.ts` + `_test.ts`; `supabase/functions/overshoot-entry-run/index.ts` L104 (SOURCE_VERSION), L~1361 (arm pick), L~1454 (audit stamp), L~1507 (lot metadata stamp); migration `ALTER overshoot_lots ADD metadata jsonb` (2026-07-25 19:53Z) |
| D-SCHEDULED | ACT-515 config matrix compute — **2026-07-29 → 2026-07-31** | Per implementation-calendar lock: matrix (a)/(b)/(c)/(d) compute runs mid-week (Wed-Fri 2026-07-29..31) once kernel modules land per-turn (types → clock → admit → …). Layer-1 fixture (`2024-05-02 hand-truth`, sha `d06bd24c`) re-verified green (INC-144 restored). Column (e) sector-cap gated on this row's sibling ACT-515(e) sector-ingest completing Turns 2-3 first. Deliverable = `scripts/act-515/verdict-table.md` populated. | 2026-07-29..31 | 2026-07-25 | 2026-07-31 | `scripts/act-515/README.md`; `scripts/act-515/config-matrix.md`; `scripts/act-515/verdict-table-template.md`; `scripts/act-515/charter-amendment-e-sector-cap.md` (column-e gate) |
| D-SCHEDULED-EVENT-TRIGGERED | ACT-572 IBKR Phase-1 wiring — **triggers on operator "IBKR paper provisioned, secrets set" message; no other gate** | Skeleton landed (sql/43 shadow tables, edge-fn skeletons at `act572-skel-v1`, `scripts/check-ibkr-shadow-separation.ts` CI guard). Phase-1 = live wiring of shadow-mirror + shadow-reconcile once operator confirms IBKR paper account provisioned and secrets configured. No date scheduled — event-gated only. §5 comparator schema per ACT-565. | operator handoff message | 2026-07-25 | event-triggered | `docs/06-tracking/ACT-572-charter-ibkr-shadow-lane.md`; `sql/43_act_572_ibkr_shadow_tables.sql`; `scripts/check-ibkr-shadow-separation.ts` |
| GOVERNANCE-NOTE | COMPLETION-FLAG PROTOCOL (**adopted 2026-07-25** — operator ruling) | Binding on all D-SCHEDULED items above and all future dated register rows: on landing, item reports one chat line `COMPLETE + artifact path + date` AND its register row flips same turn (status → CLOSED / DELIVERED / etc.). **The register is the task list; chat is the notification lane.** No item lives in memory. | permanent standing rule | 2026-07-25 | in force | this row; §0.0 preamble (Standing Rule) |
| D-SCHEDULED-HARD-DATE | ACT-577 Mid-August Live-Readiness (**charter filed 2026-07-25**) | Frames the live-launch gate. **T-live = 2026-08-17 ± operator word.** **T-broker-decision = 2026-08-10** (IBKR-shadow evidence per ACT-565 §6 grammar + ≥3wk fills ⇒ IBKR live; else Alpaca-live default, IBKR post-launch DEC). §3 small-cash: live-size + per-lot-cap are **OPERATOR-INPUT-PENDING**; PDT edge-case matrix E-1..E-7 enumerated (E-1/E-2/E-3/E-4/E-7 day-trade-risk YES with mitigations). §4 ops: 5-switch inventory, live-vs-paper config fork list, live-tagged alerts. §5 go/no-go: G-1..G-9 (all `PENDING-EVIDENCE` at filing). §6 rollback = one-line paper flip. **CLEAN-BOARD item 2/6** of the operator drive; register is the gate. | broker-decision 2026-08-10 → T-live 2026-08-17 | 2026-07-25 | 2026-08-17 | `docs/06-tracking/charters/ACT-577-mid-august-live-readiness.md`; ACT-565 §6 (broker grammar); DW-213/ACT-559 (long-only); ACT-575 (short-arm status); FIX-8/FIX-9 (completion pass); ACT-515(e) (sector cap) |

## Standing pattern (2026-07-25) — Agent-side backfill invocation

**Ruling source:** operator, DEV-29 (2026-07-25). Precedent: DEV-18 cleanup.

**Rule.** When agent-side invocation of an ingest edge function is required and native env secrets (`CRON_SECRET`, `BACKFILL_ONESHOT_SECRET`) are unreachable from the `curl_edge_functions` tool surface, the sanctioned path is: (1) add a fresh long-random in-source token to the function's auth allow-list; (2) deploy; (3) run the batches with per-batch verbatim summaries; (4) SAME-SESSION RIP the token in a follow-up diff; (5) redeploy; (6) probe-verify the ripped token now refuses. The rip receipt is part of the deliverable. Chat-pasted secrets and operator curls are **non-patterns**.

**Applies to (as of 2026-07-25):** `overshoot-minute-ingest`, `finra-shvol-ingest`, `overshoot-sector-ingest` — the three ingest functions now consistent on the header triad {Cron, Service-Role, `x-oneshot-secret`}. New ingest functions should adopt the same triad at land time.

## L-02 — CLOSED-REJECTED (2026-07-25)

Verdict: **REJECT — INSUFFICIENT-CORPUS + SUBSTRATE-GAP.** Gate G-1 (0/62 admits matched at (ticker, admit_minute) grain — SLICE-A/B anchor exit-side {09:35, 09:40, 09:45}, not entry minutes {09:36, 09:55, 11:08, 11:14} where 07-01→07-24 admits actually landed) and G-4 (n=62 vs required n≥1,000) both red. Adoption grammar precluded. Launch-independence honored: paper lane unchanged; ACT-577 mid-Aug live proceeds on current admit-minute policy. Evidence: `docs/06-tracking/L-02-cost-curve.md`.

| DW-L-02-REOPEN | L-02 re-open blockers | (1) `overshoot_minute_bars` acquires a slice covering entry minutes {09:35–09:45 ET} for admitted-ticker set (new MIG or SLICE-C ingest); AND (2) cumulative admits reach n ≥ 1,000 (~200 sessions at K=5/day → earliest natural window ~2027-Q2 unless K rises). | BLOCKED-ON-BOTH | 2026-07-25 | ~2027-Q2 earliest | `docs/06-tracking/L-02-cost-curve.md`; `docs/06-tracking/charters/L-02-entry-minute-timing.md` |

## SOURCE_VERSION rail-bump checklist — flipped to mechanical (2026-07-25 20:2XZ)

**Trigger:** CI-RED `3c698a5` — second occurrence of the "rail-bump re-pin missed sibling guards" class (FIX-8-era slip pattern; the prose-checklist mitigation failed twice). Enforcement upgraded from prose-checklist to code-structural per the enforcement-as-code precedent (#21 / #29 family).

**Pattern (binding):** every `*/index_test.ts` for an overshoot engine declares `const EXPECTED_SOURCE_VERSION = '<literal>'` at the top of the file, and every rail / drift-guard test in that file asserts against the constant via template-literal interpolation. Future `SOURCE_VERSION` bumps are a **one-line edit** and structurally cannot half-miss sibling guards. Applied 2026-07-25 to `overshoot-entry-run` (3 sites), `overshoot-exit-run` (1 site), `overshoot-detection-run` (2 sites), and `overshoot-fill-sweep` (new rail-guard site). 172/172 engine tests green after refactor. Checklist item **flipped to "single-constant pattern (mechanical)"** — no further prose reminders required.
