...
| D | ACT-571 (**2026-07-25 — BUILD-COMPLETE / ACCEPTANCE-BLOCKED-BY-DEV-9**) | overshoot universe-refresh source re-point Polygon `I:RUT` → IVV+IJH composite (reuse `parseIsharesCsv` + fetch primitives from ACT-548); sanity band `[850,950]` unchanged; provenance `source='ivv_ijh_composite'`; delete Polygon `I:RUT` from default path (probe-only retained); 7-test set GREEN; deployed build `0c5ad0d9`; manual acceptance run returned typed `both_sources_failed` (iShares CDN block from Deno edge eu-central-1 — same class as INC-123/124); fail-closed proven (905 rows unchanged); operator ruling requested Options A/B/C in evidence pack | **HARD GATE — Monday 2026-07-27 10:00:00Z** (Monday fire will fail-closed under DEV-9 unless amended) | 2026-07-24 | 2026-07-27 10:00Z | `docs/06-tracking/2026-07-25-act-571-build-and-cdn-block.md`; `docs/06-tracking/ACT-571-charter-refresh-repoint.md`; INC-140; DEV-9; INC-123/124 (block-class ancestry); ACT-548 (seed bridge) |
