...
### DW-155: Pre-live short-availability / locate source wiring (Polygon / Tradier / equivalent) — live-fire ratification blocker (DEC-068 clause (p) / ACT-328)

| Field | Value |
|-------|-------|
| **ID** | DW-155 (next-free after DW-154). |
| **Status** | open. **BLOCKING for live-fire ratification.** NOT blocking for paper v1 — paper proceeds under DEC-068 clause (p) LONG-ONLY typed-absence posture (env-flag `ALPACA_PAPER_LOCATE_AVAILABLE=false`; the `locateFetcher` is omitted from `createLiveBrokerInterfaces` and the composer routes every short candidate to the typed-absence skip path; `summary.locate_unavailable=true`; trigger surfaces `long_only_mode: true`). |
| **Tier** | A — money-path regulatory + structural constraint (Reg-SHO / borrow infrastructure). Failure to wire a real locate source before live = the short side of a dollar-neutral strategy is unexecutable on real capital = the strategy ships beta-exposed (long-only) on live, which is a return-profile regression vs the model's designed neutral-book posture. Equally critical: without a real source, structural-typed-absence and a real htb cluster remain indistinguishable in the audit envelope on live (the latent masking bug clause (p) names). |
| **Title** | Wire a real short-availability / locate data source (Polygon / Tradier / equivalent) into the §7 preflight composer's `locateFetcher?` injection slot (the STEP-C REVISION-FIX following clause (p) makes the slot optional, mirroring the `ssrStatusFetcher?` slot). On wiring, flip `ALPACA_PAPER_LOCATE_AVAILABLE` (or its live-venue equivalent) to `true` and inject the chosen source's `<source>-locate-fetcher.ts` adapter. |
| **Scope** | (a) STEP-A investigation: Polygon vs Tradier vs equivalent locate / short-availability coverage matrix (do they expose per-symbol short-availability + locate-id + qty for the rank-30-pool universe?), latency (intra-tick freshness vs verify_short_availability staleness contract), `available` semantics + partial-fill / partial-qty mapping (the §11.0.7 #4 `available && qty_available < qty_requested → failure_handled` rule must hold against the chosen source's qty semantics), `locate_id` propagation to the broker order (does the chosen source produce a locate-id the live broker accepts on order submission? — venue-specific contract); (b) author `<source>-locate-fetcher.ts` (both edge-resident under `supabase/functions/_shared/longshort-broker/` AND src-resident under `src/features/longshort/services/broker/<source>/` per the ACT-317 broker-parity discipline; the existing `alpaca-locate-fetcher.ts` is the structural template — 4xx → `available:false` fail-closed contract preserved across vendors); (c) extend `BrokerInterfaces` factory `createLiveBrokerInterfaces` to inject the new fetcher behind the env-flag (the slot is in place after STEP-C; wiring is INJECTION, not re-architecture); (d) extend `check-broker-parity.ts` + `check-broker-parity_test.ts` with a locate-scenario (parity between edge + src adapters on the chosen source); (e) verify the composer's typed-absence path no longer fires for the chosen source's covered symbol set (`summary.locate_unavailable=false` on a clean rebalance) AND a two-sided book is verified end-to-end (longs + shorts both place, both reconcile, both snapshot); (f) live-fire ratification DEC (Phase-8 / live-money DEC) cites DW-155 closure evidence explicitly. |
| **Reason for Deferral** | Alpaca paper structurally lacks `POST /v2/short_locates` (operator-confirmed 404 / code `40410000` "endpoint not found" at corr `f0b0c2e7-...`); wiring a real source now would require committing to a vendor + secrets surface + parity-test extension before the strategy has cleared paper-v1 plumbing validation. Per the §2 axiom, typed-absence is the honest representation of the structural gap until the vendor decision is operator-driven (pairs with DW-154 / DW-138 / DW-148 as the cluster of pre-live data-source gates). The composer's `locateFetcher?` slot (after STEP-C) is in place, so DW-155 closure is a wiring + STEP-A + parity-test effort, not a re-architecture. |
| **Blocking Dependencies** | (i) Operator decision on data-source vendor (Polygon vs Tradier vs alternative — driven by the STEP-A coverage + locate-id-propagation matrix); (ii) DW-138 (Alpaca / data-vendor secrets surface — the same secrets infrastructure DW-155's source registers under); (iii) STEP-C REVISION-FIX (the composer's `locateFetcher?` optionalization + env-flag wiring — sequenced immediately after this governance PR; non-blocking for DW-155 registration); (iv) DW-154 (the SSR-source twin — same vendor decision; same injection-slot pattern; pairs into a single pre-live resolution unit). NOT blocked by FP-056 paper-closure (paper closes under clause (p) LONG-ONLY posture per the architectural twin of clause (n)). |
| **Forward Binding (HARD)** | **Live-fire ratification (Phase-8 / live-money DEC) MUST NOT proceed with the short side unexecutable.** DW-155 closure (a real locate source wired + the composer's typed-absence path verified-as-no-longer-firing for the chosen source's covered symbol set + a two-sided book verified end-to-end on paper-with-real-locate-source OR on live in supervised dry-run) is a NAMED prerequisite on the live-fire ratification DEC, paired with DW-154. The live-fire DEC's "closed pre-live blockers" section MUST cite DW-155 closure evidence (ACT entry + parity-test green + composer-injection green + two-sided book verified) explicitly. |
| **Pairing with DW-154 (architectural — observation, not separate binding)** | DW-154 (SSR) and DW-155 (locate) are the SSR-typed-absence and locate-typed-absence twins on the §7 preflight composer. Same vendor-decision space (Polygon / Tradier / equivalent), same injection-slot pattern, both short-side, both pre-live blockers. At the pre-live phase they should be resolved as a **single FP / single resolution unit** (one vendor decision, one parity-test extension, one live-fire DEC closing both). Registered as two DWs because they index two distinct injection slots, NOT because they are independent work items. |
| **Future Owner Phase** | Phase 8 (live-money preparation) or a Phase-7 short-side-infrastructure FP — whichever lands first. Pairs naturally with DW-154 (SSR) + DW-138 (Alpaca / data-vendor secrets) + DW-148 (PRE-LIVE Alpaca data-tier decision) as the cluster of pre-live data-source gates. |
| **Resolution shape** | A follow-up that authors: (i) STEP-A locate/short-availability source investigation report (coverage / latency / `available` + qty semantics / locate-id propagation for Polygon vs Tradier vs alternative); (ii) the `<source>-locate-fetcher.ts` adapter pair (edge + src, parity-discipline per ACT-317); (iii) the `createLiveBrokerInterfaces` factory wiring (`locateFetcher` injection behind the env-flag); (iv) parity-test extension (`check-broker-parity.ts` + `_test.ts` locate scenario); (v) DEC-068 addendum (APPEND-ONLY) ratifying the chosen source + superseding clause (p)'s typed-absence posture for the covered symbol set; (vi) the live-fire ratification DEC citing DW-155 closure evidence + a two-sided book verified end-to-end. |
| **Cross_ref** | **DEC-068 clause (p)** (the paper-v1 LONG-ONLY typed-absence ratification this DW promises to close pre-live; the architectural twin of clause (n)); **DEC-068 clause (n)** (the SSR typed-absence twin — same pattern, paired pre-live resolution); **ACT-328** (this DW's registration action + DEC-068 clause (p) authoring); **DW-154** (the SSR-source pre-live wiring twin — same vendor decision, single pre-live resolution unit); **DW-150** (the §8.9 `ssr_violation` REJECTION-class deferral — distinct from DW-155's PRE-FLIGHT scope; they compose); **DW-138** (Alpaca / data-vendor secrets — the parallel pre-live secrets gate); **DW-148** (PRE-LIVE Alpaca data-tier decision — the parallel pre-live data-tier gate); **DW-153** (calendar-aware htb TTL — adjacent paper-evidence-gated refinement on the same htb surface); `supabase/functions/_shared/longshort-execution/preflight-composer.ts:122` (the `locateFetcher` slot — required at clause-(p) authoring; optional after STEP-C REVISION-FIX; the in-code anchor for DW-155 wiring); `supabase/functions/_shared/longshort-broker/alpaca-locate-fetcher.ts` (the structural template — 4xx → `available:false` fail-closed contract preserved across vendors per §11.0.7 #4); `src/features/longshort/services/broker/alpaca/alpaca-locate-fetcher.ts` (the src-side parity twin per ACT-317); CROSSWIND §11.0.7 #4 (the locate verifier spec DW-155's source will be verified against); **Corr `f0b0c2e7-...`** (the surfacing fire — first live `full_rebalance` exposing the structural-404); **Operator 404-probe** (same session — empirical confirmation that Alpaca paper structurally lacks `POST /v2/short_locates`); **ACT-317** (the Phase-1 broker-adapter transcription pattern DW-155 follows). |
