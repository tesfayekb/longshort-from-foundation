...
**Future-split trigger update (post-instance-#4):** Instance #3 + instance #4 are BOTH platform/tool-behavior-shaped (vs schema/column/path-shaped) — instance #3 referenced §22.8.5(b) platform commit-boundary behavior, instance #4 referenced gate-execution output behavior. The original instance-#3 trigger said: *"if a fourth instance lands AND is also §22.8.5-platform-constraint-shaped (vs schema/column/path reference), codify the narrower discipline as DW-078."* Instance #4 is platform-tool-output-shaped (gate execution), not §22.8.5-constraint-shaped specifically, but it IS within the broader "tool-behavior-state" category that instance #3 opened. With 2-of-2 most-recent recurrences in the tool-behavior-state subcategory, the trigger threshold for **DW-078 — supervisor §22.3 (l) tool-behavior-state inline-citation extension** moves from "instance #5 also of this shape" to "instance #5 anywhere in the defect-#36 family." DW-077 stays the unified discipline tracker; DW-078 (if/when authored) becomes the narrower tool-behavior-state extension covering both §22.8.5 platform constraints AND executable-tool-output preconditions. Operator decision deferred to instance #5; lineage preserved here.

**CI-FIX-01 Path 1 scope expansion (2026-05-27 / operator-ratified):** Per operator ruling, CI-FIX-01 scope expanded from the original 3 TS error fixes + 4 governance files (8 total) to additionally include (a) `scripts/check-wall-clock.ts` detector defect fix (regex-application-order via new `stripCommentsOnlyWithState` helper + `requireLiteralEmpty` per-pattern flag preserving string literals for the `new-Date-noarg` discriminator; `.test.ts` naming-convention exclusion added alongside existing `_test.ts`), (b) `ishares-constituent-fetcher.ts` + `_test.ts` Option α refactor (`findHeaderRowIndex` return type `number` → `number | null` per DEC-034 clause (2) typed-absence discipline; caller `parseISharesCsv` line 116 updated from `headerIdx < 0` to `headerIdx === null`; test line 41 updated to `idx !== null && idx > 0`). Anti-bundling honored: expansion strictly = direct prerequisites for the named CI-FIX-01 goal "verify Gates 1-9 green end-to-end" — NOT erratum addenda (GOV-ERRATA-01), NOT panel-switcher / role-UX / admin grant (DEFECT-PLATFORM-01), NOT Polygon resilience / cron / UI (FP-009). Path 2 (split into CI-FIX-02) rejected to avoid AC-32 attestation-chain fragmentation across two SHAs. Path 3 (revert) rejected to preserve the 3 verified-clean TS fixes. Cumulative diff: 10 files vs pre-execution SHA `4bdbabb`. All 9 gates verified locally CLEAN at closure: Gate 1 (audit-writer-trap) clean, Gate 2 (`deno test scripts/`) 76 passed / 0 failed, Gate 3 (longshort verifier suite) 71 passed / 0 failed, Gates 5/6/7/8/9 (sentinel / wall-clock / paper-only-url / unguarded-parseFloat / catch-returns-zero) all clean. Gate 4 (Vitest + ESLint) runs in CI build per workflow definition. **AC-32 mechanical satisfaction REALIZED at this CI-FIX-01 closure SHA** per the FP-008 closure conditional clause + FP-007 + FP-006-via-ACT-099 chain. |
