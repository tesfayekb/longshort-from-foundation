...
| **Scope — what's deferred** | **Two-part work item.** **Part A — remediate the 2 known orphan files with type errors** (named, scoped, immediate-cost-bounded): (1) `supabase/functions/longshort-reconciliation-tick/index.ts` — 1× TS2322 `SupabaseClient` generic identity mismatch between `https://esm.sh/@supabase/supabase-js@2.107.0/dist/index.d.mts` and `file:///dev-server/supabase/functions/node_modules/.deno/@supabase+supabase-js@2.107.0/node_modules/@supabase/supabase-js/dist/index.d.mts` resolved paths (root cause: dual-resolution of the same package via two different specifiers across the call chain; fix likely an import-path alignment + sibling anchor test). (2) `supabase/functions/send-signup-nudge/index.ts` — 3× TS2339 `Property 'user' does not exist on type 'User'` (likely stale `getUser()` response shape; fix: confirm whether the call wants `data.user` vs the directly-typed `User`, then correct + sibling anchor test). Both are non-trivial type-system surfaces — quarterly-style fixer-pass commit, separate from Phase 2 critical-path work. **Part B — author CI lint (future Gate 13) enforcing the orphan-entry-point ban structurally**: enumerate `supabase/functions/<name>/index.ts` files; for each, fail if no sibling `index_test.ts` or `index.test.ts` exists; allowlist exception slots only via explicit `# orphan-allowed: <reason>` sentinel in the file itself (forces a justification audit-trail when an exception is taken). Gate 13 step lives in `.github/workflows/strong-evidence.yml`; runs in parallel with Gates 1-12. **Migration step preceding Gate 13 enablement**: backfill thin anchor `_test.ts` files (single `import "./index.ts"` + `assertExists(import.meta.url)` Deno.test, per INC-30 pattern) across the remaining 44 orphans (46 minus the 2 fixed in Part A); the 44 are mostly auth/RBAC/admin/health/jobs CRUD endpoints — bulk backfill is mechanical and the anchor tests carry no behavioral risk. |
| **Blocking deps** | Phase 2 test-strategy refinement — DW-082 closure is the natural home for the next round of edge-function authoring conventions (same-PR test-file requirement, anchor-test boilerplate, exception-sentinel grammar). Specifically: any Phase 2 sub-step that (a) adds new edge functions under `supabase/functions/<name>/` (the same-PR `_test.ts` requirement MUST be in place before that work begins, not after), OR (b) authors the signal-stack edge functions (where orphan blind-spots in money paths would be a tier-A defect class) — whichever comes first. Part A (the 2 type-error fixes + anchor tests) can land independently and earlier as a small fixer commit if any Phase 2 contributor's path crosses either file; Part B (Gate 13) lands after the bulk backfill of 44 anchor tests so the gate's first run is green. |
| **Future phase** | Phase 2 test-strategy refinement (preferred — same forum as DW-081's integration-test convention codification; the two are companion work items and likely share an authoring session). Part A can land at any FP-009+ fixer-pass earlier than Phase 2 if convenient; Part B is gated on the bulk-backfill prerequisite. |
| **Resolution shape** | **Part A closure** = 2 commits (or 1 batched): `longshort-reconciliation-tick/index.ts` + sibling anchor `_test.ts` (TS2322 fixed); `send-signup-nudge/index.ts` + sibling anchor `_test.ts` (3× TS2339 fixed); `deno check` exit 0 on both; Gate 11 test count +2; INC-30 Status row updated to reflect the 2 fixes; this DW row Status updated. **Part B closure** = 3 components: (1) bulk-backfill PR — 44 anchor `_test.ts` files added across the remaining orphan entry points; (2) `scripts/check-orphan-edge-functions.ts` + `_test.ts` (Gate 13 enforcer following the same pattern as `scripts/check-eligibility-bypass.ts` from Commit 2 / Gate 12); (3) Gate 13 step added to `.github/workflows/strong-evidence.yml` running the orphan check; first CI run on the closure SHA expected green with all 53 entry points covered by Gate 11's type-check seed walk. Operator approval of the exception-sentinel grammar (`# orphan-allowed: <reason>`) at codification time selects between this and any alternative escape-hatch shape. |
| **Risk acknowledged** | (a) Until Part A lands, `longshort-reconciliation-tick` and `send-signup-nudge` carry latent type errors invisible to CI — both are operational paths (reconciliation tick is the runtime safety net for kill-switch / position-divergence detection; signup nudge is auth-adjacent), so the latent-defect risk is real but bounded (these have been on `main` through the FP-008 sequence without runtime manifestation, which suggests the type errors are diagnostic-shape rather than runtime-fatal — but that's an absence-of-evidence claim, not evidence-of-absence). (b) Until Part B lands, every new edge function added to the platform reproduces the orphan blind-spot pattern — Phase 2 contributors must manually remember to add the sibling anchor test in the same PR; the discipline addendum from DW-079 (full local pre-flight before push) applies until Gate 13 enforces structurally. (c) The 44-file bulk backfill is mechanical but introduces 44 anchor `Deno.test` invocations that each carry one `import "./index.ts"` — Gate 11 wall-time will grow by the type-check cost of the full edge-function tree (~6-15s estimated); not material at current scale but worth flagging if Phase 2 adds significantly more edge functions before Gate 13 lands. |
| **Attestation** | Forward-binding entry. Part A closure SHAs + Part B closure SHA + Gate 13 first-green CI run on the Part B closure SHA to be filled at resolution time. Sibling-shape companion to DW-081; the two are likely co-resolved at the Phase 2 test-strategy refinement step. |
