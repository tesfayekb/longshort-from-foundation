...
> **Owner:** longshort strategy module | **Phase:** Phase 2.7 / FP-043 | **Status:** fetcher + compute + orchestrator + coordinator/worker + manual handler + token-bucket pacer landed; disarmed `job_registry` row (MIG-078) + `signal_registry` planned→live flip applied. Cron wiring + enable-flip pending operator-run DEC-043 attestation.
