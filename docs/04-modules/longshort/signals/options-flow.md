...
> **Owner:** longshort strategy module | **Phase:** Phase 2.7 / FP-043 (compute) + FP-045 Phase 4 (engine) | **Status:** ✅ **LIVE-ON-QUEUE — ARMED — DEC-043 ATTESTATION CLOSED (2026-06-11).** The FP-043 fetcher + compute + classifier + within-sector z-score + token-bucket pacer are all reused VERBATIM behind the DEC-047 cursor-drain queue-worker engine. The chunked coordinator (`runOptionsFlowCoordinator`) is retired and `longshort-options-flow-worker` returns 410 Gone (handler preserved per FP-043 promise; chunk-runner module remains in tree as the canonical per-ticker semantics pin, mirrored by `options-flow-queue-adapter.ts`). `signal_registry.options_flow_imbalance_5d.status='live'` at MIG-085; `job_registry.longshort.options_flow.compute.enabled=true` at MIG-086 (operator `cron.job` jobid:87 `0 22 * * 1-5`, active=true, byte-match against registry schedule per DEC-040). **Natural-fire attestation (2026-06-10 22:00 UTC, zero human involvement):** run `af88ad2b-6d1f-413e-a63f-4d5df88913be`, `started_at=22:00:02.15668+00`, `completed_at=22:11:00.952+00`, wall `659 s`, `outcome=completed`, `universe_size=839`, `persisted_count=56`, `skip_counts={no_qualifying_flow:778, data_unavailable:5}` — mass balance `56+778+5=839 ✓`. Within-sector z (n=67 present): mean=−0.0056, std=0.8735, [min,max]=[−1.58, +3.00]. `signal_queue_runs` row `069bedac-43a5-4733-afb8-8f1e7b5a1179` `status='completed'` (11 slices, CAS-clean), zero `slice.failed`/`run.failed`/`sweeper.stale_heartbeat`. `cron.job_run_details` jobid=87 `last_status='succeeded'`. **DEC-043 end-to-end attestation CLOSED** (see `docs/08-planning/approved-decisions.md` DEC-043 closure block).
