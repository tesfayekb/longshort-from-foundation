...
> **Owner:** longshort strategy module | **Phase:** Phase 2.7 / FP-043 (compute) + FP-045 Phase 4 (engine) | **Status:** ✅ **LIVE-ON-QUEUE — DW-095 closed by FP-045 Phase 4 (2026-06-10).** The FP-043 fetcher + compute + classifier + within-sector z-score + token-bucket pacer are all reused VERBATIM behind the DEC-047 cursor-drain queue-worker engine. The chunked coordinator (`runOptionsFlowCoordinator`) is retired and `longshort-options-flow-worker` returns 410 Gone (handler preserved per FP-043 promise; chunk-runner module remains in tree as the canonical per-ticker semantics pin, mirrored by `options-flow-queue-adapter.ts`). `signal_registry.options_flow_imbalance_5d.status='live'` at MIG-085 (cadence: `daily (after-close; queue-drained ~11 min; interim per DEC-048 — §4.4.7 5-min intraday target deferred per DEC-046 v2)`); `job_registry.longshort.options_flow.compute.enabled=false` (DISARMED — combined arm-up with PEAD is the next operator step per DEC-040/043).
