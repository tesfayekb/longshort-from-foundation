...
**Status:** phase-3-complete-stop-ARMED (FP-049 / ACT-180, 2026-06-12 — MIG-092 applied: `job_registry.longshort.catalyst.compute.enabled` flipped false→true; operator-applied `cron.job` jobid 91 at `45 21 * * 1-5` UTC active=true; DEC-040 byte-match attestation **CLOSED** (`cron.job.schedule == job_registry.schedule == '45 21 * * 1-5'`, byte-identical). Manual-fire validation (run `c50a6eb3`) confirmed honest INC-75 counters on live data (articles_scanned=2231, verb_gate_drops=328, ~15% keyword pass-rate) with deterministic reproduction vs run `ce74ea97` (identical 173/58/115/0/33/118). **CRON ARMED, DEC-043-pattern attestation OPEN** — first natural cron-fire is tonight 21:45 UTC; closes on the first `signal_compute_log` row whose `completed_at` carries the wall-clock signature of 21:45 UTC (distinct from the manual-fire `as_of`-derived midnight signature). Previous: phase-3b-complete-stop (ACT-177, 2026-06-13 — MIG-091 DISARMED + registry flip + 9-entry JOB_ID_TO_SIGNAL_ID + DEC-057 §(f) addendum + DW-098 follow-up).).
