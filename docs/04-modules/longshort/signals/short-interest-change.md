...
> **Owner:** longshort strategy module | **Phase:** Phase 2.3 / FP-041 | **Status:** ✅ **ARMED.** compute + entitlement-aware fetchers + orchestrator + cron/manual handlers + `job_registry` row (MIG-076, subsequently flipped `enabled=true` by operator wire-up) + `signal_registry` planned→live flip landed. Operator `cron.job` row carries the twice-monthly schedule `'0 21 1,15 * *'`, active=true (byte-match against registry schedule per DEC-040). DEC-043 end-to-end attestation OPEN pending the next natural fire on 2026-06-15 21:00 UTC (only manual fires have landed to date: three rows in `signal_compute_log` on 2026-06-08 at 16:28 / 16:51 / 17:06 UTC, the latter two `outcome=completed, persisted_count=836/839`). **Revision-fix (FP-041 addendum):** `si_pct_float` is DERIVED in the orchestrator as `short_interest / share_class_shares_outstanding` (NOT read as a phantom field from the SI endpoint, which doesn't exist).
