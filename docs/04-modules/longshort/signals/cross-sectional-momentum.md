...
2. **Per-ticker fetch + raw momentum** — bounded-concurrency `pLimitedMap` (default `concurrency=20`) over the universe; each worker calls `PolygonPriceHistoryFetcher.fetchPriceHistory(ticker, as_of, 280)` and then `computeMomentum(bars)`. Per-ticker failures become `SignalSkip` rows (never throw) per FP-008.4 #23 pattern: Polygon 404 → `fetch_error`, other throws → `fetch_error` with `err.message`, `null` from `computeMomentum` → `insufficient_history` with bar-count detail.
...
3. `polygon-price-history-fetcher.ts` (shared) — Polygon `/v2/aggs/` with 280-day window.
