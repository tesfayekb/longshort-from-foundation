...
- **`signal_compute_log`** — MIG-065 (FP-009 Bucket C Commit C1) + MIG-071 (FP-022 / C-F4 — `skipped_detail` additive column). PK `run_id uuid`. Columns: `signal_id`, `as_of_date`, `outcome text CHECK IN ('completed','failed')`, `universe_size`, `persisted_count`, `skip_counts jsonb` (all four `SignalSkipReason` keys always present so the JSON shape is stable — the **aggregate** count-only view), `skipped_detail jsonb NOT NULL DEFAULT '[]'` (the **raw per-ticker `SignalSkip[]` array** — `{ticker, reason, detail?}` per element; coexists with `skip_counts` for diagnosability of degraded fires; empty array `[]` is the truthful clean-fire value, not a sentinel), `failure_reason text NULL`, `started_at`, `completed_at`, `operator_id`. Two indexes: `(operator_id, signal_id, as_of_date DESC)` for operator dashboards; `(signal_id, outcome)` for cross-operator monitoring. RLS identical to `signal_observations`.
