...
> **Owner:** longshort strategy module | **Phase:** Phase 2.6 / FP-044 | **Status:** ✅ **ARMED (MIG-086, 2026-06-10).** compute + Finnhub Estimate-1 fetcher pair + orchestrator + cron/manual handlers + `job_registry` row (MIG-081, flipped `enabled=true` at MIG-086) + `signal_registry` rename ('pead' → 'pead_sue_20d') + planned→live flip landed. Operator `cron.job` jobid:88 `'0 23 * * 1-5'`, active=true, byte-match against registry schedule verified per DEC-040. DEC-043 end-to-end attestation OPEN pending tonight's 23:00 UTC natural fire. Vendor lock per DEC-053 (split-vendor: FMP for Signal #1, **Finnhub Estimate-1 for Signal #2**, both retained at $144/mo within the $150 ceiling).
