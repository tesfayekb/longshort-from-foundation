...
> **Owner:** longshort strategy module | **Phase:** Phase 2.6 / FP-044 | **Status:** ✅ **ARMED — DEC-043 ATTESTATION CLOSED (2026-06-11).** compute + Finnhub Estimate-1 fetcher pair + orchestrator + cron/manual handlers + `job_registry` row (MIG-081, flipped `enabled=true` at MIG-086) + `signal_registry` rename ('pead' → 'pead_sue_20d') + planned→live flip landed. Operator `cron.job` jobid:88 `'0 23 * * 1-5'`, active=true, byte-match against registry schedule per DEC-040. **Natural-fire attestation (2026-06-10 23:00 UTC, zero human involvement):** run `5c2d015d-0107-4cdc-8e95-e310061a4aa4`, `started_at=23:00:02.468542+00`, `completed_at=23:09:01.083+00`, wall `539 s`, `outcome=completed`, `universe_size=839`, `persisted_count=835`, `skip_counts={no_recent_earnings:3, missing_sector:1, zero_dispersion:1}`. Within-sector z (n=834 present): mean=−0.0156, std=0.8865, [min,max]=[−3.00, +3.00]. `signal_queue_runs` row `5d75151b-2772-4af2-8c42-63dc14a48ed5` `status='completed'`, zero `slice.failed`/`run.failed`/`sweeper.stale_heartbeat`. `cron.job_run_details` jobid=88 `last_status='succeeded'`. **DEC-043 end-to-end attestation CLOSED** (see `docs/08-planning/approved-decisions.md` DEC-043 closure block). Vendor lock per DEC-053 (split-vendor: FMP for Signal #1, **Finnhub Estimate-1 for Signal #2**, both retained at $144/mo within the $150 ceiling).
