...
## Execution Layer (FP-056 / DEC-068 — CLOSED 2026-06-25 at ACT-327; triple-evidence ladder complete)

**Status:** **CLOSED 2026-06-25 at ACT-327 — E6-FIRE / E_evidence_3 LANDED** (closure SHA `75071e65`). [FP-056](../../08-planning/feature-proposals.md#fp-056-phase-5-paper-exec-execution-layer--sequential-submitter--two-phase-state-machine--autonomous-three-tier-resolution-longshort) is closed by a live-broker-proven phase gate: operator `spot_check` SPY 1 share filled at Alpaca paper (corr `d2d94356-cf8d-401b-be1c-11fa69c12d5b`, real `order_id=426edfe3-823d-4577-acb7-8c61dede0a9b`) drove a real `reconciliation_events` row (`event_id=1da5cac1-e57f-40e5-b746-cd1350350878`) with all 8 invariants PASS and audit envelope `triggered → completed`. **Triple-evidence ladder (DEC-068 clause g) COMPLETE:** E1 replay + E2 writer_smoke §22.5.1 (corr `e9725ae4-794e-40b5-98f0-5eaa291738a6`, ACT-326) + E3 live ground-truth (corr `d2d94356`, ACT-327). The corr-`bb3810bf` four-violation writer defect (fixed at ACT-326 via the centralized `createSupabaseReconciliationEventWriter` + DEC-068 clause (o) tier-mapping ratification) is confirmed dead by this real-broker-response round-trip. **SSR posture:** typed-absence per DEC-068 clause (n) — paper v1 carries no Reg SHO exposure; **DW-154 remains the named pre-live-ratification blocker** (NOT a paper-closure blocker). **HISTORICAL — preserved per Constitution Rule 8:** CHARTERED 2026-06-24 at ACT-305 by [DEC-068](../../decisions/DEC-068-longshort-execution-authorization.md) (longshort v1 execution authorization); governance-only at the charter commit; E1–E6 build performed across ACT-307 / ACT-310 / ACT-311 / ACT-312 / ACT-313 / ACT-314 / ACT-316 / ACT-317 / ACT-322 / ACT-326 / ACT-327.
...
**Amendment 2026-06-24 (ACT-306) — DEC-068 clause (j) added: bounded sector-aware substitution (book-construction layer).** When a top-20 name fails §7 pre-flight, the book-construction layer scans the same-side `combiner_rankings` (top-30 per side, carrying `long_score` / `short_score` + `gics_sector`) for the next candidate that is BOTH sector-legal (would not breach §7.1's 6-per-(side, sector) cap) AND pre-flight-passing; substitutes it in. Scan-cap = **rank 30** (`SUBSTITUTION_SCAN_CAP_RANK=30`, data-driven from V1 live 2026-06-23 score distribution — long rank 20→30 = −14% smooth near-linear, no cliff). Per-side daily attempts cap = **10** (`MAX_SUBSTITUTION_ATTEMPTS_PER_SIDE_PER_DAY=10`). If the bounded scan exhausts without a sector-legal + pre-flight-passing substitute, that side **operates with one fewer name for the day** — the §8.4 / §8.6.2 line 187 "book operates one fewer name" posture is preserved as the post-cascade fallback (NOT removed). **Partial supersession of CROSSWIND §8.4** (no-substitution prohibition lifted; one-fewer fallback preserved) is carried in DEC-068 clause (j).6 per Constitution Rule 8 — the spec file is NOT amended. **Clause (b) (execution-layer autonomous three-tier retry/skip) is UNCHANGED** by this amendment — clause (j) is the BOOK-CONSTRUCTION layer (runs BEFORE E1's delta computation), clause (b) is the EXECUTION layer (per-target retry/skip during order placement). The substitution layer is **PURE COMPUTE** over `(rankings, injected_preflight_results, current_positions, accepted_set_so_far)`; the §7 gate fetches happen at the BOUNDARY (orchestrator) and the results are injected into the pure layer as a `Map<symbol, PreflightResult>`, preserving E1's testability-without-credentials. **FP-056 E1 scope EXPANDED** — input becomes `combiner_rankings` top-30 per side + injected pre-flight results + current positions (NOT the fixed 40-row `longshort_target_positions`); the final selected set (post-substitution) feeds the delta computation. The unit-fork (notional at E1, shares at E2), noop tolerance, close-enumeration, and intent classification are UNAFFECTED. Evidence basis: V1 (smooth-near-linear score decay through rank ~30) + V2 (live 2026-06-23 short-side ConsDisc at 6/6 — sector-awareness non-hypothetical on day 1). See DEC-068 clauses (j).1–(j).8 for the full rule + named constants + spec-delta carry + dependency extensions.
