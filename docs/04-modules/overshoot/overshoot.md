...
## Sizing (W3.6.c LANDED — pure module; entry-time execution deferred to W3.6.e)

Sizing lives in `supabase/functions/_shared/overshoot-execution/sizing.ts` as a pure module. `computeTargetSizing({ snapshot, side, capacityPerSide, entryReferencePrice })` returns a discriminated `TargetSizingResult`: on ok it echoes `{ slotNotional = equity × sideAllocationPct / capacityPerSide, shares = FLOOR(slotNotional / entryReferencePrice), sideAllocationPct, equityBasis }`; on refusal it returns one of four typed codes — `equity_snapshot_unavailable` (passthrough when the account snapshot is refused — no phantom sizing), `capacity_non_positive`, `reference_price_non_positive`, `reference_price_exceeds_slot_notional` (FLOOR-would-be-zero, surfaced as a typed refusal rather than a silent 0-share write). Provenance constants `OVERSHOOT_SIDE_ALLOCATION_PCT_LONG = 0.25` / `OVERSHOOT_SIDE_ALLOCATION_PCT_SHORT = 0.25` — conservative first-light per I3, raised only on evidenced first-light outcomes via operator-ratified proposal.
