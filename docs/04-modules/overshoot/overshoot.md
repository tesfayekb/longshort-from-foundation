...
**I5 pre-open re-check (DEFAULT-DENY; LONG partially-evidenced ACT-488, SHORT UNTESTED-OPERATIONAL).** `evaluateI5PreOpenRecheck({ snapshot, side, tCloseRef, preEventRef, asOf })` refuses ANY selection whose pre-open reversion exceeds the side-appropriate tolerance: `OVERSHOOT_I5_REVERSION_MAX_LONG = 1.00` (ACT-488 2026-07-08 DEC; VI.I/VI.J provenance in `i5-recheck.ts` header) and `OVERSHOOT_I5_REVERSION_MAX_SHORT = 0.50` (unchanged; VI.I.D established I5 as net-protective on the SHORT arm — change requires a separate DEC). Side-directional: LONG reversion = `(tCloseRef − preOpenMid) / (tCloseRef − preEventRef)`, SHORT is symmetric. Boundary: exactly the per-side tolerance is ACCEPTED (strict `>` for refusal). Seven refusal codes: the four snapshot classes + `reference_prices_malformed` + `degenerate_overshoot_magnitude` (|tClose − preEvent| < $0.01 — guards silent 0-denominator) + `i5_reversion_exceeded` (reason string interpolates the side-appropriate threshold).
...
**Fill-sweep corrective hardening (ACT-489 / INC-90, 2026-07-08).** `overshoot-fill-sweep` is the broker-truth catch-up path for entry orders that fill after the entry-run poll window closes. It discovers submitted entry orders from `overshoot_audit_logs` where `action='overshoot.entry.submitted.entry'`, `created_at` is inside `[session_date - 14 days, session_date + 2 days)`, and `metadata->>'order_id'` is absent from `overshoot_lots.source_order_id`; it intentionally does **not** filter on `metadata->>'session_date'` because real run `3ab99ad5` submitted rows do not emit that field. The response envelope now echoes `sweep_version='inc90-fill-sweep-v2-20260708'` and `discovery_query_fingerprint='sha256:inc90-created-at-window-action-order-id-v2'` as the INC-84 bundle-content proof. A5 now distinguishes a discovery artifact from genuine post-adoption divergence: `candidates_discovered=0 && ledger_count=0 && broker_count>0` emits `overshoot.fill_sweep.discovery_shortfall`, adds a warning to the envelope, and suppresses `kill_switch_system_pause`; genuine live mismatches after discovery/adoption still emit `overshoot.fill_sweep.a5_divergence` and invoke the system pause RPC. The Deno suite includes branch tests for both A5 paths plus a contract fixture copied from the real `3ab99ad5` audit-row shape.

**Unattended cron/auth + independent-watchdog hardening (INC-97, 2026-07-10).** The fill sweep accepts the scheduled `X-Cron-Secret` path before manual JWT/RBAC authentication; the cron's anon bearer is transport authorization, not an operator session. Every live tick writes `overshoot.fill_sweep.tick`, which is the sole watchdog liveness artifact for `overshoot.fill_sweep` (an entry-run timestamp is forbidden as a proxy). The alerts dispatcher also computes broker-vs-ledger A5 independently from Alpaca positions and open `overshoot_lots`; this is deliberately outside the sweep so a sweep auth/runtime failure cannot silence reconciliation alerting. Version echoes: sweep `inc97-cycle1-v2-20260710`, dispatcher `inc97-independent-a5-v1-20260710`.

**Per-lot error isolation (ACT-468 H0, 2026-07-06).** The per-lot for-body in `overshoot-exit-run/index.ts` is wrapped in a full try/catch keyed on a `perLotStage` tag (`session_age_query` / `snapshot_fetch` / `exit_price` / `submit`). Any per-lot failure — a `fetchPolygonSnapshot` network throw, a session-age SQL error, or an unexpected exception in the exit-price / submit path — produces a TYPED per-lot outcome (`overshoot.exit.session_age_query_failed` / `overshoot.exit.snapshot_fetch_failed` / `overshoot.exit.per_lot_unexpected`), persists an audit row with the error class + correlation, increments the matching tally counter, and lets the loop **CONTINUE** to the next lot. One bad lot MUST NOT abandon the rest of the day's exits. **Boundary (explicit):** run-level failures — boot assertion, kill-switch / disarmed / probe branches, `/v2/clock`, broker positions fetch, open-lots SELECT, DB URL / Polygon key config — are NOT per-lot-wrapped; they stay run-level and abort the entire tick via the outer `apiError(500, 'exit_run_unhandled_error')`. The accounting identity extends: `positions_examined = exits_submitted + reconciliation_refusals (4) + session_age_no_fire + session_age_query_failed + snapshot_fetch_failed + per_lot_unexpected + exit_price_refusals (4) + market_closed_skips + submissions_failed`. Evidence: 24/24 tests in `overshoot-exit-run/index_test.ts` (6 new ACT-468 sentinels).
...
**AMENDMENT (ACT-481.AMEND, 2026-07-07 — supervisor-caught, ground-truth-proven; binding).** The milestone-evening walkthrough surfaced four corrections vs the AI's initial session-plan draft. All four are now BINDING against the §9 runbook below and any operator-facing walkthrough derived from it:

1.  **§9.1 Session 1 is DRY-THEN-LIVE under ONE bracket, not dry-only.** Step 4 splits into **4-i authenticated DRY-RUN** (`dry_run:true` — captures `dry_run_evidence` = deploy attestation + tier snapshot) followed by **4-ii authenticated LIVE run** `{ as_of:'<today ET>', dry_run:false }` (persists `overshoot_detection_events` + `overshoot_target_positions` rows that tomorrow's entry engine consumes). A dry-only evening feeds the entry engine NOTHING per the T2.3 persistence gates — the LIVE run is the only path that materializes selections for Session 2.
2.  **§9.2.ii Entry bracket = ratified manual bracket, NO cron arming.** The seven-step flow (fence → ARM `overshoot.entry.run` registry → I6 token-mint data-write → authenticated DRY entry run → SECOND token-mint (different hex) → LIVE entry run → DISARM → fence) is the ONLY sanctioned path this wave. Cron arming remains structurally impossible and explicitly gated: `SELECT count(*) FROM cron.job WHERE jobname LIKE 'overshoot%'` = 0 (fence-proven twice on 2026-07-07), `sql/32` and `sql/33` are authored-only NOT-EXECUTED, and **executing those schedules is the DEC-043-analog ARMING GATE explicitly blocked on hardening-wave H2**. No natural fire can occur; any operator instruction that would execute `sql/32`/`sql/33` this wave is out-of-scope and STOPs the bracket.
3.  **§9.2.i exit BEHAVIORAL proof is a CARRIED attestation at T+10 sessions after first fills.** The morning-of Session 2 exit surface is probe-scope ONLY (Alpaca probe + Polygon probe + zero-lot dry-run — §9.2.i as written). The end-to-end exit BEHAVIORAL proof (first real exit through the full pipeline) is a SEPARATE FUTURE BRACKET scheduled T+10 sessions after the first entry fills — not the same morning. Do NOT expect exit behavioral evidence in the milestone-evening session bundle.
4.  **Exit-run registry stays DISARMED throughout first light.** Nothing entering the lot table tomorrow morning will be 10 sessions old for at least 10 trading sessions; the exit engine has no work to do this wave. Its probe attestation (§9.2.i steps 1–3) is sufficient evidence of deployed-state + boot-assertion + zero-lot truthful accounting. `overshoot.exit.run` registry row remains `enabled=false` end-to-end.

These amendments override any prior chat-authored walkthrough that conflicted with them. The §9.1 / §9.2.i / §9.2.ii sub-sections below are read UNDER this amendment; where the prose predates it, the amendment governs.
...
4. **DETECTION-RUN INVOCATION.** POST `overshoot-detection-run` `{ as_of: '<today ET session date>', dry_run: false }` via §7.5 direct-localStorage script (below). Paste back the full JSON response verbatim.
    ...
    **Runbook amendment (ACT-481.AMEND, 2026-07-07) — DRY-THEN-LIVE required.** Step 4 executes as an ORDERED PAIR under the single bracket, both authenticated:

    - **4-i (DRY-RUN — evidence capture).** POST `overshoot-detection-run` `{ as_of: '<today ET session date>', dry_run: true }` first. Paste `dry_run_evidence` (deploy attestation + tier snapshot) verbatim. Confirms deploy-state + tier semantics against today's inputs WITHOUT persisting selection rows.
    - **4-ii (LIVE run — persistence).** ONLY on a coherent 4-i response, POST `overshoot-detection-run` `{ as_of: '<today ET session date>', dry_run: false }`. Persists `overshoot_detection_events` + `overshoot_target_positions` (the rows tomorrow's entry engine reads via the ratified `resolveDetectionRunForEntry` lookup). Without this LIVE run, Session 2 has NOTHING to enter — a dry-only evening feeds the entry engine zero per the T2.3 persistence gates. Paste the full LIVE response verbatim. `run_id` from 4-ii is the one carried into Session 2 readiness.

    If 4-i surfaces a defect: DISARM (step 6) + FENCE RE-PROOF (step 7) + STOP; do NOT proceed to 4-ii. If 4-ii surfaces a defect: same DISARM/FENCE/STOP; evidence to supervisor.
...
**ACT-481.AMEND (2026-07-07) — scope boundary.** Steps 1–3 below are the ONLY exit-side attestations owed in the milestone-evening / first-light session. The end-to-end exit BEHAVIORAL proof (first real exit through the full pipeline: broker positions ↔ open lots reconciliation → session-age → constructed exit limit → Alpaca LIMIT submission) is a **CARRIED ATTESTATION scheduled T+10 sessions after the first entry fills** — a SEPARATE FUTURE BRACKET, not the same morning. `overshoot.exit.run` registry row remains `enabled=false` throughout first light; nothing entering the lot table tomorrow morning will be 10 sessions old for at least ten trading sessions, so there is no work for the exit engine to do this wave.
...
**ACT-481.AMEND (2026-07-07) — NO CRON ARMING boundary (binding, verbatim).** The seven-step bracket below (fence → ARM `overshoot.entry.run` registry → I6 token-mint (data-write) → authenticated DRY entry run → second token-mint (different hex) → LIVE entry run → DISARM → fence) is the ONLY sanctioned entry path this wave. Cron arming remains structurally impossible: `SELECT count(*) FROM cron.job WHERE jobname LIKE 'overshoot%'` = 0 (fence-proven twice on 2026-07-07); `sql/32` and `sql/33` are authored-only NOT-EXECUTED; "natural fire" cannot occur. **Executing `sql/32` or `sql/33` is the DEC-043-analog ARMING GATE explicitly blocked on hardening-wave H2** and is out-of-scope this wave. Any operator instruction that would execute those schedules STOPs the bracket immediately; evidence to supervisor.
...
## §10 W4 CLOSURE — Console arc complete (2026-07-05, ACT-465.e)

**W4.e Config Panel** landed via the ratified atomicity mechanism **(a) SECURITY DEFINER RPC**. With it, the OVERSHOOT operator console W4 arc is CLOSED.

**Write-surface invariant (BINDING).** The console has **exactly one** write path into `public.overshoot_strategy_config`: `supabase.rpc('overshoot_update_strategy_config', …)` called from `OvershootConfigEditDialog.submit`. Direct browser `UPDATE` is refused by RLS + absence of authenticated GRANTs on the base table. Any future console mutation to strategy config MUST flow through this RPC (or a sibling RPC authored under the same DEFINER pattern) — no client-side multi-statement approximations, no direct table INSERT for audit shims.

**RPC contract (mirrors the three cited precedents verbatim).** `public.overshoot_update_strategy_config(p_account_key text, p_allocation_pct numeric, p_margin_multiplier numeric) RETURNS public.overshoot_strategy_config` — SECURITY DEFINER + `SET search_path = public`. Body order (BINDING): (1) `has_permission(auth.uid(), 'overshoot.manage')` gate — typed `insufficient_privilege` on deny, evaluated BEFORE bounds / UPDATE / audit; (2) parameter shape validation; (3) internal bounds re-check `strategy_allocation_pct ∈ (0, 1]` and `margin_multiplier ∈ [1.00, 2.00]` — typed `check_violation` (defense in depth beyond the table CHECKs); (4) row-locked SELECT capturing before-values (`FOR UPDATE` — no lost-update race); (5) atomic UPDATE returning after-values; (6) same-body INSERT into `overshoot_audit_logs` with action `overshoot.config.updated`, `target_type='overshoot_strategy_config'`, `target_id = p_account_key`, `metadata = {account_key, before:{strategy_allocation_pct, margin_multiplier, updated_at, updated_by}, after:{…}}`, `operator_id = auth.uid()`. `REVOKE ALL FROM PUBLIC; GRANT EXECUTE TO authenticated, service_role`. `COMMENT ON FUNCTION` cites `write_universe_eligibility_coverage`, `kill_switch_soft_pause`, `promote_combiner_model` as precedents.

**UI contract.** `OvershootConfigPanel` renders the config rows read-only for `overshoot.view`; the Edit button is wrapped in `<RequirePermission permission="overshoot.manage">` and swaps to a `view-only` badge otherwise. The edit dialog validates via a zod schema that mirrors the RPC's internal bounds check verbatim (the two bounds must stay lock-step; the exported schema is what the test file asserts against). Submit posts to the RPC; success renders the RPC-returned row (authoritative post-update snapshot) + refreshes the read cache; failure keeps the dialog OPEN with a destructive `<Alert>` carrying the RPC error message verbatim — **no optimistic UI**.
...
## §12 W3.8-T1 — Pure modules + ratified constants (LANDED 2026-07-07, ACT-478)

**Scope.** First build tranche of the ratified H0→T5 W3.8 sequence (H0 landed at ACT-468). Pure-module + constant work only; **zero engine byte-touch**. The runtime allocation lever changes at engine layer via the R-3 constant swap alone (per the "each lever is a constant swap" T3 clause of §V.A5), without any handler/detector/session-age code edit this turn.

**R-1 (per-side holding, `_shared/overshoot-execution/intents.ts`).** Added `OVERSHOOT_EXIT_TIME_HOLDING_SESSIONS_LONG = 10` (ACT-471: LONG days 6-10 ≈ 42.7 bps/day, indistinguishable from days 2-5) and `OVERSHOOT_EXIT_TIME_HOLDING_SESSIONS_SHORT = 5` HARD (ACT-472: `SHORT_T1_R10_NEGATIVE`). New `holdingSessionsForSide()` accessor. **DEPRECATED-ALIASED** uniform `OVERSHOOT_EXIT_TIME_HOLDING_SESSIONS = 5` preserved to keep session-age.ts + overshoot-exit-run/index.ts (+ sentinel tests) byte-untouched at engine layer this tranche; alias value 5 equals SHORT so short-side behavior is byte-identical, LONG session-age wiring is a T3 change. Alias removal plan pinned in-source: T3 rewires `session-age.computeSessionAge` to accept `side: OvershootSide` and read via `holdingSessionsForSide()`.

**R-3 (allocation + capacity, `_shared/overshoot-execution/sizing.ts`).** `OVERSHOOT_SIDE_ALLOCATION_PCT_LONG 0.50 → 0.90`, `_SHORT 0.50 → 0.10` per ACT-475 §V.B2 paper-tier operator directive verbatim (*"long-primary allocation — long 0.90/capacity 36, short 0.10/capacity 4 for the paper phase; pure long-only 1.00/40 PRE-AUTHORIZED as the W8 live default if paper confirms the study's short verdict"*). New named `OVERSHOOT_CAPACITY_LONG = 36`, `OVERSHOOT_CAPACITY_SHORT = 4` (handlers keep passing their own `capacityPerSide` until T3). Slot-concentration invariant: **exactly 2.5 %** of sizingBase per slot both sides (0.90/36 = 0.10/4 = 0.025) — pinned by test. Nameplate sum 1.00 pre-margin — pinned by test. W8 pure-long auto-election (1.00/40) NOT implemented this tranche; live default remains a future lever conditional on paper-window SHORT verdict.

**R-4 (regime governor, NEW `_shared/overshoot/regime.ts`).** Pure classifier `computeRegime({ spyClosesAscending })` → `{ regime: 'BULL'|'CORRECTION'|'BEAR', drawdownFromPeakPct, lastClose, peakClose, barsConsumed }`. Bands per ACT-473 Part IV: `OVERSHOOT_REGIME_CORRECTION_THRESHOLD = -0.05` (BULL floor), `OVERSHOOT_REGIME_BEAR_THRESHOLD = -0.15` (CORRECTION floor). Expanding-window peak; strict inequalities on floors. `SINGLE_BEAR_EPISODE_SAMPLE` honesty stamp pinned in module docstring (182 BEAR sessions = one 2022 rate-hike bear; N=1 for BEAR replication). Injected data only — zero DB / clock / vendor. Degenerate inputs (empty / single-bar / non-positive / NaN close) all typed-refused, never silently classified BULL. 12 tests including exact -5 % / -15 % edge boundaries.

**Runtime-behavior delta.** R-3 constant change flows through `computeTargetSizing` (called at `overshoot-entry-run/index.ts:646`) without engine byte-touch — slot notional under 100 k sizingBase moves from 12 500 (0.50/4) to 2 500 (0.90/36 or 0.10/4). Overshoot cron layer remains DISARMED (no `cron.job` rows); first-light execution is manual bracketed per §9.1, so no autonomous fires against the new sizing until operator arm. R-1 alias-at-5 means engine-layer holding behavior is byte-identical this tranche; LONG H=10 activates at T3.
