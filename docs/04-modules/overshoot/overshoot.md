...
**CI membrane:** `scripts/check-overshoot-separation.ts` + `.github/workflows/overshoot-guards.yml` enforce the separation contract at every PR: overshoot files may import ONLY the A3-allowlisted longshort leaf utilities. **W1b tightening (ACT-456)** removed `longshort-universe-interfaces.ts` from the allowlist by decoupling `HttpFetch` into an overshoot-owned redeclaration at `_shared/overshoot/http-fetch.ts` (signature-identical). Current allowlist (3 entries): `longshort-universe/shared/fetch-with-timeout.ts`, `longshort-clock.ts`, `parse-as-of-date.ts`. Longshort files may not import anything overshoot. Adding a fourth allowlist entry requires an FP-069 amendment. The workflow pins Deno to `v1.46.3` for guard reproducibility.
