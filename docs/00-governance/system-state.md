...
| longshort | phase-1-validated (FP-008 — Phase 1 universe component: constituent ingestion + §3.2 filters + §3.3 hard-exclusions + quarterly atomic refresh + continuous refresh + verify_universe_membership real + cross-check + health monitoring + replay-test integration + 4 runbooks; MIG-048-054. **2026-05-30 honest re-closure addendum:** FP-008.2 (constituent-source resolution: operator-seeded quarterly + Wikipedia automated cross-check; §3 contract changed after Polygon/iShares/FMP-free/Finnhub probes falsified affordable automated authoritative paths) + FP-008.3 (`verify_universe_membership` side-awareness contract fix; chokepoint no longer over-fires on every long lookup) landed; AC-17/AC-19/AC-26/AC-31 runtime portions evidenced via FP-011→FP-014→enrich-and-filter `df55cb4f`→D-1→Step C `reconciliation_events 7619bf86` (`universe_cross_check`, `expected_divergence_handled`)→Step A dashboard UI; ADR-007 superseded in part. NON-NEGOTIABLE eligibility-caveat contract locked: `universe_membership.{long,short}_eligible` reflects §3.2 + §3.3d only; §3.3a/b/c/e live feeds must wire AND mutate flags before any order path consumes them (Phase 2-5+). 8-item known-follow-ups catalog at closure §E. FP-006 Phase 0B foundation preserved.) | 2026-05-30 |
