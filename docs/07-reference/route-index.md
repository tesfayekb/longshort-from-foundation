...
| Field | Value |
|-------|-------|
| **Page** | `SignalsHubPage` (FP-023). Tabbed via `?tab=` (`HubTabs`): `all` (default post-FP-038 — **IMPLEMENTED at FP-038** as `AllSignalsTab`: registry-driven 10-row overview of all 9 signals + composite, with status / spec / cadence / last-fire / coverage / staleness / drift columns; live rows join last-fire from `signal_compute_log`, planned rows show "—", drift shows "Insufficient history" until N ≥ 30 distinct `as_of_date`s; read-only against `signal_registry` + `signal_compute_log`), `rankings` (**IMPLEMENTED at FP-024** as `RankingsTab`: signal/date/sector/ticker controls + **top-N selector (20/30/50)** + `SignalDistributionBand` SVG hero + top-N / bottom-N candidate tables + server-side paginated full rankings, read-only against `signal_observations`), `runs` (**IMPLEMENTED at FP-028** as `ComputeRunsTab`: signal selector + server-side paginated runs table with cron/manual freshness indicator + outcome badges + per-row expansion of `skipped_detail` for FP-022 diagnostics; read-only against `signal_compute_log` post-MIG-073/FP-027), `coverage` (**IMPLEMENTED at FP-029** as `CoverageTab`: §3.3a–e sub-rule wiring matrix per `(operator_id, as_of_date)` + derived `complete` badge + legend + PhaseContextNote citing DW-063 / DEC-038.1; server-side paginated; read-only against `universe_eligibility_coverage` — RLS already permission-scoped at MIG-055, no fix needed). |
| **Module** | longshort |
| **Classification** | authenticated, privileged |
| **Auth required** | Yes |
...
| **Scope** | system-wide |
| **Panel** | trading-panel |
| **Related tests** | `src/pages/trading/longshort/hub/HubTabs.test.tsx` (FP-023.1 — `?tab=` round-trip + default-tab fallback); `src/pages/trading/longshort/signals/__tests__/AllSignalsTab.test.tsx` (FP-038 — registry renders 10 rows + live vs planned vs composite + drift insufficient-history); `src/pages/trading/longshort/signals/__tests__/SignalDistributionBand.test.tsx` (FP-024 — top/bottom accent assignment + absent-not-plotted invariant + empty-state); `src/pages/trading/longshort/signals/__tests__/RankingsTab.test.tsx` (FP-024 — top-N / bottom-N tables + absent-count annotation + server-pagination + filter-resets-page + **FP-037 top-N selector**); `src/pages/trading/longshort/signals/__tests__/ComputeRunsTab.test.tsx` (FP-028 — phase-context note + outcome badge + freshness indicator + server-pagination + skipped_detail expansion); `src/features/longshort/hooks/__tests__/useSignalComputeRuns.test.ts` (FP-028 — `classifyFireSource` + `totalSkips` pure helpers); `src/pages/trading/longshort/signals/__tests__/CoverageTab.test.tsx` (FP-029 — phase-context note + §3.3 sub-rule columns + wired-vs-deferred badges + incomplete badge for today-truth + server-pagination); `src/features/longshort/hooks/__tests__/useEligibilityCoverage.test.ts` (FP-029 — `SUB_RULES` schema-order + `isCoverageComplete` truth-table) |
| **Related functions** | `SignalsHubPage`, `HubTabs`, `HubEmptyState`, `AllSignalsTab`, `useSignalRegistry`, `deriveStaleness`, `RankingsTab`, `SignalDistributionBand`, `ComputeRunsTab`, `CoverageTab`, `useAvailableSignals`, `useSignalDates`, `usePresentObservations`, `useAbsentCount`, `usePaginatedRankings`, `useAvailableComputeSignals`, `usePaginatedComputeRuns`, `classifyFireSource`, `totalSkips`, `usePaginatedEligibilityCoverage`, `isCoverageComplete`, `SUB_RULES` |
| **Implementation** | **IMPLEMENTED** — `<Route path="longshort/signals" element={<PermissionGate permission="longshort.view"><SignalsHubPage /></PermissionGate>}>` in `src/App.tsx`. All-Signals tab wired to `AllSignalsTab` (FP-038, default); Rankings tab wired to `RankingsTab` (FP-024); Runs tab wired to `ComputeRunsTab` (FP-028); Coverage tab wired to `CoverageTab` (FP-029). |
| **Lifecycle** | active |
| **Added by** | FP-023 (ACT-134) — registered in route-index by FP-023.1 (ACT-135). Rankings tab promoted to IMPLEMENTED by FP-024 (ACT-136). Runs tab promoted to IMPLEMENTED by FP-028 (ACT-140) post-FP-027 (ACT-139 / MIG-073) RLS unblock. Coverage tab promoted to IMPLEMENTED by FP-029 (ACT-141) — no RLS fix needed (`universe_eligibility_coverage` shipped permission-scoped at MIG-055). All-Signals tab added + promoted to IMPLEMENTED + made default by FP-038 (ACT-152) post-MIG-075 (`signal_registry` index table). |
...
### POST /longshort-combiner-forward-returns-manual — Manual Forward-Return Accrual (FP-052 3.M-iv)

| Field | Value |
|-------|-------|
| **Path** | `POST /longshort-combiner-forward-returns-manual` |
| **Classification** | privileged |
| **Owner Module** | longshort (combiner — forward-return accrual; DEC-059 evidence surface) |
| **Auth** | Bearer JWT required (`authenticateRequest`) |
| **Permission** | `longshort.manage` (`checkPermissionOrThrow`) |
| **Rate Limit** | default (DEC-023 envelope via `_shared/handler.ts`) |
| **Request Body** | `{ "as_of": "YYYY-MM-DD" }` — the RUN date (the date the orchestrator anchors maturation + Polygon lookback against; NOT a per-tuple seed). Parsed by `parseAsOfDate`; future `as_of` rejected via `productionClock` (sole sanctioned wall-clock site). |
| **Response** | `{ status: 'ok', operator_id, as_of, as_of_date, outcome: 'completed'\|'failed', tuples_considered, tuples_after_anti_join, distinct_tickers_fetched, rows_written, by_horizon: {1,5,20}, by_status: {success, polygon_404, fetch_error}, failure_reason?, correlation_id }` |
| **Audit** | Dual envelope (FP-052 / DEC-033 strategy-audit): `longshort.combiner.forward_returns.manual_triggered` BEFORE orchestrator → `longshort.combiner.forward_returns.manual_completed` / `manual_failed` AFTER. Written to `longshort_audit_logs` (NEVER the platform `audit_logs` — T4). |
