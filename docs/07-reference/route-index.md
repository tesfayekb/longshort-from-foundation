...
| **Related functions** | `SignalsHubPage`, `HubTabs`, `HubEmptyState`, `RankingsTab`, `SignalDistributionBand`, `ComputeRunsTab`, `CoverageTab`, `useAvailableSignals`, `useSignalDates`, `usePresentObservations`, `useAbsentCount`, `usePaginatedRankings`, `useAvailableComputeSignals`, `usePaginatedComputeRuns`, `classifyFireSource`, `totalSkips`, `usePaginatedEligibilityCoverage`, `isCoverageComplete`, `SUB_RULES` |
| **Implementation** | **IMPLEMENTED** — `<Route path="longshort/signals" element={<PermissionGate permission="longshort.view"><SignalsHubPage /></PermissionGate>}>` in `src/App.tsx`. Rankings tab wired to `RankingsTab` (FP-024); Runs tab wired to `ComputeRunsTab` (FP-028); Coverage tab wired to `CoverageTab` (FP-029). |
| **Lifecycle** | active |
| **Added by** | FP-023 (ACT-134) — registered in route-index by FP-023.1 (ACT-135). Rankings tab promoted to IMPLEMENTED by FP-024 (ACT-136). Runs tab promoted to IMPLEMENTED by FP-028 (ACT-140) post-FP-027 (ACT-139 / MIG-073) RLS unblock. Coverage tab promoted to IMPLEMENTED by FP-029 (ACT-141) — no RLS fix needed (`universe_eligibility_coverage` shipped permission-scoped at MIG-055). |
