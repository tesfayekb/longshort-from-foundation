...
| `LongShortDashboardPage` | `src/pages/trading/longshort/LongShortDashboardPage.tsx` | Long-Short Overview hub page (existing dashboard surface; serves the `/trading/longshort` root). FP-031: reorganized into `HubTabs` with Universe / Reconciliation tabs and `?tab=` URL-sync. | App route `/trading/longshort` |
