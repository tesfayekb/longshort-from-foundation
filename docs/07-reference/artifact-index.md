...
| **Artifact ID** | ART-047 |
| **Type** | sql-artifact |
| **Title** | `sql/33_overshoot_entry_run_cron_schedule.sql` — wires `overshoot-entry-run` to `pg_cron` at DUAL SLOTS (`35 13 * * 1-5` + `35 14 * * 1-5`) (FP-069 W3.6.e-ii) |
| **Source Path** | `sql/33_overshoot_entry_run_cron_schedule.sql` |
| **Owning Phase** | FP-069 W3.6.e-ii (authored at e-ii; operator-applied at W3.6.e-iii first-light bracket). |
| **Status** | `authored-pending-apply` — file lives in `sql/` (not `supabase/migrations/`) per MIG-031 precedent; two `cron.schedule` calls + POST-APPLY verification block are NOT executed at authoring. Arming gated on deploy (done this turn) + authenticated 2xx boot-probe + zero-lot dry-run truthful-accounting + first-real-entry end-to-end DEC-043 evidence at first-light bracket. |
| **Related Actions** | ACT-464.e-ii (authored + handler deployed + MIG-154 + MIG-155 seeded). |
| **Related Decisions** | DEC-023, DEC-033, DEC-034 (4), DEC-040, DEC-043, plus PIN-1 + PIN-2 (2026-07-04) and the LIVE-PRICE SOURCE CONTRACT (2026-07-04); DUAL-SLOT cron ratification (2026-07-05, e-i turn). |
| **Notes** | Mirrors `sql/32_overshoot_exit_run_cron_schedule.sql` verbatim shape with **TWO** `cron.schedule` calls (jobnames `overshoot-entry-run-slot-a` `'35 13 * * 1-5'` + `-slot-b` `'35 14 * * 1-5'`) pointing at the SAME handler `/overshoot-entry-run`. DUAL-SLOT semantics: winter (slot-b fires 09:35 EST; slot-a lands at 08:35 EST and refuses `market_closed`); summer (slot-a fires 09:35 EDT; slot-b lands at 10:35 EDT and returns `run_already_exists` typed no-op via the handler's session-marker idempotency gate). ONE registry row (MIG-155) — two cron identities pointing at one handler; one arming decision. Three placeholders (PROJECT_REF / YOUR_ANON_KEY / YOUR_CRON_SECRET_VALUE), no secret committed. Three-step POST-APPLY verification block covering BOTH slots: (1) `cron.job` introspection (expected 2 rows, byte-match schedules, both `active=true`, both with resolved PROJECT_REF + `X-Cron-Secret` header); (2) PROJECT_REF-literal sweep (INC-64 sentinel; expected 0 rows); (3) ARM step (`UPDATE job_registry SET enabled=true` for the single row `id='overshoot.entry.run'`). REUSES existing `CRON_SECRET`. ASCII-only self-check performed at authoring (0 non-ASCII matches). |
