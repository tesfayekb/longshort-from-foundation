...
| **Tests** | `scripts/insider-discovery-egress_test.ts` — 14 hermetic Deno tests covering: (a) master.idx parse → REST payload shape (Form 4 + 4/A surviving the post-parse filter; `rowFromEntry` parity); (b) `iterateTradingDays` skips weekends + the 2026-05-25 Memorial Day NYSE holiday; (b) backfill mode drives per-day across 3 trading days; (c1) empty-Form-4 day inserts exactly one heartbeat sentinel row; (c2) 404 unavailable day inserts heartbeat + marks `data_unavailable`; (c3) `buildHeartbeatRow` shape (sentinels + CHECK-valid `form_type='4'`); (d) SEC HTTP 403 throws `EdgarFetchError` (no Supabase insert); (d) SEC network throw bubbles as `EdgarFetchError`; (e) parseArgs daily + backfill happy paths; (e) rejects mode-mixing, missing flags, malformed dates, inverted ranges, unknown args. Fully injectable via `RunDeps`; no network, no real EDGAR, no Supabase. |
| **Single-source-of-parsing-truth invariant** | Reuses `EdgarDailyIndexFetcher` from `supabase/functions/_shared/longshort-signals/insider-transactions/edgar-daily-index-fetcher.ts` (F1 master.idx parser — UNCHANGED). The existing drift sentinels — `assertMatch(/master\.\d{8}\.idx$/)` + `assertNotMatch(/form\.\d{8}\.idx$/)` in `edgar-daily-index-fetcher_test.ts` — protect BOTH call sites. Trading-day iteration reuses `isTradingDay` + `parseIsoDate` + `isoDate` from `supabase/functions/_shared/longshort-universe/shared/trading-days.ts` (single NYSE-holiday source through 2027). |
