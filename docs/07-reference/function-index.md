...
| Field | Value |
|-------|-------|
| **Module** | longshort (FP-057 Sub-step 3 — DEC-070 clause (d) / ACT-341) |
| **Classification** | edge function — intraday combiner recompute trigger. **Tick-poll-dirty-bit** primitive (Option d, reconciled dual-investigation; supersedes the rejected event-triggered Option c). NEW primitive — NOT a sibling of the date-grain `longshort-combiner-assemble` / `-rank` crons (which stay for the once-daily slot-0 path). Composes the existing `createFeatureAssemblyOrchestrator` + `createRankerOrchestrator` factories with `{ intraday_slot: N }`. |
| **Auth** | cron-only — `verifyCronSecret` against `X-Cron-Secret` (401 on mismatch). |
| **Wall-clock discipline** | `as_of = productionClock.getWallClockTs()` SOLE wall-clock site. **Slot is DATA-DERIVED, NOT clock-derived** — `slot = COALESCE(MAX(intraday_slot),-1) + 1` over today's `combiner_rankings` (replay-deterministic; DEC-034 cl.4; crash-recovery-safe; DST/skew-safe). Daily build owns slot 0; first intraday slot = 1. |
| **`job_registry` row** | `longshort.combiner.tick` (operator-armed; MIG-127 seed `enabled=false` per disarm-fire-enable). |
| **Dirty-bit basis** | `signal_observations.computed_at` (timestamptz NOT NULL DEFAULT now(); migration `20260605113546`). FREE basis — no new column. DIRTY iff `MAX(signal_observations.computed_at) WHERE as_of_date=today` is strict-gt `MAX(combiner_rankings.computed_at) WHERE as_of_date=today` (or any signals exist and no rank exists). |
| **Skip gates (in order, BEFORE orchestrator; each emits `.skipped` with typed reason — NO write)** | (1) Global kill-switch — `__kill_switch__` row `enabled=false` → `reason='global_kill_switch_active'`. (2) Job disarmed — own `job_registry` row `enabled=false` → `reason='job_disarmed'`. (3) **Clean dirty-bit** — no new signals since the latest rank → `reason='clean_no_new_signals'` (cheap indexed MAX read; default-case of a 5-min tick). |
| **Partial-assemble guard** | The ranker's `combiner_feature_vectors` read is `eq('intraday_slot', N)` (slot-eq-filtered — see `ranker-orchestrator.ts`); the rank-side gate `assembleCompletedForAsOfDateSlot(as_of_date, N)` requires the slot-N `.completed` marker; this handler emits `assemble.completed` with `intraday_slot=N` ONLY AFTER `assembleOrch.run` returns (i.e., AFTER the chunked UPSERT lands). Together these prevent rank-on-half-written-slot — the partial-assemble race. |
| **Audit envelope** | `longshort.combiner.tick.started` BEFORE the dirty check → `.skipped` from any of the three gates → on dirty: `longshort.combiner.assemble.started/completed\|failed` (slot=N) and `longshort.combiner.rank.started/completed\|failed` (slot=N), followed by `longshort.combiner.tick.completed` / `.failed`. All carry `trigger:'tick'`. Written to `longshort_audit_logs` (T4). |
| **Failure handling** | 200 on completed / skipped / orchestrator-`failed`. 500 ONLY on dirty-bit-read failure (`cron_combiner_tick_dirty_bit_read_failed`), slot-assignment failure (`cron_combiner_tick_slot_assignment_failed`), or unexpected orchestrator throw (`cron_combiner_tick_failed`). |
| **Idempotency** | Each tick claims a UNIQUE monotonic slot; same-tick retries cannot collide with prior slots. A second clean tick is a guaranteed no-op (no write). |
| **Schedule (operator-applied — Sub-step 4)** | Target `*/5 9-16 * * 1-5` (5-min cadence, NYSE-cash window). NOT armed in Sub-step 3 — disarm-fire-enable. |
| **File** | `supabase/functions/longshort-combiner-tick/index.ts` |
| **Tests** | `index_test.ts` — 10 source-sentinel Deno tests (ALL PASS): (t1) cron-auth; (t2) `productionClock` sole wall-clock + no leaks (slot is data-derived); (t3) `JOB_REGISTRY_ID='longshort.combiner.tick'` AND does not reuse the date-grain cron ids; (t4) dirty-bit basis is `signal_observations.computed_at` with strict-gt comparator (no new column); (t5) slot is data-derived monotonic `MAX+1`; (t6) full audit-marker quartet with `trigger:'tick'`; (t7) all three skip reasons emitted BEFORE assembler construction (the BEFORE-orchestrator ordering invariant); (t8) PARTIAL-ASSEMBLE GUARD — `.completed` marker emitted textually AFTER `assembleOrch.run(...)`; (t9) slot threaded into BOTH `assembleOrch.run(as_of, {intraday_slot})` AND `rankOrch.run(as_of, {intraday_slot})`; (t10) exactly THREE `apiError(500)` sites (dirty-bit-read, slot-assignment, orchestrator throw). |
| **Added by** | FP-057 Sub-step 3 (ACT-341) — DEC-070 clauses (a–g) settled; (a) `intraday_slot` schema rotation, (b) working-order visibility, (c) ranking-freshness gate, (d) trigger model = Option (d). |
...
### FP-056 ACT-326 — Centralized `reconciliation_events` writer + `classify-submission-event` + `PLACEMENT_CALL_NAME` (the four-violation REVISION-FIX surfaced at corr `bb3810bf`; re-proven by E2 writer_smoke + E3 live ground-truth)

| Field | Value |
|---|---|
| **Module** | longshort/execution (FP-056 — DEC-068 clause (o) tier-mapping ratification). Centralizes the `public.reconciliation_events` writer used by BOTH `longshort-rebalance-submit` (placement path) AND `longshort-execute` (advance path). |
| **`reconciliation-event-writer.ts` public surface** | `createSupabaseReconciliationEventWriter(opts: { operator_id?: string, fetcher_source: FetcherSource }): ReconciliationEventWriter` — the ONE production writer for `public.reconciliation_events` (placement + advance paths). `mapKernelTierToReconciliationTier(t: 'tier1'\|'tier2'\|'tier3'): ReconciliationTier` — the DEC-068 clause (o) ratified mapping (`tier1→medium`, `tier2→strong`, `tier3→strong_plus`; `weak` intentionally unmapped on the placement path — every placement event touches money and is at least medium-consequential). Enforces the MIG-043 decomposed shape (`expected_value` / `observed_value` / `divergence` / `tolerance` — NO `payload` column) with back-compat fallback for legacy orchestrator emits; sources `engine_version` from the canonical `ENGINE_VERSION` constant; takes `fetcher_source` from the caller (live placement passes `'live'`); defaults `operator_id` to the canonical operator uuid. Propagates the underlying Postgres error per DEC-034 clause 3 (no swallow + phantom-success). |
| **`classify-submission-event.ts` public surface** | `PLACEMENT_CALL_NAME = 'longshort.rebalance.placement'` — the call_name registered for placement-path reconciliation events. `classifySubmissionEvent(r: SubmissionResult): EmittedExecutionEvent` — decomposes a `SubmissionResult` into the MIG-043 contract per Option (a) of the corr-`bb3810bf` investigation: `expected_value ← INTENT` (`{symbol, side, intent, shares?, limit_price?, selection_reason, original_rank, substituted_from_symbol, sector, computed_at}`); `observed_value ← BROKER DISPOSITION` (kind-specific union over `accepted / rejected / pending_timeout / zero_share_skipped / quote_stale_skipped / insufficient_buying_power_skipped / noop_skipped`); `divergence` = `null` for accepted + skip kinds, non-null record for `rejected` / `pending_timeout` naming which intent invariant the broker disposition contradicted (`intent_acceptance` / `intent_acceptance_within_timeout`); `tier`+`outcome` = `tier1`+`false_positive_within_tolerance` for accepted + skip kinds, `tier2`+`failure_handled` for rejected + pending_timeout. |
| **The four violations this writer prevents (the recurrence guard)** | (a) `payload` column — DOES NOT EXIST in MIG-043; the prior in-line writers inserted into it and the insert errored with the schema-cache message that surfaced at corr `bb3810bf`. (b) `tier='tier1'|'tier2'|'tier3'` — kernel vocabulary, not a valid `reconciliation_tier` enum value (the enum is `strong_plus / strong / medium / weak`). (c) `engine_version` NOT NULL was missing. (d) `fetcher_source` NOT NULL was missing. All four resolved by routing through this centralized factory; the recurrence guard is structural (the in-line writers were deleted from both edge fns). |
| **Files** | NEW: `supabase/functions/_shared/longshort-execution/reconciliation-event-writer.ts`; `supabase/functions/_shared/longshort-execution/classify-submission-event.ts`. EDITED: `supabase/functions/longshort-rebalance-submit/index.ts` + `supabase/functions/longshort-execute/index.ts` (deleted in-line writers; both consume the shared factory); `supabase/functions/_shared/longshort-reconciliation-types.ts` (`PLACEMENT_CALL_NAME` registered in `VerifyCallName`); `docs/decisions/DEC-068-longshort-execution-authorization.md` (clause (o) addendum); `docs/06-tracking/incidental-findings.md` (INC-80 — the orphaned fill at corr `bb3810bf`); `docs/06-tracking/action-tracker.md` (ACT-326). |
| **Verification** | **E2 — writer_smoke §22.5.1** (corr `e9725ae4-794e-40b5-98f0-5eaa291738a6`): two synthetic events (SMOKE-A accepted + SMOKE-R rejected) drove the centralized writer end-to-end against the real `reconciliation_events` table; 8/8 invariants PASS on both rows (kernel-tier mapping live, NOT-NULL fields populated, divergence-null invariant per kind). **E3 — live ground-truth** (corr `d2d94356-cf8d-401b-be1c-11fa69c12d5b`, ACT-327): real Alpaca SPY fill drove `event_id 1da5cac1-...` with all 8 invariants PASS — observed.order_id is the REAL Alpaca id `426edfe3-...` (not `smoke-` prefix), observed.accepted_at is the real broker timestamp; the corr-`bb3810bf` failure mode is confirmed dead. |
| **Added by** | FP-056 ACT-326 / DEC-068 clause (o) / re-proven at ACT-327 / NO migration / NO live broker call / NO grant / NO cron. |
...
### ACT-329 — DEC-068 clause (p) LONG-ONLY POSTURE: locate typed-absence at composer + env-flag gate at broker-bootstrap + named `long_only_mode` at trigger (the SSR-pattern twin for `/v2/short_locates`)

| Field | Value |
|---|---|
| **Module** | longshort/execution (FP-056 — DEC-068 clause (p) implementation; paired with clause (n)). Mirrors the SSR typed-absence pattern for the short-availability/locate source. Posture-declaration only — no gate weakening; the locate adapter's 4xx→`available:false` fail-closed remains UNCHANGED as defense-in-depth. |
| **Composer surface change (`preflight-composer.ts`)** | `PreflightComposerDeps.locateFetcher: BrokerLocateFetcher` → `locateFetcher?: BrokerLocateFetcher` (optional; mirrors clause-(n)'s `ssrStatusFetcher?`). NEW `PreflightComposerSummary.locate_unavailable: boolean` = `deps.locateFetcher === undefined`. When `locateUnavailable === true`: every SHORT candidate is recorded FAILED with `reason: 'short_availability_source_unavailable'` (NOT `failed_verifiers:['verify_short_availability']`), the per-candidate `verifiers_skipped` carries `'verify_short_availability'`, the htb-consult + broker locate loop is SHORT-CIRCUITED (broker NEVER called), and NO `reconciliation_events` row is written by the composer for the locate verifier (broker not called → no row). The **distinguishability contract** (DEC-068 clause (p)): typed-absence shape ≠ transient-reject shape — the latter still carries `failed_verifiers:['verify_short_availability']` and a `preflight_failed:` reason via the unchanged locate-PRESENT path. |
| **Broker-bootstrap surface change (`broker-bootstrap.ts`)** | `createLiveBrokerInterfaces` now reads `Deno.env.get('ALPACA_PAPER_LOCATE_AVAILABLE')` (pure env lookup — creds-free + network-free; module-load gate stays green). Default `false` on paper (Alpaca paper has no `POST /v2/short_locates`; operator-confirmed 404). When `false`, `AlpacaLocateFetcher` is NOT instantiated and `locateFetcher` is OMITTED from the returned `BrokerInterfaces` (composer takes the typed-absence path). When `true`, the real `AlpacaLocateFetcher` is injected (DW-155 closure posture). **Boot-time probe explicitly REJECTED** per clause (p) (module-load-creds-free invariant + no-wall-clock-at-construction discipline + semantic-honesty: the absence is a known venue property, not a discovered runtime fact). |
| **Trigger surface change (`longshort-rebalance-submit/index.ts`)** | Boundary narrow no longer requires `locateFetcher`. Composer call passes `locateFetcher` only when present (`...(locateFetcher ? { locateFetcher } : {})`). NEW response top-level fields + audit-completed metadata fields: `long_only_mode: boolean` (derived EXPLICITLY from `summary.locate_unavailable \|\| summary.ssr_unavailable` — clause (p) line 532 forbids heuristic detection) and `shorts_skipped_locate_unavailable: string[]` (the typed-absence short-symbol list, parallel to clause-(n)'s `shorts_placed_without_ssr_check`). On `spot_check` (long-only by design) both fields surface the posture (`long_only_mode=true`, list empty). |
| **Fail-closed invariant (UNCHANGED — binding)** | `AlpacaLocateFetcher`'s 4xx→`available:false` behavior is UNCHANGED. If the env-flag is ever misconfigured to `true` against a 404ing venue, the runtime adapter still fails closed (belt-and-suspenders). Shorting without a positive locate determination remains forbidden per §11.0.7 #4. |
| **Tests** | NEW composer tests (2 — the distinguishability twins): (1) `no locateFetcher → SHORT FAILED with reason short_availability_source_unavailable; NO failed_verifiers entry; locate NEVER called; summary.locate_unavailable=true`; (2) `locateFetcher PRESENT + locate returns available:false → SHORT fails with failed_verifiers:[verify_short_availability]; broker called; summary.locate_unavailable=false` — the two together assert the DEC-068 clause (p) DISTINGUISHABILITY INVARIANT. NEW broker-bootstrap tests (2): env-flag UNSET → `locateFetcher` omitted + construction is creds-free + network-free (no `fetchImpl` wired and factory still returns); env-flag=`'true'` → `locateFetcher` instantiated. NEW trigger tests (2): `long_only_mode=true` when `locateFetcher` omitted from broker + `shorts_skipped_locate_unavailable` enumerates short candidates + broker locate never called; both-flag-true case. Full longshort suite: **175/175 PASS** (was 160). |
| **§22.5.1 verification gate** | The `long_only_mode: true` field landing in a REAL `longshort_audit_logs` row (with `shorts_skipped_locate_unavailable` populated) is the audit-shape gate per clause (p) line 524. Satisfied by the **OPERATOR-GATED clean `full_rebalance` re-fire (post-landing)** — NOT by this PR's unit tests (which exercise the in-process audit-metadata payload but not the DB write). The §22.5.1 discipline applies (audit-shape failures cannot be mock-validated; see corr `bb3810bf` lesson). |
| **Files** | EDITED: `supabase/functions/_shared/longshort-execution/preflight-composer.ts`; `supabase/functions/_shared/longshort-execution/preflight-composer_test.ts`; `supabase/functions/_shared/longshort-execution/broker-bootstrap.ts`; `supabase/functions/_shared/longshort-execution/broker-bootstrap_test.ts`; `supabase/functions/longshort-rebalance-submit/index.ts`; `supabase/functions/longshort-rebalance-submit/index_test.ts`; `docs/07-reference/function-index.md`; `docs/06-tracking/action-tracker.md` (ACT-329). |
| **Added by** | ACT-329 / DEC-068 clause (p) / paired with DW-155. NO migration. NO live broker call. NO grant. NO cron. NO live fire (operator-gated post-landing re-fire is the §22.5.1 gate). |

### FP-069 W1a/W1b (ACT-455/ACT-456) — `overshoot-backfill-bars-manual` + `overshoot-backfill-earnings-manual`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — separate strategy; import-guarded from longshort per `scripts/check-overshoot-separation.ts`). |
| **`overshoot-backfill-bars-manual`** | Edge function that batches Polygon `/v2/aggs/ticker/{T}/range/1/day/{from}/{to}` calls via `PolygonDailyOhlcvFetcher` (`_shared/overshoot/polygon-daily-ohlcv-fetcher.ts`), writes `overshoot_bars` (PK: `ticker, trade_date`), records one `overshoot_runs` row per invocation with `source='polygon'`, and supports `{probe:true}` (single-ticker read; no writes) and resume-by-cursor via response `next_cursor`. Uses overshoot-owned `HttpFetch` type (`_shared/overshoot/http-fetch.ts`) — no longshort import. Service-role JWT; no permission gate at W1b (backfill orchestration). |
| **`overshoot-backfill-earnings-manual`** | Edge function that pages Finnhub `/calendar/earnings` (per-ticker) and FMP `/earning_calendar` (bulk range) via `FinnhubEarningsFetcher` / `FmpEarningsCalendarFetcher` (`_shared/overshoot/earnings-calendar-fetcher.ts`), writes `overshoot_earnings` (PK: `ticker, announcement_date, source`) preserving NULL hour (typed absence — Finnhub-only session flag). Honors 429 `Retry-After` with paced ≤60/min. Writes `overshoot_runs` row per invocation. |
| **Separation invariant** | Both fetchers import `HttpFetch` from `./http-fetch.ts` (overshoot-owned, signature-identical to `_shared/longshort-universe-interfaces.ts:105`). Guard `scripts/check-overshoot-separation.ts` — allowlist reduced to 3 leaf-util paths at W1b (interfaces.ts removed). |
| **Files** | `supabase/functions/overshoot-backfill-bars-manual/index.ts`; `supabase/functions/overshoot-backfill-earnings-manual/index.ts`; `supabase/functions/_shared/overshoot/{http-fetch,polygon-daily-ohlcv-fetcher,earnings-calendar-fetcher}.ts` (+ `_test.ts`). |
| **Added by** | ACT-455 (W1a build); ACT-456 (W1b corrective — decoupled HttpFetch, allowlist tightening, guard v1.46.3 pin). D-gate evidence (D-0..D-7) will amend this entry in later W1b turns. |

### FP-069 W3.3.b.i (ACT-460.b.i) — `overshoot-short-interest-compute`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — parallel-tree sibling of the longshort short-interest orchestrator; import-guarded from longshort per `scripts/check-overshoot-separation.ts`). |
| **Classification** | edge function — signal-compute (money-path-adjacent). Writes the `overshoot_short_interest` derivation cache the detector kernel will consume; upstream of every overshoot money-path decision that reads SI% / DTC. |
| **File** | `supabase/functions/overshoot-short-interest-compute/index.ts` |
| **Tests** | `supabase/functions/overshoot-short-interest-compute/index_test.ts` — 17 source-sentinel tests (DEC-023 envelope + RBAC `overshoot.manage` + wall-clock discipline + three skip gates + probe-before-gates ordering + A3 divide byte-verbatim + SI-unavailable-no-row + idempotent upsert on A6 PK + source_run_id attribution + four audit verbs incl. `.skipped` + alpaca-probe last-4-only NO-secret contract + polygon-probe NO-DB-write + POLYGON_API_KEY structured error + DEFECT-3 batch caps + membrane guards + handler-path pin + POST-only guard). |
| **Auth** | `authenticateRequest` + `checkPermissionOrThrow('overshoot.manage')`. Cron wiring (`sql/30_overshoot_short_interest_cron_schedule.sql`) is AUTHORED at b.i but NOT executed — operator arms at b.iii after DEC-043 attestation. |
| **Contract** | POST `{ as_of?: 'YYYY-MM-DD', probe?: 'alpaca'|'polygon', tickers?: string[], batch_size?: number, resume_from?: string }`. Probe modes short-circuit BEFORE the skip gates (a disarmed/kill-switched system MUST still be probeable). Alpaca probe returns `{ ok, account_last4, status, paper: true, correlation_id }` — NEVER emits `account_number` / key / secret material (last-4 only comparator; PA37Y0DBAZD5 → `'AZD5'` for overshoot #2 vs `'SVZO'` for longshort #1, INC-77). Polygon probe returns `{ ok, status, report_count?, correlation_id }` — writes NOTHING to the DB. |
| **Derivation contract (A3, byte-verbatim)** | `si_pct_float = r.short_interest / shares` — mirrors `_shared/longshort-signals/short-interest-change/short-interest-orchestrator.ts:334` verbatim. Defensive `Number.isFinite(shares) && shares > 0` re-check at the divide site (mirrors orchestrator.ts:320). CONSCIOUS APPROXIMATION: current shares-outstanding denominates historical SI counts (pinned in the fetcher, schema COMMENT, module doc, and the divide-site header). |
| **Typed-absence** | SI fetch `unavailable` → NO row written (NEVER a fabricated zero-SI row). Shares fetch `unavailable` → rows STILL written with `si_pct_float=NULL`, `dtc` carried through (typed null on denominator; NEVER a fabricated denominator). `dtc=NULL` when Polygon omits both `days_to_cover` and a positive `avg_daily_volume`. |
| **Idempotency** | `.upsert(rows, { onConflict: 'as_of_date,ticker' })` — targets A6 PK `(as_of_date, ticker)` on `overshoot_short_interest`. Re-invocation on the same batch produces zero net (as_of_date, ticker) delta; only `si_pct_float` / `dtc` / `source_run_id` / `computed_at` refresh. |
| **Run attribution** | `source_run_id = crypto.randomUUID()` stamped on every upserted row. No `overshoot_backfill_runs` INSERT (its `kind CHECK` excludes `'short_interest'` — extending it exceeds b.i scope); a formal `overshoot_signal_runs` table is tracked as a b.iii follow-up. |
| **Skip gates (3, mirroring `longshort-combiner-assemble/index.ts:32-44`)** | (1) `job_registry` id=`__kill_switch__` enabled=false → `.skipped` reason=`global_kill_switch_active`; (2) `job_registry` id=`overshoot.short_interest.compute` enabled=false → `.skipped` reason=`job_disarmed`; (3) probe modes short-circuit BEFORE (1)+(2) so credential hygiene stays observable when disarmed. |
| **Registry row (ART-043 / MIG-151)** | `enabled=false` at seed (disarm-fire-enable). Schedule `'0 21 1,15 * *'` byte-matches sql/30 cron.schedule template. |
| **Separation invariant** | Overshoot tree. Zero imports from `_shared/longshort-signals/**`, `_shared/longshort-combiner/**`, `_shared/longshort-broker/**`, `_shared/longshort-execution/**`, `_shared/longshort-universe/**`. Uses overshoot-owned fetchers under `_shared/overshoot/*` + overshoot-owned Alpaca client under `_shared/overshoot-broker/*`. |
| **Files** | `supabase/functions/overshoot-short-interest-compute/index.ts` + `_test.ts`; `supabase/migrations/20260704043127_05b95ae3-cfe0-43ef-86f0-45b2d932b0d3.sql` (MIG-151 / ART-043); `sql/30_overshoot_short_interest_cron_schedule.sql` (ART-044, authored-pending-apply). |
| **Added by** | ACT-460.b.i (FP-069 W3.3.b tranche 1/3 — code + seed + wiring artifact + local gates; no deploy, no probes invoked, no batch). |

### FP-069 W3.5.a (ACT-462.a) — `polygon-grouped-daily-fetcher.ts` + `bars-append.ts` + `forward-earnings-append.ts`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — pure library modules feeding the W3.5.b detection edge function; import-guarded from longshort per `scripts/check-overshoot-separation.ts`; 0 violations at build). |
| **Classification** | signal-compute / money-path-adjacent. Data producers for the detector kernel + orchestrator ratified at W3.4.a-c. Every refusal is typed; zero silent sentinels; zero synthetic fill. |
| **`polygon-grouped-daily-fetcher.ts`** | Decodes `GET https://api.polygon.io/v2/aggs/grouped/locale/us/market/stocks/{date}?adjusted=true` in one request. Probe evidence (2026-07-02): `resultsCount=12462`; all 14 detector benchmarks (SPY, QQQ, IWM, XLE, XLF, XLK, XLV, XLI, XLY, XLP, XLU, XLB, XLC, XLRE) present. Holiday (2026-07-04): `status:OK resultsCount:0`. Typed absence for `vwap`/`trade_count` (Polygon omits on 3 of 12462 rows); fractional volume preserved (DEFECT-1 discipline). Non-2xx (incl. 404) throws `OvershootFetchError` — grouped 404 = endpoint unreachable, NOT delisting. Binds `POLYGON_API_KEY_PROD_PROBE` per operator D2 ruling. **Naming caveat (recorded here per ACT-462.a):** the `_PROD_PROBE` suffix is legacy — this is the production-plan Polygon credential; single named key, no fallback chains. Runtime validity is proven ONLY by the W3.5.c GATE-ZERO edge-runtime probe against this endpoint. |
| **`bars-append.ts`** | Pure orchestrator: `buildBarsAppendRows({ groupedResponse, universe, sourceRunId, fetchedAsOf, requiredBenchmarks? })` → `DailyBarUpsertRow[]` shaped 1:1 to `overshoot_daily_bars` columns for direct `.upsert({ onConflict: 'ticker,trade_date' })`. Typed refusals: `BarsMissingForAsofError` (empty whitelist intersection — non-session / vendor outage; refuse whole run, never partial-write); `BenchmarksMissingError` (≥1 benchmark absent — excess-return denominator would silently drift). Exports `REQUIRED_BENCHMARKS` as the single load-bearing set. |
| **`forward-earnings-append.ts`** | Pure orchestrator + staleness predicate. `appendForwardEarnings({ fetcher, asOf, exclusionWidthDays, marginDays, sourceRunId, fetchedAsOf, capRows })` fetches `GET /stable/earnings-calendar?from=&to=` via `FmpEarningsCalendarFetcher` (bulk range, one request). Window = `[as_of + 1 calendar day, as_of + exclusionWidthDays + marginDays]`. **Operator-ratified defaults (ACT-462.a):** `exclusionWidthDays=5` sourced from detector's own named-parameter config (NOT from `overshoot_study_runs.params_jsonb` — the study sweeps grid `{0,3,5,7}`); `marginDays=2` calendar days (membrane-clean, no trading-calendar import); forward window = 7 calendar days. **Cap-breach:** `rows.length >= capRows` (default 4000) throws `EarningsCalendarCapBreachError` BEFORE any upsert; retry-with-smaller-window FORBIDDEN as silent narrowing. Probe evidence (2026-07-04): FMP `/stable/earnings-calendar` returns exactly 4000 for a 30d window (silent truncation risk); 14d peak-season = 2877 (72% headroom); 14d quiet = 971 (76% headroom); 7d forward window is comfortably below cap in all observed regimes. `isEarningsCalendarStale({ lastFetchedAt, asOf, thresholdHours? })` — pure predicate; default `thresholdHours=26` (24h cron + 2h grace; symmetric with `si_stale` cadence per D1). |
| **Refusal taxonomy** | `bars_missing_for_asof` (session gap), `benchmarks_missing` (denominator drift), `earnings_calendar_cap_breach` (silent-truncation guard), `earnings_calendar_stale` (predicate; wired at W3.5.b). |
| **Tests** | `polygon-grouped-daily-fetcher_test.ts` (7 cases: apiKey required; happy path incl. fractional volume; typed absence vwap/trade_count; malformed-row drop; holiday resultsCount=0 no-throw; 404 throws; 401 throws). `bars-append_test.ts` (8 cases: happy path + deterministic sort + attribution; typed absence preserved; empty-response refusal; irrelevant-tickers refusal; missing-SPY refusal; case-insensitive match; empty sourceRunId rejected; benchmark override). `forward-earnings-append_test.ts` (11 cases: window arithmetic verbatim URL check; happy path with attribution; cap-breach at exactly 4000; cap-breach above 4000; parameter validation; staleness null/within/at-boundary/beyond/override/negative). |
| **Membrane** | Overshoot tree only. Imports: `./http-fetch.ts` (overshoot-owned); `../longshort-universe/shared/fetch-with-timeout.ts` (A3 allowlist); `./polygon-daily-ohlcv-fetcher.ts` (`OvershootFetchError` re-use); `./earnings-calendar-fetcher.ts` (`FmpEarningsCalendarFetcher` re-use — bulk-range decoder generalises verbatim). Zero longshort-signals / longshort-combiner / longshort-broker / longshort-execution imports. Separation guard: 0 violations. |
| **DB touch (deferred)** | None this tranche — pure modules. W3.5.b edge function performs the actual `.upsert()` into `overshoot_daily_bars` and `overshoot_earnings_calendar` (both have `source_run_id` FK to `overshoot_backfill_runs(run_id)`; FK reconciliation — reuse vs schema follow-up — is a W3.5.b concern). Zero detector/kernel/fixture edits. |
| **Files** | `supabase/functions/_shared/overshoot/polygon-grouped-daily-fetcher.ts` (+ `_test.ts`); `supabase/functions/_shared/overshoot/bars-append.ts` (+ `_test.ts`); `supabase/functions/_shared/overshoot/forward-earnings-append.ts` (+ `_test.ts`). |
| **Added by** | ACT-462.a (FP-069 W3.5 tranche 1/3 — code + tests + docs; NO edge function, NO cron, NO deploy, NO detector edits). Runtime key validity (D2) proven only at W3.5.c GATE-ZERO. |

### FP-069 W3.5.b (ACT-462.b) — `overshoot-detection-run`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — the EOD detection-run edge function consuming the W3.5.a append legs + the W3.4 detector kernel; import-guarded from longshort per `scripts/check-overshoot-separation.ts`). |
| **Classification** | edge function — signal-compute / money-path setup. Reads production commons + writes `overshoot_events` + `overshoot_target_positions`; upstream of every overshoot broker decision at W3.6+. |
| **File** | `supabase/functions/overshoot-detection-run/index.ts` |
| **Tests** | `supabase/functions/overshoot-detection-run/index_test.ts` — 14 source-sentinel tests (DEC-023 envelope + productionClock ban on Date.now() in kernel/orchestrator surface + boot-assertion ordering + probe short-circuit ordering before skip gates + RBAC deny before pipeline + all four typed-refusal → outcome mappings + `dry_run` zero-persist gating + `append_run_ids` shape + kernel live-parameterization pattern + detector-import purity + A4 persistence column lists match schema + append-leg ordering (backfill_runs row inserted BEFORE the upsert, both legs) + earnings-calendar staleness predicate wire-up + `POLYGON_API_KEY_PROD_PROBE` single-key binding with zero fallback). |
| **Auth** | `authenticateRequest` + `checkPermissionOrThrow('overshoot.manage')`. Cron wiring (`sql/31_overshoot_detection_run_cron_schedule.sql`) is AUTHORED at b-ii but NOT executed — operator arms at W3.5.c after GATE-ZERO + dry-run + first-real-detection attestation. |
| **Contract** | POST `{ as_of?: 'YYYY-MM-DD', probe?: boolean, dry_run?: boolean, correlation_id?: string }`. `probe` short-circuits BEFORE the two skip gates so credential hygiene stays observable when disarmed. `dry_run` runs the FULL pipeline (all seven stages) but gates the `overshoot_events` + `overshoot_target_positions` inserts on `!dryRun` — the detection_runs row is always written with `dry_run: true` stamped into `durations_ms`. |
| **Pipeline (7 stages, verbatim order)** | (1) method + JSON parse → (2) RBAC `overshoot.manage` → (3) **boot assertion** against `overshoot_study_runs` verifying `RATIFIED_STUDY_RUN_ID` full-UUID equality + `RATIFIED_PARAM_GRID_HASH_PREFIX` LIKE + `outcome='completed'` (fail-fast typed `boot_assertion_failed_priors_not_found` 500) → (4) probe short-circuit → (5) two skip gates (kill-switch on strategy_key='overshoot'; job-disarmed on `overshoot.detection.run.enabled=false`) → (6) pipeline: **bars-append leg** (`overshoot_backfill_runs` row `kind='bars'` inserted BEFORE upsert to satisfy the `source_run_id` FK; run_id captured into `append_run_ids.bars`) → **forward-earnings-append leg** (`kind='earnings_fmp'` row inserted BEFORE upsert; run_id captured into `append_run_ids.earnings`; leg wrapped in try/catch mapping each typed refusal to its outcome/reason without swallowing) → `isEarningsCalendarStale` predicate (26h threshold — 24h cron + 2h grace) → kernel live-parameterization (`EVENT_DETECTION_SQL` byte-identical to study runner, bound via `stripStatementBody()` + `bindNamed()` + `DETECTION_PARAM_ORDER` — runner-parity gate) → SI read within 20d staleness window → `runDetector` (pure, unmodified from W3.4) → (7) persistence: `overshoot_events` (all rows) + `overshoot_target_positions` (only `!dryRun`; PK `(run_id, ticker, side)` DO NOTHING) + `overshoot_detection_runs` finalize (`outcome`, `event_count`, `selected_count`, `durations_ms`, `append_run_ids`). |
| **Refusal taxonomy → outcome mapping** | `BarsMissingForAsofError` → `outcome=no_op, reason=bars_missing_for_asof`; `BenchmarksMissingError` → `outcome=failed, reason=benchmarks_missing`; `EarningsCalendarCapBreachError` → `outcome=failed, reason=earnings_calendar_cap_breach`; `isEarningsCalendarStale=true` → `outcome=no_op, reason=earnings_calendar_stale`. All four refusal classes reach the detection_runs row (zero swallowed errors); the run row is inserted at the point of refusal with the `append_run_ids` captured to that point. |
| **Append-attribution design (MIG-152)** | `overshoot_detection_runs.append_run_ids jsonb NULL` (MIG-152) with reserved shape `{"bars": <uuid>, "earnings": <uuid \| null>}` — NEVER stashed in `durations_ms` (lying-name class). Detection-run row inserted with `append_run_ids=null` at boot; each leg captures its `overshoot_backfill_runs.run_id` and `finalizeRun()` writes the final shape. |
| **Boot-assertion shared surface (closes W3.4.c deferral)** | Imports `RATIFIED_STUDY_RUN_ID` + `RATIFIED_PARAM_GRID_HASH_PREFIX` + `assertStudyProvenance` + `StudyProvenanceAttestation` from `_shared/overshoot/detector/detector.ts` (single home, no new constants, zero detector edits). Same-tree contract as the pure detector surface listed above (row for `_shared/overshoot/detector/*`, ACT-462.a-adjacent) — this is the FIRST edge-function consumer of those exports. Convention: shared surfaces are registered in the row of the module that authors them (detector.ts) and cross-referenced from every consumer's row (this row) — mirrors the `REQUIRED_BENCHMARKS` cross-reference between `bars-append.ts` (author) and the detection function (consumer). |
| **Kernel live-parameterization (runner-parity bind)** | Reuses `EVENT_DETECTION_SQL` byte-identical to the study runner source-of-truth via the same `stripStatementBody()` + `bindNamed()` + `DETECTION_PARAM_ORDER` pattern as `overshoot-study-run/index.ts:126-134`. `event_date_min=event_date_max=asOfDay` bind produces exactly one candidate slice at the detection as_of. Study-cell lookup keyed to `RATIFIED_STUDY_RUN_ID`. |
| **A4 column-alignment attestation (persistence-code time)** | Verified verbatim vs W3.1 migration (`20260704023836_…sql`): `overshoot_events` INSERT column list matches schema L42-65 (event_id via DB default); `overshoot_target_positions` INSERT column list matches schema L146-156 including PK; `overshoot_detection_runs` INSERT column list matches schema L8-21 + MIG-152 `append_run_ids` column. No mismatch surfaced. |
| **Secrets** | `POLYGON_API_KEY_PROD_PROBE` (D2 ruling — production-plan Polygon credential despite the `_PROBE` suffix; single named key, no fallback chains; runtime validity proven ONLY by W3.5.c GATE-ZERO edge-runtime probe against the grouped endpoint), `FMP_API_KEY`, service-role JWT for DB writes. |
| **Separation invariant** | Overshoot tree. Zero imports from `_shared/longshort-signals/**`, `_shared/longshort-combiner/**`, `_shared/longshort-broker/**`, `_shared/longshort-execution/**`, `_shared/longshort-universe/**` (except the A3-allowlisted `fetch-with-timeout.ts` leaf transitively via the W3.5.a fetchers). Guard `scripts/check-overshoot-separation.ts` — 0 violations. |
| **Files** | `supabase/functions/overshoot-detection-run/index.ts` + `_test.ts`; `supabase/migrations/20260704083003_738694d1-d16f-4867-8b83-a5c714c835b1.sql` (MIG-152 — registry seed + `append_run_ids` column); `sql/31_overshoot_detection_run_cron_schedule.sql` (ART-045, authored-pending-apply). |
| **Added by** | ACT-462.b-i (function + tests) + ACT-462.b-ii (sql/31 + reference index + docs). Runtime key validity + deploy + dry-run + first-real-detection evidence land at W3.5.c. |

### FP-069 W3.6.d-ii (ACT-463.d-ii) — `overshoot-exit-run`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — T+5 time-stop exit-cron + operator-manual liquidation path; consumer of the three W3.6.d-i pure modules; import-guarded from longshort). |
| **Classification** | edge function — money-path (order-submitting). Reads broker truth (Alpaca `/v2/clock`, `/v2/positions`) + Polygon quote snapshots + `overshoot_lots` (status='open'); writes `overshoot_audit_logs` on every named refusal or submission; submits Alpaca `/v2/orders` LIMIT day-TIF on matched lots via `OvershootAlpacaOrderSubmitter`. |
| **File** | `supabase/functions/overshoot-exit-run/index.ts` |
| **Tests** | `supabase/functions/overshoot-exit-run/index_test.ts` — 18 source-sentinel tests (DEC-023 envelope; `productionClock` + `Date.now()` present exactly once as the I6 manual-window cutoff, non-money non-decision; boot-assertion + drift-canary `void`-references to the three W3.6.d-i exported constants BEFORE probe short-circuit BEFORE two skip gates BEFORE I6 gate BEFORE pipeline; I6 second-confirm gate with 15-min window + `metadata->>'confirm_token'` match + 428 refusal; every named refusal writes to `overshoot_audit_logs` — reconciliation `.<class>` × 4, exit-price `.<class>` × 4, `submit_failed`, `submitted.<intent>`; accounting identity fields present in response envelope; dry-run guard precedes `submitOrder`; CID + intent contract `exit_time` on cron / `exit_manual` on manual with `attempt: 0` run-scoped; LIMIT + day-TIF; PIN-2 clock consumer + `market_closed` refusal + `minutes_to_close`; kill-switch + disarmed gates; `POLYGON_API_KEY_PROD_PROBE` single-key zero-fallback; **separation-guard** zero `data.alpaca.markets` / `/v2/stocks/` / `useDataUrl=true` consumers per LIVE-PRICE SOURCE CONTRACT; all four A5 reconciliation refusal classes surfaced in tally; all four exit-price refusal classes surfaced in tally; PIN-1 SPY prior-session SQL with earliest-lot `MIN(entry_ts)::date` anchor; probe taxonomy `alpaca` \| `polygon` else 400). |
| **Auth** | `authenticateRequest` + `checkPermissionOrThrow('overshoot.manage')`. Cron wiring (`sql/32_overshoot_exit_run_cron_schedule.sql`) is AUTHORED but NOT executed — operator arms at W3.6.d-arm after boot-probe + zero-lot dry-run attestation. |
| **Contract** | POST `{ as_of?: 'YYYY-MM-DD', probe?: 'alpaca'\|'polygon', dry_run?: boolean, manual_confirm?: boolean, second_confirm_token?: string }`. Probe short-circuits BEFORE the three skip gates. `dry_run=true` runs the full pipeline with zero broker submissions. `manual_confirm=true` triggers the I6 second-confirm gate. |
| **Pipeline (verbatim order)** | (1) method + JSON parse → (2) RBAC `overshoot.manage` → (3) **boot assertion** (`RATIFIED_STUDY_RUN_ID` row equality + `RATIFIED_PARAM_GRID_HASH_PREFIX` LIKE + `outcome='completed'` fail-fast `boot_assertion_failed_priors_not_found` 500; PLUS `void` drift-canary references to `OVERSHOOT_EXIT_TIME_HOLDING_SESSIONS` / `OVERSHOOT_EXIT_MARKETABLE_LIMIT_SLIPPAGE_BPS` / `OVERSHOOT_EXIT_SNAPSHOT_MAX_AGE_MS` so a d-i rename surfaces at edge module-load) → (4) probe short-circuit (`alpaca` probes `/v2/account`; `polygon` round-trips SPY snapshot) → (5) skip gates: kill-switch on `strategy_key='overshoot'` + job-disarmed on `overshoot.exit.run.enabled=false` → (6) **I6 second-confirm gate** (manual path only): requires `second_confirm_token` matching a recent `overshoot.exit.manual_triggered` audit row within `OVERSHOOT_MANUAL_CONFIRM_WINDOW_MS = 15 * 60 * 1000` for the same actor; else 428 typed refusal `manual_confirm_token_missing_or_invalid` → (7) **pipeline**: Alpaca `/v2/clock` (PIN-2 seam; `market_closed` typed refusal + `minutes_to_close`) → `OvershootAlpacaPositionFetcher.listOpenPositions` → open lots → `reconcileOpenPositions` (all four A5 refusal classes persisted to `overshoot_audit_logs`, never skipped) → per matched (symbol,side): cron path `computeSessionAge` (earliest-lot `MIN(entry_ts)::date` anchors T+5; PIN-1 semantics via injected `clock`); manual path bypasses session-age → Polygon `/v2/snapshot/locale/us/markets/stocks/tickers/{sym}` quote → `constructExitLimitPrice` (four typed refusals each audit-written) → `OvershootAlpacaOrderSubmitter` LIMIT day-TIF with `exit_time` / `exit_manual` CID from `buildOvershootClientOrderId({runId, ticker, side, intent, attempt:0})` (attempt run-scoped; next cron tick = fresh `run_id`). |
| **Refusal taxonomy → audit action** | Reconciliation: `overshoot.exit.reconciliation_refusal.<lot_without_broker_position\|unknown_broker_position\|side_mismatch\|qty_mismatch>`. Exit-price: `overshoot.exit.price_refusal.<polygon_snapshot_unavailable\|_stale\|_malformed\|_crossed>`. Submission: `overshoot.exit.submit_failed`. Success: `overshoot.exit.submitted.<intent>`. Every branch writes exactly one audit row — never-silent-drop. |
| **Accounting identity (response envelope)** | `positions_examined = broker_positions.length + open_lots.length` reported alongside `matched_count` + `exits_submitted` + full `refusals` tally (4 reconciliation + 4 exit-price + `session_age_no_fire` + `submissions_failed`) + `minutes_to_close` + `run_id` + `intent` + `dry_run` + `manual` + `correlation_id`. Observability contract: for any run, `positions_examined` MUST equal the sum of `exits_submitted` + every counter in `refusals` + `matched_count - exits_submitted - session_age_no_fire - Σ(exit_price refusals) - submissions_failed`. |
| **d-i modules byte-untouched (headline gate)** | `git diff --stat HEAD -- supabase/functions/_shared/overshoot-execution/{session-age,exit-price-construction,position-reconciliation}.ts supabase/functions/_shared/overshoot/detector/ supabase/functions/overshoot-detection-run/` → empty diff-stat. HEADLINE GATE HELD. |
| **Boot-assertion shared surface (consumer)** | Imports `RATIFIED_STUDY_RUN_ID` + `RATIFIED_PARAM_GRID_HASH_PREFIX` from `_shared/overshoot/detector/detector.ts` (single-home, no new constants, zero detector edits). Cross-reference of the same shared surface authored in the detector row and consumed by both `overshoot-detection-run` (ACT-462.b-i) and this row. |
| **Boot-assertion additional import** | `OVERSHOOT_EXIT_TIME_HOLDING_SESSIONS` (session-age), `OVERSHOOT_EXIT_MARKETABLE_LIMIT_SLIPPAGE_BPS` + `OVERSHOOT_EXIT_SNAPSHOT_MAX_AGE_MS` (exit-price-construction) — statically imported + `void`-referenced to survive tree-shaking; any rename lands as a compile-time break at deploy, not at first fire (drift canary). |
| **Secrets** | `POLYGON_API_KEY_PROD_PROBE` (D2 ruling; single named key, zero fallback), `ALPACA_PAPER_KEY_OVERSHOOT` + `ALPACA_PAPER_SECRET_OVERSHOOT` (account #2 for broker truth), service-role JWT for DB reads/writes. |
| **Separation invariant** | Overshoot tree. Zero imports from `_shared/longshort-*/**`. Guard `scripts/check-overshoot-separation.ts` — 0 violations. Standing LIVE-PRICE SOURCE CONTRACT enforced by test-suite grep: zero `data.alpaca.markets` / `/v2/stocks/` / `useDataUrl=true` consumers in this file. |
| **Files** | `supabase/functions/overshoot-exit-run/index.ts` + `_test.ts`; `supabase/migrations/<MIG-153>` (registry seed for `overshoot.exit.run` enabled=`false`, 17-col MIG-152 shape byte-identical); `sql/32_overshoot_exit_run_cron_schedule.sql` (ART-046, authored-pending-apply). |
| **Added by** | ACT-463.d-ii (function + tests + MIG-153 + sql/32 + deploy). Authenticated 2xx probe + zero-lot dry-run truthful-accounting + first-real-exit end-to-end evidence land at W3.6.d-arm (post-W3.6.e). |

### FP-069 W3.6.e-ii (ACT-464.e-ii) — `overshoot-entry-run`

| Field | Value |
|---|---|
| **Module** | overshoot (FP-069 — pre-open ENTRY cron + operator-manual entry path; consumer of the four W3.6.e-i pure modules + sizing + `overshoot_strategy_config` + broker fetchers; import-guarded from longshort). |
| **Classification** | edge function — money-path (order-submitting). Reads broker truth (Alpaca `/v2/clock`, `/v2/account`, `/v2/assets/{sym}`, `/v2/orders/{id}`) + Polygon quote snapshots + `overshoot_events` + `overshoot_strategy_config` + `overshoot_daily_bars`; writes `overshoot_audit_logs` on every named refusal / submission / session-marker + UPSERTs `overshoot_target_positions` (INC-83) + INSERTs `overshoot_lots` on filled_qty > 0; submits Alpaca `/v2/orders` LIMIT day-TIF via `OvershootAlpacaOrderSubmitter`. |
| **File** | `supabase/functions/overshoot-entry-run/index.ts` |
| **Tests** | `supabase/functions/overshoot-entry-run/index_test.ts` — 26 source-sentinel tests (DEC-023 envelope; `productionClock` + `Date.now()` present exactly once as the I6 manual-window cutoff; boot-assertion + FIVE e-i drift-canary `void`-refs BEFORE probe BEFORE two skip gates BEFORE I6 BEFORE pipeline; `run_already_exists` DUAL-SLOT idempotency gate; INC-83 UPSERT `overwrites-on-commit` + `sentinel-persists-on-I5-refuse` (ordering-asserted); R-γ cumulative BP guard; shortability gate shorts-only; entry-price 4-class tally; CID + `entry` intent contract; LIMIT + day-TIF; PIN-2 clock + `market_closed` refusal + `minutes_to_close`; lots-on-fill (broker-truth cost basis; partial-fill leaves lot at filled qty); `POLYGON_API_KEY_PROD_PROBE` zero-fallback; **separation-guard** zero `data.alpaca.markets` / `/v2/stocks/` / `useDataUrl=true` consumers per LIVE-PRICE SOURCE CONTRACT; single ratified account key `overshoot-paper-primary`). |
| **Auth** | `authenticateRequest` + `checkPermissionOrThrow('overshoot.manage')`. Cron wiring (`sql/33_overshoot_entry_run_cron_schedule.sql`) is AUTHORED but NOT executed — operator arms at W3.6.e-iii first-light bracket. |
| **Contract** | POST `{ as_of?: 'YYYY-MM-DD', probe?: 'alpaca'\|'polygon', dry_run?: boolean, manual_confirm?: boolean, second_confirm_token?: string, slot?: 'a'\|'b' }`. Probe short-circuits BEFORE the three skip gates. `dry_run=true` runs the full pipeline with zero broker submissions. `slot` is diagnostic only — the handler routes both slots to the same pipeline; `run_already_exists` collapses the second slot to a typed no-op. |
| **Pipeline (verbatim order)** | (1) method + JSON parse → (2) RBAC `overshoot.manage` → (3) **boot assertion** (`RATIFIED_STUDY_RUN_ID` fail-fast `boot_assertion_failed_priors_not_found` 500; PLUS `void` drift-canary refs to `OVERSHOOT_SIDE_ALLOCATION_PCT_LONG` / `_SHORT` / `OVERSHOOT_ENTRY_MARKETABLE_LIMIT_SLIPPAGE_BPS` / `OVERSHOOT_ENTRY_SNAPSHOT_MAX_AGE_MS` / `OVERSHOOT_I5_REVERSION_MAX_LONG` / `OVERSHOOT_I5_REVERSION_MAX_SHORT` [ACT-488] so an e-i rename surfaces at edge module-load) → (4) probe short-circuit (`alpaca` probes `/v2/account`; `polygon` round-trips SPY snapshot) → (5) skip gates: kill-switch on `strategy_key='overshoot'` + job-disarmed on `overshoot.entry.run.enabled=false` → (6) **I6 second-confirm gate** (manual only; recent `overshoot.entry.manual_triggered` audit row within `OVERSHOOT_MANUAL_CONFIRM_WINDOW_MS = 15 * 60 * 1000`; else 428 `manual_confirm_token_missing_or_invalid`) → (7) **pipeline**: Alpaca `/v2/clock` (PIN-2 seam; `market_closed` typed refusal + `minutes_to_close`) → **`run_already_exists` idempotency gate** (cron only; same-session-date `overshoot.entry.session_marker` audit row → typed no-op; DUAL-SLOT DST collapse) → `resolveDetectionRunForEntry` (three ratified typed refusals audited via `overshoot.entry.detection_linkage_refusal.<code>`) → strategy config read (typed `strategy_config_absent` + audit) → `OvershootAlpacaAccountFetcher.fetchAccountSnapshot` (typed `equity_snapshot_unavailable` passthrough + audit) → session marker write → per selected target from `overshoot_events WHERE run_id=<detection.runId> AND selected_for_entry=true`: `evaluateI5PreOpenRecheck` (DEFAULT-DENY; on refuse audit + `continue` BEFORE UPSERT — INC-83 sentinel persists) → `computeTargetSizing` (`sizingBase = equity × strategy_allocation_pct × margin_multiplier`; per-side capacity = selected count) → `assertBuyingPowerCoversNotional` (R-γ; cumulative `intendedNotional` check BEFORE each submission) → `OvershootAlpacaShortabilityFetcher.fetchShortability` (shorts only; typed `not_shortable`) → Polygon `/v2/snapshot` quote → `constructEntryLimitPrice` (four typed refusals) → **INC-83 UPSERT** `INSERT INTO overshoot_target_positions ... ON CONFLICT (run_id, ticker, side) DO UPDATE SET target_shares, target_notional, computed_at` (overwrites-on-commit proof) → `OvershootAlpacaOrderSubmitter` LIMIT + day-TIF with `entry` CID (`buildOvershootClientOrderId({runId, ticker, side, intent:'entry', attempt:0})`; Alpaca side `buy` for LONG / `sell` for SHORT; CID `S1` + audit `orderSide_semantic:'sell_short'` carry semantic) → `OvershootAlpacaFillFetcher.fetchFill` → INSERT `overshoot_lots (symbol, entry_ts, qty=filled_qty, cost_basis=avg_fill_price*filled_qty, side, status='open')` on `filled_qty > 0` (partial-fill: lot at filled qty, order remains tracked). |
| **Refusal taxonomy → audit action** | `overshoot.entry.detection_linkage_refusal.<detection_run_missing_for_prior_session\|_stale\|_not_completed\|no_prior_spy_session\|malformed_session_date>`; `overshoot.entry.strategy_config_absent`; `overshoot.entry.equity_snapshot_unavailable`; `overshoot.entry.session_marker` (idempotency anchor); `overshoot.entry.i5_refusal.<code>`; `overshoot.entry.sizing_refusal.<code>`; `overshoot.entry.buying_power_refusal.insufficient_buying_power`; `overshoot.entry.shortability_refusal.not_shortable`; `overshoot.entry.price_refusal.<polygon_snapshot_unavailable\|_stale\|_malformed\|_crossed>`; `overshoot.entry.submit_failed`; `overshoot.entry.submitted.entry`. Every branch writes exactly one audit row — never-silent-drop. |
| **Accounting identity (response envelope)** | `targets_loaded = orders_submitted + Σ (i5_refusals, sizing_refusals, buying_power_refusals, shortability_refusals, entry_price 4-class, submissions_failed) + fill_unfilled_no_lots`. Response also reports `run_id`, `detection_run_id`, `prior_spy_session`, `session_date`, `sizingBase`, `strategy_allocation_pct`, `margin_multiplier`, `minutes_to_close`, `slot`, full `submissions[]` with `filled_qty` + `avg_fill_price` + `lot_ids`, and `correlation_id`. |
| **INC-83 resolution** | STRUCTURALLY RESOLVED this file. Two proof obligations enforced by source-sentinel tests: (a) `INC-83 overwrites-on-commit proof: UPSERT with ON CONFLICT DO UPDATE`; (b) `I5 default-deny with INC-83 sentinel-persists-on-I5-refuse proof` (ordering `idxI5refuse < idxUpsert` + `inc83_sentinel_persists: true` audit metadata). |
| **e-i modules + detector + exit-run byte-untouched (headline gate)** | `git diff --stat -- supabase/functions/_shared/overshoot/ _shared/overshoot-execution/ overshoot-detection-run/ overshoot-exit-run/` → empty diff-stat. HEADLINE GATE HELD. |
| **Boot-assertion shared surface (consumer)** | Imports `RATIFIED_STUDY_RUN_ID` + `RATIFIED_PARAM_GRID_HASH_PREFIX` from `_shared/overshoot/detector/detector.ts` (same shared surface as `overshoot-detection-run` and `overshoot-exit-run`). |
| **Boot-assertion additional imports** | Five e-i constants (sizing R-α x2 + entry-price 2 + i5 tolerance) — statically imported + `void`-referenced (drift canary). |
| **Secrets** | `POLYGON_API_KEY_PROD_PROBE` (single named key, zero fallback), `ALPACA_PAPER_KEY_OVERSHOOT` + `ALPACA_PAPER_SECRET_OVERSHOOT` (account #2), service-role JWT. |
...
| **File** | `supabase/functions/mcp/index.ts` — **OWNED as of ACT-476** (gate-0B corrective). The `AUTO-GENERATED` banner was deleted, disabling the `@lovable.dev/mcp-js@0.20.0` Vite-plugin overwrite. Tool surface FROZEN at exactly `app_info` + `echo` per H-SEC-5; regeneration is incompatible with a frozen, lint-gated surface. Hand-edits are limited to lint-parity changes (e.g. `var` → `const`) with byte-equivalent behavior; any tool addition/removal requires the H-SEC-5 FP + OAuth 2.1 path and reversion to plugin ownership. Sources of record remain `src/lib/mcp/index.ts` + `src/lib/mcp/tools/{app-info,echo}.ts`. |
...
| **Added by** | Operator-initiated via Lovable MCP feature (commits `82b372fb..513a6528`, reclassified per §22.5.4). Ratified ACT-475 (H-SEC-5 line + this row). Ownership transferred ACT-476 (gate-0B corrective, HEAD `f7027df3`). |
