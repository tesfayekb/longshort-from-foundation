...
| **Exports** | `const FEATURE_ORDER: readonly string[]` (16-key LOCKED contract: 2 criticals catalog-order, then 7 `(value, is_present)` pairs catalog-order — the 3.3b-ii trainer binds to this); `const FEATURE_VECTOR_LENGTH = 16`; `const NON_CRITICAL_MISSING_SENTINEL = -999`; `class LgbmTreeDumpParseError`; `interface LgbmTree` + `interface LgbmEnsemble`; `function parseLgbmTreeDump(text): LgbmEnsemble` (accepts only numerical `<=` splits — `decision_type=2`); `function featuresToOrderedArray(features): Float64Array` (applies §6.5.2 sentinel substitution: `is_present=0` → value=-999 regardless of stored `__value`); `function scoreLgbm(ensemble, features16): number` (sums `shrinkage × leaf_value` per tree + bias; raw lambdarank output). |
