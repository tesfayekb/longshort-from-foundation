...
### MIG-113: ACT-276 / DW-131 second-stage corrective — canonical PostgREST service-role claim read in DW-119 RBAC helper guard

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-22 against project ref `sftatlxatbdrotivxcip`. File: `supabase/migrations/20260622085933_9909be6d-8b60-4c4c-8a7a-823c8956938f.sql`. |
| Applied | 2026-06-22; live-DB post-apply `pg_get_functiondef` shows the three guarded RBAC helpers now use `COALESCE(NULLIF(current_setting('request.jwt.claim.role', true), ''), NULLIF(current_setting('request.jwt.claims', true), '')::jsonb ->> 'role') = 'service_role'`. `pg_proc.proacl` still shows `{postgres, service_role, authenticated}=X/postgres` for all four RBAC helpers. Supabase linter result unchanged: the same 11 by-design WARN-0029 entries remain for authenticated-callable SECURITY DEFINER functions; no new finding class introduced. |
| Pattern | `CREATE OR REPLACE FUNCTION` for the three DW-119-guarded helpers only: `has_permission(uuid,text)`, `is_superadmin(uuid)`, `has_role(uuid,text)`. Function semantics unchanged except the service-role exemption now reads the canonical PostgREST `request.jwt.claims` JSON role claim while retaining the legacy scalar setting as a backward-compatible first arm. Same-migration `GRANT EXECUTE` block repeated for all four canonical helpers per DW-131 binding rule. NO edge-function code change, NO table/RLS/view change, NO grant beyond `{authenticated, service_role}`, NO dropped overload re-introduction. |
| Purpose | MIG-112 restored the missing EXECUTE grants, but the admin console still returned uniform 403 because the DW-119 service-role predicate read only `current_setting('request.jwt.claim.role', true)`. In the deployed PostgREST RPC context, the service-role client exposes the role through `request.jwt.claims` JSON; `auth.uid()` is NULL for the service-role RPC and the scalar setting is empty, so the guard returned `false` before evaluating the target user's real permissions. MIG-113 corrects the binding source while preserving the DW-119 arbitrary-`_user_id` leak closure: allowed callers remain self, service_role, or a real superadmin caller. |
| Dependency | MIG-109 / DW-119 introduced the guard with the scalar-only service-role predicate; MIG-112 restored helper reachability after the recreate dropped EXECUTE grants. MIG-113 is the forward delta that makes the restored wire evaluate the intended service-role exemption in the actual PostgREST claim shape. |
| Sub-step authority | ACT-276 (runtime-regression corrective under supervisor/runtime-error report). Scope: database helper predicate only + canonical source/doc updates. |
| AC evidence | (a) User-reported runtime/network evidence after MIG-112 still showed 403 on `list-users`, `list-roles`, `list-permissions`, `query-audit-logs`, and `get-user-stats`. (b) Live `pg_proc.proacl` before MIG-113 confirmed grants were already present, ruling out the MIG-112 grant layer as the remaining root cause. (c) Live `pg_get_functiondef` before MIG-113 showed scalar-only `current_setting('request.jwt.claim.role', true) = 'service_role'` in all three guarded helpers. (d) Live DB role mapping for the caller showed `superadmin` + `user`, and the direct mapping query confirmed superadmin logical inheritance should authorize the calls once the service-role RPC can reach the target-user evaluation branch. (e) Post-MIG-113 helper definitions carry the canonical JSON claim read and same-migration grants. |
| Capability-gap surface (§22.8.5) | MIG-109's verification was false-green for the deployed PostgREST service-role path because it simulated `request.jwt.claim.role` rather than the actual `request.jwt.claims` JSON source. Forward verifier guidance: service-role helper tests must set/read the same claim shape PostgREST uses, or exercise the real edge-function RPC path; scalar-claim simulation alone is insufficient. |
| Cross-references | ACT-276 (this corrective); DW-131 (same regression family); MIG-109 (guard introduction); MIG-112 (grant restoration); `sql/02_rbac_security_helpers.sql` (canonical helper source updated with the JSON-claim predicate); `supabase/functions/_shared/authorization.ts` `checkPermissionOrThrow`; admin edge functions `get-user-stats` / `list-users` / `list-roles` / `list-permissions` / `query-audit-logs`. Rollback (NOT RECOMMENDED — re-opens uniform admin 403 on service-role RPC): recreate the three helpers with scalar-only `current_setting('request.jwt.claim.role', true) = 'service_role'`. |

### MIG-114: ACT-279 — `signal_decay_returns` + `signal_decay_log` (per-signal close-to-next-open alpha decay instrument — measurement-only)

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-22 against project ref `sftatlxatbdrotivxcip`. |
| Applied | 2026-06-22; live-DB §22.5.1 verification post-apply via `supabase--read_query` on `pg_class.relrowsecurity`, `pg_policies`, `pg_constraint`, and `has_table_privilege` for `{anon, authenticated, service_role}`. Supabase linter delta: ZERO new finding classes; the 11 reported WARN-0029 entries are the pre-existing DW-119 / MIG-109 / MIG-111 / MIG-112 SECURITY DEFINER auth-helper baseline retained by design. |
| Pattern | Two-table create (`signal_decay_returns` + `signal_decay_log`), four-step structure for each per the public-schema grants rule: (1) CREATE TABLE, (2) `GRANT SELECT, INSERT, UPDATE, DELETE TO authenticated; GRANT ALL TO service_role` (no `anon` grant — every policy scopes via `has_permission(auth.uid(),'longshort.view')`), (3) ENABLE ROW LEVEL SECURITY, (4) CREATE POLICY ×4 each (read gated by `longshort.view`; INSERT/UPDATE/DELETE deny-write for `authenticated` — sibling pattern of `combiner_forward_returns`). Plus two BTREE indexes on `signal_decay_returns` (`(operator_id, signal_id, seed_as_of_date)` and `(operator_id, price_source_status, computed_at DESC)`) and one on `signal_decay_log` (`(operator_id, as_of_date DESC, completed_at DESC)`). |
| Purpose | Measurement-only evidence plumbing for DEC-048 (cadence decision at Phase 7), the fast-signal overnight-weighting question, and the Phase 4/5 exit-threshold design. Captures per-signal close-to-next-open price decay (`open_decay_return = next_open / seed_close - 1`) keyed back to the prior EOD `signal_observations` row via `(operator_id, signal_id, seed_as_of_date, ticker, horizon_label='next_open')`. Nothing consumes these tables; the instrument banks evidence rows on the cron's daily fire (`35 13 * * 1-5`, registered OOB via `sql/22`). |
| Schema discipline | `signal_decay_returns_price_source_status_check` admits 7 values: `success`, `unreconciled_single_source`, `polygon_404`, `fetch_error`, `halted_at_open`, `universe_dropped`, `hard_excluded_since_seed`. `success` is RESERVED for the cross-source reconcile (DW-135); the Phase-1 accruer NEVER stamps it. `signal_decay_returns_typed_absence_chk` enforces: data-bearing statuses ⇒ `seed_close`, `next_open`, `open_decay_return`, `seed_close_date`, `next_open_date` ALL NOT NULL; non-data-bearing statuses ⇒ `next_open` AND `open_decay_return` NULL. NEVER `0` / `-999` as a sentinel (anti-phantom defaults). |
| Idempotency | UPSERT `onConflict='operator_id,signal_id,seed_as_of_date,ticker,horizon_label'` with `success`-status anti-join in the orchestrator's pre-fetch step. Typed-absence AND `unreconciled_single_source` rows remain re-fetchable: the former so a newly-settled open promotes them off `fetch_error`; the latter so a future cross-source reconcile (DW-135) can promote them in place to `success` without losing audit history. |
| Dependency | Sibling: `combiner_forward_returns` (FP-052 / MIG-094) — schema shape, GRANT/RLS pattern, typed-absence CHECK, and `success`-only anti-join discipline all mirrored verbatim. Caller: `supabase/functions/longshort-signal-decay-capture/index.ts` (new edge fn, ACT-279). Cron: `sql/22_longshort_signal_decay_cron_schedule.sql` (operator-applied OOB; collision-free at `35 13 * * 1-5`; hard-ASCII per DW-133). |
| Sub-step authority | ACT-279 (this commit). Supervisor-authorized scope: measurement-only instrument; pure-additive; nothing consumes the output. |
| AC evidence | (a) Pre-MIG-114 `read_query`: neither table exists. (b) Post-MIG-114 `read_query` (`pg_class`): both tables present with `relrowsecurity=true`. (c) `pg_policies`: 4 rows per table (read gated by `longshort.view`; INSERT/UPDATE/DELETE deny `WITH CHECK false` / `USING false`). (d) `pg_constraint` for `signal_decay_returns`: 5 rows — PK + `horizon_label_check` + `price_source_check` + `price_source_status_check` + `typed_absence_chk`. (e) `has_table_privilege` ×3 roles: `anon`={F,F,F,F}, `authenticated`={T,T,T,T} (writes blocked by RLS deny policies — same as sibling `combiner_forward_returns`), `service_role`={T,T,T,T}. (f) Consumer leak sweep `rg 'signal_decay_returns\|signal_decay_log'`: 0 hits outside `_shared/longshort-signals/decay/` + `longshort-signal-decay-capture/` + `sql/22_*`. (g) Gate 6 (`scripts/check-wall-clock.ts`): CLEAN — 0 violations. (h) ASCII gate on `sql/22`: 0 non-ASCII bytes. (i) Unit tests: 14/14 PASS in `signal-decay-accruer_test.ts` (incl. `dec-14` anti-phantom-confidence — no row EVER stamps `success` in Phase-1). |
| Capability-gap surface (§22.8.5) | (1) Halt detection is structurally `false` in Phase-1 because no production `BrokerHaltStatusFetcher` exists. The schema admits `halted_at_open` and the accruer handles it; the orchestrator simply cannot positively set the flag yet. Wiring deferred to DW-135 (it is the right co-vehicle since the Tradier daily-bar fetcher + halt-feed wiring share the same boundary surface). (2) The `success` status is reserved for cross-source reconcile (DW-135). Phase-7 evidence consumers MUST apply a confidence-discount to `unreconciled_single_source` until DW-135 closes. (3) Cron registration via `sql/22` is operator-applied OOB; the §22.5.1 freshness gate (Step 4) is the next-fire evidence and is intentionally NOT pre-asserted in this MIG entry. |
| Cross-references | ACT-279 (this commit); DW-135 (cross-source reconcile follow-up that promotes `unreconciled_single_source` → `success`); DEC-048 (the locked cadence decision this evidence feeds); DEC-034 clause 4 + FP-047 (injected-clock discipline mirrored verbatim — `productionClock.getWallClockTs()` read ONLY at handler top-of-call-chain); DW-133 (hard-ASCII gate on cron-body); `_shared/longshort-combiner/forward-return-{accruer, orchestrator, constants}.ts` (canonical sibling); `sql/19_longshort_combiner_shadow_cron_schedule.sql` (canonical cron-template sibling); MIG-094 (the `combiner_forward_returns` migration whose four-step shape this MIG mirrors). Rollback (NOT RECOMMENDED — discards the evidence plumbing the Phase-7 cadence decision depends on): `DROP TABLE public.signal_decay_returns; DROP TABLE public.signal_decay_log;` (no FK references — safe to drop in isolation). |

### MIG-115: ACT-283 — FP-052.3 (3.3a) promotion-gate foundation: `combiner_forward_returns.horizon_td CHECK` widened to admit T+10 (§6.1/§6.2 LOCK) + atomic `promote_combiner_model` / `rollback_combiner_model` `SECURITY DEFINER` `service_role`-only RPCs (single-active-per-side via retire-first ordering)

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-23 against project ref `sftatlxatbdrotivxcip`. |
| Applied | 2026-06-23; live-DB §22.5.1 verification post-apply via `supabase--read_query` on `pg_constraint.conname='combiner_forward_returns_horizon_td_check'` (definition = `CHECK ((horizon_td = ANY (ARRAY[1, 5, 10, 20])))`) + `pg_proc` (both RPCs present with identity args `(uuid)` and `(text)`) + `pg_proc.proacl` (`promote_combiner_model` and `rollback_combiner_model` both = `postgres=X/postgres,service_role=X/postgres` only — authenticated/anon/PUBLIC EXECUTE revoked). Supabase linter delta: ZERO new finding classes; the 11 reported WARN-0029 entries are the pre-existing DW-119 / MIG-109 / MIG-111 / MIG-112 / MIG-113 SECURITY DEFINER auth-helper baseline retained by design (verbatim mirror of MIG-114 sourcing). |
| Pattern | Three-part additive migration in one transaction: (1) `ALTER TABLE … DROP CONSTRAINT IF EXISTS combiner_forward_returns_horizon_td_check; ALTER TABLE … ADD CONSTRAINT … CHECK (horizon_td IN (1, 5, 10, 20))` — strict superset of the prior `(1, 5, 20)` constraint; every prior-valid row remains valid; zero data migration. (2) `CREATE OR REPLACE FUNCTION public.promote_combiner_model(p_model_id uuid) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public` — locks candidate `FOR UPDATE`; raises `insufficient_privilege` for non-service-role callers via the canonical PostgREST scalar+JSON claim read (mirrors MIG-113 / DW-131); raises `invalid_transaction_state` if `status <> 'candidate'`; UPDATE prior active → `retired` (stamping `retired_at`) FIRST; UPDATE candidate → `active` (stamping `promoted_at`) SECOND. Ordering is LOAD-BEARING — the `uq_combiner_model_registry_active_per_side` PARTIAL UNIQUE index forbids any transient two-active state. Returns `jsonb { success, model_id, side, promoted_at, prev_active_id, prev_active_retired_at }`. (3) `CREATE OR REPLACE FUNCTION public.rollback_combiner_model(p_side text) RETURNS jsonb` — same shape; retires current active first, restores most-recently-retired prior model second (selected via `ORDER BY retired_at DESC NULLS LAST, updated_at DESC LIMIT 1`); raises `no_data_found` if no active or no prior-retired exists. Both RPCs: `REVOKE EXECUTE FROM PUBLIC, anon, authenticated; GRANT EXECUTE TO service_role` (so they do NOT contribute to WARN-0029). |
| Purpose | Lands the promotion-gate foundation for FP-052.3 (3.3a). The widened CHECK admits the §6.1/§6.2-locked T+10 training-label horizon so the 3.M forward-returns fetcher (paired-edit at `supabase/functions/_shared/longshort-combiner/forward-return-constants.ts`) can begin accruing T+10 rows from this date forward; the RPCs are the atomic mutation surface a future 3.3b promotion edge function will call once a candidate model exists and clears the FP-052.3 criteria evaluator. Per DEC-063 the v1 promotion gate carves SHAP/C6 out (re-gated at DW-136 landing per DEC-063 Clause 3) and otherwise preserves the §10.7 + §6.5.4 exit gates verbatim. |
| Schema discipline | Widened constraint is a strict superset (additive). No table/column added; no RLS policy added (the existing `combiner_forward_returns` policies are unchanged); no enum added. The two RPCs are `SECURITY DEFINER` `SET search_path = public` (DW-120 pattern) and gate via the canonical PostgREST service-role claim read (DW-131 pattern), so they do not introduce a new auth-helper pattern. Single-active-per-side invariant remains enforced by `uq_combiner_model_registry_active_per_side` (pre-existing partial unique index from MIG-099); the RPCs' retire-first ordering is the mechanism that keeps the invariant intact at every transaction-intermediate state — the index is the safety net that rejects ordering bugs. |
| Idempotency | (a) The horizon-CHECK widen is `DROP CONSTRAINT IF EXISTS` then `ADD CONSTRAINT` — safe to re-apply (re-application no-ops if the constraint already admits 10, modulo a brief lock to re-check). (b) The two RPCs use `CREATE OR REPLACE FUNCTION` — safe to re-apply. (c) Calling `promote_combiner_model` on an already-`active` row raises `invalid_transaction_state`; calling it on a `retired` row raises the same. Calling `rollback_combiner_model` when no prior-retired exists raises `no_data_found`. There is no silent no-op path; every state transition is explicit. |
| Dependency | Sibling: `combiner_model_registry` (MIG-099 — the registry the RPCs mutate; the `uq_combiner_model_registry_active_per_side` partial unique index is the load-bearing invariant). `combiner_forward_returns` (MIG-100 — the table whose CHECK is widened). Caller (future, 3.3b): the LightGBM-LambdaRank training pipeline's promotion edge function (separate prompt). Caller (future, 3.3c): the DW-136 SHAP re-gate execution per DEC-063 Clause 3 (rollback on verifier failure). Code paired-edit: `supabase/functions/_shared/longshort-combiner/forward-return-{constants,orchestrator}.ts` (`HORIZONS_TD = [1,5,10,20]`; `MATURATION_FLOOR_CAL_DAYS[10] = 10`; `emptyByHorizon` derived from `HORIZONS_TD` — no hardcoded keys; T+1/5/20 logic byte-unchanged). |
| Sub-step authority | FP-052.3 sub-step 3.3a; ACT-283 (this commit). Supervisor-authorized scope: one additive migration; no LightGBM, no training, no SHAP write, no cron arming, no model promotion fire. |
| AC evidence | (a) Pre-MIG-115 `read_query`: horizon CHECK = `CHECK ((horizon_td = ANY (ARRAY[1, 5, 20])))`; neither RPC exists. (b) Post-MIG-115 `read_query`: horizon CHECK = `CHECK ((horizon_td = ANY (ARRAY[1, 5, 10, 20])))`; both RPCs present with identity args `(uuid)` and `(text)`; `proacl` rows for both = `postgres=X/postgres,service_role=X/postgres` ONLY (no authenticated grant — confirms `REVOKE EXECUTE FROM authenticated` succeeded). (c) Supabase linter delta: ZERO new WARN classes; 11 WARN-0029 entries match the pre-existing baseline (the two new RPCs do NOT add to WARN-0029 because EXECUTE is revoked from `authenticated`). (d) `combiner_model_registry WHERE status='active'` remains 0; nothing was promoted (no model exists). (e) Gate-3.0 exit-gate queries continue to return 0/0 (the foundation closure is preserved). (f) Pure-evaluator Deno tests pass: empty-input → all `not_computable`; SHAP/C6 carve-out invariant; `TRAINING_HORIZON_TD === 10`; C10 25-vs-3 boundary; C9 label-density boundary; current-substrate snapshot in `{not_computable, not_yet}`; C1 perfect-rank → `pass`. (g) Forward-returns fetcher edit is paired with the schema (HORIZONS_TD admits 10 only AFTER MIG-115; orchestrator `emptyByHorizon` derives from HORIZONS_TD so no hardcoded keys lag the constant). |
| Capability-gap surface (§22.8.5) | (1) The two RPCs are landed but have no caller at 3.3a — first caller is the 3.3b promotion edge function (separate prompt). Service-role gating means the caller MUST authenticate as service-role (no superadmin shortcut). (2) T+10 labels accrue with a 10-RTH-day lag from MIG-115 landing; pre-MIG-115 seeds will never have a T+10 row (the PK includes `horizon_td=10`, so structural absence, not silent fill). (3) The promotion gate's substrate-evaluable criteria (C1/C2/C9/C10/C12) return `not_computable`/`not_yet` against current substrate by construction — this is the correct v1 state per the FP-052.3 acceptance criteria, not a defect. (4) Per DEC-063, SHAP/C6 is carved out of the v1 promotion gate; operator-dashboard "SHAP coverage: DEFERRED" indicator REQUIRED at first promotion (lands at 3.3b). (5) Threshold values in the evaluator are CANDIDATE — production calibration requires ≥20 RTH-days of post-MIG-115 labeled substrate. |
| Cross-references | ACT-283 (this commit); DEC-063 (the load-bearing scope governance: SHAP/C6 carve-out + atomic RPC contract + horizon widen); DEC-062 (DW-100 inversion below FP-052.3 — the precursor that surfaced the dependency direction); FP-052.3 entry in `feature-proposals.md`; DW-136 (the re-gate trigger per DEC-063 Clause 3 — cross-referenced both ways); MIG-099 (`combiner_model_registry` create + `uq_combiner_model_registry_active_per_side` partial unique index — the invariant the RPCs honor); MIG-100 (`combiner_forward_returns` create — the table whose CHECK this MIG widens); MIG-114 (sibling additive MIG whose linter-baseline sourcing language this MIG mirrors verbatim); CROSSWIND §6.1 / §6.2 (T+10 LOCK); CROSSWIND §10.7 line 322 (the exit gate DEC-063 temporally relocates); ADR-008a (sentinel-introduction site for 3.3b). Rollback (NOT RECOMMENDED — re-opens both forks): `ALTER TABLE public.combiner_forward_returns DROP CONSTRAINT combiner_forward_returns_horizon_td_check; ALTER TABLE … ADD CONSTRAINT … CHECK (horizon_td IN (1,5,20)); DROP FUNCTION public.promote_combiner_model(uuid); DROP FUNCTION public.rollback_combiner_model(text);` (safe: no FK references; the dropped constraint pre-existed at MIG-100). |

### MIG-116: ACT-287 — FP-052.3 (3.3b-ii-A) artifact-store consumption surface: `storage.objects` RLS scoped to `combiner-models/` bucket (service_role write / `longshort.view` read per DEC-065 Clause 4) + `purge_retired_combiner_artifacts()` `SECURITY DEFINER` `service_role`-only function (12-week retention purge per DEC-065 Clause 3) + `pg_cron` daily 04:15 UTC schedule `longshort.combiner.artifact_retention_purge`

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-23 against project ref `sftatlxatbdrotivxcip`. |
| Applied | 2026-06-23; cron schedule returned `jobid=105`. The `combiner-models/` Storage bucket itself is OPERATOR-PROVISIONED (Dashboard / Storage CLI; 3.3b-ii-B scope) — until the operator creates it, the RLS policies bind to zero rows and the purge function processes zero models (forward-compatible inert state, by design). |
| Pattern | Three-part additive migration in one transaction: (1) two `CREATE POLICY` statements on `storage.objects` scoped to `bucket_id = 'combiner-models'` — `FOR ALL TO service_role` (trainer + purge writes) and `FOR SELECT TO authenticated USING (has_permission(auth.uid(), 'longshort.view'))` (operator-dashboard diagnostic reads); both prefixed with `DROP POLICY IF EXISTS` for idempotency. NO anon access (bucket is private per DEC-065 Clause 4). (2) `CREATE OR REPLACE FUNCTION public.purge_retired_combiner_artifacts() RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public` — iterates `combiner_model_registry WHERE status='retired' AND retired_at < now() - interval '12 weeks'`; for each, `DELETE FROM storage.objects WHERE bucket_id='combiner-models' AND name LIKE '{model_id}/%'` (Supabase storage stack handles backend object cleanup via its existing triggers); emits one `combiner.artifact_purged` audit event per non-empty purge; returns `jsonb { success, cutoff, models_purged, ran_at }`. In-function role gate raises `insufficient_privilege` on any caller that is not `service_role` / `postgres` / `supabase_admin`. `REVOKE EXECUTE FROM PUBLIC, anon, authenticated; GRANT EXECUTE TO service_role` (so the function does NOT contribute to WARN-0029). (3) `SELECT cron.unschedule('longshort.combiner.artifact_retention_purge') WHERE EXISTS …; SELECT cron.schedule('longshort.combiner.artifact_retention_purge', '15 4 * * *', $$SELECT public.purge_retired_combiner_artifacts();$$)` — daily at 04:15 UTC, off the combiner compute / signal cron hours. |
| Purpose | Lands the in-substrate consumption surface for the `combiner-models/` Supabase Storage bucket the FP-052.3 3.3b-ii-B Python trainer will write to. Pairs with `model-artifact-loader.ts` (this same commit) which is the read-side consumer (real `LoadModelArtifact` impl + DEC-064 Clause 4 `feature_order_hash` refusal). Together they close the consumption side of the substrate Architecture-A substitution (DEC-064 + DEC-065 per ACT-286) WITHOUT building the trainer or provisioning the bucket — both of which are operator-GHA scope (3.3b-ii-B). |
| Schema discipline | No public-schema table created; no column added; no enum added. The two `storage.objects` policies are additive (`DROP POLICY IF EXISTS` makes re-apply safe); the purge function is `CREATE OR REPLACE`; the cron schedule uses `unschedule WHERE EXISTS` then `schedule` (idempotent re-apply). `search_path = public` (DW-120 pattern). Service-role-only function gate via the canonical PostgREST claim read + `current_user` defensive fallback (covers `pg_cron` execution context). |
| Idempotency | (a) Both policy `CREATE`s are preceded by `DROP POLICY IF EXISTS` — safe to re-apply. (b) Purge function is `CREATE OR REPLACE` — safe to re-apply. (c) Calling the purge function repeatedly is idempotent: a model whose objects were already deleted produces a zero-row `DELETE` and no audit event is emitted (the `IF v_deleted_count > 0` guard). (d) Cron schedule uses `unschedule … WHERE EXISTS` then `schedule` — re-applying the migration replaces the schedule at the same jobname without duplicating it. |
| Dependency | Sibling: `combiner_model_registry` (MIG-099 — the registry the purge function reads `status='retired' AND retired_at` from). Read-side consumer: `supabase/functions/_shared/longshort-combiner/model-artifact-loader.ts` (same commit — uses the same bucket name `combiner-models`, parses `storage://combiner-models/{model_id}/model.txt`, derives `meta.json` sibling). Write-side producer (FUTURE, 3.3b-ii-B): Python LightGBM trainer + GHA workflow + operator-provisioned bucket + operator-provisioned `SUPABASE_SERVICE_ROLE_KEY` GHA secret. Pre-existing: `public.has_permission(uuid, text)` (used by SELECT policy); `cron` schema (pg_cron extension pre-enabled on platform foundation); `public.audit_logs` (`combiner.artifact_purged` event sink). |
| Sub-step authority | FP-052.3 sub-step 3.3b-ii-A; ACT-287 (this commit). Supervisor-authorized scope: ONE additive migration (RLS + retention function + cron); pure-TS loader + tests; widened `LoadModelArtifact` callback shape; NO Python, NO `.github/`, NO `requirements.txt`, NO bucket provisioning, NO secret provisioning, NO cron arming on any signal/compute path, NO model promotion fire. |
| AC evidence | (a) Migration applied 2026-06-23; cron schedule jobid=105 returned. (b) Supabase linter delta: ZERO new WARN-0029 entries — the 11 WARN-0029 baseline (DW-119 / MIG-109 / MIG-111 / MIG-112 / MIG-113 auth-helper subset) is preserved verbatim; `purge_retired_combiner_artifacts` has `REVOKE EXECUTE FROM authenticated` so it does NOT contribute. (c) `model-artifact-loader.ts` 7 Deno tests pass (mal-1..mal-8 incl. the LOAD-BEARING mal-4 hash-mismatch refusal test); full combiner suite 132/132 green under pinned Deno 1.46.3. (d) `supabase/functions/deno.lock` re-verified `"version": "3"` (Catalog #58 no v5 regression). (e) Loader Gate-6 wall-clock self-scan (mal-8) green; `scripts/check-wall-clock.ts` CLEAN. (f) Bucket-absent inert state honored: until the operator provisions the bucket, the RLS policies bind to zero rows and the purge function returns `models_purged: 0` — no failure surface in the meantime. |
| Capability-gap surface (§22.8.5) | (1) The `combiner-models/` bucket itself does NOT exist yet — operator-provisioned in 3.3b-ii-B. Loader calls against missing-bucket return `ArtifactDownloadError` (the loader does NOT silently succeed). (2) No `SUPABASE_SERVICE_ROLE_KEY` GHA secret — required for the 3.3b-ii-B Python trainer to upload artifacts; operator-provisioned. (3) The purge function deletes `storage.objects` rows; the underlying Storage backend object cleanup is handled by the Supabase storage stack's existing DELETE triggers — verify in production that bucket-side bytes are actually reclaimed within the storage SLO. (4) No artifact exists yet (no trained model), so the purge function runs nightly returning `models_purged: 0` until 3.3b-ii-B lands AND a model is retired AND 12 weeks elapse from its retirement — accepted forward-compatible state. |
| Cross-references | ACT-287 (this commit); DEC-065 Clause 3 (12-week retention via pg_cron purge — this MIG's mechanism); DEC-065 Clause 4 (RLS contract — service_role write, longshort.view read — this MIG's policies); DEC-064 Clause 4 (`feature_order_hash` refusal — implemented by `model-artifact-loader.ts` same commit); MIG-099 (`combiner_model_registry` — the registry the purge reads); MIG-115 (sibling 3.3a foundation this MIG builds atop); `model-artifact-loader.ts` (the read-side consumer paired with this migration); CROSSWIND §6.4 (12-week retention intent — PRESERVED via this MIG's purge, NOT superseded). Rollback (safe — re-opens forward-compatible inert state): `SELECT cron.unschedule('longshort.combiner.artifact_retention_purge'); DROP FUNCTION public.purge_retired_combiner_artifacts(); DROP POLICY "combiner-models: service_role write" ON storage.objects; DROP POLICY "combiner-models: longshort.view read" ON storage.objects;` (safe: no FK references; the bucket itself is operator-managed). |

### MIG-117: ACT-292 — FP-052.2 (3.2-b) market-regime: `signal_registry` rows for the two DEC-066 §(f) grounded features + DISARMED `job_registry` seed for `longshort.spy_regime.compute` (NO DDL on `signal_observations` — Gate-3.2 "without migration" preserved)

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-23 against project ref `sftatlxatbdrotivxcip`. |
| Applied | 2026-06-23; live-DB §22.5.1 verification post-apply via `supabase--read_query`: (a) both `signal_registry` rows present with `status='planned'`, `cadence='daily'`, `criticality=NULL`, `job_registry_id='longshort.spy_regime.compute'`, `display_order` 100 / 101; (b) `job_registry` row `id='longshort.spy_regime.compute'` present with `enabled=false` (DISARMED), `schedule='0 19 * * 1-5'`, `handler_path='supabase/functions/longshort-spy-regime-compute/index.ts'`, `status='registered'`. Supabase linter delta: ZERO new finding classes; the 11 reported WARN-0029 entries are the pre-existing DW-119 / MIG-109 / MIG-111 / MIG-112 / MIG-113 / MIG-115 / MIG-116 SECURITY DEFINER auth-helper baseline retained by design (verbatim mirror of MIG-115 / MIG-116 sourcing). |
| Pattern | Two-part additive seed migration in one transaction (NO DDL): (1) `INSERT INTO public.signal_registry … VALUES (…) ON CONFLICT (signal_id) DO NOTHING` — two new rows (`market_24m_cumulative_return` + `market_realized_vol_6m`); both `status='planned'` at seed (live-flips at 3.2-d when the assembler reads them, preserving the existing signal-FP "planned→live in the same migration that arms compute" precedent); `criticality=NULL` because the market-level structural category is neither per-name `critical` nor per-name `non_critical` — typed fail-loud lives at the assemble layer per DEC-066 §(e); CHECK admits NULL identically to the existing `composite` row. (2) `INSERT INTO public.job_registry … VALUES ('longshort.spy_regime.compute', 'longshort', …, '0 19 * * 1-5', false, …) ON CONFLICT (id) DO NOTHING` — disarmed cron seed at 19:00 UTC weekdays (collision-free; well BEFORE assemble `35 23 * * 1-5` and rank `50 23 * * 1-5` — ordering pre-positioned for 3.2-c regime-broadcaster which will read these regime rows during feature assembly). |
| Purpose | Wires the registry + scheduler layers for the DEC-066 §6.5.1.1 market-level structural category that FP-052.2 / 3.2-b lands at the code layer (handler + orchestrator + pure compute). 3.2-b is the PRODUCER commit: it writes regime rows to `signal_observations` under sentinel ticker `__MARKET__` (FP-052.2 / ACT-291) but NO consumer reads them yet — the rows are INERT until 3.2-c lands the assembler regime-broadcaster, 3.2-d flips `FEATURE_VECTOR_LENGTH` 16→18, and 3.2-e re-validates the trainer hash. `feature_order_hash` MUST NOT flip in 3.2-b. |
| Schema discipline | NO public-schema table created; NO column added; NO enum added; NO RLS policy added or modified; NO GRANT changes. NO DDL on `signal_observations` (FP-052.2 / ACT-291 live probe pre-confirmed admissibility: `ticker NOT NULL` satisfied by `__MARKET__` sentinel; `signal_id` free-text admits the two new IDs; both live CHECKs satisfied by always-present fail-loud regime rows). Gate-3.2 "without migration on `signal_observations` / `combiner_feature_vectors`" HOLDS. |
| Idempotency | (a) `signal_registry` insert uses `ON CONFLICT (signal_id) DO NOTHING` — safe to re-apply; existing rows preserved verbatim. (b) `job_registry` insert uses `ON CONFLICT (id) DO NOTHING` — safe to re-apply; existing schedule/enabled state preserved. (c) NO `cron.schedule(...)` application in this migration (operator-applied at 3.2-c-d per the disarm-fire-enable convention — MIG-066 / MIG-074 / MIG-076 / MIG-102 precedent). |
| Dependency | Sibling: `signal_registry` (MIG-075 — the index table; this migration adds rows, not DDL). `signal_observations` (MIG-064 + MIG-101 — the carrier table; this migration does NOT touch it, but the orchestrator's runtime upsert depends on the live `value_is_present_check` + `carried_forward_present_check` admitting `value NOT NULL + is_present=true + carried_forward=false`). `job_registry` (the scheduler table; row seeded DISARMED). Pre-existing infra: `PolygonPriceHistoryFetcher` (the SPY data source per DEC-066 §(c)); `POLYGON_API_KEY` secret (already provisioned). Code paired-commit (same FP-052.2 / 3.2-b unit): `_shared/longshort-signals/market-regime/{compute-regime,regime-orchestrator}.ts` + `longshort-spy-regime-compute/index.ts` + `longshort-spy-regime-compute-manual/index.ts`. Downstream (FUTURE, 3.2-c): assembler regime-broadcaster reads `signal_observations WHERE signal_id LIKE 'market\_%' ESCAPE '\\' AND ticker='__MARKET__'`. Downstream (FUTURE, 3.2-d): paired TS+Python `FEATURE_ORDER` expansion 16→18; `feature_order_hash` flips (additive-not-break at change time, POST-PROMOTION LOCK active per DEC-066 §(f)). |
| Sub-step authority | FP-052.2 sub-step 3.2-b; ACT-292 (this commit). Supervisor-authorized scope: ONE additive registry+scheduler seed migration (NO DDL); pure-compute + orchestrator + cron handler + manual sibling; `REGIME_PRICE_HISTORY_LOOKBACK_DAYS = 730` constant; NO assembler edit; NO `lgbm-inference.ts` / `feature_contract.py` / `FEATURE_ORDER` / `EXPECTED_FEATURE_KEY_COUNT` change; NO cron arming; NO fire. `feature_order_hash` MUST NOT flip in 3.2-b. |
| AC evidence | (a) `read_query` on `public.signal_registry`: both new rows confirmed with the exact field values listed in the Applied row. (b) `read_query` on `public.job_registry`: row present, `enabled=false` (DISARMED), `schedule='0 19 * * 1-5'`. (c) `pg_cron`: no `cron.schedule(...)` call applied — DISARMED at seed, operator-armed at 3.2-c-d. (d) `supabase/functions/deno.lock` re-verified `"version": "3"` (Catalog #58 no v5 regression). (e) Gate-6 wall-clock self-scan (`scripts/check-wall-clock.ts`): CLEAN — 0 violations. (f) Deno tests: `compute-regime_test.ts` 19/19 PASS; `regime-orchestrator_test.ts` 9/9 PASS; `longshort-spy-regime-compute/index_test.ts` 13/13 PASS. (g) NO assembler / `lgbm-inference.ts` / `feature_contract.py` / `EXPECTED_FEATURE_KEY_COUNT` / `feature_order_hash` touch in any file in this commit (pinned by handler tests `(7)` + `(8)`). (h) Cron-collision check vs existing schedules: 19:00 UTC weekdays is free (taken weekday slots: 20:00 / 21:00 / 21:15 / 21:30 / 22:30 / 23:35 assemble / 23:50 rank). |
| Capability-gap surface (§22.8.5) | (1) `REGIME_PRICE_HISTORY_LOOKBACK_DAYS=730` is at the trading-day threshold (730 × 252/365 ≈ 504); holiday-cluster compression may produce fewer than 504 trading bars on a real fire and trip `regime_data_insufficient_history` cold-start before live SPY history actually warrants it. Mitigation: observe at 3.2-c-d operator §22.5.1 smoke runs; if trip rate is non-zero, widen to 760+ via a separate FP — do NOT widen as a "while we're at it" change here (ROI-guardrail). (2) The two `signal_registry` rows are `status='planned'` until 3.2-d arms the FEATURE_ORDER expansion; if 3.2-c lands but 3.2-d does not, the rows remain planned. Operator-flippable via a 1-line corrective migration at 3.2-d. (3) The 19:00 UTC slot is OK for SPY (US-market close at 20:00 UTC ≈ 16:00 ET on standard time, 21:00 UTC ≈ 16:00 ET on DST); SPY EOD bar may not be Polygon-published at 19:00 UTC during DST months — first arming should fire during a known-published window. Mitigation: at 3.2-c-d, operator picks a fire time after Polygon's daily-bar publish SLA; the seeded `0 19` slot is a SHAPE placeholder, not an arming commitment. |
| Cross-references | ACT-292 (this commit); ACT-291 (FP-052.2 + signal_observations constraint-admissibility probe — the live-DB investigation that pre-cleared "no DDL needed"); ACT-290 (DEC-066 governance — the structural category being seeded); DEC-066 §(c) (Polygon-SPY data-source binding); DEC-066 §(e) (typed-fail-loud distinct reasons — `regime_data_missing_current_bar` vs `regime_data_insufficient_history`); DEC-066 §(f) (grounded parameters — 504 trading days per Daniel & Moskowitz 2016; 126 trading days per Barroso & Santa-Clara 2015; sqrt(252) annotated convention-not-grounding); DEC-066 §(g) (CANDIDATE parks — `market_63d_return` + rolling-z NOT in v1); DEC-066 §(h) (the `signal_observations` constraint question — resolved by ACT-291 probe); MIG-075 (`signal_registry` schema); MIG-064 (`signal_observations` original CHECK); MIG-101 (`signal_observations.carried_forward` column + additive CHECK); CROSSWIND §6.5.1.1 (the additive sub-clause the rows materialize). Rollback (safe — re-opens pre-3.2-b state): `DELETE FROM public.job_registry WHERE id = 'longshort.spy_regime.compute'; DELETE FROM public.signal_registry WHERE signal_id IN ('market_24m_cumulative_return','market_realized_vol_6m');` (safe: no FK references; no rows written under the new signal_ids yet since the handler is disarmed). |

### MIG-118 — longshort_target_positions table (FP-055 Step A / ACT-302)

| Field | Value |
|---|---|
| Migration file | `supabase/migrations/20260624040628_*.sql` |
| Applied | 2026-06-24 |
| Status | applied |
| Author | longshort (FP-055 Step A — Portfolio Construction) |
| Purpose | Create the `longshort_target_positions` table — the daily per-ticker target dollar state computed off `combiner_book` + `BrokerBuyingPower.account_equity`. Read-only audit/output surface for the new portfolio-construction layer; the (future) execution layer (DW-046) will diff against these rows to derive orders. |
| Schema | Columns: `operator_id uuid NOT NULL`, `as_of_date date NOT NULL`, `side text NOT NULL CHECK (side IN ('long','short'))`, `ticker text NOT NULL`, `target_notional numeric NOT NULL CHECK (>=0)`, `target_shares numeric NULL CHECK (NULL OR >=0)` (NULL at compute — needs fill price), `allocation_pct numeric NOT NULL CHECK (>0 AND <=1)` (D4 named param), `leverage numeric NOT NULL CHECK (>=1 AND <=2)` (D5 — DB admits the Phase-8 range; kernel currently asserts === 1.0 per DW-137), `sizing_basis text NOT NULL CHECK (IN ('account_equity'))`, `sizing_basis_value numeric NOT NULL CHECK (>0)`, `capital_base numeric NOT NULL CHECK (>0)`, `book_size int NOT NULL CHECK (>0)`, `ranker_source text NOT NULL`, `book_ref_computed_at timestamptz NOT NULL`, `computed_at timestamptz NOT NULL DEFAULT now()`. PK `(operator_id, as_of_date, ticker)`. UNIQUE `(operator_id, as_of_date, side, ticker)`. Index `(operator_id, as_of_date)`. |
| Access (RLS + GRANTs) | Mirrors `combiner_book` (MIG-099) verbatim. `GRANT SELECT TO authenticated`, `GRANT ALL TO service_role`. Read policy: `SELECT TO authenticated USING (public.has_permission(auth.uid(), 'longshort.view'))`. RESTRICTIVE deny policies on `INSERT`/`UPDATE`/`DELETE` for `authenticated` — only service-role writes (DEC-032 clause-4: NO new execution-adjacent permission key introduced). |
| Dependency | Sibling: `combiner_book` (MIG-099 — the upstream book that targets derive from). Reads `combiner_rankings.gics_sector` (MIG-099) for the §7.1 sector-cap WITNESS in the orchestrator. No FK to `combiner_book` — by design, so the targets table can absorb a book that gets re-written/superseded without cascade. Code paired-commit (same FP-055 unit): `supabase/functions/_shared/longshort-targets/{target-position-builder,target-position-orchestrator,stub-capital-fetcher}.ts` + tests + `supabase/functions/longshort-targets-compute/index.ts` + `…-manual/index.ts`. Downstream (FUTURE): the Phase-5 paper-exec layer (DW-046) will read these rows to compute deltas vs broker positions. Pre-cut socket: the `longshort.targets.published` audit event is the trigger surface. |
| Sub-step authority | FP-055 / ACT-302. Supervisor-authorized scope: ONE additive table migration (CREATE TABLE + RLS + GRANTs + index); pure-compute kernel + orchestrator + edge fns; ZERO order, ZERO broker write, ZERO `longshort.execute`, ZERO cron arming. |
| AC evidence | (a) Migration applied; LIVE `read_query` against `pg_tables` and `information_schema.columns` confirms shape (verified via subsequent INSERT against the schema succeeded). (b) Live INSERT of 40 derived rows from `combiner_book WHERE as_of_date='2026-06-23'` succeeded with `ON CONFLICT … DO UPDATE`; aggregated query confirms 20L+20S, each `target_notional=2500.00`, gross per side `$50,000`, dollar-neutral, `capital_base=$100,000`, `allocation_pct=1.0`, `leverage=1.0`, `ranker_source='count_normalized_fallback'`. (c) RLS verified by inheritance from the `combiner_book` template pattern (verbatim policy bodies). (d) `deno test … target-position-builder_test.ts` → 15/15 PASS. (e) Both `deno.lock` files re-asserted at `version: 3`. |
| Capability-gap surface (§22.8.5) | (1) The migration triggered the existing-functions SECURITY DEFINER linter warning bank (11 WARNs on `has_permission` / `has_role` / etc.) — these are pre-existing on the helpers MIG-118 calls; NOT caused by this migration; out of scope for FP-055 (they are governance-tracked separately). (2) The `leverage` column CHECK admits `>=1 AND <=2` per the future Phase-8 DEC range, but the kernel asserts `===1.0` — a future leverage change in code that bypasses the kernel could persist rows with `leverage > 1.0` until the orchestrator-side gate catches it. Mitigation: the kernel is the SOLE writer through the orchestrator; no other writer path exists; CHECK + kernel assertion are layered defense. (3) `target_shares` is NULL at Step A; the execution layer (DW-046) must compute shares from a fill price BEFORE attempting any order placement. The NULL is intentional and audited. |
| Cross-references | FP-055; ACT-302; DW-137 (Phase-8 leverage authorization DEC); DW-138 (Alpaca live capital-fetcher wiring); DEC-032 clause-4 (`longshort.execute` permission key — NOT introduced); DEC-034 clause (4) (wall-clock containment — `computed_at = ts.toISOString()` injected); DEC-033 v4.1 (strategy audit writer — used for all `longshort.targets.*` events); CROSSWIND §1.5 (per-name sizing — 2.5% verbatim at full book / allocation=1 / leverage=1); CROSSWIND §1 L95/L155 (no-leverage invariant — honored via `leverage===1.0` assertion). Rollback (safe — pure additive, no FK in): `DROP TABLE IF EXISTS public.longshort_target_positions CASCADE;` (safe: nothing references it; the kernel + orchestrator just fail-loud on missing-table after rollback). |

### MIG-119 — `longshort_short_availability_cache` table (FP-056 E4 / ACT-312 / DEC-068 clause-e HYBRID resolution)

| Field | Value |
|-------|-------|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-24 against project ref `sftatlxatbdrotivxcip`. |
| Applied | 2026-06-24. Table created; both `longshort.view`-SELECT and service_role-ALL RLS policies installed; `expires_at` btree index created. The 11 pre-existing SECURITY DEFINER linter warnings observed at apply time are the same set flagged at ACT-302 line 3178 / MIG-118 (`has_permission` / `has_role` / `is_superadmin` / `kill_switch_*` / `promote_combiner_model` / `rollback_combiner_model` / `purge_retired_combiner_artifacts` / `write_universe_eligibility_coverage` / `signal_queue_*` / `compare_reconciliation_baseline`) — NOT caused by this migration; the new table itself contributes ZERO new warnings. |
| Pattern | Single-statement-block additive migration: (1) `CREATE TABLE IF NOT EXISTS public.longshort_short_availability_cache (symbol TEXT PRIMARY KEY, marked_htb_at TIMESTAMP WITH TIME ZONE NOT NULL, expires_at TIMESTAMP WITH TIME ZONE NOT NULL, created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now())`. (2) `CREATE INDEX IF NOT EXISTS idx_longshort_short_availability_cache_expires_at ON public.longshort_short_availability_cache (expires_at)` — supports the verifier's pre-flight read `WHERE expires_at > now()`. (3) GRANT block: `SELECT` to `authenticated`; `ALL` to `service_role`. (4) `ALTER TABLE … ENABLE ROW LEVEL SECURITY`. (5) Two `DROP POLICY IF EXISTS` + `CREATE POLICY` pairs — `longshort.view`-gated SELECT for authenticated; FOR ALL for service_role. |
| Purpose | Within-day correction record for Alpaca's once-each-morning `/v2/assets` shortable snapshot (per Alpaca docs — intraday borrow changes are not reflected until next-morning refresh). The MIG-119 record breaks the otherwise-uninterrupted htb re-reject loop the substitution layer cannot catch (pre-flight passes stale snapshot → broker rejects htb → next tick re-passes same stale snapshot → re-rejects → … until next-morning assets refresh). `verify_short_availability` consults this table as a fast-fail gate BEFORE the broker locate call (the LOAD-BEARING wiring — without the consult, the record is written but never read and the loop is NOT broken). Genuine-success locate (`qty_available >= qty_requested`) DELETEs the row; partial locate (`qty_available < qty_requested`) does NOT clear (the symbol is still constrained). §2 axiom is satisfied: this is a within-day CORRECTION over a known-stale prime, not a derivative drifting from a live prime — the latter pattern is exactly why halted + transient-BP get NO cache (their primes are live; a cache would INTRODUCE staleness they don't have). |
| Schema discipline | One new public-schema table, four columns, one PRIMARY KEY, one btree index. No FK in (intentional — the cache is keyed by raw symbol; FK to a universe table would couple the loop-break to membership state, which is the wrong semantics). `created_at` defaults to `now()` for diagnostic auditing of write age; the operational lifetime fields are `marked_htb_at` (rejection ts injected by E4) and `expires_at` (`marked_htb_at + 24h` wall-clock TTL backstop). |
| Idempotency | (a) `CREATE TABLE IF NOT EXISTS` — re-apply safe. (b) `CREATE INDEX IF NOT EXISTS` — re-apply safe. (c) Both `CREATE POLICY` statements preceded by `DROP POLICY IF EXISTS` — re-apply safe. (d) Propagator-side UPSERT uses `ON CONFLICT (symbol) DO UPDATE SET marked_htb_at = EXCLUDED.*, expires_at = EXCLUDED.*` — repeated rejections for the same symbol within TTL are absorbed (latest-wins). (e) `verify_short_availability` clear path is `DELETE … WHERE symbol = $1` — DELETEing a non-existent row is a no-op. |
| TTL backstop choice (SURFACE / DEC-068 append flagged) | 24-hour wall-clock TTL. Rationale: no daily assets-refresh job exists (grep confirmed only quarterly universe refresh), so clear-on-genuine-success is the only operational clear path; without a backstop a never-re-attempted name stays htb-marked indefinitely. 24h is a conservative wall-clock approximation chosen over trading-calendar math to keep the migration small. **DEC-068 append flagged**: confirm whether a trading-calendar-aware TTL (e.g. "expires at next morning's NYSE open + assets-refresh") is required before E5 cron arm, or whether 24h wall-clock is acceptable for v1 paper. Risk if 24h is wrong-low: a name briefly htb on Friday afternoon goes Monday-morning-stale before the weekend close, causing one spurious re-reject Monday morning — minor noise. Risk if wrong-high: a name that briefly went htb stays marked for an extra hour or two past the next-morning refresh — minor false-negative on shortability. Neither is catastrophic; the right answer is operator-affirmation. |
| Dependency | Read consumer: `supabase/functions/_shared/longshort-verifiers/verify_short_availability.ts` (the pre-flight CONSULT wiring + the clear-on-GENUINE-success DELETE — the LOAD-BEARING reads). Write consumer: `supabase/functions/_shared/longshort-execution/cache-propagator-io.ts` (`createSupabaseHtbCacheWriter` UPSERT). Both consult `public.has_permission(auth.uid(), 'longshort.view')` via the SELECT policy. NOT consumed by halted / transient-BP propagation paths — those are observability-emit only (NO write, NO read). |
| Sub-step authority | FP-056 E4; ACT-312 (this commit). Supervisor-authorized scope: ONE additive migration (htb cache table + RLS + GRANTs + index); pure `cache-propagator.ts` + I/O shell `cache-propagator-io.ts` + their tests; one verifier touch (`verify_short_availability.ts` — pre-flight consult + clear-on-genuine-success); one E3 wiring (inline propagator dep on `lifecycle-orchestrator.ts`); NO halted or transient-BP cache tables, NO pause-class handling, NO `longshort.execute` (E5), NO partial-fill (DW-140), NO broker call from E4 itself. |
| AC evidence | (a) Migration applied 2026-06-24 (atomic create+apply, standard executor env per §22.3(f) — no capability mismatch). (b) Live `information_schema.columns` SELECT confirms exact column shape: `symbol` (text, not null), `marked_htb_at` (timestamptz, not null), `expires_at` (timestamptz, not null), `created_at` (timestamptz, not null). (c) §22.5.1 live-DB round-trip: UPSERTed marker rows `E4_VERIFY_GME` + `E4_VERIFY_PARTIAL_CASE`; DELETEd `E4_VERIFY_GME` (simulating clear-on-genuine-success); verified read returns ONE row (`E4_VERIFY_PARTIAL_CASE`) with `ttl_interval = 1 day` exact + `within_ttl = true` (the `WHERE expires_at > now()` indexed read works). Verification rows cleaned up. (d) `deno test --allow-read --allow-env --no-check --no-lock supabase/functions/_shared/longshort-execution/ supabase/functions/_shared/longshort-verifiers/` → **209/209 PASS** (~1s). (e) `deno check supabase/functions/_shared/longshort-verifiers/verify_short_availability.ts` → clean (no type errors from the new cache-propagator-io import). (f) Both `deno.lock` files re-asserted byte-identical to baseline at `version: 3` (SHA-256 hashes match `/tmp/deno.lock.bak` and `/tmp/supafunc.lock.bak`). (g) Gate-6 wall-clock self-scan tests on both new pure + I/O modules → CLEAN (no `Date.now(` / no `performance.now(` / no no-arg `new Date()`). |
| Capability-gap surface (§22.8.5) | (1) Pre-existing 11 SECURITY DEFINER WARN-0029 entries are NOT contributed-to by this migration (the new table introduces zero new functions). (2) 24h wall-clock TTL is a conservative approximation; a trading-calendar-aware TTL is the right v2 (flagged for DEC-068 append). (3) The TTL backstop assumes the propagator clock advances normally — a clock skew of >24h would either keep records longer than intended (wall-clock) or expire them prematurely; mitigated by the injected `ts` from the lifecycle-orchestrator (single Date source per DEC-034 (4)). (4) The cache is keyed by raw `symbol` only — no `operator_id` partitioning. Acceptable at v1 because Crosswind is a single-operator deployment; multi-operator partitioning is reserved as a future schema change if/when operators diverge. |
| Cross-references | FP-056 E4; ACT-312 (this commit); DEC-068 clause-e HYBRID resolution (halted + transient-BP observability-emit only; htb persisted-state due to Alpaca assets-snapshot staleness); ACT-311 (FP-056 E3 — the lifecycle-orchestrator this wires the inline propagator seam into); ACT-310 (FP-056 E2 — the I/O-shell + injected-writer pattern this mirrors); DW-150 / DW-151 / DW-152 (pause-class deferrals: ssr_violation / pdt_block / persistent-BP); DW-140 (partial-fill — out of E4 scope); DW-141 (modify-vs-cancel — out of E4 scope); DEC-034 clause (4) (injected clock — `ts` is the sole Date source); DEC-033 v4.1 (strategy-audit writer — `broker_rejection_propagation` emits via the existing `ReconciliationEventWriter`); §2 axiom (external snapshots are primes; the htb cache is a within-day correction over a stale prime, NOT a derivative drifting from a live prime). CROSSWIND §8.9 L257-274 (the propagation table the failure_action strings are pinned to verbatim); §8.10 L285 (LULD subsumed by halted-class handling — NOT separate scope); §11.0.7 #4 (verify_short_availability semantics — synthesized observation preserves the explicit-unavailable contract). Rollback (safe — pure additive, no FK in): `DROP TABLE IF EXISTS public.longshort_short_availability_cache CASCADE;` (safe: nothing references it; the verifier read is gated on `cache?.reader` injection so legacy call sites are unaffected, and the propagator's UPSERT fails loud with a write-failure tier-3 event if the table is missing). |

### MIG-120 — `longshort.execute` permission seed (FP-056 E5 / ACT-313 / DEC-068 clause d + DEC-032 clause 4 satisfied)

| Field | Value |
|---|---|
| Migration version | Lovable `supabase--migration` tool (atomic create+apply per §22.5.1) applied 2026-06-24 against project ref `sftatlxatbdrotivxcip`. Filename: `supabase/migrations/20260624191356_*.sql`. |
| Applied | 2026-06-24. One row inserted into `public.permissions` (`key='longshort.execute'`, description-anchored to DEC-068 clause d + DEC-032 clause 4 compliance, 786-char description). The 11 pre-existing SECURITY DEFINER linter warnings observed at apply time are the same set flagged at ACT-302 / ACT-312 — NOT caused by this migration; the new permission row introduces ZERO new functions, ZERO new warnings. |
| Pattern | Single-statement INSERT into the existing `public.permissions` table, idempotent via `ON CONFLICT (key) DO NOTHING`. No new table, no new policy, no new function, no new index. Mirrors MIG-037 (Step 5.2 `longshort.view` + `longshort.manage` seed) verbatim in shape. |
| Purpose | Seed the highest-privilege long-short permission — `longshort.execute` — which gates the long-short execution edge function (`longshort-execute`) that drives the autonomous three-tier order lifecycle (E3 `advanceTick`) against the broker. Two-segment per DEC-031 sub-point 3. Per DEC-032 clause 4 the key is introduced now because the first consuming code lands in the SAME PR (the edge fn at `supabase/functions/longshort-execute/index.ts` with `await checkPermissionOrThrow(authCtx.user.id, 'longshort.execute')` immediately after `authenticateRequest`). |
| Schema | (no schema change — INSERT only) |
| Access (RLS + GRANTs) | (no new table; `permissions` table policies unchanged) |
| Dependency | Existing `public.permissions` table (introduced at the platform RBAC bootstrap). Coupled to: `supabase/functions/longshort-execute/index.ts` (the first consumer — same PR per DEC-032 clause 4); `src/features/longshort/index.ts` (`LONGSHORT_PERMISSION_KEYS` updated to include `'longshort.execute'`); the existing `public.has_permission(_user_id, _permission_key)` SECURITY DEFINER function (the gate's underlying check). |
| Sub-step authority | FP-056 / E5 / ACT-313. Supervisor-authorized scope: ONE additive INSERT (no DROP, no destructive ALTER); permission SEEDED but NOT default-granted to any role per DEC-031 sub-point 10 (granting is a separate operator action; superadmin inherits via the existing wildcard pattern). |
| AC evidence | (a) Migration applied 2026-06-24 (standard executor env per §22.3(f) — no capability mismatch). (b) **§22.5.1 live-DB verification**: `SELECT p.key, length(p.description) AS desc_len, (SELECT count(*) FROM public.role_permissions rp WHERE rp.permission_id = p.id) AS role_grants_default FROM public.permissions p WHERE p.key='longshort.execute'` → 1 row, `desc_len=786`, `role_grants_default=0` (DEC-031 sub-point 10 satisfied — NOT granted to any role by default). (c) Both `deno.lock` files re-asserted at `version: 3` (the migration introduces no deps). (d) `deno test --allow-read --no-check supabase/functions/_shared/longshort-execution/` → 141/141 PASS including 6 new tick-scheduler envelope tests covering the edge-fn composition seam. (e) Gate-6 `deno run --allow-read scripts/check-wall-clock.ts` → CLEAN, 0 violations. |
| Capability-gap surface (§22.8.5) | (1) Pre-existing 11 SECURITY DEFINER WARN-0029 entries are NOT contributed-to by this migration (zero new functions). (2) Granting `longshort.execute` to any role is a SEPARATE operator action; no migration in this PR performs a grant (deliberate per DEC-031 sub-point 10). (3) The first consumer's broker boundary deliberately THROWS `LiveBrokerNotProvisionedError` until DW-138 + E6 wires `AlpacaPaperClient` — the permission seed is operational; the money-path side-effect is gated downstream. |
| Cross-references | FP-056 E5; ACT-313 (this commit); DEC-068 clause (d) (introduction trigger AUTHORIZED); DEC-032 clause (4) (key-only-when-code-exists — SATISFIED in same PR); DEC-031 sub-point 3 (two-segment) + sub-point 10 (no-default-grants); DEC-033 v4.1 (strategy-audit writer for `longshort.execute.*` events); DEC-034 clause (3) (errors propagate — the live-factory throw at `broker-bootstrap.ts`); MIG-037 (Step 5.2 — the seed pattern this mirrors verbatim); DW-047 (the deferred-work item that moves to `resolving` at this PR; closes at FP-056 full closure); DW-138 (Alpaca secrets — E6 closure gate, OPEN). Rollback (safe — single-row INSERT, idempotent): `DELETE FROM public.permissions WHERE key='longshort.execute'`; the edge fn's `checkPermissionOrThrow` would then deny all callers (fail-closed; safer than fail-open). |

### MIG-121 — `longshort_equity_snapshots` table (ACT-324 / FP-057)

| Field | Value |
|-------|-------|
| Migration file | `supabase/migrations/20260624225809_1a0a3d5e-79ab-4247-b873-4a161c15f19f.sql` |
| Applied | 2026-06-24 (Lovable `supabase--migration` tool — atomic create+apply per §22.5.1 against project ref `sftatlxatbdrotivxcip`). |
| Status | applied |
| Author | longshort (ACT-324 / FP-057 — equity-snapshot pipeline; data source for the portfolio growth chart). |
| Purpose | Create the `longshort_equity_snapshots` table — the equity time-series that the operator-facing portfolio growth chart (Yahoo-style, D/W/M/3M/6M/1Y range toggles, built on `recharts ^2.15.0` per DEC-069) reads. Written by the `longshort-rebalance-submit` placement trigger on every `full_rebalance` fire (`source='rebalance_fire'`); the daily-snapshot cron (fast-follow, NOT this PR) will fill non-fire days with `source='daily_cron'`. The chart accumulates one point per fire from today onward; the curve emerges over time. |
| Schema | Columns: `operator_id uuid NOT NULL`, `ts timestamptz NOT NULL`, `account_equity numeric NOT NULL CHECK (>=0)`, `cash numeric NULL`, `long_mv numeric NOT NULL CHECK (>=0)`, `short_mv numeric NOT NULL CHECK (>=0)` (absolute value of negative Alpaca position market_value), `gross numeric NOT NULL CHECK (>=0)` (`long_mv + short_mv`), `net numeric NOT NULL` (`long_mv - short_mv` — may be negative), `source text NOT NULL CHECK (IN ('rebalance_fire','daily_cron'))`, `mode text NULL CHECK (NULL OR IN ('full_rebalance','spot_check'))`. PK `(operator_id, ts)` (uniqueness by construction of the injected `ts`; no UPSERT needed). Index `(operator_id, ts DESC)` — supports the chart's "latest N points" range queries. |
| Access (RLS + GRANTs) | Mirrors `longshort_target_positions` (MIG-118) verbatim. `GRANT SELECT TO authenticated`, `GRANT ALL TO service_role`. Read policy: `SELECT TO authenticated USING (public.has_permission(auth.uid(), 'longshort.view'))`. RESTRICTIVE deny policies on `INSERT`/`UPDATE`/`DELETE` for `authenticated` — only service-role writes (the placement trigger writes via `supabaseAdmin`; DEC-032 clause-4 preserved: NO new execution-adjacent permission key introduced). |
| Idempotency | (a) `CREATE TABLE IF NOT EXISTS`; (b) `CREATE INDEX IF NOT EXISTS`; (c) all four `CREATE POLICY` statements preceded by `DROP POLICY IF EXISTS`; (d) writes are pure INSERTs and `(operator_id, ts)` is unique by construction — no UPSERT semantics needed; re-fires within the same wall-clock millisecond are pathological and not in the operational envelope. |
| Dependency | Sibling pattern: `longshort_target_positions` (MIG-118) — same RLS/GRANT template. Consumed by: `supabase/functions/longshort-rebalance-submit/index.ts` (the writer — non-fatal try/catch around the INSERT; the order placement is authoritative, the snapshot is observational). Future consumers: the equity-chart React component (separate frontend build, reads via Supabase RLS-gated SELECT), the daily-snapshot cron (fast-follow). |
| Sub-step authority | ACT-324 / FP-057. Supervisor-authorized scope: ONE additive table migration (CREATE TABLE + RLS + GRANTs + index); ONE non-fatal INSERT wiring on the existing placement trigger (no new broker call); `recharts ^2.15.0` added to `package.json` co-equally with DEC-069; ZERO order behavior change, ZERO cron arming, ZERO live fire. |
| AC evidence | (a) Migration applied via the Lovable `supabase--migration` tool. (b) **§22.5.1 live-DB verification**: `SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='longshort_equity_snapshots'` → 10 columns. `SELECT relrowsecurity FROM pg_class WHERE relname='longshort_equity_snapshots'` → `true`. `SELECT array_agg(polname ORDER BY polname) FROM pg_policy WHERE polrelid='public.longshort_equity_snapshots'::regclass` → `{longshort_equity_snapshots_deny_delete, longshort_equity_snapshots_deny_insert, longshort_equity_snapshots_deny_update, longshort_equity_snapshots_read_longshort_view}` (4 policies — mirrors MIG-118 verbatim). `has_table_privilege('authenticated', ..., 'SELECT')` → `true`; `has_table_privilege('authenticated', ..., 'INSERT')` → `false`; `has_table_privilege('service_role', ..., 'INSERT')` → `true`; `has_table_privilege('service_role', ..., 'DELETE')` → `true` (the GRANT block is in effect; authenticated cannot write; service_role has ALL). (c) Trigger module tests: 9/9 PASS in `longshort-rebalance-submit/index_test.ts` including 4 new snapshot tests (math, payload, non-fatal failure, spot_check-skips). (d) Both `deno.lock` files at `version: 3` (the migration introduces no Deno deps; recharts is a Node/npm dep, separate dependency surface). |
| Capability-gap surface (§22.8.5) | (1) Pre-existing 11 SECURITY DEFINER WARN-0029 entries are observed at apply time — same set as MIG-118/119/120; NOT contributed to by this migration (zero new functions). (2) The `spot_check` mode does NOT write a snapshot in this PR (data not in hand on that path; would require a new broker call — directive-forbidden). The daily-snapshot cron (fast-follow) closes this gap by writing on non-fire days regardless of mode. (3) The snapshot writer is NON-FATAL by design — repeated write failures are silent to the operator. Diagnostic surface for write failures is `console.error('longshort_equity_snapshot.write_failed', ...)` to the edge-fn logs; an alert rule on this log line is a fast-follow. |
| Cross-references | ACT-324; FP-057; DEC-069 (recharts authorization — co-equal); MIG-118 (the RLS/GRANT template mirrored verbatim); FP-056 E5.5 PHASE-2 / ACT-322 (the placement trigger this writes inside); DEC-068 clauses (d)+(g)+(n) (the trigger's governance lineage — UNTOUCHED by this PR); DEC-034 clause (4) (wall-clock containment — `ts` is the injected boundary `ts`, NOT a fresh wall-clock read inside the snapshot path); FP-023.1 forward-binding (per-FP zero-new-dep discipline — superseded for the charting surface by DEC-069). Rollback (safe — pure additive, no FK in): `DROP TABLE IF EXISTS public.longshort_equity_snapshots CASCADE;` (safe: only the placement trigger writes, wrapped in non-fatal try/catch — drop causes the snapshot insert to error and be swallowed; orders still place; chart degrades to "no data"). |

### MIG-122 — `combiner_rankings` + `combiner_shap_attribution` intraday_slot PK rotation (FP-057 Sub-step 1 / DEC-070 clause a / ACT-339)
| Applied | 2026-06-26. Status `active`. `ADD COLUMN intraday_slot smallint NOT NULL DEFAULT 0` on both tables (DEFAULT 0 populates all existing rows BEFORE any key change → table never observed half-keyed). Single transaction. Dependent FK `combiner_shap_attribution_operator_id_as_of_date_ticker_fkey` dropped, both PKs rotated to append `intraday_slot` as the last key element, FK re-created on the rotated composite — all atomic. Backward-compat: 1400 existing rankings rows + 0 shap rows all slot=0 post-migration; readers (`createSupabaseRankingsReader` in `rebalance-submit-orchestrator.ts:172` `ORDER BY as_of_date DESC`, `feature-assembler-orchestrator.ts:116` floor-query, `shadow-ranker-orchestrator.ts:175`) never name `intraday_slot` and return the identical slot-0 row. RLS policies + GRANTs intact (row-level, do not reference PK columns). Pre-existing 11 SECURITY-DEFINER WARN-0029 entries observed at apply (same set as MIG-118/119/120/121); zero new linter findings contributed by this migration. |
| Linked | ACT-339; DEC-070 clause (a) keystone; FP-057 Sub-step 1; co-equal with MIG-123/124/125/126. |

### MIG-123 — `combiner_feature_vectors` intraday_slot PK rotation (FP-057 Sub-step 1 / DEC-070 clause a / ACT-339)
| Applied | 2026-06-26. Status `active`. `ADD COLUMN intraday_slot smallint NOT NULL DEFAULT 0`; PK rotated `(operator_id, as_of_date, ticker) → (operator_id, as_of_date, ticker, intraday_slot)`. Single transaction; no FK dependents. 4195 rows all slot=0 post-migration. Backward-compat preserved as MIG-122. |
| Linked | ACT-339; DEC-070 clause (a); FP-057 Sub-step 1. |

### MIG-124 — `combiner_book` intraday_slot PK+UNIQUE rotation (FP-057 Sub-step 1 / DEC-070 clause a / ACT-339)
| Applied | 2026-06-26. Status `active`. `ADD COLUMN intraday_slot smallint NOT NULL DEFAULT 0`; BOTH constraints rotated in same transaction: PK `(operator_id, as_of_date, side, rank_within_side) → (…, intraday_slot)` and UNIQUE `(operator_id, as_of_date, ticker) → (…, intraday_slot)`. 200 rows all slot=0 post-migration. |
| Linked | ACT-339; DEC-070 clause (a); FP-057 Sub-step 1. |

### MIG-125 — `combiner_book_shadow` intraday_slot PK+UNIQUE rotation (FP-057 Sub-step 1 / DEC-070 clause a / ACT-339)
| Applied | 2026-06-26. Status `active`. `ADD COLUMN intraday_slot smallint NOT NULL DEFAULT 0`; BOTH constraints rotated: PK `(operator_id, as_of_date, variant, side, rank_within_side) → (…, intraday_slot)` and UNIQUE `(operator_id, as_of_date, variant, ticker) → (…, intraday_slot)`. 2880 rows all slot=0 post-migration. |
| Linked | ACT-339; DEC-070 clause (a); FP-057 Sub-step 1. |

### MIG-126 — `combiner_forward_returns` intraday_slot PK rotation (FP-057 Sub-step 1 / DEC-070 clause a / ACT-339)
| Applied | 2026-06-26. Status `active`. `ADD COLUMN intraday_slot smallint NOT NULL DEFAULT 0`; PK rotated `(operator_id, source_table, variant, seed_as_of_date, ticker, horizon_td) → (…, intraday_slot)`. T+H horizon arithmetic in `accrueReturns` UNCHANGED for slot 0 (DEC-064 §6.1/6.2 algorithm-lock preserved — slot is an additive partition within `seed_as_of_date`, not a date-axis redefinition). 4600 rows all slot=0 post-migration. |
| Linked | ACT-339; DEC-070 clauses (a) + (e); DEC-064 §6.1/6.2 (algorithm-lock); FP-057 Sub-step 1. |

### MIG-137 — `combiner_feature_vectors.gated_signals` additive column (DEC-071 sub-step 3c / ACT-358)
| Applied | 2026-06-28. Status `active`. File `supabase/migrations/20260628055353_bc7b892e-c3a9-4eff-9db4-79f25450068b.sql`. Single statement: `ALTER TABLE public.combiner_feature_vectors ADD COLUMN IF NOT EXISTS gated_signals jsonb NULL` with a `COMMENT ON COLUMN` documenting the sanctioned-null vs bug-null contract. NO PK rotation; NO constraint change; NO default value beyond NULL — every pre-existing row keeps `gated_signals = NULL` and is byte-identical under the ranker's gated-set check (`gatedSet === null` → no critical is treated as sanctioned). RLS/GRANTs unchanged (column-level, inherits table policy). |
| Purpose | DEC-071 sub-step 3c — the §4.3.5 assembler carve-out's keystone safety property: a critical signal whose value is `null` is GATED (skip per-name, DEC-074) only when its id appears in this column; otherwise it remains a BUG and the ranker still throws `IncludedRowInvariantError`. Today only `short_term_reversal_1w` can populate this column (gated reversal carve-out). Persists the in-memory `FeatureVectorRow.gated_signals` field through the orchestrator → ranker → composite path so the discriminator survives the `combiner_feature_vectors` round-trip. |
| Idempotency | `ADD COLUMN IF NOT EXISTS` (re-apply = no-op). Reads in `ranker-orchestrator.ts` use `r.gated_signals ?? null`, so pre-migration deployed code that hadn't selected the column still yields the legacy `gated_signals=null` behavior under PostgREST projection-omission. |
| AC evidence | (a) Migration applied via `supabase--migration`. (b) **§22.5.1 live-DB verification**: `SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_schema='public' AND table_name='combiner_feature_vectors' AND column_name='gated_signals'` → `[{column_name: gated_signals, data_type: jsonb, is_nullable: YES}]` (1 row, exact match). (c) Combiner test suite: 148 prior + 13 new DEC-071 3c tests = 161 PASS / 0 fail. Tests cover: gated-by-news + gated-by-catalyst → included; no-row / non-gated-skip / no-skip / gate_inputs_unavailable → still excluded with MISSING_CRITICAL_7; momentum carve-out rejected; features[reversal]=null + gated_signals=[reversal] (never fabricated zero); ranker skips gated-null (per-name DEC-074, numerator+presentCount unchanged); ranker still throws on non-gated null critical (bug-detection intact); ranker still throws when empty gated_signals array; must-not-move byte-identical composite for normal-reversal name. (d) Both `deno.lock` files at `version: 3` (no deps added). |
| Capability-gap surface (§22.8.5) | (1) The `gated_signals` column is jsonb but operationally an array of catalog signal_ids; no CHECK constraint enforces this — writer (`feature-assembler.ts`) is the single producer and emits `[REVERSAL_SIGNAL_ID]` xor `null`, so the type-narrowness is producer-side, not DB-side. (2) The downstream model-active branch in `ranker-orchestrator.ts` does NOT yet consult `gated_signals` for its feature-vector ordering (`featuresToOrderedArray` reads `features` directly). A gated reversal produces `features[reversal]=null` which the trained-combiner ADR-008a sentinel construction will surface — but the gated-vs-bug distinction is not yet wired into the LightGBM path. Out of scope for 3c (fallback-ranker carve-out only); FP-058 closes the model-path side. (3) Pre-existing 11 SECURITY-DEFINER WARN-0029 entries observed at apply time — NOT contributed to by this migration (zero new functions). |
| Cross-references | ACT-358; DEC-071 sub-step 3c; DEC-074 (per-name denominator semantics inherited verbatim — gated reversal drops numerator + presentCount for that name only, NO symmetric-when-any-gated); §4.3.5 (critical-coverage gate the carve-out weakens GATED-ONLY); §9 typed-absence (the `features[reversal]=null` shape, never a fabricated zero); MIG-099 (the parent `combiner_feature_vectors` table); MIG-134 (the `signal_observations.skip_reason` column the assembler reads to make the gated-vs-missing decision); 186 verified-real gated rows on 2026-06-26 from sub-step 3b (the live evidence base 3c was designed against). Rollback (safe — pure additive, no FK in, no consumer requires the column): `ALTER TABLE public.combiner_feature_vectors DROP COLUMN IF EXISTS gated_signals;` (safe: ranker reader uses `?? null` fallback; pre-3c rows already carry NULL; the carve-out reverts to the pre-DEC-071 hard-exclude behavior — gated names then fail MISSING_CRITICAL_7 again). |
| Linked | ACT-358; DEC-071 sub-step 3c; DEC-074. |
