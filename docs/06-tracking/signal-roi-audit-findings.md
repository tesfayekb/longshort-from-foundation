...
Sequencing intent: execute the weekend-buildable rows (DW-170 + DW-172 + DW-173 + DW-178 + the DEC-071 construction fix bundled with the DW-176 ungated shadow ride-along — DW-171 stays accrual-gated; DEC-072 stays charter-deferred) AFTER the remaining audits (#4, #8, #9, #3) complete, as one combined scope-disciplined PR per the operator's "review all first, then build together" framing. The DEC-071 now-fix is the audit's FIRST construction fix (departure from the #6/#2/#5/#1 shadow-or-charter-only pattern), justified by the unmitigated-gap + unambiguous-literature; the others remain measure-later shadow stand-ups or time-sensitive forward-data accruals. New weekend-buildable findings from the pending audits will land in this table as they're surfaced.
...
- Deferred-work register: `docs/08-planning/deferred-work-register.md` (DW-169 … DW-182 as of this writing).
- Phase-7 measure-and-lock home: `docs/08-planning/feature-proposals.md` → FP-058.
- Audit-action records: `docs/06-tracking/action-tracker.md` → ACT-350 (ledger creation + #6/#2 findings); ACT-351 (#5 findings + DW-173/174/175 + weekend-build-list section); ACT-352 (#7 findings + DEC-071 now-fix charter + DW-176/177 + weekend-build-list now-fix row); **ACT-353 (#1 findings + DEC-072 charter-deferred + DW-178 TIME-SENSITIVE + DW-179/180/181/182 + weekend-build-list updates)**.
- Decisions: `docs/decisions/DEC-071-reversal-cross-signal-gate-and-magnitude-cap.md` (the now-fix charter — PROPOSED, not yet built); `docs/decisions/DEC-072-analyst-credibility-weight-brokerage-tier-proxy.md` (the analyst credibility-weight charter — PROPOSED, BUILD DEFERRED to post-audit slate).
