...
### Clause (p) — LONG-ONLY PAPER POSTURE: structural short-side unexecutability on Alpaca paper (locate endpoint + SSR source both absent); typed-absence at the composer with NAMED `long_only_mode` declaration; ENV-FLAG-gated injection (boot-time probe explicitly rejected); BETA-EXPOSURE caveat on paper P/L — ADDENDUM (ACT-328, 2026-06-25)

(Clauses (a)–(o) and prior addenda are BYTE-UNCHANGED by this addendum; clause (p) is APPEND-ONLY. (p) is next-emitted after (o).)

**THE TRIGGER.** First live `full_rebalance` (corr `f0b0c2e7-...`, 2026-06-25 15:07 GMT): the operator-triggered fire placed 14 long orders and ZERO short orders, despite the preflight summary reporting `short_count: 30` / `passed_count: 30` at the candidate-categorization layer. Root-cause probe (operator, same session, against Alpaca paper `POST /v2/short_locates`) returned HTTP `404` / code `40410000` "endpoint not found" — Alpaca paper does NOT expose the locate endpoint. The locate adapter (`alpaca-locate-fetcher.ts:46`) correctly fail-closed on the 4xx per §11.0.7 #4 (`available:false` on every short candidate; the gate held). Combined with clause (n) (no paper SSR source), the short side has **two structural paper-only data-source absences**, both Reg-SHO/borrow infrastructure that fundamentally requires live-market data. This is NOT a code defect — the system behaved correctly. The defect is the audit-envelope shape: structural-404-absence and a real transient broker-reported htb reject both flow through `failed_verifiers:['verify_short_availability']`, leaving the two **indistinguishable** in the audit envelope — a latent masking bug that would mask a future real short-side issue once a real locate source is wired pre-live.

**THE POSTURE.** Paper v1 is **LONG-ONLY** on the placement path. The short-side execution machinery (planner short-deltas, `verify_short_availability` verifier, htb cache E4 wiring, short-side state-machine, rejection classifier) is BUILT and unit-tested but is **not fireable on paper** until a real short-availability/locate source is wired (DW-155, registered same PR). Long-side execution + the scheduler + the reconciliation surface + the equity-snapshot pipeline + the audit envelope are validated end-to-end on paper.

**THE BETA-EXPOSURE CAVEAT (binding).** Long-only paper P/L validates **plumbing** (placement, reconciliation, snapshot, audit envelope, scheduler), NOT the strategy's **return profile**. The dollar-neutral book the strategy is designed around is not exercised on paper v1; paper returns are **beta-exposed** to broad market direction, NOT the neutral cross-sectional alpha the model targets. **Paper P/L MUST NOT be cited as evidence of strategy return-profile validity in any live-fire ratification artifact.** Any future DEC, FP, ACT, or runbook that cites paper-v1 P/L as return-profile evidence is a clause-(p) violation. Paper-v1 P/L may be cited as plumbing-correctness evidence (execution + reconciliation + audit + snapshot pipelines exercised end-to-end on real broker dispositions); it MUST NOT be cited as strategy-correctness evidence.

**THE MECHANICS (BINDING — composer + trigger contract; carried by the STEP-C REVISION-FIX following this clause).** The composer + placement-trigger MUST surface the long-only posture as a NAMED mode, not as inferred attrition:

1. **Composer** (`preflight-composer.ts`): `PreflightComposerDeps.locateFetcher` becomes OPTIONAL (`locateFetcher?`), mirroring clause-(n)'s `ssrStatusFetcher?` typed-absence pattern. When `locateFetcher === undefined`, every SHORT candidate is recorded as FAILED with reason `short_availability_source_unavailable` (NOT `failed_verifiers:['verify_short_availability']`), the per-candidate `verifiers_skipped` carries `'verify_short_availability'`, and the batch `summary.locate_unavailable: boolean` is `true`. The broker locate endpoint is NEVER CALLED on the typed-absence path — typed-absence short-circuits before the htb-consult-and-locate loop.
2. **Trigger** (`longshort-rebalance-submit/index.ts`): derives `long_only_mode: boolean = summary.locate_unavailable || summary.ssr_unavailable` and surfaces it as a NAMED top-level response field + `longshort.rebalance.completed` audit metadata field. Adds `shorts_skipped_locate_unavailable: string[]` (the typed-absence short-symbol list), parallel to clause-(n)'s `shorts_placed_without_ssr_check`.
3. **Broker bootstrap** (`broker-bootstrap.ts` / `createLiveBrokerInterfaces`): the `locateFetcher` omission is **ENV-FLAG-GATED via `ALPACA_PAPER_LOCATE_AVAILABLE`** (default `false` on paper). When the flag is `false`, the factory does NOT instantiate or inject `AlpacaLocateFetcher`. When DW-155 closure wires a real source (Polygon/Tradier/equivalent), the flag flips to `true` and the real `locateFetcher` is injected.

**THE ENV-FLAG MECHANISM (binding) — boot-time probe REJECTED.** The structural-absence signal is the env-flag `ALPACA_PAPER_LOCATE_AVAILABLE`. A boot-time live probe (calling `POST /v2/short_locates` at factory construction to detect the 404 dynamically) is **explicitly REJECTED** as the mechanism, for three load-bearing reasons:
1. **Module-load-creds-free invariant (E5/E6 discipline).** A boot probe couples `createLiveBrokerInterfaces()` to a live broker round-trip at factory construction. Module load becomes network-dependent and creds-dependent, which is precisely what the diagnostic-503 pre-flight pattern (`alpacaCredsPresent()` at the HTTP-handler edge, NOT at module load) was designed to keep OUT of construction. Probing at construction violates this separation.
2. **No-wall-clock-at-construction (DEC-034 clause 4 family).** A probe introduces a latency-variable, network-coupled side-effect at module load — not a `Date` source per se, but the same family of "construction depends on the outside world" failure mode the discipline forbids. Cold-start latency becomes a function of broker reachability.
3. **Semantic honesty.** The absence is a **known property of the venue** (operator-asserted: "this venue has no locate endpoint"), not a discovered runtime fact. An env-flag is a declarative posture statement; a probe is re-discovering a known fact on every cold start — wasted call, wasted latency, with edge cases ("what if the probe was wrong last boot, succeeded transiently?") the env-flag does not have.

The env-flag is deterministic, free, operator-controlled, and flips cleanly at DW-155 closure. The locate adapter's 4xx→`available:false` fail-closed behavior (§11.0.7 #4) is UNCHANGED and remains the runtime defense-in-depth net if the flag is ever misconfigured to `true` against a venue that actually 404s — belt-and-suspenders preserved.

**THE DISTINGUISHABILITY INVARIANT (binding).** Structural unavailability (env-flag `false` → typed-absence at composer; broker NEVER called) MUST remain **distinguishable** from a transient broker-reported htb reject (env-flag `true` → broker called → locate returns `available:false` or partial-qty) in the audit envelope. The two carry distinct shapes:
- Typed-absence: `failed_verifiers` does NOT include `'verify_short_availability'`; reason field is `short_availability_source_unavailable`; `verifiers_skipped` carries `'verify_short_availability'`; `summary.locate_unavailable: true`; no `reconciliation_events` row for the locate verifier (broker not called).
- Transient reject: `failed_verifiers` DOES include `'verify_short_availability'`; reason field begins `preflight_failed:`; `summary.locate_unavailable: false`; a `reconciliation_events` row IS written by the verifier (broker called, observed.available=false recorded).

Conflating the two by routing structural absence through the transient-reject path (or vice versa) would mask a future real short-side issue and is a clause-(p) violation.

**THE FAIL-CLOSED INVARIANT (binding — UNCHANGED).** The locate adapter's 4xx→`available:false` behavior (§11.0.7 #4) is UNCHANGED and CORRECT. Shorting without a positive locate determination is forbidden on every venue, paper or live. The clause-(p) surfacing is **posture-declaration, NOT gate-weakening**. Any change that allows a short to be submitted without a positive locate determination (e.g. defaulting `available` to `true` when the source is absent; treating typed-absence as "assume available"; weakening the 4xx → `available:false` adapter contract) is a clause-(p) violation.

**THE 22.5.1 VERIFICATION GATE (binding — extends clause (o) §22.5.1 to the long_only_mode audit shape).** The STEP-C REVISION-FIX adds `long_only_mode: boolean` to the placement-trigger's `longshort.rebalance.completed` audit metadata. Per the corr-`bb3810bf` lesson, audit-metadata shape is a DB write that mock tests do not fully validate — the §22.5.1 discipline applies: a clean `full_rebalance` re-fire (operator-gated, post-STEP-C) MUST verify the `long_only_mode: true` field lands in a real `longshort_audit_logs` row, with the typed-absence short symbols enumerated in `shorts_skipped_locate_unavailable`. No new harness needed (the existing audit-envelope write is the surface); the gate is the operator-gated clean re-fire, NOT a mock-only test.

**THE PAIRING WITH CLAUSE (n) (architectural symmetry — observation, not a separate binding).** Clause (n) and clause (p) are the SSR-typed-absence and locate-typed-absence twins on the §7 preflight composer. Same architectural pattern (optional injection slot + per-candidate typed-absence skip + batch-level `summary.<source>_unavailable` boolean + trigger-level named surfacing). Same pre-live closure path (DW-154 for SSR, DW-155 for locate). Same vendor-decision space (Polygon / Tradier / equivalent). DW-154 + DW-155 are a **single pre-live resolution unit** at the pre-live phase (one FP, one vendor decision, one parity-test extension); they are registered as two DWs because they index two distinct injection slots, not because they are independent work items.

**SCOPE BOUNDARIES (binding).**
- **PAPER-V1-ACCEPTABLE.** Clause (p) ratifies the long-only posture for paper v1 ONLY.
- **EXPLICITLY NOT LIVE-ACCEPTABLE.** Live-fire ratification MUST NOT proceed with the short side unexecutable. DW-155 closure (real locate source wired + a two-sided book verified) is a NAMED pre-live blocker on the live-fire ratification DEC, paired with DW-154.
- **NO SCOPE EXPANSION.** Clause (p) does NOT amend, supersede, or weaken clauses (a)–(o). It composes with clause (n) (the SSR typed-absence twin) and does not touch the broker-rejection-side `ssr_violation` / `htb` routing (clause (e) / §8.9, deferred per DW-150).
- **NO HEURISTIC INFERENCE.** The `long_only_mode` declaration is derived from explicit composer flags (`summary.locate_unavailable` / `summary.ssr_unavailable`), NOT from "all 30 shorts failed" heuristics. Any future inference-based long-only detection (e.g. counting failed shorts to decide the mode) is a clause-(p) violation — it would falsely trip on a real near-full-book htb cluster.

**CROSS-REFERENCES.**
- **Clause (n)** (the SSR typed-absence twin — same architectural pattern; clause (p) mirrors it for the locate source).
- **DW-155** (the pre-live locate-source wiring blocker — registered same PR; live-fire ratification prerequisite; paired with DW-154 as a single pre-live resolution unit).
- **DW-154** (the pre-live SSR-source wiring blocker — the architectural twin's pre-live closure).
- **§2 axiom** (prime-vs-derivative, no synthetic sentinels) — the basis for typed-absence over default-on-failure on both the SSR and locate surfaces.
- **CROSSWIND §11.0.7 #4** (locate verifier spec: "skip short entry; do NOT substitute long; do NOT default to 'assume available'" — the fail-closed semantic clause (p)'s env-flag mechanism preserves).
- **`alpaca-locate-fetcher.ts:46`** (the 4xx→`available:false` fail-closed adapter behavior — UNCHANGED by clause (p); remains the defense-in-depth net).
- **`preflight-composer.ts:122`** (the `locateFetcher` injection slot — STEP-C transitions from required to OPTIONAL, mirroring `ssrStatusFetcher?` at line 126).
- **`broker-bootstrap.ts` / `createLiveBrokerInterfaces`** (the env-flag-gated factory branch — STEP-C wires `ALPACA_PAPER_LOCATE_AVAILABLE` here).
- **`longshort-rebalance-submit/index.ts`** (the trigger surfacing `long_only_mode` + `shorts_skipped_locate_unavailable` in the response + audit metadata).
- **DEC-034 clause (4)** (no-wall-clock-at-construction discipline — the family the boot-probe-rejection rationale belongs to).
- **Corr `f0b0c2e7-...`** (the surfacing fire — first live `full_rebalance` that exposed the structural-404 / audit-conflation defect).
...
## Clause (s) — Paper-v1 HALT posture: typed-absence, degraded-documented, NOT live-acceptable — AMENDMENT (ACT-416, 2026-06-30)

(Clauses (a)–(r) and prior addenda are BYTE-UNCHANGED by this amendment; clause (s) is APPEND-ONLY. The next-emitted clause letter is (s), not (q) — the operator-supplied prompt named (q), but grep at HEAD `0e9ac02a` confirms (q) and (r) already exist [SSR typed-absence at line 552; Polygon decision-price feed fix at line 620]; (s) is the literal-over-mental-model next-free letter. Letters (l) and (m) remain intentionally unused per clause (n)'s reserved-by-skip convention; no further skips are introduced here. The structure mirrors clause (n) verbatim-in-shape — POSTURE / RATIONALE / SCOPE BOUNDARY / GUARDRAIL 1 / GUARDRAIL 2 / GUARDRAIL 3 — because clause (s) is halt's SSR-twin on the typed-absence axis.)

**THE POSTURE.** The in-hand Polygon tier ("Stocks Advanced", confirmed real-time SIP-consolidated at clause (r) line 635 / consumed today at `broker-bootstrap.ts:246` for NBBO) exposes **no dedicated halt boolean at any REST endpoint** — empirically confirmed by the ACT-400 (FP-062 sub-step 6I.1b) read-only probe of `/v1/marketstatus/now`, `/v3/snapshot`, `/v2/snapshot/.../tickers/...`, `/v3/reference/tickers/...`, `/v2/last/trade`, `/v3/trades` (see `deferred-work-register.md:1469` for the verbatim probe inventory). Halt status under the tier we own is **inferable only by decoding per-trade condition codes** (T1 resumption / T12 paused / LULD / regulatory) on the trade tape — a real-time mechanism that is a BUILD on the WebSocket channel the tier already includes, NOT a procurement on a higher tier. The `verify_halt_status` tick-dispatch is INTENTIONALLY UNWIRED in the production reconciliation tick (the existing intentional gating at `supabase/functions/longshort-reconciliation-tick/index.ts:124`, comment verbatim: *"A verify_halt_status dispatch is NOT added here — it splits to post-6I.1b"*); `AlpacaHaltStatusFetcher` (`supabase/functions/_shared/longshort-broker/alpaca-halt-status-fetcher.ts`) remains **structurally phantom** for live-order code paths — it reads `/v2/assets/{symbol}.status`, which is static tradability metadata, not real-time per-second halt state. Halt status on paper v1 is therefore **TYPED-ABSENT** per the §2 prime-vs-derivative axiom — NOT a fabricated `halted: false` sentinel, NOT a default-on-failure ("phantom non-halt fetcher is structurally worse than no halt check," per the original DW-058-B2 wording at `deferred-work-register.md:1465`).

**THE RATIONALE.** A halt is a market-structure event. On PAPER there is **no capital at risk** — a paper position in a halted name cannot lose money to the halt the way a real position can. Halt-avoidance protects nothing a paper book can lose. Wiring a real halt feed now would add (i) a live data-feed dependency (WS subscriber + reconnect/gap-replay + condition-code decoder), (ii) a new failure axis (feed outage blocks all placements or — worse — silently misses halts), and (iii) a non-trivial build (no Polygon WS infrastructure exists in the repo at HEAD `0e9ac02a` — zero `WebSocket(`s + `polygon-io.com/stocks/realtime` consumers under `supabase/functions/_shared/longshort-broker/`). Typed-absence ("we don't know if halted") is the honest encoding of what we actually know on paper, strictly better than a phantom "not halted" — the §2 axiom (*No silent sentinels in money paths*; *Missing data uses typed Optional / explicit enums, never fabricated zeros*) is the basis.

**THE HALT-vs-SSR DIFFERENCE (load-bearing distinction from clause (n)).** A halt affects BOTH sides of the book (long entries, short entries, long exits, short exits) — SSR is short-only. Under clause (s) typed-absence, a halted name could be entered LONG on paper without the halt check catching it (clause (n) under SSR typed-absence could only mis-route SHORTS without the SSR check). The paper-v1 mitigation is identical to clause (n)'s: no capital is at risk on paper, and the signal-layer degraded-volume/spread defense (DW-067 registered note in the §3.3c halt-feed-unavailable runbook at `docs/04-modules/longshort/universe/runbooks/halt-feed-unavailable-runbook.md`) provides defense-in-depth — degraded liquidity profiles on recently-halted names propagate to signal quality even without the halt boolean. Live-fire ratification does NOT inherit this mitigation argument; see SCOPE BOUNDARY.

**THE SCOPE BOUNDARY (load-bearing).** This posture is **PAPER-ONLY-ACCEPTABLE and EXPLICITLY NOT LIVE-ACCEPTABLE.** Live-fire ratification CANNOT proceed with halt-coverage as typed-absence — a real halt fetcher MUST be wired (and verified) before live. **FP-064** (chartered concurrently with this clause, same PR, ACT-416) is the named pre-live blocker (Polygon Advanced WS condition-code halt fetcher BUILD — WS-probe-first Step A → fetcher → `verify_halt_status` non-phantom wire → verification); FP-064 closure is a hard prerequisite for the live-money DEC. Live-fire is also independently gated by DW-154 (clause (n) SSR), DW-155 (clause (q) assets.shortable validation), and INC-81 (clause (q) advance-path short-side wiring) — clause (s) ADDS FP-064 to that list, it does not replace any item on it.

**GUARDRAIL 1 (this clause — DEC ratification).** The paper-v1 halt typed-absence posture is governance-ratified HERE (clause (s)). It cannot quietly persist into live by inertia: live-fire ratification must explicitly close FP-064 and reference its evidence (WS-probe verdict + fetcher-build + non-phantom wire + verification against real halt-class condition codes on the live tape).

**GUARDRAIL 2 (binding placement / tick-surface diagnostic — operator-visible).** Wherever a placement (or the autonomous tick) acts on a name whose halt status is **typed-absent**, the operator MUST SEE the absence as a named field — NOT silently swallowed, NOT buried under a debug flag, NOT deferred to a follow-up dashboard. The required shape mirrors clause (n)'s `summary.ssr_unavailable` / `shorts_placed_without_ssr_check` precedent, adapted to halts:
- The placement-trigger response and audit metadata MUST surface `summary.halt_status_unavailable: boolean` (true when `haltStatusFetcher === undefined` OR when injected but every probed name returned indeterminate / not-supported on the tier).
- The placement-trigger response and audit metadata MUST surface `names_placed_without_halt_check: string[]` enumerating the symbols that were entered (long or short) without a real halt-status confirmation on this tick.
- The reconciliation-tick (the existing intentional gating at `longshort-reconciliation-tick/index.ts:124`) MUST emit a comment-stamped diagnostic line (or `reconciliation_events` row, classification TBD at FP-064 wire time) on every tick where `verify_halt_status` dispatch remained skipped, so the absence is auditable per-tick — not silent.

A placement-path or tick build that omits this surfacing FAILS the clause-(s) contract. The exact response-schema and audit-row shapes are bound at the FP-064 wire turn — clause (s) binds the REQUIREMENT to surface, not the field names beyond `halt_status_unavailable` / `names_placed_without_halt_check`.

**GUARDRAIL 3 (named live-fire blocker — FP-064).** FP-064 is registered concurrently with this clause (same PR, ACT-416, in `feature-proposals.md`) as a named, OPEN, live-fire-ratification-blocking deferred build. Its closure prerequisites are: **(Step A — WS PROBE FIRST, mandatory)** a one-shot read-only probe opening the Polygon Advanced WS `T.*` trades channel with the production key, confirming halt-class condition codes (T1 resumption / T12 paused / LULD / regulatory) flow in real-time on the tier we already own — the vendor-doc claim is NOT in-repo-verified and MUST be verified before any fetcher code branch opens; **(Step B — BUILD)** WS subscriber on `T.*` + condition-code halt-state decoder + reconnect / gap-replay + `/v3/trades` REST backfill/replay path + idempotency + tests against the live feed → real per-symbol `halted_at` / `resumed_at` events → `verify_halt_status` tick-dispatch wired non-phantom and verified against the §11.0.7 #6 tier-strong / low-tolerance contract. The live-fire ratification DEC must cite FP-064 closure evidence (probe verdict + build attestation + verification log).

**THE PROCUREMENT-FRAMING CORRECTION (load-bearing).** The DW-058-B2 ACT-400 probe addendum at `deferred-work-register.md:1469` records *"Clean per-second halt signal requires the SIP halts WebSocket feed (Stocks Advanced+ tier) — Phase-7 / tier-upgrade-gated CONFIRMED"*. That sentence is **REST-only-probe-scoped** and is **FALSE as a procurement claim**: (i) Polygon ships NO dedicated halt-boolean REST field on ANY tier, so "tier-upgrade-gated" is not a real procurement remedy; (ii) the SIP-consolidated WS trade tape (the canonical halt mechanism via condition-code decoding) is included on the **Stocks Advanced tier we already own** (DEC-068 clause (r) line 635; consumed today at `broker-bootstrap.ts:246`); (iii) there is no tier above Advanced that adds a halt boolean to substitute for the WS condition-code build. The real pre-live discharge is a BUILD on the tier we hold, NOT a procurement on a tier we don't. The ACT-400 addendum is PRESERVED as the audit record of the REST-only probe finding (clause-(p) preserved-record precedent); the correction is appended as a Rule-8 addendum in `deferred-work-register.md` (DW-058-B2-ADD-02) and recorded HERE in this clause for the DEC-resident reader.

**CROSS-REFERENCES.**
- **DW-058-B2** (the halt-feed deferred-work entry; ACT-400 probe addendum at `deferred-work-register.md:1469` preserved; new DW-058-B2-ADD-02 correction addendum appended concurrent with this clause).
- **FP-064** (the pre-live halt-coverage build — WS-probe-first Step A → condition-code fetcher → `verify_halt_status` non-phantom wire; live-fire ratification prerequisite; same-PR registration).
- **Clause (n) / DW-154** (the SSR twin — same typed-absence-on-paper, NOT-LIVE-ACCEPTABLE shape; clause (s) is the halt analog).
- **Clause (r)** (Polygon "Stocks Advanced" tier confirmed real-time SIP-consolidated — the in-hand tier the FP-064 build sits on).
- **ACT-400** (the FP-062 6I.1b REST-only probe — the audit record this clause's posture and FP-064's WS-probe-first Step A both reference; explicitly NOT re-litigated by clause (s)).
- **`supabase/functions/longshort-reconciliation-tick/index.ts:124`** (the existing intentional `verify_halt_status` dispatch gating — the in-code anchor for the typed-absence posture; FP-064 Step B's wire-point).
- **`supabase/functions/_shared/longshort-broker/alpaca-halt-status-fetcher.ts`** (the structurally-phantom existing fetcher — REPLACED, not consumed, by the FP-064 WS condition-code fetcher).
- **`supabase/functions/_shared/longshort-verifiers/verify_halt_status.ts`** (the verifier whose tick-dispatch wire is FP-064's terminal acceptance criterion).
- **`supabase/functions/_shared/longshort-execution/broker-bootstrap.ts:246`** (the `PolygonQuoteFetcher` injection seam confirming we own the Stocks Advanced tier today — the procurement-framing correction's empirical anchor).
- **§2 axiom** (prime-vs-derivative, no synthetic sentinels) — the basis for typed-absence over default-on-failure.
- **CROSSWIND §11.0.7 #6** (`verify_halt_status` contract — strong tier, low_tolerance, skip-this-name-this-tick failure action — the spec FP-064 Step B will wire against).
- **`docs/04-modules/longshort/universe/runbooks/halt-feed-unavailable-runbook.md`** + **DW-067** + **DW-063** (the §3.3c universe-layer halt-feed-unavailability registered notes; the defense-in-depth context for the halt-vs-SSR difference paragraph).
- **ACT-416** (this clause's authoring action; the governance-only PR landing clause (s) + FP-064 + the DW-058-B2 procurement-framing correction).

**WHAT THIS CLAUSE DOES NOT DO.** It does NOT resolve halt-coverage for LIVE (live remains gated on FP-064 closure). It does NOT run the WS probe (that is FP-064 Step A, scheduled at FP-062 gap-(e) discharge, not this turn). It does NOT edit the ACT-400 addendum (preserved-record precedent — the correction is APPENDED as DW-058-B2-ADD-02). It does NOT close FP-062 Phase-6 (gap (e) live-fills validation remains the discrete-closure gate; clause (s) discharges gap (c) only). It does NOT charter a procurement (FP-064 is a BUILD on the tier we hold).
